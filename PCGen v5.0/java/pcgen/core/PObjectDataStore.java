 ...
 *
 * $Id: PObjectDataStore.java,v 1.1 2006/02/21 01:07:42 vauchers Exp $
 */
package pcgen.core;
...
public class PObjectDataStore
{
	...
	public void add(PObject obj)
	{
		...
		Globals.debugPrintIf(was != null, "Replacing " + containedType + " keyed " + key);
		String upperName = obj.getName().toUpperCase();
		was = byUpperName.put(upperName, obj);
		Globals.debugPrintIf(was != null, "Replacing " + containedType + " upper named " + upperName);
		...
		Globals.debugPrintIf(was != null, "Replacing " + upperType + " " + containedType + " named " + upperName);
		clearVariableNameCache();
	}
	...
	public boolean hasVariableNamed(Collection collectionOfNames, String variableName)
	{
		...
		for (Iterator e = collectionOfNames.iterator(); e.hasNext();)
		{
			...
			if (obj == null)
			{
				if (Globals.isDebugMode())
				{
					Globals.debugPrint("No PObject for " + containedType + " " + wpName);
				}
				continue;
			}
	...
	private final void cacheVariableNames()
	{
		...
		if (Globals.isDebugMode())
		{
			Globals.debugPrint("Final variable name cache: " + nameSet);
		}
	}
}
