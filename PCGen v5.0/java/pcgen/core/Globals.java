...
//	private static final ArrayList helpContextFileList = new ArrayList(30);
	...
	private static final ArrayList eqSlotList = new ArrayList(20);
	...
	private static final Map eqSlotMap = new HashMap();
	...
	static
	{
		...
		{
			errorPrint("Can't find the VersionNumber property.", mrex);
		}
		finally
		{
...
//	}
	 ...
	 **/
	static String getFilePath(String aString)
	{
	...
	public static Equipment getEquipmentFromName(final String baseName)
	{
		...
		// Separate the "non-size" descriptors int 2 ArrayLists.
...
//	}
	 ...
	 * Print information message if PCGen is debugging.
	 *
	 * @param printIfTrue If true, print the message
	 * @param param1 String information message
	 */
	public static void debugPrintIf(final boolean printIfTrue, final String param1)
	{
		if (Globals.isDebugMode() && printIfTrue)
		{
	...
	private static Equipment findEquipment(final String aName, final ArrayList preNameList, final ArrayList postNameList, final ArrayList sizList, final String omitString)
	{
		...
		if (preNameList != null)
		{
			ArrayList nameList = preNameList;
	 ...
	 * @param newName
	 */
	private static void appendNameParts(ArrayList nameList, final String omitString, final StringBuffer newName)
	{
	...
	/**
	 * returns an Array of LevelInfo objects
	 **/
...
/////////////////////////////////////////////////
		...
		if (SettingsHandler.getShowHPDialogAtLevelUp())
		{
			...
			for (; ;)
			{
				Object selectedValue = javax.swing.JOptionPane.showInputDialog(Globals.getRootFrame(), "Randomly generate a number between " + min + " and " + max + "." + Constants.s_LINE_SEP + "Select it from the box below.", Globals.getGameModeHitPointText() + " for " + Utility.ordinal(level) + " level of " + name, javax.swing.JOptionPane.INFORMATION_MESSAGE, null, rollChoices, new Integer(roll));
	...
	public static void executePostExportCommand(String fileName)
	{
		...
		if (!"".equals(aString))
		{
			...
			{
				errorPrint("Could not execute " + aString + " after exporting " + fileName, ex);
			}
		}
	}
}
