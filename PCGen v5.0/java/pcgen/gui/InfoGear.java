...
final class InfoGear extends FilterAdapterPanel
{
	 ...
	 */
	private abstract class GearActionListener implements ActionListener
	{
		int qty = 0;
		...
		protected abstract void updateEquipment(Equipment eq, int qty);
	}
	...
	private class BuyGearActionListener extends AvailableGearActionListener
	{
		...
		protected void updateEquipment(Equipment eq, int qty)
		{
			buySpecifiedEquipment(eq, qty);
		}
	}
	...
	private class BuyRateGearActionListener extends AvailableGearActionListener
	{
		...
		protected void updateEquipment(Equipment eq, int qty)
		{
			buySpecifiedEquipmentRate(eq, qty);
		}
	}
	...
	private class RemoveGearActionListener extends SelectedGearActionListener
	{
		...
		protected void updateEquipment(Equipment eq, int qty)
		{
			removeSpecifiedEquipment(eq, qty);
		}
	}
	...
	private class SellRateGearActionListener extends SelectedGearActionListener
	{
		...
		protected void updateEquipment(Equipment eq, int qty)
		{
			...
			{
				sellSpecifiedEquipmentRate(eq, qty);
			}
		}
	}
	...
	private class GearPopupMenu extends JPopupMenu
	{
		...
		private GearPopupMenu(JTreeTable treeTable)
		{
			if (treeTable == availableTable)
			{
				 ...
				 *    inherited in the enclosing type 'pcgen/gui/InfoGear'.
				 ...
				 */
				GearPopupMenu.this.add(createBuyMenuItem("Buy  1", 1, "control EQUALS"));
				GearPopupMenu.this.add(createBuyMenuItem("Buy  5", 5, null));
				GearPopupMenu.this.add(createBuyMenuItem("Buy 10", 10, null));
				GearPopupMenu.this.add(createBuyMenuItem("Buy 20", 20, null));
				GearPopupMenu.this.add(createBuyMenuItem("Buy  n", -1, "alt N"));
				this.addSeparator();
				GearPopupMenu.this.add(createBuyRateMenuItem("Buy  1 at...", 1, null));
				GearPopupMenu.this.add(createBuyRateMenuItem("Buy  5 at...", 5, null));
				GearPopupMenu.this.add(createBuyRateMenuItem("Buy 10 at...", 10, null));
				GearPopupMenu.this.add(createBuyRateMenuItem("Buy 20 at...", 20, null));
...
*/
			...
			else // selectedTable
			{
				 ...
				 * author: Thomas Behr 08-02-02
				 */
				GearPopupMenu.this.add(createRemoveMenuItem("Remove  1", 1, "control MINUS"));
				GearPopupMenu.this.add(createRemoveMenuItem("Remove  5", 5, null));
				GearPopupMenu.this.add(createRemoveMenuItem("Remove 10", 10, null));
				GearPopupMenu.this.add(createRemoveMenuItem("Remove 20", 20, null));
				GearPopupMenu.this.add(createRemoveMenuItem("Remove  n", -1, null));
				this.addSeparator();

				GearPopupMenu.this.add(createSellMenuItem("Sell  1", 1, null));
				GearPopupMenu.this.add(createSellMenuItem("Sell  5", 5, null));
				GearPopupMenu.this.add(createSellMenuItem("Sell 10", 10, null));
				GearPopupMenu.this.add(createSellMenuItem("Sell 20", 20, null));
				GearPopupMenu.this.add(createSellMenuItem("Sell  n", -1, null));
				this.addSeparator();

				// FREQ 606205 -- sage_sam 11 Feb 2003
				GearPopupMenu.this.add(createSellRateMenuItem("Sell One at...", 1, null));
	...
	private void resortSelected(int sort, boolean sortOrder)
	{
		...
		ArrayList eqList = aPC.getEquipmentMasterList();
	...
	private class ResortComparator implements Comparator
	{
	...
	private void editChargesButtonClicked()
	{
		...
		try
		{
			...
			{
				JOptionPane.showMessageDialog(null, "Value out of range", Constants.s_APPNAME, JOptionPane.ERROR_MESSAGE);
				return;
			}
	...
	private Object getCurrentAvailableTableItem()
	{
		...
		if (row >= 0)
		{
			final TreePath treePath = availableTable.tree.getPathForRow(row);
	...
	private double removeSpecifiedEquipment(Equipment selectedEquipment, double qty)
	{
		...
		if (selectedEquipment.getHeaderChildCount() == 0)
		{
			...
			{
				if (!selectedEquipment.acceptsChildren() || ((sellQty % 1) == 0))
				{
	...
	/**
	 * This method causes the character to sell a number of the given equipment item
	 * in inventory at the rate specified in the rate combo box.
	...
	/**
	 * This method causes the character to sell all copies of the given equipment item
	 * in inventory at the rate specified at the time of the action.
	 * @param selectedEquipment Equipment to sell
	 *
	 * sage_sam 27 Feb 2003 for FREQ 606205
	 */
	...
	private void sellSpecifiedEquipmentRate(Equipment selectedEquipment, double qty)
	{
		...
		Object input = JOptionPane.showInputDialog(this, "Enter sell price percentage:", "Sell at Percent", JOptionPane.QUESTION_MESSAGE, null, null, defaultValue);
		if (input != null)
		{
			...
			{
				GuiFacade.showMessageDialog(this, "You must enter an integer value.", "Error", JOptionPane.ERROR_MESSAGE);
			}
		}
	}
	...
	/**
	 * This method causes the character to sell a number of the given equipment item
	 * in inventory at the rate specified.
	 * @param selectedEquipment Equipment to sell
	 * @param qty double number of the item to sell
	...
	/**
	 * This method causes the character to buy a number of the given equipment item
	 * in inventory at the rate specified in the rate combo box.
	 * @param selectedEquipment Equipment to sell
	...
	{
		Object defaultValue = cmbSellPercent.getSelectedItem().toString();
		Object input = JOptionPane.showInputDialog(this, "Enter buy price percentage:", "Buy at Percent", JOptionPane.QUESTION_MESSAGE, null, null, defaultValue);
		if (input != null)
		{
			String iString = input.toString().trim();
			try
	...
	private void buySpecifiedEquipmentRate(Equipment selectedEquipment, double qtyToBuy, int buyRate)
	{
		try
		{
			...
			if (selectedEquipment.acceptsChildren() && (buyQty % 1) != 0)
			{
				JOptionPane.showMessageDialog(null, "You cannot buy, own or carry non-integral numbers of containers\ni.e. Half a sack is nonsensical.", Constants.s_APPNAME, JOptionPane.ERROR_MESSAGE);
				return;
			}
			...
			{
				JOptionPane.showMessageDialog(null, "Insufficient funds for purchase of " + qtyToBuy + " " + selectedEquipment.getName(), Constants.s_APPNAME, JOptionPane.INFORMATION_MESSAGE);
			}
		}
		catch (Exception exc)
		{
	...
	private final class EquipmentModel extends AbstractTreeTableModel
	{
		...
		private void addItemToModel(Equipment aEq, boolean fireEvent)
		{
			...
			if (fireEvent)
			{
				...
				switch (currentMode)
				{
					...
					default:
						Globals.errorPrint("In InfoGear.EquipmentModel.addItemToModel the mode " + currentMode + " is not supported.");
						break;
			...
			switch (currentMode)
			{
				...
				default:
					Globals.errorPrint("In InfoGear.EquipmentModel.addItemToModel (second switch) the mode " + currentMode + " is not supported.");
					break;
		...
		private void resetModel(int mode, boolean available)
		{
			...
			switch (mode)
			{
				...
				default:
					Globals.errorPrint("In InfoGear.EquipmentModel.resetModel the mode " + mode + " is not supported.");
					break;
			...
			PObjectNode rootAsmyPONode = (PObjectNode) root;
			...
			if ((p.getChildCount() == 0) && (obj != null) && (obj instanceof Equipment) && ((Equipment) obj).equals(e))
			{
				...
				switch (column)
				{
					case COL_QTY_SRC:
						...
						break;
					default:
						Globals.errorPrint("In InfoGear.EquipmentModel.setValueForItemInNodes the column " + column + " is not supported.");
						break;
				}
			}
			else
		...
		public String getColumnName(int column)
		{
			switch (column)
			{
				...
				case COL_INDEX:
					return "Order";
				default:
					Globals.errorPrint("In InfoGear.EquipmentModel.getColumnName the column " + column + " is not supported.");
					break;
			}
			return "";
		}
...
//					return Float.class;
				...
				default:
					Globals.errorPrint("In InfoGear.EquipmentModel.getColumnClass the column " + column + " is not supported.");
					break;
			}
			return String.class;
		}
		...
		public Object getValueAt(Object node, int column)
		{
			...
			switch (column)
			{
				case COL_NAME: // Name
					...
					{
						Globals.errorPrint("Somehow we have no active node when doing getValueAt in InfoGear.");
						return "";
					}
				case COL_COST: // Cost
					if (aEq != null)
				...
				case -1:
					...
					break;
				default:
					Globals.errorPrint("In InfoGear.EquipmentModel.getValueAt the column " + column + " is not supported.");
					break;
		...
		public void setValueAt(Object value, Object node, int column)
		{
			...
			switch (column)
			{
				...
				case COL_INDEX:
					...
					break;
				default:
					Globals.errorPrint("In InfoGear.EquipmentModel.setValueAt the column " + column + " is not supported.");
					break;
