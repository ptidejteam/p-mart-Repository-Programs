...
// Consider replacing ResetableIterator with ResetableListIterator, if any
// classes look like they want to be bidirectional.

public class PObjectNode implements Cloneable, ResetableIterator
{
	...
	// All this is for free when we get rid of PObjectNode[] for a Collection.  XXX
	private int mark = 0;
		...
		if (item instanceof PObject)
		{
			...
			if (checkFeatState)
			{
				...
				switch (aFeat.getFeatType())
				{
					...
					default:
						return "|" + Color.red.getRGB() + "|" + itemName;
				}
	...
	void setChildren(PObjectNode[] newChildren, boolean generateEvent)
	{
	...
	public void addChild(PObjectNode aChild)
	{
		...
		/* This is GROSSLY inefficient; replace with a
		 * standard data structure which does the memory
		 * handling for you.  XXX */
	...
	private void addChild(PObjectNode aChild, int index)
	{
		...
		/* This is GROSSLY inefficient; replace with a
		 * standard data structure which does the memory
		 * handling for you.  And duplicative.  XXX */
	...
	private boolean removeChild(PObjectNode aChild)
	{
		...
		// Bizarre -- they can mismatch and it still removes.
		// XXX
