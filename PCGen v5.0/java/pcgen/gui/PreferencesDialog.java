	...
	private void applyOptionValuesToControls()
	{
		...
		switch (SettingsHandler.getTabPlacement())
		{
			...
			case SwingConstants.RIGHT:
				...
				break;
			default:
				Globals.errorPrint("In PreferencesDialog.applyOptionValuesToControls (tab placement) the tab option "
		...
		switch (SettingsHandler.getChaTabPlacement())
		{
			...
			case SwingConstants.RIGHT:
				...
				break;
			default:
				Globals.errorPrint("In PreferencesDialog.applyOptionValuesToControls (cha tab placement) the tab option "
		...
		switch (SettingsHandler.getNameDisplayStyle())
		{
			...
			case Constants.DISPLAY_STYLE_NAME_RACE_CLASS:
				...
				break;
			default:
				Globals.errorPrint("In PreferencesDialog.applyOptionValuesToControls (name display style) the tab option "
		...
		switch (Globals.getSourceDisplay())
		{
			...
			case Constants.SOURCEWEB:
				...
				break;
			default:
				Globals.errorPrint("In PreferencesDialog.applyOptionValuesToControls (source display) the option "
	...
	private void setOptionsBasedOnControls()
	{
		...
		switch (mainTabPlacementCombo.getSelectedIndex())
		{
			...
			case 3:
				...
				break;
			default:
				Globals.errorPrint("In PreferencesDialog.setOptionsBasedOnControls (mainTabPlacementCombo) the index "
		...
		switch (charTabPlacementCombo.getSelectedIndex())
		{
			...
			case 3:
				...
				break;
			default:
				Globals.errorPrint("In PreferencesDialog.setOptionsBasedOnControls (charTabPlacementCombo) the index "
		...
		switch (tabLabelsCombo.getSelectedIndex())
		{
			...
			case 3:
				...
				break;
			default:
				Globals.errorPrint("In PreferencesDialog.setOptionsBasedOnControls (tabLabelsCombo) the index "
		...
		else if (skinnedLookFeel.isSelected())
		{
			...
			else
			{
				...
				//JOptionPane.showMessageDialog(null, SkinLFResourceChecker.getMissingResourceMessage() + missingLibMsg, Constants.s_APPNAME, JOptionPane.WARNING_MESSAGE);
		...
		SettingsHandler.setBrowserPath( new File(browserPath.getText()) ); 
		...
		switch (sourceOptions.getSelectedIndex())
		{
			...
			case 3:
				...
				break;
			default:
				Globals.errorPrint("In PreferencesDialog.setOptionsBasedOnControls (sourceOptions) the index "
		...
		{ 
			...
			{ 
				...
				{ 
					...
					dialogOpened=true; 
					GuiFacade.showMessageDialog(PreferencesDialog.this, "File does not exist; preferences were not set.", "Invalid Path", JOptionPane.ERROR_MESSAGE); 
	...
	private final class PrefsButtonListener implements ActionListener
	{
		public void actionPerformed(ActionEvent actionEvent)
		{
			...
			else if (source == pcgenFilesDirButton)
			{
				...
				final JTextField textField = pcgenFilesDir;
				final File returnFile = askForPath(currentPath, dialogTitle, textField);
			}
				...
				if (fc.showOpenDialog(getParent()) == JFileChooser.APPROVE_OPTION)
				{
					...
					else
					{
						if (newTemplate.getName().startsWith("csheet"))
						{
							...
							SettingsHandler.setTemplatePath(newTemplate.getParentFile().getParentFile().getParentFile().getParentFile());
						}
						else
						{
							//it must be a psheet
	...
	private JPanel buildSourcesPanel()
	{
		...
		loadURL.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent evt)
			{
				...
				if (cb.isSelected())
				{
					GuiFacade.showMessageDialog(null, PropertyFactory.getString("in_Prefs_urlBlocked"), Constants.s_APPNAME, GuiFacade.INFORMATION_MESSAGE);
				}
			}
		});
