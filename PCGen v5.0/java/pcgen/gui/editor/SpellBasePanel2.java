 ...
 *
 * @(#) $Id: SpellBasePanel2.java,v 1.1 2006/02/21 01:07:44 vauchers Exp $
 */
...
public class SpellBasePanel2 extends JPanel implements PObjectUpdater
{
	...
	private void initComponents()
	{
		...
		try
		{
			btnAddVariant = new JButton(new ImageIcon(Globals.getRootFrame().getClass().getResource("resource/Forward16.gif")));
			btnRemoveVariant = new JButton(new ImageIcon(Globals.getRootFrame().getClass().getResource("resource/Back16.gif")));
		}
		catch (Exception exc)
		{
	...
	public void updateView(PObject thisPObject)
	{
		...
		final ArrayList variants = ((Spell) thisPObject).getVariants();
		if (variants.size() != 0)
		{
		...
		for (e = Globals.getSpellMap().values().iterator(); e.hasNext();)
		{
			...
			if (obj instanceof ArrayList)
			{
				...
				{
					EditUtil.addPObjectTypes((Spell) ((ArrayList) obj).get(i), availableList);
				}
			}
			else
			{
	...
	public void updateData(PObject thisPObject)
	{
		...
		s.setVariants(".CLEAR");
		s.setVariants(EditUtil.delimitArray(sel, '|'));
	}
