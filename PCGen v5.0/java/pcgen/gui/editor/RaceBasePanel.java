 ...
 *
 * @(#) $Id: RaceBasePanel.java,v 1.1 2006/02/21 01:07:44 vauchers Exp $
 */
...
public class RaceBasePanel extends BasePanel
{
	...
	private JComboBox cmbCR;
	private JTextField txtDisplayName;
	private JComboBox cmbHands;
	private JTextField txtHitDiceAdvancement;
	private JComboBox cmbHitDiceNumber;
	private JComboBox cmbHitDiceSize;
	private JComboBox cmbLegs;
	private JTextField txtLevelAdj;
	private JComboBox cmbMonsterClass;
	private JComboBox cmbMonsterLevel;
	private JComboBox cmbReach;
	private JComboBox cmbSize;
	private JComboBox cmbSkillMult;
...
//	private AvailableSelectedPanel pnlTemplateTypes;
	...
	private void initComponents()
	{
		...
		cmbHitDiceNumber = new JComboBox();
		cmbHitDiceSize = new JComboBox();
		cmbLegs = new JComboBox();
		txtLevelAdj = new JTextField();
		cmbMonsterClass = new JComboBox();
		cmbMonsterLevel = new JComboBox();
		cmbReach = new JComboBox();
		cmbSize = new JComboBox();
		cmbSkillMult = new JComboBox();
	...
	public int getCR()
	{
		...
		try
		{
			final String txtCR = cmbCR.getSelectedItem().toString();
			if (txtCR == null)
			{
				return 0;
			}
	...
	public void setHitDiceAdvancement(
		final int[] advancement,
		final boolean isAdvancementUnlimited)
	{

		if (advancement == null || (advancement.length == 0))
		{
		...
		else
		{
			...
			for (int index = 0; index < advancement.length; index++)
			{
				if (index > 0)
				{
					adv.append(',');
				}
				if (advancement[index] == -1 && isAdvancementUnlimited)
				{
					adv.append('*');
				}
				else
				{
					adv.append(String.valueOf(advancement[index]));
				}
			}
	...
	public void updateView(PObject thisPObject)
	{
		...
		setHitDiceAdvancement(thisRace.getHitDiceAdvancement(), thisRace.isAdvancementUnlimited());
		setLegs(thisRace.getLegs());
		setLevelAdjustment(thisRace.getLevelAdjustmentFormula());
		setMonsterClass(thisRace.getMonsterClass(false));
		setMonsterLevel(thisRace.getMonsterClassLevels(false));
		setRaceSize(thisRace.getSize());
		setReach(thisRace.getReach());
		setSkillMultiplier(thisRace.getInitialSkillMultiplier());
		setHitDiceNumber(thisRace.hitDice(false));
		setHitDiceSize(thisRace.getHitDiceSize(false));
	}
