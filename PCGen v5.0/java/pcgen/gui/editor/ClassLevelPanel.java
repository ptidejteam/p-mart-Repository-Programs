 ...
 *
 * @(#) $Id: ClassLevelPanel.java,v 1.1 2006/02/21 01:07:44 vauchers Exp $
 */
...
import pcgen.gui.JTableEx;
import pcgen.gui.TableSorter;
			...
			public void actionPerformed(ActionEvent evt)
			{
				...
				if (x >= 0 && x < levelTagList.size())
				{
					...
					{ // this is a pre-existing one that needs to be removed
						...
						Globals.errorPrint("This tag " + lt.getTag() + ":" + lt.getValue() + " needs to be hand-deleted from customClasses.lst");
						return;
					}
	...
	public void updateView(PObject po)
	{
		...
		if (aList != null)
		{
			for (Iterator i = aList.iterator(); i.hasNext();)
			{
				...
				LevelTag lt = new LevelTag(la.level(), "ADD", la.getList());
				levelTagList.add(lt);
			}
		}
		...
		if (aList != null)
		{
			for (Iterator i = aList.iterator(); i.hasNext();)
			{
				...
				LevelTag lt = new LevelTag(l, "ADDDOMAINS", t);
				levelTagList.add(lt);
			}
		}
		for (int x = 0; x < obj.getBonusList().size(); ++x)
		{
			...
			int y = c.indexOf('|');
			LevelTag lt = new LevelTag(c.substring(0, y), "BONUS", c.substring(y + 1));
			levelTagList.add(lt);
		}
		for (int x = 0; x < obj.getVariableCount(); ++x)
		{
			...
			int y = c.indexOf('|');
			LevelTag lt = new LevelTag(c.substring(0, y), "DEFINE", c.substring(y + 1));
			levelTagList.add(lt);
		}
		for (int x = 0; x < obj.getCastList().size(); ++x)
		{
			...
			if (!c.equals("0"))
			{
				LevelTag lt = new LevelTag(x + 1, "CAST", c);
				levelTagList.add(lt);
			}
		}
		...
		for (int x = 0; x < oList.length; ++x)
		{
			...
			if (!c.equals("0"))
			{
				LevelTag lt = new LevelTag(x + 1, "KNOWN", c);
				levelTagList.add(lt);
			}
		}
		...
		while (flag)
		{
			...
			if (src != null)
			{
				...
				LevelTag lt = new LevelTag(src, "SPELL", lev);
				levelTagList.add(lt);
			}
			else
			{
		...
		while (flag)
		{
			...
			if (src != null)
			{
				...
				String lev = src.substring(y + 1);
				src = src.substring(0, y);
				LevelTag lt = new LevelTag(src, "DR", lev);
				levelTagList.add(lt);
			}
			else
			{
		...
		if ((aList != null) && (aList.size() != 0))
		{
			for (Iterator se = aList.iterator(); se.hasNext();)
			{
				...
				LevelTag lt = new LevelTag(lev, "SA", sa.toString());
				levelTagList.add(lt);
			}
		}
		...
		while (flag)
		{
			...
			if (src != null)
			{
				...
				String lev = src.substring(y + 1);
				src = src.substring(0, y);
				LevelTag lt = new LevelTag(src, "SR", lev);
				levelTagList.add(lt);
			}
			else
			{
		...
		if ((aList != null) && (aList.size() != 0))
		{
			for (Iterator se = aList.iterator(); se.hasNext();)
			{
				...
				int y = c.indexOf(':');
				LevelTag lt = new LevelTag(c.substring(0, y), "FEAT", c.substring(y + 1));
				levelTagList.add(lt);
			}
		}
		...
		if (aCol != null)
		{
			for (Iterator se = aCol.iterator(); se.hasNext();)
			{
				...
				try
				{
					LevelTag lt = new LevelTag(c.substring(0, y), "FEATAUTO", c.substring(y + 1));
					levelTagList.add(lt);
				}
				catch (Exception exc)
				{
					Globals.errorPrint("Unrecognized FEATAUTO format:" + c, exc);
				}
			}
		}
		aCol = obj.vFeatList();
		if (aCol != null)
		{
			for (Iterator se = aCol.iterator(); se.hasNext();)
			{
				String c = (String) se.next();
				int y = c.indexOf(':');
				LevelTag lt = new LevelTag(c.substring(0, y), "VFEAT", c.substring(y + 1));
				levelTagList.add(lt);
			}
		}
...
//				s = s.substring(y + 1);
//			}
//			LevelTag lt = new LevelTag(l, "KIT", s);
//			levelTagList.add(lt);
//		}
		...
		if (s != null && !s.equals(""))
		{
			...
			LevelTag lt = new LevelTag(l, "REGION", s);
			levelTagList.add(lt);
		}
		for (int x = 0; x < obj.getTemplates().size(); ++x)
		{
			...
			int y = c.indexOf('|');
			LevelTag lt = new LevelTag(c.substring(0, y), "TEMPLATE", c.substring(y + 1));
			levelTagList.add(lt);
		}
		...
		if ((aList != null) && (aList.size() != 0))
		{
			for (Iterator se = aList.iterator(); se.hasNext();)
			{
				String c = (String) se.next();
				int y = c.indexOf('|');
				LevelTag lt = new LevelTag(c.substring(0, y), "UMULT", c.substring(y + 1));
				levelTagList.add(lt);
			}
		}
		...
		if (aList != null)
		{
			for (int x = 0; x < aList.size(); ++x)
			{
				...
				if (!c.equals(""))
				{
					LevelTag lt = new LevelTag(x + 1, "UDAM", c);
					levelTagList.add(lt);
				}
			}
		}
	...
	public void updateData(PObject po)
	{
		...
		try
		{
			URL a = new URL(obj.getSourceFile());
		...
		{
			Globals.errorPrint(exc.getMessage());
		}
	}
		...
		public Object getValueAt(final int rowIndex, final int columnIndex)
		{
			if ((rowIndex >= 0) && (rowIndex < ClassLevelPanel.levelTagList.size()))
			{
				...
				switch (columnIndex)
				{
					...
					default:
						Globals.errorPrint("In ClassLevelPanel.LevelModel.getValueAt the column " + columnIndex + " is not supported.");
						break;
				}
			}
			return null;
	...
	static class ListFrame extends JDialog
	{
		...
		public ListFrame(String title, ArrayList aList)
		{
			super(Globals.getRootFrame(), title, true);
			initComponents();
