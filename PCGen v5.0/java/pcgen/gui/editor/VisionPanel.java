 ...
 *
 * @(#) $Id: VisionPanel.java,v 1.1 2006/02/21 01:07:44 vauchers Exp $
 */
...
final class VisionPanel extends JPanel
{
	...
	private JComboBox cmbVisionType;
	...
	private void initComponents()
	{
		...
		cmbVisionType = new JComboBox(new String[]{"Normal", "Low-light", "Keen Low-light", "Darkvision", "Blindsight", "Keen Scent", "Tremorsense"});
	...
	private void btnAddActionPerformed()
	{
		...
		{
			JOptionPane.showMessageDialog(null, PropertyFactory.getString("in_demVisionInfoMissing"), Constants.s_APPNAME, JOptionPane.ERROR_MESSAGE);
			return;
		}
	 ...
	 *
	 * @param argSelected An arraylist of string vision values.
	 */
	...
	/**
	 * Split a single vision string into an array of its components - the vision type,
	 * and vision distance
	 *
	 * @param visionString The string to be converted
	 * @return A String[3] array containing the elements of the vision string.
	 */
	static String[] splitVisionString(String visionString)
	{
		...
		if (idx >= 0)
		{
			...
			returnValue[0] = visionString.substring(0, idx);
			if (visionString.charAt(idx + 1) == '*' || visionString.charAt(idx + 1) == '/')
			{
				returnValue[1] = visionString.substring(idx + 1, idx + 2);
				returnValue[2] = visionString.substring(idx + 2);
			}
			else
			{
				returnValue[1] = "";
				returnValue[2] = visionString.substring(idx + 1);
			}
		}
