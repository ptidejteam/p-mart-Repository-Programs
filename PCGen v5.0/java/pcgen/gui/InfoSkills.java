...
import pcgen.util.ResetableIterator;
...
class InfoSkills extends FilterAdapterPanel
{
	 ...
	 **/
	public final class SkillWrapper
	{
		...
		private final Skill getSkWrapSkill()
		{
			return _aSkill;
		}

		private final Integer getSkWrapMod()
		{
			return _mod;
		}

		private final Float getSkWrapRank()
		{
			return _ranks;
		}

		private final Integer getSkWrapOutputIndex()
		{
			return _outputIndex;
		}
	}
		...
		private class ResetSkillActionListener extends SkillActionListener
		{
			...
			public void actionPerformed(ActionEvent evt)
			{
				...
				if (aSkill != null)
				{
					...
					//
					if ((aSkill.getRank().doubleValue() == 0.0) && !(aSkill.getUntrained().charAt(0) == 'Y'))
					{
	...
	private void initComponents()
	{
		...
		selectedTable.getColumnModel().getColumn(COL_SRC).setCellRenderer(new OutputOrderRenderer(new String[]{"Hidden"}));
	 ...
	 * eg. lvl 1 class a, lvl2 class b, lvl 3 class a. If all points for level 1
	 * have been spent, but points for levels 2 and 3 remain to be spent, class a
	 * will erroneously be selected. This can only be corrected one skill points
	 * are tracked by PCLevelInfo
	 */
	private void setCurrentClassCombo()
	{
		boolean foundClass = false;
		PCClass aClass = null;
		// Search for a class with remaining points. Search is done in the order
		// levels are assigned to hopefully get the earliest class with remianing points.
		for (int idx = 0; idx < aPC.getLevelInfoSize(); ++idx)
		{
			aClass = aPC.getClassKeyed(aPC.getLevelInfoClassKeyName(idx));
			if ((aClass != null) && (aClass.skillPool().intValue() > 0))
			{
		...
		// If no points remain to be spent, foucs on the last class to be leveled up.
		if (!foundClass)
		{
...
//				asplit.setDividerLocation((int)(InfoSkills.this.getSize().getWidth() - 408));
	...
	private void skillChoiceActionPerformed()
	{
		...
		if (selection >= 0 && selection <= 2 && selection != oldSelection)
		{
		...
		availableTable.getSelectionModel().addListSelectionListener(new ListSelectionListener()
		{
			public void valueChanged(ListSelectionEvent e)
			{
				if (!e.getValueIsAdjusting())
				{
					...
					{
						lastSkill = null;
						JOptionPane.showMessageDialog(null, "No skill selected. Try again.", Constants.s_APPNAME, JOptionPane.ERROR_MESSAGE);
						return;
					}
					...
					else if (e.getClickCount() == 2)
					{
						...
						SwingUtilities.invokeLater(new Runnable()
						{
							public void run()
							{
								...
								if (theSkill != null)
								{
									if (theSkill.getChoiceList() == null || theSkill.getChoiceList().size() == 0)
									{
										addSkill(-1);
									}
									else
	...
	private boolean modRank(Skill aSkill, double points)
	{
		...
		if (bSkill != null)
		{
			...
			//
			if ((bSkill.getRank().doubleValue() == 0.0) && !(bSkill.getUntrained().charAt(0) == 'Y'))
			{
...
//		}
	...
	private final class OutputOrderRenderer extends DefaultTableCellRenderer
	{
		...
		private OutputOrderRenderer(String[] choices)
		{
			super();
	...
	private final class SkillModel extends AbstractTreeTableModel
	{
		 ...
		 * items ignore any skill the PC doesn't qualify
		 * for.
		 **/
		private class DisplayableSkillsIterator implements ResetableIterator
		{
			...
			public void reset()
			{
				it = available ? Globals.getSkillList().iterator() : aPC.getSkillList().iterator();
				peek = null;
			}
		}
		 ...
		 **/
		private void populateNode(PObjectNode node, ResetableIterator skillsIt, InfoSkillsSorter sorter, boolean available)
		{
			final TreeSet set = new TreeSet(new PObjectNode.Comparator());
			...
			while (skillsIt.hasNext())
			{
				...
				else
				{
					...
					nameNode.setIsValid(skill.passesPreReqTests());
					set.add(nameNode);
				}
			}
		 ...
		 **/
		private void createRootNode(PObjectNode node, ResetableIterator skillsIt, InfoSkillsSorter sorter, boolean available)
		{
		 ...
		 * included in the list.  Only Weapon, Armor and
		 * Shield type items should be checked for
		 * proficiency.
		 */
