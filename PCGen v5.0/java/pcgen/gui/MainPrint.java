...
//		buttonPanel.add(templatePathButton);
	...
	private void export()
	{
		...
		fcExport.setCurrentDirectory(new File(SettingsHandler.getPDFOutputSheetPath()));
		if (!partyMode)
		{
		...
		for (int loop = 0; loop < pcExports.length; loop++)
		{
			...
			try
			{
				...
				if (((String) templateList.getSelectedValue()).endsWith(".xslt"))
				{
					...
					File template = new File(SettingsHandler.getTemplatePath() + File.separator + (String) templateList.getSelectedValue());
	...
	private void print()
	{
		...
		for (int loop = 0; loop < pcExports.length; loop++)
		{
			...
			try
			{
				 ...
				 */
				File tmpFile = null;
				System.out.println((String) templateList.getSelectedValue());
				if (((String) templateList.getSelectedValue()).endsWith(".xslt"))
				{
					System.out.println("Printing using XML/XSLT");
					tmpFile = File.createTempFile("currentPC_", ".xml");
					printToXMLFile(tmpFile, pcExports[loop]);
					File template = new File(SettingsHandler.getTemplatePath() + File.separator + (String) templateList.getSelectedValue());
					fh.setInputFile(tmpFile, template);
				...
				{
					System.out.println("Printing using FO sheets");
					tmpFile = File.createTempFile("currentPC_", ".fo");
					printToFile(tmpFile, pcExports[loop]);
					fh.setInputFile(tmpFile);
				}
...
//  				unblock();
			...
			{
				Globals.errorPrint("Could not print " + pcName, ex);
				JOptionPane.showMessageDialog(null, "Could not print " + pcName + ". Try again.", "PCGen", JOptionPane.ERROR_MESSAGE);
			}
			unblock();
		}
...
//  			aPC.print(template, bw);
	...
	{	// In XML file mode, there is no such thing as party mode.
		...
		//final BufferedWriter bw = new BufferedWriter(new FileWriter(outFile));
		final BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(outFile), "UTF-8"));
		final File template = new File(Globals.getDefaultPath() + File.separator + "outputsheets" + File.separator + "base.xml");
		SettingsHandler.setSelectedCharacterPDFOutputSheet(template.getAbsolutePath());
		final PlayerCharacter oldPC = Globals.getCurrentPC();
		PlayerCharacter aPC;
		if (partyMode)
	...
	private class ButtonListener implements ActionListener
	{
		public void actionPerformed(ActionEvent e)
		{
			...
			else if (src.equals(printButton))
			{
				if (templateList.getSelectedValue() == null)
				{
					JOptionPane.showMessageDialog(null, PropertyFactory.getString("in_printNoTemplate"), "PCGen", JOptionPane.ERROR_MESSAGE);
				}
				else
				{
					(new Thread(new Runnable()
