...
public class PCGen_Frame1 extends JFrame
{
	...
	private void jbInit() throws Exception
	{
		...
		if (game.getTabShown(Constants.TAB_SOURCES))
		{
	...
	private void handleQuit()
	{
		...
		// writing of custom files done when editor dialog closed
//		CustomData.writeCustomFiles();
		// However, still need to (possibly) write customEquipment.lst
	...
	private boolean getUserChoice()
	{
		...
		{
			Globals.errorPrint("Error in PCGen_Frame1::getUserChoice", e);
		}
		return false;
	}
	...
	private void doNewItem()
	{
		if (!Globals.displayListsHappy())
		{
			JOptionPane.showMessageDialog(null, PropertyFactory.getString("in_newCharNoSources"), Constants.s_APPNAME, JOptionPane.ERROR_MESSAGE);
			return;
		}
	...
	boolean loadPartyFromFile(File file)
	{
		...
		else
		{
			...
			Globals.errorPrint("PCGen_Frame1: Error in loadPartyFromFile");
			return false;
		}
	...
	private void partySaveItem(boolean saveas)
	{
		...
		if (saveas || newParty)
		{
			...
			if (returnVal == JFileChooser.APPROVE_OPTION)
			{
				...
				if (file.isDirectory())
				{
					JOptionPane.showMessageDialog(null, "You cannot overwrite a directory with a party.", Constants.s_APPNAME, JOptionPane.ERROR_MESSAGE);
					return;
				}
		...
		{
			JOptionPane.showMessageDialog(null, "Could not save " + partyFileName, Constants.s_APPNAME, JOptionPane.ERROR_MESSAGE);
	...
	void revertToSavedPopupItem_actionPerformed()
	{
		...
		if (aPC.getLoadCompanion() && !aPC.getFollowerList().isEmpty())
		{
			for (Iterator aF = aPC.getFollowerList().iterator(); aF.hasNext();)
			{
				...
				if (!aLoaded)
				{
					...
					{
						//todo: i18n these messages
						JOptionPane.showMessageDialog(null, "Unrecoverable problems occurred while loading a companion or follower.", "Error", JOptionPane.ERROR_MESSAGE);
					}
				}
			}
		}
	...
	private boolean closePCTabAt(int index, boolean isClosing)
	{
		...
		if ((aPC != null) && (aPC.isDirty()))
		{
			...
			if (isClosing)
			{
				reallyClose = JOptionPane.showConfirmDialog(this, aPC.getDisplayName() + " changed.  Save changes before closing?", "Save " + aPC.getDisplayName() + " before closing?", JOptionPane.YES_NO_CANCEL_OPTION);
			}
			else // reverting
			{
				reallyClose = JOptionPane.showConfirmDialog(this, aPC.getDisplayName() + " changed.  Save changes before reverting?", "Save " + aPC.getDisplayName() + " before reverting?", JOptionPane.YES_NO_CANCEL_OPTION);
			}
		...
		if (bSave && (index >= FIRST_CHAR_TAB))
		{
			...
			//if (isClosing)
			//{
			//	menuBar.openRecentPCMenu.add(aPC.getDisplayName(), new File(aPC.getFileName()));
			//}
		}
	...
	{
		//JOptionPane.showMessageDialog(null, FOPResourceChecker.getMissingResourceMessage() + missingLibMsg, Constants.s_APPNAME, JOptionPane.WARNING_MESSAGE);
	...
	private void checkResources()
	{
		...
		{
			//JOptionPane.showMessageDialog(null, JEPResourceChecker.getMissingResourceMessage() + missingLibMsg, Constants.s_APPNAME, JOptionPane.WARNING_MESSAGE);
...
//			return (CharacterInfo)getBaseTabbedPane().getComponentAt(index);
	...
	final class MenuItems extends JMenuBar
	{
		...
		public MenuItems()
		{
			...
			newItem = Utility.createMenuItem("mnuFileNew", frameActionListener.newActionListener, "file.new", "control N", "New16.gif", false);
			fileMenu.add(newItem);

			openItem = Utility.createMenuItem("mnuFileOpen", frameActionListener.openActionListener, "file.open", "control O", "Open16.gif", true);
			...
			closeItem = Utility.createMenuItem("mnuFileClose", frameActionListener.closeActionListener, "file.close", "control W", "Close16.gif", false);
			...
			saveItem = Utility.createMenuItem("mnuFileSave", frameActionListener.saveActionListener, "file.save", "control S", "Save16.gif", false);
			fileMenu.add(saveItem);

			saveAsItem = Utility.createMenuItem("mnuFileSaveAs", frameActionListener.saveAsActionListener, "file.saveas", "shift control S", "SaveAs16.gif", false);
			...
			revertToSavedItem = Utility.createMenuItem("mnuFileRevertToSaved", frameActionListener.revertToSavedActionListener, "file.reverttosaved", "control R", null, false);
			...
			printItem = Utility.createMenuItem("mnuFilePrint", frameActionListener.printActionListener, "file.print", "control P", "Print16.gif", false);
			...
			fileMenu.addSeparator();
			addKit = Utility.createMenuItem("mnuFileAddKit", frameActionListener.addKitActionListener, "assign.kit", "control K", "Information16.gif", false);
			fileMenu.add(addKit);
		...
		private final class DocsFrame extends JFrame
		{
			public DocsFrame()
			{
				...
				{
					JOptionPane.showMessageDialog(null, "Could not open docs in external browser. " + "Have you set your default browser in the " + "Preference menu? Sorry...", Constants.s_APPNAME, JOptionPane.ERROR_MESSAGE);
