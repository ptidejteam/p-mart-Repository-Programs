...
public final class CampaignOutput
{
	...
	{
		File outFile = new File(SettingsHandler.getPccFilesLocation().getAbsolutePath() + File.separator + campaign.getDestination());
		...
		try
		{
			...
			FileAccess.newLine(out);
			FileAccess.write(out, "LICENSED:" + (campaign.isLicensed() ? "YES" : "NO"));
			FileAccess.newLine(out);
			...
			FileAccess.newLine(out);
			ArrayList aList = campaign.getOptionsList();
		...
		{
			Globals.errorPrint("Error while writing to " + outFile.toString(), exc);
		...
		finally
		{
			...
			{
				Globals.errorPrint("Can't close " + outFile.toString(), ex); //Not much more to do really...
			}
