 ...
 *
 * $Id: SpellLoader.java,v 1.1 2006/02/21 01:07:48 vauchers Exp $
 */
...
final class SpellLoader
{
	...
	public static void parseLine(Spell obj, String inputLine, URL sourceURL, int lineNum) throws PersistenceLayerException
	{
		int i = 0;
		...
		while (colToken.hasMoreElements())
		{
			...
			{
				Globals.errorPrint("EFFECTS: tag deprecated - use DESC: instead in " + sourceURL.toString());
				obj.setDescription(colString.substring(8));
			}
			else if (colString.startsWith("EFFECTTYPE:"))
			{
				Globals.errorPrint("EFFECTTYPE: tag deprecated - use TARGETAREA: instead in " + sourceURL.toString());
