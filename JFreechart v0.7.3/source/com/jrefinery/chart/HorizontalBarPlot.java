 ...
 * Contributor(s):   -;
 *
 * $Id: HorizontalBarPlot.java,v 1.1 2007/10/10 18:57:06 vauchers Exp $
 *
...
//	    usable = usable - ((seriesCount-1) * categoryCount * seriesGap);
	...
	if (data!=null) {
            if (this.renderer.isStacked()) {
                result = Datasets.getMinimumStackedRangeValue(data);
            }
            else {
                result = Datasets.getMinimumRangeValue(data);
            }
        }
	...
	if (data!=null) {
            if (this.renderer.isStacked()) {
                result = Datasets.getMaximumStackedRangeValue(data);
            }
            else {
                result = Datasets.getMaximumRangeValue(data);
            }
	}
