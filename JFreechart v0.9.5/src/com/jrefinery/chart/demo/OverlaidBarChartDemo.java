 ...
 * Contributor(s):   -;
 *
 * $Id: OverlaidBarChartDemo.java,v 1.1 2007/10/10 19:54:11 vauchers Exp $
 *
 * Changes
 * -------
...
public class OverlaidBarChartDemo extends ApplicationFrame {
    ...
    public OverlaidBarChartDemo(String title) {
        ...
        CategoryDataset dataset1 = DatasetUtilities.createCategoryDataset("S", "Category", 
                                                                          data1);
        ...
        List keys = dataset1.getColumnKeys();
        Comparable[] categories = new Comparable[keys.size()];
        for (int i = 0; i < keys.size(); i++) {
            categories[i] = keys.get(i).toString();
        }
        OverlaidVerticalCategoryPlot plot 
            = new OverlaidVerticalCategoryPlot("Category", "Value", categories);
        plot.add(plot1);
        plot.add(plot2);
