/**
 * AttributeField - A Component which receives InputEvents and/or renders attribute value in a Document
 *
 * Copyright (c) 2005
 ...
 *  <li>output only - set the <code>id</code> property and the 
 *      <code>attributeName</code> (note: default attributeName is "value") 
 *      property.</li>
 *  <li>input and output - set the <code>parameter</code>, <code>id</code> 
 *  and <code>attributeName</code> properties.</li>
 * </ul>
 *
 * <p>When used for output, the value is rendered in the XML document element as a
 * attribute by default. You can also render values to the additional element attributes.
...
public class AttributeField extends AbstractField {
  ...
  public AttributeField(int javaDataType) {
    super(javaDataType);
    attribute = createAttribute("value", javaDataType);
    attribute.setModel(this.getModel());
    renderer = createDefaultRenderer();
  }
   ...
   * By Default this method returns a new VisibleRenderer.
