/**
 * Form - Top level container for applicaions
 *
 * Copyright (c) 2002
...
import com.taursys.xml.event.CloseFormDispatcher;
import com.taursys.xml.event.CloseFormEvent;
import com.taursys.xml.event.CloseFormListener;
import com.taursys.xml.event.InitContextDispatcher;
import com.taursys.xml.event.InitContextEvent;
import com.taursys.xml.event.InitContextListener;
import com.taursys.xml.event.InitFormDispatcher;
import com.taursys.xml.event.InitFormEvent;
import com.taursys.xml.event.InitFormListener;
import com.taursys.xml.event.OpenFormDispatcher;
import com.taursys.xml.event.OpenFormEvent;
import com.taursys.xml.event.OpenFormListener;
import com.taursys.xml.render.DocumentElementRenderer;
import com.taursys.xml.render.FormRenderer;
...
public class Form extends DocumentElement {
  ...
  private InitContextDispatcher initContextDispatcher =
    new InitContextDispatcher(this);
  ...
  private CloseFormDispatcher closeFormDispatcher =
    new CloseFormDispatcher(this);
  ...
  private DocumentAdapterBuilder documentAdapterBuilder;
   ...
   * been dispatched.
   * Parses Document and Creates DocumentAdapter using DocumentAdapterBuilder.
   * This method sets the initialized flag to true.
  ...
  protected void initForm() throws Exception {
    if (documentAdapter == null && documentURI != null) {
      //TODO after refactor of URI/URL "resource" feature, change this to URL
      documentAdapter = getDocumentAdapterBuilder().build(documentURI);
    }
    initialized = true;
  }

  //TODO add an InitDocument event and move these methods there
  ...
  // =======================================================================
  //                 Document and Document Adapter Methods
  // =======================================================================

  /**
   * Get the documentURI which is used to create the Document
   * @return the documentURI which is used to create the Document
   * @see com.taursys.dom.DocumentAdapterBuilder#build(String) for URI syntax
   */
   ...
   * @param documentURI which is used to create the Document
   * @see com.taursys.dom.DocumentAdapterBuilder#build(String) for URI syntax
   */
  ...
  public void setDocument(org.w3c.dom.Document newDocument)
    ...
    documentAdapter =
      DOMDocumentAdapterFactory.getInstance().newDOMDocumentAdapter(
        newDocument);
  }
  ...
  //TODO add proxy method for import Document here also

  /**
   * Get the DocumentAdapterBuilder for this Form and create it if null.
   * @return the DocumentAdapterBuilder for this Form
   */
  public DocumentAdapterBuilder getDocumentAdapterBuilder()
    throws DocumentAdapterBuilderException {
    if (documentAdapterBuilder == null) {
      documentAdapterBuilder =
        DocumentAdapterBuilderFactory.getInstance().newDocumentAdapterBuilder();
    }
    return documentAdapterBuilder;
  }

  /**
   * Set the DocumentAdapterBuilder for this Form
   * @param builder the DocumentAdapterBuilder for this Form
   */
  public void setDocumentAdapterBuilder(DocumentAdapterBuilder builder) {
    documentAdapterBuilder = builder;
  }
