...
public class AttributeRenderer extends AbstractSubcomponentRenderer {
  ...
  public void render() throws RenderException {
    ...
    if (component.isVisible()) {
      try {
        da.setAttributeText(id,
            ((Attribute) component).getAttributeName(),
            ((Attribute) component).getModel().getText());
      } catch (ModelException ex) {
        throw new RenderException(RenderException.REASON_MODEL_EXCEPTION, ex);
      }
    } else {
      da.removeAttribute(id, ((Attribute) component).getAttributeName());
    }
  }
}
