...
public abstract class AbstractRenderer {
  ...
  protected void init() throws RenderException {
    ...
    parentDocumentAdapter = parent.getDocumentAdapter();
    if (parentDocumentAdapter == null)
      throw new RenderException(RenderException.REASON_DOCUMENT_IS_NULL);
    Document currentDoc = parentDocumentAdapter.getDocument();
    if (currentDoc == null)
      throw new RenderException(RenderException.REASON_DOCUMENT_IS_NULL);
    if (doc != currentDoc) {
      doc = currentDoc;
    ...
    if (componentNode == null) {
      id = ((DocumentComponent)component).getId();
      if (id != null) {
        componentNode = parentDocumentAdapter.getElementById(id);
        if (componentNode != null) {
  ...
  /**
   * Renders the Component by adding or removing it from the document based
   * on visible property and invokes renderContents if component is visible.
  ...
  /**
   * Invoked by render method if component's visible property is true, to
   * render the contents of this component.
   * @throws RenderException
   */
  protected abstract void renderContents() throws RenderException;
