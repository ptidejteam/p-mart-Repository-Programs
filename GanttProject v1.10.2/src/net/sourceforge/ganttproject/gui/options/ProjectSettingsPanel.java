...
public class ProjectSettingsPanel extends GeneralOptionPanel {
	...
	{
		super(GanttProject.correctLabel(GanttLanguage.getInstance().getText("project")),
			  GanttLanguage.getInstance().getText("settingsProject"),owner);
		
        myProject = project;
		Box vbproject = Box.createVerticalBox();

		/*JPanel filePanel = new JPanel(new BorderLayout());
		filePanel.add(new JLabel(language.getText("file")), BorderLayout.WEST);
		vbproject.add(filePanel);
		vbproject.add(tfFile = new JTextField());
	...
	{
		/*if(appli.projectDocument!=null) {
			String filename = appli.projectDocument.getFilePath();
			if(appli.askForSave)
				filename+="*";
