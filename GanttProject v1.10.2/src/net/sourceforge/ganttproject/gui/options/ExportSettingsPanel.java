...
public class ExportSettingsPanel extends GeneralOptionPanel {
	...
	{
		super(GanttProject.correctLabel(GanttLanguage.getInstance().getText("export")),
				  GanttLanguage.getInstance().getText("settingsExport"),parent);
		appli = parent;
...
//		export the 3D border of the task
		...
		bordersPanel.add(new JLabel(language.getText("border3D")), BorderLayout.CENTER);
		vb.add(bordersPanel);
		vb.add(new JPanel());	
	...
	public boolean applyChanges(boolean askForApply) {
		if(getExportName() == appli.getOptions().getExportName() && 
				getExportComplete() == appli.getOptions().getExportComplete() &&
				getExportRelations() == appli.getOptions().getExportRelations() && 
				getExport3DBorder() == appli.getOptions().getExport3dBorders())
		{
			bHasChange = false;
		}
		else
		{
			...
			{			
				...
				appli.getOptions().setExport3dBorders(getExport3DBorder());
			}
		}
		return bHasChange;
	}
