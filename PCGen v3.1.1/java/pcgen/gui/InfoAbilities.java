...
class InfoAbilities extends JPanel
{
	...
	//private JButton rollButton = new JButton("Reset");
...
*/
		...
		buildConstraints(gbc, 0, 2, 1, 1, 0.0, .5);
...
*/
		PCGen_Frame1.getStatusBar().setText("Click on Choose if you have any bonus languages to select.");
		updateCharacterInfo();
	}
		...
		if (checkPurchasePoints)
		{
			...
			if ((availablePool > 0) && (usedStatPool >= availablePool))
			{
				JOptionPane.showMessageDialog(null,
				  "You have reached or exceded the maximum points of " + availablePool + " as specified by the method \"" + SettingsHandler.getPurhaseModeMethodName() + "\"",
				  Constants.s_APPNAME,
				  JOptionPane.INFORMATION_MESSAGE);
			}
		}
