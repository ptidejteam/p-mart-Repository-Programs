...
*/
	...
	private void initComponents()
	{
		...
		poolPanel.add(new JLabel("Stat Cost:"));
  	...
  	final int availablePool = SettingsHandler.getPurchaseModeMethodPool();
		...
		if (SettingsHandler.isPurchaseStatMode())
		{
			if (checkPurchasePoints)
			{
				...
				{
					JOptionPane.showMessageDialog(null,
						"You have attempted to spend more than the maximum of " + availablePool + " points, as specified by the method \"" + SettingsHandler.getPurhaseModeMethodName() + "\"",
						Constants.s_APPNAME,
						JOptionPane.INFORMATION_MESSAGE);
					return false;
				}
...
*/
	...
	protected class StatTableModel extends AbstractTableModel
	{
		...
		public void setValueAt(Object obj, int rowIndex, int columnIndex)
		{
			...
			{
				((PCStat)aPC.getStatList().getStats().get(rowIndex)).setBaseScore(Integer.parseInt(obj.toString()));
				setStatLabelText();
				updatePool();
			}
		}
/*
		public void setValueAt(Object obj, int rowIndex, int columnIndex)
		{
			if (rowIndex>=0 && rowIndex<aPC.getStatList().getStats().size() && columnIndex==1)
			{
				...
				{
					JOptionPane.showMessageDialog(null,
						"You may not set a stat to above " + SettingsHandler.getPurchaseScoreMax() + ", as specified by the purchase mode method \"" + SettingsHandler.getPurhaseModeMethodName() + "\"",
						Constants.s_APPNAME,
						JOptionPane.INFORMATION_MESSAGE);
					return;
				}
				...
				{
					JOptionPane.showMessageDialog(null,
						"You may not set a stat to below " + SettingsHandler.getPurchaseScoreMin() + ", as specified by the purchase mode method \"" + SettingsHandler.getPurhaseModeMethodName() + "\"",
						Constants.s_APPNAME,
						JOptionPane.INFORMATION_MESSAGE);
					return;
				}
