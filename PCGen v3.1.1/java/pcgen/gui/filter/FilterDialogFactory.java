...
public class FilterDialogFactory implements FilterConstants
{
	 ...
	 */
	public static void setFilters(List availableFilters, List selectedFilters)
	{
	 ...
	 * for the selected (and therefore active) Filterable i.e. tab
	 *
	...
	public static String getSelectedFiltersToolTipText()
	{
		...
		if (f != null)
		{
			...
			{
				buffer.append("<br>Currently selected filters:");
				int mode = f.getFilterMode();
				if (mode == MATCH_ALL)
				{
					buffer.append("<br>Match all of");
				}
				else if (mode == MATCH_ALL_NEGATE)
				{
					buffer.append("<br>Match not all of");
				}
				else if (mode == MATCH_ANY)
				{
					buffer.append("<br>Match any of");
				}
				else if (mode == MATCH_ANY_NEGATE)
				{
					buffer.append("<br>Match not any of");
				}
				buffer.append("<ul>");
				for (Iterator it = f.getSelectedFilters().iterator(); it.hasNext();)
				{
					buffer.append("<li>");
					buffer.append(((PObjectFilter)it.next()).toString());
					buffer.append("</li>");
				}
				buffer.append("</ul>");
			}
			...
			{
				buffer.append("<br>Currently no selected filters.");
			}
		}
		else
		{
			buffer.append("<br>Currently active tab does not support filtering.");
		}
 ...
 * @author Thomas Behr
 */
class FilterSelectDialog extends JDialog implements ActionListener, FilterConstants
{
	...
	private static PObjectFilter displayOnlyFilter = new PObjectFilter()
	{
		...
		{
			return "Active tab does not support filtering.";
		}
		...
		public boolean accept(pcgen.core.PObject pObject)
		{
			return false;
		}
	};
	...
	{
		super(Globals.getRootFrame(), "Filter Options");
		init();
	}

	public FilterSelectDialog(java.awt.Dialog owner)
	{
		super(owner, "Filter Options");
		init();
	}

	public FilterSelectDialog(java.awt.Dialog owner, String title)
	{
		super(owner, title);
		init();
	}

	public FilterSelectDialog(java.awt.Dialog owner, String title, boolean modal)
	{
		super(owner, title, modal);
		init();
	}

	public FilterSelectDialog(java.awt.Dialog owner, boolean modal)
	{
		super(owner, "Filter Options", modal);
		init();
	}

	public FilterSelectDialog(java.awt.Frame owner)
	{
		super(owner, "Filter Options");
		init();
	}

	public FilterSelectDialog(java.awt.Frame owner, String title)
	{
		super(owner, title);
		init();
	}

	public FilterSelectDialog(java.awt.Frame owner, String title, boolean modal)
	{
		super(owner, title, modal);
		init();
	}

	public FilterSelectDialog(java.awt.Frame owner, boolean modal)
	{
		super(owner, "Filter Options", modal);
		init();
	}
...
//                  final Dimension buttonDimension = new Dimension(95, 27);
		...
		leftPanel.add(new JLabel("Available Filters: "), 0, 0, 1, 1);
...
//  		availableScroll.setPreferredSize(new Dimension(200, 295));
		...
		addButton = new JButton(new ImageIcon(getClass().getResource("/pcgen/gui/resource/Forward16.gif")));
		addButton.addActionListener(this);
		removeButton = new JButton(new ImageIcon(getClass().getResource("/pcgen/gui/resource/Back16.gif")));
		...
		rightPanel.add(new JLabel("Selected Filters: "), 0, 0, 1, 1);
...
//                  saveButton.setPreferredSize( buttonDimension );
		...
		// Option buttons
		matchAllRadio = new JRadioButton("Match All");
		matchAllRadio.setEnabled(true);
		matchAnyRadio = new JRadioButton("Match Any");
		matchAnyRadio.setEnabled(false);
		negateBox = new JCheckBox("Negate/Reverse");
...
//  		mainPanel.add(rightButtonPanel, 1, 2, 1, 1);
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		else if (src.equals(applyButton))
		{
			if (filterable != null)
			{
				storeFilters(filterable.getAvailableFilters(),
		...
		else if (src.equals(saveButton))
		{
			...
			final String operand = (matchAllRadio.isSelected()) ? AND : OR;

			if (selectedModel.size() < 1)
			{
				// show warning
				JOptionPane.showMessageDialog(null,
				  "No filters selected.\nNo custom filter will be created!",
				  "Warning",
			...
			else
			{
				filter = FilterFactory.createCompoundFilter((PObjectFilter)selectedModel.get(0),
				  (PObjectFilter)selectedModel.get(1),
				  operand);
				for (int i = 2; i < selectedModel.size(); i++)
				{
					filter = FilterFactory.createCompoundFilter(filter,
					  (PObjectFilter)selectedModel.get(i),
					  operand);
				}
			}
	...
	private void setMode(int mode)
	{
		switch (mode)
		{
			case SINGLE_SINGLE_MODE:
				activateSingleSingleMode();
				break;
			case SINGLE_MULTI_MODE:
				activateSingleMultiMode();
				break;
			case MULTI_MULTI_MODE:
...
//  		selectedModel.addElement("Active tab does not support filtering.");
	...
	private void settings(Filterable f)
	{
		...
		else
		{
			...
			if (f.getSelectionMode() < SINGLE_SINGLE_MODE)
			{
				return;
			}
			...
			setFilters(f.getAvailableFilters(),
	 ...
	 * <br>author: Thomas Behr 09-02-02
	 *
	 * @param availableFilters   an array of strings holding names of available filters;
	 *                           if availableFilters is null, then there will be
	 *                           no items in the available list
	 * @param selectedFilters    an array of strings holding names of selected filters;
	 *                           if selectedFilters is null, then there will be
	 *                           no items in the selected list
	 */
	public void setFilters(String[] availableFilters,
	  String[] selectedFilters)
	{
		...
		if (availableFilters != null)
		{
			...
			{
				availableModel.addElement(availableFilters[i]);
			}
		}
		if (selectedFilters != null)
		{
			...
			{
				selectedModel.addElement(selectedFilters[i]);
			}
		}
	}
	 ...
	 *                           no items in the selected list
	 */
	public void setFilters(List availableFilters,
	 ...
	 *                           previously stored data in this List will be lost
	 */
	private void storeFilters(List availableFilters,
	  List selectedFilters)
	{
		if (availableFilters != null)
		{
			availableFilters.clear();
			for (Enumeration enum = availableModel.elements(); enum.hasMoreElements();)
			{
				availableFilters.add(enum.nextElement());
			}
		}
		if (selectedFilters != null)
		{
			selectedFilters.clear();
			for (Enumeration enum = selectedModel.elements(); enum.hasMoreElements();)
			{
				selectedFilters.add(enum.nextElement());
			}
		}
	}
	...
	/**
	 * convenience method<br>
	 * returns the available (unselected) filters as a string array
	 *
	 * <br>author: Thomas Behr 09-02-02
	 *
	 * @return a PObjectFilter[] containing the available filter names
	 */
	public PObjectFilter[] getAvailableFilters()
	{
		PObjectFilter[] available = new PObjectFilter[availableModel.size()];
		for (int i = 0; i < available.length; i++)
		{
			available[i] = (PObjectFilter)availableModel.get(i);
		}
		return available;
	}
	...
	/**
	 * convenience method<br>
	 * returns the selected filters as a string array
	 *
	 * <br>author: Thomas Behr 09-02-02
	 *
	 * @return a PObjectFilter[] containing the selected filter names
	 */
	public PObjectFilter[] getSelectedFilters()
	{
		PObjectFilter[] selected = new PObjectFilter[selectedModel.size()];
		for (int i = 0; i < selected.length; i++)
		{
			selected[i] = (PObjectFilter)selectedModel.get(i);
		}
		return selected;
	}
	...
	{
		ArrayList illegalNames = new ArrayList();
...
class SortedListModel extends DefaultListModel
{
	...
	private int binarySearch(Object key)
	{
		...
		while (low <= high)
		{
			...
			int cmp = midVal.compareTo(keyString);
 ...
 * @author Thomas Behr
 */
class FilterList extends JList
{

	private static final Border NO_FOCUS_BORDER = BorderFactory.createEmptyBorder(1, 1, 1, 1);

	public FilterList(ListModel model)
	{
		super(model);
	...
	private class FilterListCellRenderer extends JLabel implements ListCellRenderer
	{
 ...
 * @author Thomas Behr
 */
class FilterCustomDialog extends JDialog implements ActionListener, FilterConstants
{
	private int maxSelected;
	...
	{
		super(Globals.getRootFrame(), "Filter customization");
		this.init();
	}
...
// right stuff
		rightPanel.gbc.weighty = 0;
		rightPanel.gbc.anchor = GridBagConstraints.NORTHWEST;
		rightPanel.add(new JLabel("Removed Filters: "), 0, 0, 1, 1);
...
// Removed List
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		{
			int[] indices = removedList.getSelectedIndices();
			for (int i = 0; i < indices.length; i++)
			{
				// take into account that the list shrinks by one
				// with each iteration;
		...
		else if (src.equals(applyButton))
		{
			if (filterable != null)
			{
				...
				FilterDialogFactory.setFilters(filterable.getAvailableFilters(),
				  filterable.getSelectedFilters());
...
//  		removedModel.addElement(displayOnlyFilter);
	 ...
	 */
	private void setFilters(List availableFilters,
	  List removedFilters)
	{
		availableModel.clear();
		removedModel.clear();
	  ...
	  List removedFilters)
	{
		...
		if (removedFilters != null)
		{
			...
			for (Enumeration enum = removedModel.elements(); enum.hasMoreElements();)
			{
				removedFilters.add(enum.nextElement());
			}
		}
	}
	...
	private class ListMouseHandler extends MouseAdapter
	{
		...
		public void mouseClicked(MouseEvent e)
		{
			...
			if (e.getClickCount() == 2)
			{
				...
				{
					int index = removedList.getSelectedIndex();
					if (index > -1)
					{
 ...
 * @author Thomas Behr
 */
class FilterNameDialog extends JDialog implements ActionListener, FilterConstants
{
	...
	private void _init()
	{
		...
		mainPanel.add(new JLabel("Name : "), 0, 0, 1, 1);
		mainPanel.add(nameFieldPanel, 1, 0, 1, 1);
		mainPanel.add(new JLabel("Description : "), 2, 0, 1, 1);
		mainPanel.gbc.weighty = 10;
		mainPanel.gbc.fill = GridBagConstraints.BOTH;
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		else
		{
			...
			{
				JOptionPane.showMessageDialog(null,
				  "There is already a filter named '" + name + "'!",
				  "Warning",
				  JOptionPane.ERROR_MESSAGE);
			}
			else
	 ...
	 *         -1 is returned.
	 */
	private int indexOfIllegalChar(String s)
	{
 ...
 * @author Thomas Behr
 */
class FilterEditorDialog extends JDialog implements ActionListener, FilterConstants
{
	private ArrayList customFilters;
	private Hashtable customFiltersOrigin;
	...
	private static final String HELP_MESSAGE =
	  "Create your own custom coumpound filters.\n\n" +
	  "Reference filters by their name in square brackets, e.g.:\n" +
	  "\t'[Miscellaneous :  Qualify]'\n" +
	  "You can use the arrow buttons to avoid typos.\n\n" +
	  "Concatenate filters using boolean logic.\n" +
	  "Possible tokens are:\n" +
	  "\t'AND', 'NOT', 'OR', '(' and ')'\n\n" +
	  "Both for filters and tokens do NOT write the single quotes!\n\n" +
	  "Example 1:\n" +
	  "NOT [Miscellaneous :  Qualify]\n\n" +
	  "Example 2:\n" +
	  "[Miscellaneous :  Qualify] OR NOT [Source :  WotC - Core Rulebook I - Players Hand Book]\n\n" +
	  "Example 3:\n" +
	  "([Miscellaneous :  Qualify] AND [Type :  Weapon]) OR (NOT [Miscellaneous :  Qualify] AND [Type :  Armor])\n\n" +
	  "You may use line breaks between tokens for better readability;\n" +
	  "e.g. example 3 could be written as:\n" +
	  "(\n" +
	  "[Miscellaneous :  Qualify]\n" +
	  " AND [Type :  Weapon]\n" +
	  ") OR (\n" +
	...
	public FilterEditorDialog()
	{
		...
		this.customFiltersOrigin = new Hashtable();
		this.init();
	}
...
// left stuff
		leftPanel.gbc.anchor = GridBagConstraints.NORTHWEST;
		leftPanel.gbc.weighty = 0;
		leftPanel.add(new JLabel("Standard Filters: "), 0, 0, 1, 1);
...
// Standard List
		...
		addStandardButton = new JButton(new ImageIcon(getClass().getResource("/pcgen/gui/resource/Forward16.gif")));
...
// middle stuff
		...
		middlePanel.gbc.anchor = GridBagConstraints.NORTHWEST;
		middlePanel.add(new JLabel("Editor: "), 0, 0, 1, 1);

		final Dimension editorDimension = new Dimension(scrollPaneDimension.width * 2,
		  scrollPaneDimension.height);
...
// right stuff
		rightPanel.gbc.weighty = 0;
		rightPanel.gbc.anchor = GridBagConstraints.NORTHWEST;
		rightPanel.add(new JLabel("Custom Filters: "), 0, 1, 1, 1);
...
// Custom List
		...
		addCustomButton = new JButton(new ImageIcon(getClass().getResource("/pcgen/gui/resource/Back16.gif")));
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if (src.equals(addStandardButton))
		{
			...
			for (int i = 0; i < indices.length; i++)
			{
				...
				buf.append("\n").append("[").append(filter.getCategory()).append(PObjectFilter.SEPARATOR);
		...
		else if (src.equals(cancelButton))
		{
			if (filterable != null)
			{
				unStoreFilters(filterable.getAvailableFilters(),
				  filterable.getSelectedFilters(),
				  filterable.getRemovedFilters());
			}
...
//  		customModel.addElement(displayOnlyFilter);
	...
	private void settings(Filterable f)
	{
		...
		else
		{
			...
			setFilters(filterable.getAvailableFilters(),
			  filterable.getSelectedFilters(),
			  filterable.getRemovedFilters());
	  ...
	  List removedFilters)
	{
		String origin;
		Object filter;
		for (Enumeration enum = customModel.elements(); enum.hasMoreElements();)
		{
			filter = enum.nextElement();
			origin = (String)customFiltersOrigin.get(filter.getClass().getName());
			if ((origin == null) || (origin.equals("available")))
			{
				availableFilters.add(filter);
			}
			else if (origin.equals("selected"))
			{
				selectedFilters.add(filter);
			}
			else if (origin.equals("removed"))
			{
	 ...
	 * @param removedFilters     a List in which the custom filter(name)s are to be stored
	 */
	private void unStoreFilters(List availableFilters,
	  List selectedFilters,
	  List removedFilters)
	{
		String origin;
		...
		for (Iterator it = customFilters.iterator(); it.hasNext();)
		{
			...
			else if (origin.equals("available"))
			{
				availableFilters.add(filter);
			}
			else if (origin.equals("selected"))
