 ...
 *                   David Browning;
 *
 * $Id: DemoDatasetFactory.java,v 1.1 2007/10/10 19:09:08 vauchers Exp $
 *
 * Changes
 * -------
...
public class DemoDatasetFactory {
    ...
    /**
     * Creates a sample <code>BoxAndWhiskerDataset</code>.
     *
     * @return a sample <code>BoxAndWhiskerDataset</code>.
     */
    public static BoxAndWhiskerDataset createBoxAndWhiskerDataset() {
        Date[] date = new Date[7];
        double[] max =          {2.1, 2.9, 2.5, 2.6, 1.7, 1.9, 2.6};
        double[] min =          {0.2, 0.4, 0.6, 0.5, 0.1, 0.3, 0.4};
        double[] average = new double[7];
        double[] median = new double[7];
        double[] q1Median = new double[7];
        double[] q3Median = new double[7];
        double[] replicateCount = new double[7];   // as yet not used...
        double[][] outlier = new double[7][];

        for (int i =0; i < 7; i++) {
            date[i] = DateUtilities.createDate(2003, 7, i+1, 12, 0);
            average[i] = (max[i] + min[i])/2;
            median[i] = min[i] + average[i] + 0.1;
            q1Median[i] = median[i] + (max[i] - median[i])/2;
            q3Median[i] = median[i] - (median[i]- min[i])/2;
            outlier[i] = makeOutlierArray(max, i);
        }

        return new DefaultBoxAndWhiskerDataset("Test", date, max, min, average, median, q1Median, q3Median, outlier, replicateCount);
    }

    private static double[] makeOutlierArray(double[] max, int index) {
        int len = (int)(Math.random() * 4) + 1;
        double[] array = new double[len];
        for (int i = 0; i < len; i++) {
            array[i] = Math.random()/2 + max[index];
        }
        return array;
    }
