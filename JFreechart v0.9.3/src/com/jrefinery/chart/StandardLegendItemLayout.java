 ...
 * Contributor(s):   -;
 *
 * $Id: StandardLegendItemLayout.java,v 1.1 2007/10/10 19:52:15 vauchers Exp $
 *
 * Changes
 * -------
...
public class StandardLegendItemLayout implements LegendItemLayout {
	 ...
	 * @param orientation   The orientation of the layout (HORIZONTAL or
	 *      VERTICAL).
	...
	private void doVerticalLayout(LegendItemCollection collection) {
		...
		while (iterator.hasNext()) {
			...
			if ((first) || (item.getHeight()<(this.dimension-currentColumnY))) {
				...
				currentColumnWidth =
					Math.max(currentColumnWidth, item.getWidth());
				first=false;
			}
