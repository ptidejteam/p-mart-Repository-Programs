 ...
 *
 * $Id: VerticalXYBarRenderer.java,v 1.1 2007/10/10 19:52:15 vauchers Exp $
 *
 * Changes
 * -------
	...
	implements XYItemRenderer
{
	 ...
	 *
	 * @param margin    The percentage amount to trim from the width of each
	 *      bar.
	 */
	...
	public void setMargin(double margin) {
		...
		this.margin = margin;
		this.firePropertyChanged("VerticalXYBarRenderer.margin", old,
			new Double(margin));
	   ...
	   ChartRenderingInfo info)
	{
		...
		this.translatedRangeZero = rangeAxis
	...
	public void drawItem(Graphics2D g2, Rectangle2D dataArea,
		ChartRenderingInfo info, XYPlot plot, ValueAxis domainAxis,
		ValueAxis rangeAxis,
		...
		CrosshairInfo crosshairInfo)
	{
		...
		double translatedValue = rangeAxis
			.translateValueToJava2D(valueNumber.doubleValue(), dataArea);

		Number startXNumber = intervalData.getStartXValue(series, item);
		double translatedStartX = domainAxis
			.translateValueToJava2D(startXNumber.doubleValue(), dataArea);

		Number endXNumber = intervalData.getEndXValue(series, item);
		double translatedEndX = domainAxis
			.translateValueToJava2D(endXNumber.doubleValue(), dataArea);
			...
			if (entities!=null) {
				...
				if (this.toolTipGenerator!=null) {
					tip = this.toolTipGenerator
						.generateToolTip(data, series, item);
				}
				String url = null;
				if (this.urlGenerator != null)
					url = this.urlGenerator.generateURL(data, series, item);
