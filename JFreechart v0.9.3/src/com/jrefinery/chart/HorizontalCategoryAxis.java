 ...
 *                   Jon Iles;
 *
 * $Id: HorizontalCategoryAxis.java,v 1.1 2007/10/10 19:52:14 vauchers Exp $
 *
...
/**
 * A horizontal axis that displays categories.  Used for bar charts and line
 * charts.
 * <P>
 * Note: the axis needs to rely on the plot for assistance with the placement
 * of category labels, since the plot controls how the categories are
 * distributed.
 */
    ...
    implements HorizontalAxis
{
    ...
    /** A flag that indicates whether or not the category labels should be
     *  drawn vertically.
     */
    ...
    /**
     * Constructs a HorizontalCategoryAxis, using default values where
     * necessary.
     */
     ...
     * @param labelInsets   Determines the amount of blank space around the
     *      label.
     * @param categoryLabelsVisible     Flag indicating whether or not category
     *      labels are visible.
     * @param verticalCategoryLabels    Flag indicating whether or not the
     *      category labels are drawn vertically.
     ...
     * @param categoryLabelInsets   Determines the blank space around each
     *      category label.
     * @param tickMarksVisible      Flag indicating whether or not tick marks
     *      are visible.
     * @param tickMarkStroke    The stroke used to draw tick marks (if visible).
     */
    public HorizontalCategoryAxis(String label, Font labelFont,
    ...
    /**
     * Returns a flag indicating whether the category labels are drawn
     * 'vertically'.
     *
     * @return The flag.
     */
    ...
    /**
     * Sets the flag that determines whether the category labels are drawn
     * 'vertically'.
     *
    ...
    /**
     * Draws the HorizontalCategoryAxis on a Java 2D graphics device (such as
            ...
            while (iterator.hasNext()) {
                ...
                if (obj instanceof Tick) {
                    ...
                    for (int i = 0; i < ts.length; i++)
                        g2.drawString(ts[i].getText(), ts[i].getX(),
                            ts[i].getY());
                }
            }
        }
     ...
     */
    public void refreshTicks(Graphics2D g2, Rectangle2D plotArea,
        Rectangle2D dataArea)
    {
        ...
        if (data!=null) {
            ...
            while (iterator.hasNext()) {
                ...
                float catX = (float)categoryPlot
                    .getCategoryCoordinate(categoryIndex, dataArea);
                if (this.verticalCategoryLabels) {
                    xx = (float)(catX+labelBounds.getHeight()/2
                        - metrics.getDescent());
                    ...
                    if (this.skipCategoryLabelsToFit) {
                        ...
                        ticks.add(new Tick(category, label, xx, yy));
                        categorySkip = (int)
                            ((labelBounds.getWidth() - maxWidth/2)/maxWidth)+1;
                    }
                    else {
    ...
    public double reserveHeight(Graphics2D g2, Plot plot, Rectangle2D drawArea){
        ...
        if (label!=null) {
            ...
            labelHeight =
                this.labelInsets.top+labelInsets.bottom+labelBounds.getHeight();
        }
        ...
        Rectangle2D drawArea, double reservedWidth)
    {
        ...
        if (!visible)
            return new Rectangle2D.Double(drawArea.getX(),
                drawArea.getMaxY(), drawArea.getWidth()-reservedWidth, 0.0);
     ...
     * @param vertical      A flag indicating whether or not the tick labels
     *      are drawn vertically.
     *
     * @return The maximum tick label height.
     */
    private double getMaxTickLabelHeight(Graphics2D g2, Rectangle2D drawArea,
        boolean vertical)
    {
        ...
        if (vertical) {
            ...
            while (iterator.hasNext()) {
                ...
                Rectangle2D labelBounds =
                    font.getStringBounds(tick.getText(), frc);
    ...
    protected double getMaxTickLabelWidth(Graphics2D g2, Rectangle2D plotArea) {
        ...
        while (iterator.hasNext()) {
            ...
            if (obj instanceof Tick) {
                ...
                for (int i = 0; i < ts.length; i++) {
                    Rectangle2D labelBounds =
                        font.getStringBounds(ts[i].getText(), frc);
                    if (labelBounds.getWidth() > maxWidth) {
                        maxWidth = labelBounds.getWidth();
                    }
                }
    ...
    /**
     * Configures the axis against the current plot.  Nothing required in this
     * class.
     */
     ...
     */
    private String[] breakLine(String text, int areaWidth,
        FontRenderContext frc)
    {
        ...
        AttributedString as =
            new AttributedString(text, getTickLabelFont().getAttributes());
        ...
        for(;;) {
            ...
            while (layout != null) {
                textList.add(text
                    .substring(offset,offset+layout.getCharacterCount()));
