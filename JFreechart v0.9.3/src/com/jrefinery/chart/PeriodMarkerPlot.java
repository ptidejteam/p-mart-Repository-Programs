 ...
 *
 * $Id: PeriodMarkerPlot.java,v 1.1 2007/10/10 19:52:15 vauchers Exp $
 *
 * Changes
 * -------
 ...
 * @author  sylvain
 */
public class PeriodMarkerPlot extends XYPlot
	implements HorizontalValuePlot, VerticalValuePlot
{
	 ...
	 */
	public PeriodMarkerPlot(XYDataset data, ValueAxis domainAxis,
			ValueAxis rangeAxis)
	 ...
	 * the vertical axis for a HighLowPlot.
	 *
	...
	/**
	 * Returns the maximum value in the range, since this is plotted against
	 * the vertical axis for a HighLowPlot.
	 *
		...
		ChartRenderingInfo info)
	{
		...
		// we can cast the axes because HiLowPlot enforces support of these
		// interfaces
	...
	private void drawMarkedPeriods(XYDataset data, int serie, Graphics2D g2,
		Rectangle2D plotArea)
	{
		...
		for(int itemIndex = 0; itemIndex < itemCount; itemIndex++){
			...
			Number xStart;
			if( itemIndex > 0 )
				xStart = new Long( (data.getXValue(serie,
					itemIndex).longValue()
			...
			double xxStart = getDomainAxis()
				.translateValueToJava2D(xStart.doubleValue(), plotArea);
			double xxEnd = getDomainAxis()
				.translateValueToJava2D(xEnd.doubleValue(), plotArea);
	...
	private void markPeriod(double xStart, double xEnd, double minY,
		double maxY, Graphics2D g2)
	{
