 ...
 *
 * $Id: VerticalIntervalBarRenderer.java,v 1.1 2007/10/10 19:52:15 vauchers Exp $
 *
 * Changes
 * -------
 ...
 */
public class VerticalIntervalBarRenderer extends BarRenderer
	implements CategoryItemRenderer
{
		...
		int categoryIndex, Object previousCategory)
	{
		...
		// BAR X
		double rectX =
			dataArea.getX()+dataArea.getWidth()*plot.getIntroGapPercent();
		...
		double translatedValue0 = axis
			.translateValueToJava2D(value0.doubleValue(), dataArea);

		// Y1
		Number value1 = intervalData.getStartValue(series, category);
		double translatedValue1 = axis
			.translateValueToJava2D(value1.doubleValue(), dataArea);
		...
		if (plot.getLabelsVisible()) {
			...
			Rectangle2D lowLabelArea = new Rectangle2D.Double(rectX,
				translatedValue1, rectWidth,
			...
			Rectangle2D highLabelArea = new Rectangle2D.Double(rectX,
				dataArea.getY(), rectWidth, translatedValue0 - dataArea.getY());
			drawLabel(g2, highLabel, highLabelArea, labelFont, HIGH_LABEL,
				rotate);
		}
	 ...
	 * @param labelArea     The area in which to draw the label (it may extend
	 *      beyond the sides of this rectangle).
	 * @param labelFont      The font to draw the label with.
	 * @param labelType     HIGH_LABEL or LOW_LABEL; determines how to position
	 *      the label in the provided area.
