 ...
 *
 * $Id: VerticalCategoryAxis.java,v 1.1 2007/10/10 19:52:14 vauchers Exp $
 *
...
public class VerticalCategoryAxis extends CategoryAxis implements VerticalAxis {
	...
	/** A flag that indicates whether or not the axis label should be drawn
	 *  vertically.
	 */
	 ...
	 *      label.
	 * @param verticalLabel     Flag indicating whether or not the axis label
	 *      is drawn vertically.
	...
	/**
	 * Returns a flag indicating whether or not the axis label is drawn
	 * vertically.
	 *
	 * @return The flag.
	 */
	...
	/**
	 * Draws the CategoryAxis on a Java 2D graphics device (such as the screen
	 * or a printer).
	 *
	...
	public void draw(Graphics2D g2, Rectangle2D drawArea, Rectangle2D plotArea){
        ...
        if (this.label == null ? false : !this.label.equals("")) {
			...
			if (verticalLabel) {
				...
				double xx = drawArea.getX()+labelInsets.left;
				double yy = drawArea.getY()
					+ drawArea.getHeight()/2-labelBounds.getHeight()/2;
		...
		Rectangle2D plotArea)
	{
		...
		if (data!=null) {
			...
			while (iterator.hasNext()) {
				...
				float yy = (float)(categoryPlot
					.getCategoryCoordinate(categoryIndex, plotArea)
	 ...
	 * @param plot The plot.
	 *
	 * @return A boolean indicating whether or not the axis considers the plot
	 *      is compatible.
	 */
