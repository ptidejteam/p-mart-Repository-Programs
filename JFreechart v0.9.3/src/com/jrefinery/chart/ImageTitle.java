 ...
 *
 * $Id: ImageTitle.java,v 1.1 2007/10/10 19:52:15 vauchers Exp $
 *
...
public class ImageTitle extends AbstractTitle {
	 ...
	 *      MIDDLE or BOTTOM).
	 */
	public ImageTitle(Image image, int position, int horizontalAlignment,
	 ...
	 * @param titleArea     The area within which the title (and plot) should
	 *      be drawn.
	 */
	public void draw(Graphics2D g2, Rectangle2D titleArea) {
		if (this.position == TOP || this.position == BOTTOM) {
			...
			throw new RuntimeException(
				"ImageTitle.draw(...) - invalid title position.");
		}
	}
	...
	public boolean isValidPosition(int position) {
		if (position==AbstractTitle.TOP)
			return true;
		else if (position==AbstractTitle.BOTTOM)
			return true;
		else if (position==AbstractTitle.RIGHT)
			return true;
		else if (position==AbstractTitle.LEFT)
			return true;
		else
			return false;
	}
	 ...
	 * printer).
	 * @param g2    The graphics device.
	 * @param chartArea     The area within which the title (and plot) should
	 *      be drawn.
	...
	protected Size2D drawHorizontal(Graphics2D g2, Rectangle2D chartArea) {
		...
		if (this.horizontalAlignment == CENTER) {
			startX = chartArea.getX() + leftSpace
				+ chartArea.getWidth() / 2 - this.width / 2 ;
		}
		...
		g2.drawImage(image, (int)startX, (int)startY, this.width, this.height,
			null);
