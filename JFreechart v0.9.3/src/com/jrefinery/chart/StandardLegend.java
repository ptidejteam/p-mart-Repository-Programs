 ...
 *
 * $Id: StandardLegend.java,v 1.1 2007/10/10 19:52:14 vauchers Exp $
 *
...
public class StandardLegend extends Legend {
	...
	public Rectangle2D draw(Graphics2D g2, Rectangle2D available) {
		...
		return draw(g2, available, (_anchor & HORIZONTAL)!=0,
			(_anchor & INVERTED)!=0);
		...
		float textHeight, Rectangle2D innerLegendArea)
	{
		...
		float yy = (float)innerLegendArea.getY()
			+ innerGap+(series+0.15f)*textHeight;
		return new Rectangle2D.Float(xx, yy, boxHeightAndWidth,
			boxHeightAndWidth);
...
     * <P>
	 ...
	 */
	private LegendItem createLegendItem(Graphics2D graphics, String label,
		double x, double y)
	{
