 ...
 * Contributor(s):   -;
 *
 * $Id: DefaultShapeFactory.java,v 1.1 2007/10/10 19:52:14 vauchers Exp $
 *
 * Changes
 * -------
...
public class DefaultShapeFactory implements ShapeFactory {
	...
	public Shape getShape(int series, int item, double x, double y,
		...
		if (series==0) {
			return
				new Rectangle2D.Double(x-0.5*scale, y-0.5*scale, scale, scale);
		}
		else {
			return
				new Ellipse2D.Double(x-0.5*scale, y-0.5*scale, scale, scale);
		}
