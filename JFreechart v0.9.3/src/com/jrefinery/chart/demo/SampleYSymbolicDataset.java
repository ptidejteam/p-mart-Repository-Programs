	...
	implements XYDataset, YisSymbolic
{
	...
	public SampleYSymbolicDataset(String datasetName, int xRange,
		String[] tabString)
	{
		this(datasetName, xRange, tabString, DEFAULT_SERIES_COUNT,
			DEFAULT_ITEM_COUNT, null);
	}
	...
	public SampleYSymbolicDataset(String datasetName, Double[][] xValues,
		Integer[][] yValues, String[] ySymbolicValues, int serie, int item,
		String[] serieNames)
	{
		this.datasetName = datasetName;
		...
		YisSymbolic dataset2)
	{
		...
		for (int i=0; i<sDataset2.getSeriesCount(); i++) {
			for (int j=0; j<sDataset2.getItemCount(i); j++) {
				newIndex =  ySymbolicValuesCombined
					.indexOf(sDataset2.getYSymbolicValue(i, j));
				sDataset2.setYValue(i, j, new Integer(newIndex));
			}
		}

		//Set the new list of symbolic value on the two data sets
		String[] ySymbolicValuesCombinedA =
			new String[ySymbolicValuesCombined.size()];
	...
	private static Object cloneArray(Object arr) {
		...
		Object[] newarr =
			(Object[])Array.newInstance(cls.getComponentType(), length);
