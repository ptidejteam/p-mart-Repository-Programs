 ...
 *
 * $Id: WindItemRenderer.java,v 1.1 2007/10/10 19:52:14 vauchers Exp $
 *
 * Changes
 * -------
	...
	implements XYItemRenderer
{
		...
		CrosshairInfo crosshairs)
	{
		...
		rax2 = x.doubleValue() + (windForce * Math.cos(wdir_t)*8000000.0);
		ray2 = windForce * Math.sin(wdir_t);
		...
		double aldir =
			Math.toRadians(windDir.doubleValue() * (-30.0) -90.0 - 5.0);
		ralx2 = wforce.doubleValue() * Math.cos(aldir)*(double) 8000000
			* 0.8+ x.doubleValue();
		...
		double ardir = Math.toRadians(windDir.doubleValue()
			* (-30.0) -90.0 + 5.0);
		rarx2 = wforce.doubleValue() * Math.cos(ardir)*(double) 8000000 * 0.8
			+ x.doubleValue();
