 ...
 *
 * $Id: MeterLegend.java,v 1.1 2007/10/10 19:52:15 vauchers Exp $
 *
 * Changes
 * -------
...
public class MeterLegend extends StandardLegend {
    ...
    protected Rectangle2D draw(Graphics2D g2, Rectangle2D available,
        ...
        legendItemLabels[ currentItem] = this.legendText +
                (data.isValueValid()
                    ? ( "   Current Value: " + data.getValue().toString())
                    : "");
        legendItemColors[ currentItem] = null;		// no color
        currentItem ++;
        if (updateInformation( meterPlot, data, MeterDataset.FULL_DATA,
            currentItem, legendItemLabels, legendItemColors)) {
            currentItem++;
        }
        if (showCritical && updateInformation( meterPlot, data,
            MeterDataset.CRITICAL_DATA, currentItem, legendItemLabels,
            legendItemColors)) {
            currentItem++;
        }
        if (showWarning && updateInformation( meterPlot, data,
            MeterDataset.WARNING_DATA, currentItem, legendItemLabels,
            legendItemColors)) {
            currentItem++;
        }
        if (showNormal && updateInformation( meterPlot, data,
            MeterDataset.NORMAL_DATA, currentItem, legendItemLabels,
            legendItemColors)) {
            currentItem++;
        }
        ...
        if (legendItemLabels!=null) {
            ...
            if (horizontal) {
                ...
                for (int i=0; i<legendItemLabels.length; i++) {
                    items[i] = createLegendItem(g2, legendItemLabels[i],
                        xoffset, totalHeight);
                    if ((!startingNewRow)
                ...
                double yloc = (inverted)
                    ? available.getY() + available.getHeight() - totalHeight
                        - outerGap
                ...
                double yloc = available.getY()+(available.getHeight()/2)
                    - (totalHeight/2);
