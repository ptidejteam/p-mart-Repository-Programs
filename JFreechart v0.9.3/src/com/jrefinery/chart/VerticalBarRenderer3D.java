 ...
 *                   Richard Atkinson;
 *
 * $Id: VerticalBarRenderer3D.java,v 1.1 2007/10/10 19:52:15 vauchers Exp $
 *
 * Changes
 * -------
...
public class VerticalBarRenderer3D extends VerticalBarRenderer {
        ...
        Rectangle2D axisDataArea, Shape dataClipRegion)
    {
        ...
        float y = (float)axis.translateValueToJava2D(marker.getValue(),
            axisDataArea);
        ...
        path.lineTo(x+(float)effect3d,
            y-(float)effect3d);
        path.lineTo((float)(axisDataArea.getMaxX()+effect3d),
            y-(float)effect3d);
        path.lineTo((float)(axisDataArea.getMaxX()),
            y);
        path.closePath();
        ...
        int categoryIndex, Object previousCategory)
    {
        ...
        if (value!=null) {
            ...
            if (this.upperClip<=0.0) {  // cases 1, 2, 3 and 4
                if (y>=this.upperClip) return; // bar is not visible
                base = this.upperClip;
                if (y<=this.lowerClip) {
                    y = this.lowerClip;
                }
               ...
               if (y>=this.upperClip) y=this.upperClip;
               else if (y<=this.lowerClip) y=this.lowerClip;
            }
            else { // cases 9, 10, 11 and 12
               if (y<=this.lowerClip) return; // bar is not visible
               base = this.lowerClip;
               if (y>=this.upperClip) {
                   y=this.upperClip;
               }
