 ...
 * <p>
 * NOTE:
 * The Thermometer plot utilises a meter data set, however range options
 * within this data set are not currently utilised.  This is currently under
 * consideration / development.
 *
 * The Thermometer supports a number of options
 * <ol>
 * <li>3 ranges which could be viewed as Critical, Warning and Normal ranges.
 *  </li>
 * <li>The thermometer can be run in two modes:
 *      <ul>
 *      <li>fixed range where colour changes on value, or</li>
 *      <li>3 seperate ranges where colour changes with range changes.</li>
 *      </ul>
 * </li>
...
public class ThermometerPlot extends Plot implements VerticalValuePlot {
	...
	/**
	 * Some general purpose graphics2D objects, used to stop excessive object
	...
	/**
	 * Two range variables are utilised to track which range is the current fo
	...
	/**
	 * Data Model Type : Basically used to enable / disable the code for
	 * supporting data model range changes.  Currently set to true as only
	 * experimetal and not functional.
	 **/
	 ...
	 *      plot.
	 * @param outlinePaint      The color used to draw an outline around the
	 *      plot.
	 ...
	 * Thermometer plot requires no horizontal axes, only a null axis is
	 * compatible.
	 *
	 * @param axis      The axis.
	...
	/**
	 * Returns true if the axis is compatible with the meter plot.  Since a
	 * Thermometer plot requires a VerticalNumberAxis, only a
	 * VerticalNumberAxis axis is compatible.
	 *
	 * @param  axis     The axis.
...
//            tooltips = info.getToolTipsCollection();
		...
		circle1.setFrame(midX - bulbRadius, ticksYStop, bulbDiameter,
			bulbDiameter);
		...
		outer1.setRoundRect(midX - columnRadius, plotArea.getMinY(),
			columnDiameter, k, columnDiameter, columnDiameter);
		i = columnDiameter - gapDiameter;
		j = columnRadius - gapRadius;
		outer2.setRoundRect(midX - j, plotArea.getMinY()  + gapRadius,
			i, k, i, i);
	  ...
	  /// Determine Data Range, always the same as display except in the middle
	  /// range
	  rangeData = range ;
	  ...
	  /// Draw gauge display
	  double ds = ((current - rangeInfo[range][DISPLAY_LOW])
			/ rangeInfo[range][DISPLAY_RANGE])
	  ...
	  if (k < (ticksYStop + bulbRadius)) {
		outer3.setRoundRect(midX - j, k, i,
			(ticksYStop + bulbRadius) - k, l, l);
		a4 = new Area(outer3);
		a3.add(a4);
	  }
	  ...
	  Rectangle2D drawArea =
		new Rectangle2D.Double(midX-columnRadius-legendWidth,
	  ...
	  if (rangeIndcatorStrokeSize > 0) {
		double incY = ((ticksYStop - ticksYStart)
			/ rangeInfo[range][DISPLAY_RANGE]);
		...
		if (inRange(range, rangeInfo[1][RANGE_LOW])) {
			tickY = ticksYStart ;
			tickY += (int) (incY
				*  (rangeInfo[range][DISPLAY_HI] - rangeInfo[1][RANGE_LOW])) ;
		...
		if (inRange(range, rangeInfo[2][RANGE_LOW])) {
			tickY = ticksYStart ;
			tickY += (int) (incY
				* (rangeInfo[range][DISPLAY_HI]-rangeInfo[2][RANGE_LOW])) ;
	  ...
	  switch (valueLocation) {
		case LEFT:
		  g2.drawString(valueFormat.format(current),
			midX + columnRadius + gapRadius, midY);
		  break;
		case BULB:
	...
	metrics = g2.getFontMetrics();
	tickX1 = midX - columnRadius - gapDiameter
		- metrics.stringWidth(UNITS[units]);
  ...
  /***
   * This is experimental, an early attempt to support data model ranges being
   * changed.
   *
   * @param event
   */
  public void datasetChanged(DatasetChangeEvent event) {
	...
	public Range getVerticalDataRange() {
		return new Range(rangeInfo[range][DISPLAY_LOW],
			rangeInfo[range][DISPLAY_HI]);
	}
	 ...
	 *
	 * @param v The new range colour change option
	 */
	...
	/**
	 * Get whether the thermometer paint changes with data (true) or range
	 * (false)
	...
	/**
	 * Sets the width of the range inidctators.  Set to 0 to disable range
	 * inidcator display.
	 * @param width   the width of the inidcator. valid range 0-15 inclusive
	 */
	...
	protected void setAxisRange() {
		rangeAxis.setRange(new Range(rangeInfo[range][DISPLAY_LOW],
			rangeInfo[range][DISPLAY_HI]));
	}
