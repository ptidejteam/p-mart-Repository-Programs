...
public class PCGIOHandler extends IOHandler
{
	...
	public void write(PlayerCharacter aPC, OutputStream out)
	{
		...
		String pcgString;
            ...
            //Mynex wants it all saved in the new format now... so let's give it a shot
/*                if (Boolean.getBoolean("writePCG1")) {
                        pcgString = (new PCGVer2Creator(aPC)).createPCGString();
                }
                else {
	...
	public void read(PlayerCharacter aPC, InputStream in)
	{
		...
		for (int i = 0; i < lines.size(); i++)
			pcgLines[i] = (String)lines.elementAt(i);
	...
	private void sanityChecks()
	{
		...
		if (aRace.hitDice() != 0)
		{
			...
			if (aRace.getHitPointList() == null)
			{
				...
				{
					final int roll = pcgen.core.RollingMethods.roll(iSides);
		...
		if (aPC.getClassList() != null)
		{
			for (Iterator it = aPC.getClassList().iterator(); it.hasNext();)
			{
				...
				for (int i = 0; i < aClass.getLevel().intValue(); i++)
				{
					...
					iSides = baseSides + aClass.getBonusTo("HD", "MAX", i + 1);
					if (iRoll > iSides)
					{
