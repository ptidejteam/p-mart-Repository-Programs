...
class InfoDomain extends FilterAdapterPanel
{
	...
	private final int[] deiColumnWidth = {100, 231, 36, 71};
	...
	private static String[] s_domainColList;
	...
	static
	{
		...
		s_columnNames = new String[]{in_column1, in_column2, in_column3, in_column4};
		s_domainColList = new String[]{in_column5, in_column4};
	}
	...
	private class ClassPopupMenu extends JPopupMenu
	{
		...
		ClassPopupMenu(JTableEx treeTable)
		{
			if (treeTable == deityTable)
			{
				...
				this.addSeparator();
				ClassPopupMenu.this.add(Utility.createMenuItem("Edit Deity",
					new ActionListener()
				 	{
					...
					, "editDeity", (char)0, "alt E", "Edit deity", null, true));
				ClassPopupMenu.this.add(Utility.createMenuItem("Create New Deity",
					new ActionListener()
					{
						public void actionPerformed(ActionEvent e)
						{
					...
					, "createDeity", (char)0, "alt C", "Create deity", null, true));
				ClassPopupMenu.this.add(Utility.createMenuItem("Delete Deity",
					new ActionListener()
				 	{
						public void actionPerformed(ActionEvent e)
						{
					...
					, "deleteDeity", (char)0, "DELETE", "Delete deity", null, true));
			}
	...
	private void initComponents()
	{
		...
		rowSM = domainTable.getSelectionModel();
		rowSM.addListSelectionListener(new ListSelectionListener()
		{
			public void valueChanged(ListSelectionEvent e)
			{
				...
				if (selectedRow >= 0)
				{
					final String selectedDomainName = domModel.getValueAt(selectedRow, -1).toString();
					final Domain aDomain = Globals.getDomainNamed(selectedDomainName);
					setDomainInfoText(aDomain);
				}
				else
				{
					setDomainInfoText(null);
				}
			}
		});
		...
		deityTable.getColumnModel().getColumn(0).setPreferredWidth(60); //JD
	...
	private void formComponentShown(ComponentEvent evt)
	{
		...
		if (!hasBeenSized)
		{
			...
			for (int i = 0; i < deityTable.getColumnCount(); i++)
			{
				...
				width = Globals.getCustColumnWidth("Diety", i);
				if (width != 0)
					sCol.setPreferredWidth(width);
				sCol.addPropertyChangeListener(new ResizeColumnListener(deityTable, "Diety", i));
			}
			...
			for (int i = 0; i < domainTable.getColumnCount(); i++)
			{
				...
				width = Globals.getCustColumnWidth("Domain", i);
				if (width != 0)
					sCol.setPreferredWidth(width);
				sCol.addPropertyChangeListener(new ResizeColumnListener(domainTable, "Domain", i));
			}
	...
	private void setDomainInfoText(Domain aDomain)
	{
		...
		if (aDomain != null)
		{
			...
			String aString = aDomain.getGrantedPower();
			if (aString.length() != 0)
			{
				infoText.append(" &nbsp;").append(in_domainGrant).append("</b>:").append(aString);
			}
			aString = aDomain.preReqHTMLStrings(false);
			if (aString.length() != 0)
			{
				infoText.append(" &nbsp;<b>").append(in_domainRequire).append("</b>:").append(aString);
			}
		}
	...
	private void setDeityInfoText(Deity aDeity)
	{
		if (aDeity != null)
		{
			...
			infoText.append(" &nbsp;").append(in_deityDescrip).append("</b>:").append(aDeity.getDescription());
	...
	public void updateCharacterInfo()
	{
		...
		for (int i = 0; i < aPC.getCharacterDomainList().size(); i++)
		{
			...
			if (aCD != null && aCD.getDomain() != null)
			{
				...
				final String domainName = aCDDomain.getName();
				if (!dataList.contains(domainName))
				{
					/*
					...
					if (accept(aCDDomain))
					{
						dataList.add(domainName); // list of available choices
					}
					pcDomainList.add(domainName); // list of pc's choices
				}
			}
		}
		domTotal.setText(Integer.toString(aPC.getCharacterDomainList().size()));
		domChosen.setText(Integer.toString(pcDomainList.size()) + "*"); //to identify which are chosen in the table
		if (aPC.getDeity() != null)
		{
			...
			while (aTok.hasMoreElements())
			{
				...
				if (aString.equals("ALL") || aString.equals("ANY"))
				{
					for (Iterator i = Globals.getDomainList().iterator(); i.hasNext(); )
					{
						final Domain aDomain = (Domain)i.next();
						aString = aDomain.getName();
						if (accept(aDomain) && !dataList.contains(aString))
						{
				 ...
				 * author: Thomas Behr 09-02-02
				 */
				else if (accept(Globals.getDomainNamed(aString)) && !dataList.contains(aString))
				{
					dataList.add(aString);
				}
			}
			...
			for (Iterator d = aClass.getAddDomains().iterator(); d.hasNext();)
			{
				...
				if (aClass.getLevel().intValue() >= level)
				{
					...
					while (aTok.hasMoreTokens())
					{
						...
						if (accept(Globals.getDomainNamed(dName)) && !dataList.contains(dName))
						{
							dataList.add(dName);
						}
					}
	...
	public void addDomain(int selectedRow)
	{
		...
		{
			JOptionPane.showMessageDialog(null, in_qualifyMess + domainName, Constants.s_APPNAME, JOptionPane.INFORMATION_MESSAGE);
			return;
		}
		for (int i = 0; i < aPC.getCharacterDomainList().size(); i++)
		{
	...
	public void selectDeityIndex(int selectedRow)
	{
		...
		{
			final ListSelectionModel lsm = deityTable.getSelectionModel();
			JOptionPane.showMessageDialog(null, in_reqMess + selectedDeityName + ".", Constants.s_APPNAME, JOptionPane.INFORMATION_MESSAGE);
			lsm.clearSelection();
			return;
		}
		aPC.setDeity(aDeity);
...
//  				}
	 ...
	 */
	static final class DeityModel extends AbstractTableModel
	{
		private ArrayList deities = Globals.getDeityList();
			...
			if (deities != null && row >= 0 && row < deities.size())
			{
				...
				switch (column)
				{
					...
					case 0:
						...
						break;
					case 1:
						retVal = aDeity.domainListPIString();
						break;
					case 2:
					...
					case 3:
						...
						break;
					default:
						Globals.errorPrint("In InfoDomain.getValueAt the column " + column + " is not supported.");
						break;
				}
			}
			return retVal;
	...
	class DomainModel extends AbstractTableModel
	{
		...
		public Object getValueAt(int row, int col)
		{
			...
			final String rowText = (String)dataList.get(row);
			StringBuffer retVal = new StringBuffer(rowText.length() + 20);
			switch (col)
			{
				case -1: // return name of item without html tags
					retVal.append(rowText);
					break;
				case 0:
					final Domain aDomain = Globals.getDomainNamed(rowText);
				...
				case 1:
					try
					{
						retVal.append(Globals.getDomainNamed(rowText).getSource());
					}
					catch (Exception exc)
					{
						Globals.errorPrint(in_errorMess, exc);
					}
					break;
				default:
					Globals.errorPrint("In InfoDomain.getValueAt the column " + col + " is not handled.");
					break;
			}
	...
	private void deleteDeityButtonClick()
	{
		...
		if (row >= 0)
		{
			...
			{
				final String selectedDeityName = deityModel.getValueAt(selectedRow, -1).toString();
				aDeity = Globals.getDeityNamed(selectedDeityName);
				if (aDeity != null)
				{
					if (aDeity.isType("CUSTOM"))
					{
						final int areYouSure = JOptionPane.showConfirmDialog(null, "Delete deity:" + aDeity.getName() + "?", Constants.s_APPNAME, JOptionPane.OK_CANCEL_OPTION);
					...
					{
						JOptionPane.showMessageDialog(null, "You currently cannot delete a non-custom deity.", Constants.s_APPNAME, JOptionPane.ERROR_MESSAGE);
					}
		...
		// TODO: Pop up some kind of warning message...
	}

	private void createDeityButtonClick()
	{
	...
	private void editDeityButtonClick()
	{
		...
		new DeityEditorMain(aDeity).show();
	}
