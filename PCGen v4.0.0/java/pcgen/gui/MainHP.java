...
//
	...
	private void incrementForRow(int iRow, int increment)
	{
		...
		{
			JOptionPane.showMessageDialog(null, "Roll must be at least the minimum (1)", "PCGen", JOptionPane.ERROR_MESSAGE);
		}
		else if ((iRoll + increment) > iMax)
		{
			JOptionPane.showMessageDialog(null, "Roll cannot exceed the maximum (" + iMax + ")", "PCGen", JOptionPane.ERROR_MESSAGE);
		}
		else
		{
			iRoll += increment;
	 ...
	 *
	 */
	class PCHitPointsTableModel extends AbstractTableModel
	{
		...
		public Object getValueAt(int rowIndex, int columnIndex)
		{
			...
			if (aPC != null)
			{
				...
				switch (columnIndex)
				{
					...
					case 3:		// Con
						int iConMod = aPC.getStatBonusTo("HP", "BONUS");
						return new Integer(iConMod);

					case 4:		// Total
						iHp += aPC.getStatBonusTo("HP", "BONUS");
						if (iHp < 1)
						{
							iHp = 1;
						}
