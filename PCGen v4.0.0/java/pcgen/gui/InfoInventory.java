...
class InfoInventory extends FilterAdapterPanel
{
	...
	private JTextField totalWeight = new JTextField();
	...
	private JButton equipButton = new JButton(CharacterInfo.in_equipping);
	private JButton resourceButton = new JButton(CharacterInfo.in_resources);
	private InfoEquipping infoEquipping = new InfoEquipping();
	private InfoResources infoResources = new InfoResources();
	private JFrame equippingFrame = new JFrame(CharacterInfo.in_equipping);
	private JFrame resourcesFrame = new JFrame(CharacterInfo.in_resources);
	private boolean equippingFrameHasBeenSized = false;
	private boolean resourcesFrameHasBeenSized = false;
	...
	static int viewMode = GuiConstants.INFOINVENTORY_VIEW_TYPE_SUBTYPE_NAME;		// keep track of what view mode we're in for Available
	static int viewSelectMode = GuiConstants.INFOINVENTORY_VIEW_NAME;			// keep track of what view mode we're in for Selected. defaults to "Name"
...
// create available table of equipment
		...
		aPanel = new JPanel();
		gridbag.setConstraints(aPanel, c);
		aPanel.add(equipButton);
		aPanel.add(resourceButton);
		Utility.setDescription(equipButton, "Click to display equipping options");
		Utility.setDescription(resourceButton, "Click to display resource options");
		aPanel.add(equipButton);
		equippingFrame.getContentPane().setLayout(new BorderLayout());
		equippingFrame.getContentPane().add(infoEquipping, BorderLayout.CENTER);
		...
		loader = null;
		equippingFrame.setIconImage(img);

		aPanel.add(resourceButton);
		resourcesFrame.getContentPane().setLayout(new BorderLayout());
		resourcesFrame.getContentPane().add(infoResources, BorderLayout.CENTER);
		resourcesFrame.setIconImage(img);
		rightPane.add(aPanel);
		...
		buildConstraints(c2, 0, 3, 1, 1, 0, 0);
		c2.fill = GridBagConstraints.BOTH;
		c2.anchor = GridBagConstraints.WEST;
		gridbag2.setConstraints(pnlBuy, c2);
		south.add(pnlBuy, c2);

		buildConstraints(c2, 0, 4, 1, 1, 0, 0);
		c2.fill = GridBagConstraints.BOTH;
		c2.anchor = GridBagConstraints.WEST;
	...
	void initActionListeners()
	{
		...
		equipButton.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent evt)
			{
				if (!equippingFrameHasBeenSized)
				{
//		Dimension screenSize = kit.getScreenSize();
					Dimension screenSize = PCGen_Frame1.getCharacterPane().getParent().getParent().getSize();
					Point p = InfoInventory.this.getLocationOnScreen();
					int screenHeight = screenSize.height;
					int screenWidth = screenSize.width;

					// center frame in screen
					equippingFrame.setSize(screenWidth, screenHeight);
					equippingFrame.setLocation((int)p.getX(), (int)p.getY());
					equippingFrameHasBeenSized = true;
				}
				equippingFrame.setVisible(true);
				infoEquipping.display();
			}
		});
...
//		Dimension screenSize = kit.getScreenSize();
					...
					// center frame in screen
					resourcesFrame.setSize(screenWidth, screenHeight);
//					resourcesFrame.setLocation(screenWidth / 2, screenHeight / 2);
					resourcesFrameHasBeenSized = true;
				}
				infoResources.display();
	...
	public boolean canAfford(Equipment base, Equipment selected, float newQty)
	{
		...
		final float itemCost = ((newQty - previousQty) * SettingsHandler.getInventoryTab_BuyRate()) * (float)0.01 * base.getCost().floatValue();
	...
	public void updateCharacterInfo()
	{
		...
		updateTotalWeight();
		updateTotalValue();
		capacity.setText(Globals.maxLoadForLoadScore(aPC.getVariableValue("LOADSCORE", "").intValue()).toString());
		needsUpdate = false;
	}
	...
	private void updateTotalWeight()
	{
		...
		load.setText(loadTypes[Globals.loadTypeForLoadScore(aPC.getVariableValue("LOADSCORE", "").intValue(), weight)]);
	}
...
//End multicontainers code
	...
	private void adjustGold(Equipment base, float diffQty)
	{
		...
		{
			float itemCost = diffQty * base.getCost().floatValue();
			if (diffQty > 0)
			{
...
//Begin LocationEditor classes
	...
	static final class HandRenderer extends JComboBox implements TableCellRenderer
	{
	...
	final class LocationRenderer extends JComboBox implements TableCellRenderer
	{
...
//End LocationEditor classes
	 ...
	 */
	class EquipmentModel extends AbstractTreeTableModel implements TreeTableModel
	{
		// Types of the columns.
		protected Class[] cTypes = {TreeTableModel.class, String.class};
		protected int modelType = 0; // availableModel=0,selectedModel=1
		protected int currentMode = GuiConstants.INFOINVENTORY_VIEW_TYPE_SUBTYPE_NAME;
