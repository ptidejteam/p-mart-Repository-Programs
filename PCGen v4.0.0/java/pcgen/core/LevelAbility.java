		...
		switch (type)
		{
			...
			case 6:
				...
				for (Iterator e1 = aPC.getClassList().iterator(); e1.hasNext();)
				{
					...
					else if (!bClass.getSpellType().equals("") && !bClass.getSpellType().equals(Constants.s_NONE)
		...
		switch (type)
		{
			...
			case 6:
				if (selectedList.size() > 0)
				{
					...
					//The following line is unused. TODO Check if it can be removed safely...
					SpecialAbility sa = new SpecialAbility("Bonus Caster Level for " + cString + aString, "PCCLASS|" + ownerClass.getName() + "|" + level);
