...
public class Race extends PObject
{
	...
	public void rollHeightWeight()
	{
		...
		String bString = aPC.findTemplateHeight();
		...
		while (aTok.hasMoreTokens())
		{
			...
			if (aPC.getGender().startsWith(aString))
			{
				startCounting = true;
			}
			if (i >= 0)
...
//						}
						break;
					default:
						Globals.errorPrint("In Race.rollHeightWeight (weight) the i value " + i + " is not handled (" + aString + ").");
						break;
				}
				if (i == 3)
				{
		...
		bString = aPC.findTemplateWeight();
				...
				if (i >= 0)
				{
					switch (i)
					{
						...
						case 2:
							...
							break;
						default:
							Globals.errorPrint("In Race.rollHeightWeight (height) the i value " + i + " is not handled (" + aString + ").");
							break;
					}
					if (i == 2)
					{
...
//			roll = Math.abs(Globals.getRandomInt(max - min + 1)) + min;
			for (; ;)
			{
				Object selectedValue = JOptionPane.showInputDialog(
				  ...
				  Globals.getGameModeHPText() + " for " + Utility.ordinal(x + 1) + " hit die of " + getName(),
