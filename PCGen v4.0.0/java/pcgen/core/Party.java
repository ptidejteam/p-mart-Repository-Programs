						...
						else
						{
							if (bTok.hasMoreTokens() || flag)
							{
									...
									while (pTok.hasMoreTokens())
									{
										...
										switch (j++)
										{
											...
											case 6:
												...
												break;
											default:
												Globals.errorPrint("In Party.print there is an unhandled case in a switch.");
												break;
										}
									}
									if (cMax.intValue() >= Globals.getPCList().size() && existsOnly)
										...
										while (dString.length() > 0)
										{
											...
											if (eString.startsWith("%."))
											{
												...
												if (charNum >= 0 && charNum < Globals.getPCList().size())
												{
													...
													if (aPC != null)
													{
														(new pcgen.io.ExportHandler(null)).replaceToken(aPC, eString.substring(2), output);
													}
													else
													{
														noMoreItems = true;
												...
												FileAccess.write(output, eString);
										}
								...
								else
								{
									...
									{
										final PlayerCharacter aPC = (PlayerCharacter)Globals.getPCList().get(charNum);
										(new pcgen.io.ExportHandler(null)).replaceToken(aPC, aString, output);
									}
								}
