 ...
 *                   Barak Naveh;
 *
 * $Id: StandardLegend.java,v 1.1 2007/10/10 19:39:27 vauchers Exp $
 *
...
public class StandardLegend extends Legend implements Serializable {
     ...
     * has been defined; otherwise returns <code>Double.MAX_VALUE</code>.
     * 
        ...
        if (horizontal) {
            ...
            for (int i = 0; i < legendItems.getItemCount(); i++) {
                if (this.renderingOrder == LegendRenderingOrder.STANDARD) {
                    items[i] = createDrawableLegendItem(
                ...
                if (items[i].getMaxX() + xstart > xlimit && wrappingAllowed ) {
                    // start a new row
            ...
            for (int i = 0; i < items.length; i++) {
                if (this.renderingOrder == LegendRenderingOrder.STANDARD) {
                    ...
                    items[i] = createDrawableLegendItem(
                        g2, legendItems.get(items.length - i - 1), 0, totalHeight
                    );
                }
    ...
    private void drawSeriesElements( Graphics2D g2, DrawableLegendItem[] items, 
