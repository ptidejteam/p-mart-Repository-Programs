 ...
 * Contributor(s):   -;
 *
 * $Id: IntervalXYDelegate.java,v 1.1 2007/10/10 19:39:25 vauchers Exp $
 *
...
class IntervalXYDelegate implements DomainInfo, Serializable, Cloneable, PublicCloneable {
	...
	/**
	 * updates the interval width if an item is added, i.e. relaxes the interval width to the minimum 
	 * of the actual interval width, the distance between the actual x value and 
	...
	public void itemAdded(int series, int item) {
		...
		if (item > 0) {
			double before = this.dataset.getXValue(series, item - 1).doubleValue();
		...
		if (item + 1 < this.dataset.getItemCount(series)) {
			double after = this.dataset.getXValue(series, item + 1).doubleValue();
	...
	private void calculateSeries(int series) {
		for (int item = 1, itemCount = this.dataset.getItemCount(series); item < itemCount; item++) {
