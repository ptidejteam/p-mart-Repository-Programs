...
/**
 * Notes:
 * the tooltip support have been removed for now.
 * It may be possible to add it using the 
 * org.eclipse.swt.events.MouseTrackListener interface.
                                                         ...
                                                         Printable
{
    ...
    //TODO private boolean fillZoomRectangle = false;

    /** The minimum distance required to drag the mouse to trigger a zoom. */
    private int zoomTriggerDistance;
    
    /** A flag that controls whether or not horizontal tracing is enabled. */
    //TODO private boolean horizontalAxisTrace = true;

    /** A flag that controls whether or not vertical tracing is enabled. */
    //TODO private boolean verticalAxisTrace = true;
    ...
    public ChartComposite(Composite comp, 
        ...
        canvas.addPaintListener(new PaintListener() {
            public void paintControl(PaintEvent e) {
                //System.out.println( "detected paint event" );
                ...
                if (useBuffer) {
                    ...
                    if (refreshBuffer) {
                        ...
                        chart.draw(sg2d, new Rectangle2D.Double(0, 0, 
                                chartBufferWidth, chartBufferHeight), info);
                ...
                else
                {
                    chart.draw(sg2, new Rectangle2D.Double(0, 0, 
                        getBounds().width, getBounds().height), info);
                    //FIXME cross hair does not work properly because a 
        ...
        Listener listener = new Listener() {
            public void handleEvent (Event event) {
                switch (event.type) {
                    case SWT.MouseDown:
                        zoomPoint = new Point(event.x, event.y);
                    ...
                    case SWT.MouseMove:
                         ...
                         * To work properly, it would require a class 
                         * similar to the SWT Tracker (used to handle 
                         * the zoom box).
                        ...
                        */
                        ...
                        Rectangle rect = new Rectangle(zoomPoint.x, 
                                zoomPoint.y, zoomPoint.x - event.x, zoomPoint.y
                                - event.y);
                        tracker.setRectangles(new Rectangle [] {rect});
                        tracker.open();
                        Rectangle trect = tracker.getRectangles()[0];
                        // handle zoom
                        boolean hZoom = false;
                        boolean vZoom = false;
                        ...
                        if (hZoom && vZoom) 
                        {
                            ...
                            zoomRectangle = new Rectangle2D.Double(trect.x, 
                                    trect.y, trect.width, trect.height);
                            Rectangle2D.intersect(zoomRectangle, 
                                    scaledDataArea, zoomRectangle);
                        }
                        else if (hZoom) 
                        {
                            zoomRectangle = new Rectangle2D.Double(
                                    trect.x, scaledDataArea.getMinY(),
                                    trect.width, scaledDataArea.getHeight());
                            Rectangle2D.intersect(zoomRectangle, scaledDataArea,
                                    zoomRectangle);
                        }
                        else if (vZoom) 
                        {
                            zoomRectangle = new Rectangle2D.Double(
                                    scaledDataArea.getMinX(), trect.y,
                                    scaledDataArea.getWidth(), trect.height);
                            Rectangle2D.intersect(zoomRectangle, 
                                    scaledDataArea, zoomRectangle);
                        }
                        // no break here cause we want to catch mouse up event
                    case SWT.MouseUp:
                        if (zoomRectangle == null) {
                            Rectangle2D screenDataArea 
                                    = getScreenDataArea(event.x, event.y);
                            if (screenDataArea != null) {
                                zoomPoint = getPointInRectangle(
                                        event.x, event.y, screenDataArea);
                            }
                            if (popup != null && event.button == 3) {
                                displayPopupMenu(
                                        canvas.getShell().getLocation().x 
                                        + canvas.getParent().getLocation().x 
                                        + canvas.getLocation().x + event.x, 
                                        canvas.getShell().getLocation().y 
                                        + canvas.getParent().getLocation().y 
                                        + canvas.getLocation().y + event.y);
                            }
                        }
                        else
                        {
    ...
    public Rectangle2D scale(Rectangle2D rect) {
        ...
        double x = rect.getX() * getScaleX() + insets.x;
        double y = rect.getY() * this.getScaleY() + insets.y;
     ...
     */
    private void drawVerticalAxisTrace(int x, Graphics2D sg2) 
    {
        Line2D verticalTraceLine;
        Rectangle2D dataArea = getScreenDataArea();
        sg2.setXORMode(java.awt.Color.orange);
        if (((int) dataArea.getMinX() < x) && (x < (int) dataArea.getMaxX())) {
            verticalTraceLine = new Line2D.Float(x, (int) dataArea.getMinY(),
                    x, (int) dataArea.getMaxY());
            sg2.draw(verticalTraceLine);
        }
    }
     ...
     */
    private void drawHorizontalAxisTrace(int y, Graphics2D sg2) {
        Line2D horizontalTraceLine;
        Rectangle2D dataArea = getScreenDataArea();
        sg2.setXORMode(java.awt.Color.orange );
        if (((int) dataArea.getMinY() < y ) && (y < (int) dataArea.getMaxY())) {
            horizontalTraceLine = new Line2D.Float(
                    (int) dataArea.getMinX(), y, (int) dataArea.getMaxX(), y);
            sg2.draw(horizontalTraceLine);
        }
    }
