 ...
 *                   Thierry Saura;
 *
 * $Id: StackedBarRenderer.java,v 1.1 2007/10/10 20:07:35 vauchers Exp $
 *
 * Changes
 * -------
...
public class StackedBarRenderer extends BarRenderer implements Serializable {
     ...
     */
    public StackedBarRenderer(CategoryItemLabelGenerator toolTipGenerator) {
     ...
     */
    public StackedBarRenderer(CategoryURLGenerator urlGenerator) {
    ...
    public void drawVerticalItem(Graphics2D g2,
        ...
        if (hasLinkingLines() && (pointsRegister == null)) {
            ...
            pointsRegister = new double[data.getColumnCount() * data.getRowCount() * 2];
        }
        ...
        if (value != null) {
            ...
            if (hasLinkingLines()) {
                ...
                if (column == 0) {
                    ...
                    // other categories
                    int position = (column * data.getRowCount() * 2) + (2 * row);
                    pointsRegister[position] = rectX + rectWidth;
                    if (positiveValue) {
                        // draw a line between two stacked bars
                        double lastX = pointsRegister[position - 2 * data.getRowCount()];
                        if (lastX > 0.0) {
                            Line2D line = new Line2D.Double(rectX, barY, lastX,
                                pointsRegister[position - 2 * data.getRowCount() + 1]);
