 ...
 *                   David M O'Donnell;
 *
 * $Id: ChartPanel.java,v 1.1 2007/10/10 20:07:37 vauchers Exp $
 *
...
public class ChartPanel extends JPanel implements ChartPanelConstants,
    ...
    public void zoomInHorizontal(double x) {
        ...
        ValueAxis axis = getHorizontalValueAxis(chart.getPlot());

        if (axis != null) {
            double value = axis.translateJava2DtoValue((float) x,
                                                       this.info.getDataArea(),
                                                       AxisLocation.BOTTOM);
    ...
    public void zoomInVertical(double y) {
        ...
        ValueAxis axis = getVerticalValueAxis(chart.getPlot());

        if (axis != null) {
            double value = axis.translateJava2DtoValue((float) y,
                                                       this.info.getDataArea(),
                                                       AxisLocation.LEFT);
            axis.resizeRange(0.5, value);
        }
    ...
    public void zoom(Rectangle2D selection) {
        ...
        if ((selection.getHeight() > 0) && (selection.getWidth() > 0)) {
            ...
            Rectangle2D scaledDataArea = getScaledDataArea();
            ValueAxis hva = getHorizontalValueAxis(this.chart.getPlot());
            if (hva != null) {
                lower = hva.translateJava2DtoValue((float) selection.getX(),
                                                   scaledDataArea,
                                                   AxisLocation.BOTTOM);
                upper = hva.translateJava2DtoValue((float) selection.getMaxX(),
                                                   scaledDataArea,
                                                   AxisLocation.BOTTOM);
                if (lower < upper) {
                    hva.setRange(lower, upper);
                }
                else {
                    hva.setRange(upper, lower);
                }
            }

            ValueAxis vva = getVerticalValueAxis(this.chart.getPlot());
            if (vva != null) {
                lower = vva.translateJava2DtoValue((float) selection.getMaxY(),
                                                   scaledDataArea,
                                                   AxisLocation.LEFT);
                upper = vva.translateJava2DtoValue((float) selection.getY(),
                                                   scaledDataArea,
                                                   AxisLocation.LEFT);
                if (lower < upper) {
                    vva.setRange(lower, upper);
                }
                else {
                    vva.setRange(upper, lower);
                }
            }
        }
    ...
    public void autoRangeHorizontal() {
        ValueAxis axis = getHorizontalValueAxis(this.chart.getPlot());
        if (axis != null) {
    ...
    public void autoRangeVertical() {
        ValueAxis axis = getVerticalValueAxis(this.chart.getPlot());
        if (axis != null) {
            axis.setAutoRange(true);
        }
    }
