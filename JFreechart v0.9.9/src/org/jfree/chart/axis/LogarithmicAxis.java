 ...
 *                   David M. O'Donnell;
 *
 * $Id: LogarithmicAxis.java,v 1.1 2007/10/10 20:07:40 vauchers Exp $
 *
 * Changes
 * -------
...
public class LogarithmicAxis extends NumberAxis {
    ...
    public void refreshTicks(Graphics2D g2, Rectangle2D drawArea, Rectangle2D dataArea,
        ...
        if (location == AxisLocation.TOP || location == AxisLocation.BOTTOM) {
            refreshTicksHorizontal(g2, drawArea, dataArea, location);
        }
        else if (location == AxisLocation.LEFT || location == AxisLocation.RIGHT) {
            refreshTicksVertical(g2, drawArea, dataArea, location);
        }
        ...
        for (int i = iBegCount; i <= iEndCount; i++) {
            ...
            for (int j = 0; j < jEndCount; j++) {
                ...
                if (tickVal >= lowerBoundVal - SMALL_LOG_VALUE) {
                    ...
                    double yy = translateValueToJava2D(tickVal, dataArea, location);
