...
public class InfoSkills extends FilterAdapterPanel
{
	...
	private JComboBox currCharacterClass = new JComboBox(); // now contains Strings of Class/lvl
...
//				asplit.setDividerLocation((int)(InfoSkills.this.getSize().getWidth() - 408));
	...
	private boolean modRank(Skill aSkill, double points)
	{
		...
		{
			JOptionPane.showMessageDialog(null, "You must complete selections for "+pcl.getClassKeyName()+"/"+pcl.getLevel()+" before selecting skills for another class/level", "PCGen", JOptionPane.INFORMATION_MESSAGE);
			return false;
		}
		int skillPool = pcl.getSkillPointsRemaining();
		if (points > 0.0 && points > skillPool)
		{
			JOptionPane.showMessageDialog(null, "You only have "+skillPool+" points to spend", "PCGen", JOptionPane.INFORMATION_MESSAGE);
			return false;
		}
		if (points < 0.0 && skillPool - points > pcl.getSkillPointsGained())
		{
			JOptionPane.showMessageDialog(null, pcl.getClassKeyName()+"/"+pcl.getLevel()+" can have at most "+pcl.getSkillPointsGained()+" skill points.", "PCGen", JOptionPane.INFORMATION_MESSAGE);
			return false;
		}
		Skill bSkill = aSkill;
		...
		if (aSkill != null)
		{
			...
			Collections.sort(currentPC.getSkillList(), new Comparator()
			{
...
//		}
	 ...
	 */
	private static final class OutputOrderEditor extends JComboBox implements TableCellEditor
	{
