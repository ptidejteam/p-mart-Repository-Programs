...
public final class InfoSummary extends FilterAdapterPanel
{
	...
	private static final int ADJ_COLUMN = 2;
	private static final int TOTAL_COLUMN = 3;
	private static final int MOD_COLUMN = 4;
	private static final int INC_COLUMN = 5;
	private static final int DEC_COLUMN = 6;

	static final long serialVersionUID = -2633367637476404623L;
	...
	{
		return alignmentStrings = Globals.getAlignmentListStrings(true);
	}
	...
	private void initComponents()
	{
		...
		statTable.setColAlign(0, SwingConstants.LEFT);
		...
		statTable.setColAlign(1, SwingConstants.CENTER);
		...
		statTable.setColAlign(2, SwingConstants.CENTER);
		...
		statTable.setColAlign(3, SwingConstants.CENTER);
		...
		statTable.setColAlign(4, SwingConstants.CENTER);
...
/////////////////////////
			...
			case INC_COLUMN:
				{
					...
					{
						JOptionPane.showMessageDialog(null, "Cannot raise stat above " + Integer.toString(aStat.getMaxValue()), Constants.s_APPNAME, JOptionPane.ERROR_MESSAGE);
					}
			...
			case DEC_COLUMN:
				{
					...
					{
						JOptionPane.showMessageDialog(null, "Cannot lower stat below " + Integer.toString(aStat.getMinValue()), Constants.s_APPNAME, JOptionPane.ERROR_MESSAGE);
					}
	...
	private void updatePool(boolean checkPurchasePoints)
	{
		...
		int pcTotalLevels = aPC.getTotalLevels();
		if (pcTotalLevels < 2)
		{
			aPC.setDirty(true);
		...
		public Object getValueAt(int rowIndex, int columnIndex)
		{
			...
			switch (columnIndex)
			{
				case SCORE_COLUMN:
					...
					return new Integer(currentPC.getStatList().getBaseStatFor(Globals.s_ATTRIBSHORT[rowIndex]));
				case ADJ_COLUMN:
					if (currentPC.isNonAbility(rowIndex))
					{
						return "*";
					}
					return new Integer(currentPC.getStatList().getTotalStatFor(Globals.s_ATTRIBSHORT[rowIndex]) - currentPC.getStatList().getBaseStatFor(Globals.s_ATTRIBSHORT[rowIndex]));
				case TOTAL_COLUMN:
					if (currentPC.isNonAbility(rowIndex))
					{
						return "*";
					}
					return new Integer(currentPC.getStatList().getTotalStatFor(Globals.s_ATTRIBSHORT[rowIndex]));
				case MOD_COLUMN:
					if (currentPC.isNonAbility(rowIndex))
					{
						return new Integer(0);
					}
					return new Integer(currentPC.getStatList().getStatModFor(Globals.s_ATTRIBSHORT[rowIndex]));
				case INC_COLUMN:
	...
	public void setPcNameText(JTextField pcNameText)
	{
		this.pcNameText = pcNameText;
	}
}
