 ...
 *
 * $Id: Skill.java,v 1.1 2006/02/21 01:10:50 vauchers Exp $
 */
...
public final class Skill extends PObject
{
	...
	public boolean isClassSkill(PCClass pc)
	{
		...
		if (pc.skillList().contains(name))
		{
			return true;
                ...
                // added 6/10/2003 to make SKILLIST functional. This is to fix bug 698090
                if (pc.getClassSkillList() != null)
                {
                    Iterator p = pc.getClassSkillList().iterator();
                    while (p.hasNext())
                    {
                        String classString = p.next().toString();
                        for(Iterator c = Globals.getClassList().iterator(); c.hasNext();)
                        {
                            PCClass skillClass = (PCClass) c.next();
                            if (classString.equals(skillClass.toString()))
                            {
                                if (skillClass.skillList().contains(name))
                                {
                                    return true;
                                }
                                if (skillClass.hasCSkill(name))
                                {
                                    return true;
                                }
                            }
		...
		for (Iterator i = classList.iterator(); i.hasNext();)
		{
			...
			if (aString.length() > 0 && aString.charAt(0) == '!' && (aString.substring(1).equalsIgnoreCase(pc.getName()) || aString.substring(1).equalsIgnoreCase(pc.getSubClassName())))
			{
				return false; // this is an excluded-from-class-skill list
			}
			if ("ALL".equals(aString) || aString.equals(pc.getName()) || aString.equals(pc.getSubClassName()) || (pc.getClassSkillList() != null && pc.getClassSkillList().contains(aString)))
			{
				return true;
			}
		}
		...
		for (Iterator e = aPC.getCharacterDomainList().iterator(); e.hasNext();)
		{
			...
			if (aCD.getDomain() != null && aCD.isFromPCClass(pc.getName()) && aCD.getDomain().hasCSkill(name))
			{
				return true;
			}
		}
	...
	private boolean isCrossClassSkill(PCClass pc)
	{
		...
		for (Iterator e = aPC.getCharacterDomainList().iterator(); e.hasNext();)
		{
			...
			if (aCD.getDomain() != null && aCD.isFromPCClass(pc.getName()) && aCD.getDomain().hasCCSkill(name))
			{
				return true;
			}
		}
	...
	private Float getRankAdj()
	{
		...
		return new Float(currentPC.getTotalBonusTo("SKILLRANK", skillName, true));
	}
	...
	public static boolean chooseSpokenLanguage(PlayerCharacter aPC, Skill speakLanguage)
	{
		if (aPC != null)
		{
			...
			final ArrayList selectedLangNames = new ArrayList();
