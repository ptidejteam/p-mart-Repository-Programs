 ...
 *
 * $Id: LevelAbility.java,v 1.1 2006/02/21 01:10:50 vauchers Exp $
*/
		...
		else
		{
			...
			else
			{
				...
				if ((aArrayList.size() > 0) && (type != 7))
				{
					...
					{
						Logging.debugPrint("choice: " + c.getSelectedList() + eString);
					}
		...
		switch (type)
		{
			case 4:
				if (aText.equals("DOMAIN"))
				{
					...
					final List pcCharacterDomainList = aPC.getCharacterDomainList();
			...
			case 6:
				if (selectedList.size() > 0)
				{
					...
					if (bClass == null)
					{
						...
						if (bClass == null)
						{
							Logging.errorPrint("ERROR:Expected to find a class named " + cString);
							break;
						}
