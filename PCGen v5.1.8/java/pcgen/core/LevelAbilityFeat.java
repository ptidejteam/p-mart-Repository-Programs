...
final class LevelAbilityFeat extends LevelAbility
{
	...
	void processToken(String aChoice, List aArrayList, String bString)
	{
		...
		else
		{
			...
			if (aFeat == null)
			{
				Logging.debugPrint("LevelAbilityFeat: Feat not found: ", featName);
				return;
			}
			...
			if (aFeat.passesPreReqTests() || isVFeat)
			{
	...
	public void processChoice(List aArrayList, List selectedList, String extraString)
	{
		...
		if (isVFeat)
		{
			...
			for (int index = 0; index < listSize; ++index)
			{
				...
				if (aFeat != null)
				{
					...
					aFeat = PlayerCharacter.getFeatNamedInList(aPC.vFeatList(), aFeat.getName());
					if (aFeat.isMultiples())
					{
		...
		else
		{
			...
			{
				final String featString = selectedList.get(n).toString() + extraString;
