 ...
 * Last Edited: $Date: 2006/02/21 01:10:58 $
 *
 */
...
//			{
//				equipment.setAttacks(Integer.parseInt(colString.substring(8)));
//			}
			...
			{
				Logging.debugPrint("Equipment DR tag found.");
			...
			else if ((aLen > 6) && colString.startsWith("HANDS:"))
			{
				...
				{
					throw new PersistenceLayerException("Illegal number of required hands " + sourceURL.toString() + ":" + Integer.toString(lineNum) + " \"" + colString + "\"");
				}
			}
			...
			else if ((aLen > 5) && colString.startsWith("SLOTS:"))
			{
				...
				{
					throw new PersistenceLayerException("Illegal number of required slots " + sourceURL.toString() + ":" + Integer.toString(lineNum) + " \"" + colString + "\"");
				}
			}
			...
			else if ((aLen > 6) && colString.startsWith("SPROP:"))
			{
				...
				//GMGen Section: Don't remove this, it is intended for use with GMGen.
				if(SettingsHandler.isGMGen()) {
					String treasureList = colString.substring(13);
					StringTokenizer treasureTok = new StringTokenizer(treasureList, "|", false);
					while (treasureTok.hasMoreTokens())
					{
						try
						{
							...
							int weight = new Integer(cmdTok.nextToken().trim()).intValue();
						...
						{
							throw new PersistenceLayerException("Illegal equipment info " + sourceURL.toString() + ":" + Integer.toString(lineNum) + " \"" + colString + "\"");
						}
					}
				}
				//End GMGen Section
