    ...
    public void encode(OutputStream out, byte[] object) throws IOException {
        ...
        for(int i = 0; i < object.length; i++) {
            ...
            if((b_out.getWrittenLength() + ((7 + charset.length) * count)) > (count * line)) {
                ...
                /*
                 * BUGFIX: Former code subtracted -1 from object.length
                 * when checking for folding. Subject lines exactly one character
                 * longer than the max length thus lost one charatcer by getting a
                 * premature end but no line break /LJN 20030401
                 */
                
                if(i < object.length) { // fold and start over
