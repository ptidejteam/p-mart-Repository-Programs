...
* 3) MIME content type "message" other than the primary subtype 'rfc822'.<p>
*
...
*
* The parser will allow all header fields. If an implementor whishes to be more strict about
* what header fields to allow he must call <code>useStrictHeaderFields</code>. This will make
* the parser ignore fields it does not recognize and treat them as a part of the body.
*
...
public class MessageParser {
    ...
    // use strict header fields?
    private boolean strictHeaderFields;
    
    // strict header field names 
    ...
    */
    ...
    public MessageParser(CoderTable table) {
        ...
        strictHeaderFields = false;
        
        fieldNames = new String[] { "to", "from", "sender", "cc", "bcc", "date",
    ...
    */
    ...
    /**
    * Set if the parser should use only recognized header fields. If the parser does not 
    * recognize a field it will end the current header and start a body.
    */
    
    public void useStrictHeaderFields(boolean flag) {
        strictHeaderFields = flag;
    }
    
    
    /**
    * Get header field names recognized by this parser.
    */
    
    public String[] getStrictHeaderFieldNames() {
        return fieldNames;
    }
    
    
    /**
    * Set the header field names recognized by this parser.
    */
    
    public void setStrictHeaderFieldNames(String[] names) {
        fieldNames = names;
    }
	...
	*/
	...
	private Body readBody(Body body, String boundary, PushbackInputStream in) throws IOException {
	     ...
	     */
         ...
         if(boundary.length() > 0 && old_body.length > line.length()) b_length = line.length() + 2; // add 2 for last line break
         byte[] new_body = new byte[old_body.length - b_length];
         System.arraycopy(old_body, 0, new_body, 0, new_body.length);
         //for(int i = 0; i < new_body.length; i++) new_body[i] = old_body[i];
         
         
         //System.out.println("PARSED BODY >>> ");
         //System.out.println(new String(new_body));
         ...
         if(body instanceof BinaryBody) ((BinaryBody)body).setBody(last_body);
             ...
             // interesting, we've got a message - at the moment we're not synchronized so a recursion
             // won't do. try a new parser instead
             
             MessageParser pars = new MessageParser(coderTable);
             pars.setDefaultCharset(defaultCharset);
             pars.useStrictHeaderFields(strictHeaderFields);
             pars.setStrictHeaderFieldNames(fieldNames);
             
             ((MessageBody)body).setMessage(pars.parseMessage(new PushbackInputStream(new BufferedInputStream(new ByteArrayInputStream(last_body)), 256)));
	...
	private Body getNewBody(Header header) {
	    ...
	    switch(type) {
            ...
            case DIGEST : 
                ...
                break;
            case MESSAGE : 
                body = new MessageBody(new DefaultMessage(new TextBody(), defaultCharset));
                break;
            case JPG_IMAGE : 
    ...
    private HeaderField readHeaderField(PushbackInputStream in, boolean isFirst) throws IOException {
        ...
        while(!finished) {
            ...
            // a '=' character might be the start of an encoded partion of the field, so...
                ...
                if(tmp != '?') value.append("=").append((char)tmp); // no encoding
                    ...
                    // we encounter a space or a new line or until we have four
                    // question marks
                    
                    // should we break out of anything but new lines or spaces we mustn't
                    // try to unread it
                    
                    boolean doUnread = true;
                    ...
                    // unread the character that made us stop the loop above if appropriate
                    
                    if(doUnread) unread(in, tmp); 
        ...
        if(isAddressField(name.toString())) {
            ...
            String stringName = name.toString().trim();
            
            if(v.indexOf(';') > -1) {
                
                if(!stringName.equalsIgnoreCase("content-type")) pf = new ParameterField(stringName, v.substring(0, v.indexOf(';')), new Q_Encoder(defaultCharset));
                else pf = new ParameterField(stringName, v.substring(0, v.indexOf(';')));
                ...
                if(!stringName.equalsIgnoreCase("content-type")) pf = new ParameterField(stringName, v, new Q_Encoder(defaultCharset));
                pf = new ParameterField(stringName, v);
    ...
    */
    ...
    // attempt to parse parameters to a hashtable - names and keys and values as...
        ...
        for(int i = 0; i < ch.length; i++) {
            ...
            if(ch[i] == '\"') {
                ...
                if(state != QUOTE) {
                    ...
                    table.put(tmp_name.trim(), tmp_2.trim());
