 ...
 *
 * $Id: Equipment.java,v 1.1 2006/02/21 00:57:41 vauchers Exp $
 */
...
//attempt to add DR vales for Armor
	 ...
	 */
	private int getContainedEquipmentCount()
	{
		if (d_containedEquipment == null)
		{
			return 0;
	 ...
	 */
	private Equipment getContainedEquipment(int i)
	{
	...
	private boolean checkContainerCapacity(TreeSet aTypeList, Float aQuant)
	{
		...
		if ("".equals(pickChildType(aTypeList, aQuant)))
		{
			return false;
		}
		else
	...
	{
		int dbon = DefBon.intValue() + (int) bonusTo("EQMARMOR", "DEFBONUS", true);
		if (dbon > 100)
		{
			dbon = 100;
		}
 ...
 */
				...
				(isShield() && aPC.canEquip("Shield")))
			{
				...
				if (isWeapon())
				{
					...
					if (wp == null)
					{
						...
						Globals.debugPrint("Globals: " + Globals.getWeaponProfList() + Constants.s_LINE_SEP + "Proficiency name: " + aWProf + " " + this);
						return;
					}
	...
	private void setWeightMod(String aString)
	{
		try
		{
			weightMod = Double.parseDouble(aString);
		}
		catch (NumberFormatException e)
		{
			weightMod = 0.0;
	...
	public String formatSaveLine(String sep, String endPart)
	{
		...
		if (weightMod != 0.0)
		{
			sbuf.append(sep).append("WEIGHTMOD").append(endPart).append(weightMod);
		}
	...
	public String getItemNameFromModifiers()
	{
		...
		if (Globals.sizeInt(getBaseSize()) != iSize)
		{
			...
			common.append(((SizeAdjustment) Globals.getSizeAdjustmentList().get(iSize)).toString());
		}
