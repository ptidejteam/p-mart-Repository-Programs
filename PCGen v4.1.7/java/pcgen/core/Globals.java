 ...
 *
 * $Id: Globals.java,v 1.1 2006/02/21 00:57:41 vauchers Exp $
 */
package pcgen.core;
...
public final class Globals
{
	...
	private static String pcGenVersion = null; // set by static initializer
	...
	private static final String defaultPcgPath = getDefaultPath() + java.io.File.separator + "characters";
	...
	private static final ArrayList levelInfo = new ArrayList(250);
	...
	private static final ArrayList weaponProfList = new ArrayList(400);
	private static final DenominationList denominationList =
		new DenominationList(); // derived from ArrayList
	...
	private static final TreeSet effectTypesSet = new TreeSet();
	...
	private static TreeSet getEffectTypesSet()
	{
...
//	}
	...
	private static TreeSet getRangesSet()
	{
		return rangesSet;
	}
	...
	{
		Object obj = getSpellMap().get(aKey);
...
//	}
	...
	private static TreeSet getSrSet()
	{
		return srSet;
	}
	...
	public static boolean canResizeHaveEffect(Equipment aEq, ArrayList typeList)
	{
		...
		for (int iSize = 0; iSize < sizeAdjustmentList.size(); ++iSize)
		{
			...
			if ((sadj.getBonusTo("ITEMCOST", typeList, 1.0) != 1.0) ||
				(aEq.isArmor() || aEq.isShield() && (sadj.getBonusTo("ACVALUE", typeList, 1.0) != 1.0)) ||
				((aEq.getWeightAsDouble() != 0.0) && (sadj.getBonusTo("ITEMWEIGHT", typeList, 1.0) != 1.0)) ||
				(aEq.isContainer() && (sadj.getBonusTo("ITEMCAPACITY", typeList, 1.0) != 1.0)))
			{
				return true;
			}
		}
...
//	}
	...
	private static void writePurchaseModeConfiguration()
	{
	...
	{
		final BufferedWriter bw = getWriter(customDeityFilePath(true));
		if (bw == null)
		{
			return;
		}
	...
	{
		final BufferedWriter bw = getWriter(customSkillFilePath(true));
		if (bw == null)
		{
			return;
		}
		try
		{
			...
			for (Iterator e = getSkillList().iterator(); e.hasNext();)
			{
				...
				if (aSkill.isType(Constants.s_CUSTOM))
				{
					bw.write(aSkill.getPCCText());
					bw.newLine();
				}
			}
		}
	...
	private static void writeCustomItems()
	{
	...
	public static String customSkillFilePath(final boolean usePath)
	{
		...
		return aString + File.separator + "customsources" + File.separator + "customSkills.lst";
	}
