 ...
 *
 * $Id: LevelAbility.java,v 1.1 2006/02/21 00:57:42 vauchers Exp $
*/
...
class LevelAbility extends PObject
{
	...
	final void setOwner(PCClass aOwnerClass)
	{
		ownerClass = aOwnerClass;
	}
	...
	static LevelAbility createAbility(PCClass aOwnerClass, int aLevel, String aList)
	{
	...
	final void process(ArrayList availableList)
	{
		String bString = list;
		aText = bString;
		type = 4;
		...
		switch (type)
		{
			...
			case 6:
				...
				for (Iterator e1 = aPC.getClassList().iterator(); e1.hasNext();)
				{
					...
					else if (!"".equals(bClass.getSpellType()) && !(Constants.s_NONE.equals(bClass.getSpellType()))
						&& !bClass.getKeyName().equals(ownerClass.getKeyName()))
					{
		...
		switch (type)
		{
			case 4:
				...
				for (index = 0; index < listSize; ++index)
				{
					...
					SpecialAbility sa = new SpecialAbility(zString, "PCCLASS|" + ownerClass.getName() + '|' + level);
					ownerClass.addSpecialAbilityToList(sa);
					ownerClass.addSave(zString);
				}
				break;
			case 6:
				if (selectedList.size() > 0)
				{
					...
					//final SpecialAbility sa = new SpecialAbility("Bonus Caster Level for " + cString + aString, "PCCLASS|" + ownerClass.getName() + "|" + level);

					ownerClass.addBonusList(0 + "|PCLEVEL|" + bClass.getKeyName() + "|1");
					ownerClass.addSave("BONUS|" + 0 + "|PCLEVEL|" + bClass.getKeyName() + "|1");
					...
					int dnum = (int) ownerClass.getBonusTo("DOMAIN", "NUMBER", ownerClass.getLevel().intValue());
					if (ownerClass.getLevel().intValue() > 0)
					{
						dnum -= (int) ownerClass.getBonusTo("DOMAIN", "NUMBER", ownerClass.getLevel().intValue() - 1);
					}
