 ...
 * Last Edited: $Date: 2006/02/21 00:57:42 $
 *
 */
...
public final class BioSet
{
	...
	private static HashMap raceMap = new HashMap(); // key = Dwarf.BASEAGE or Dwarf%.BASEAGE, value = tagged value
	private static HashMap userMap = new HashMap(); // for user overrides/additions, check this before raceMap
	private static HashMap ageMap = new HashMap(); // key = region.age, value = bonus adjustments
	...
	// region = region (e.g. None)
	// line = 0|Adult\tBONUS:STAT|MUS|-1
	...
	// setParameters = . delimited list of non-random settings
	// randomize = . delimited list of attributes to randomize
	// either could be AGE.WT.HT  but you can't include same paramter in both
	public static void randomize(String randomize)
	{
		...
		{
			raceName = raceName.substring(0, raceName.indexOf("(")).trim() + '%';
		}
		else
		{
			raceName += '%';
	...
	private static String replaceString(String input, String replacement, int value)
	{
	...
	{
		String r = (String) userMap.get(regionName + aPC.getRace().getName() + addKey);
		if (r != null)
		{
			return r;
		}
		r = (String) userMap.get(regionName + raceName + addKey);
		if (r != null)
		{
			return r;
		}
		r = (String) raceMap.get(regionName + aPC.getRace().getName() + addKey);
		if (r != null)
		{
			return r;
		}
