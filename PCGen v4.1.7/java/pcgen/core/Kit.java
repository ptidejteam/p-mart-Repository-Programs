...
public final class Kit extends PObject implements Comparable
{
	...
	public void addKitSkills(ArrayList thingsToAdd, ArrayList warnings)
	{
		...
		if ((aList != null) && (aList.size() != 0))
		{
			...
			for (int i = 0, x = aList.size(); i < x; ++i)
			{
				...
				KitSkillAdd sta = new KitSkillAdd(aSkill,
					Math.min(kSkill.getRank(),
						aPC.getMaxRank(skillName, pcClass).doubleValue()),
					aSkill.costForPCClass(pcClass).intValue(),
					kSkill.isFree()
				);
				skillList.add(sta);
			}
	...
	private void updatePCSpells(Spell obj, PCClass pcClass)
	{
		...
		if (spLevel < 0)
		{
			System.err.println("SPELLS: " + pcClass.getName() + " cannot cast spell \"" + obj.getName() + "\"");
			return;
		}
	...
	private void updatePCSkills(KitSkillAdd obj, PCClass pcClass)
	{
		...
		if (aString.length() > 0)
		{
			System.err.println("SKILL: " + aString);
		}
