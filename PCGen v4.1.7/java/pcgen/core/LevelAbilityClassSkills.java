 ...
 *
 * $Id: LevelAbilityClassSkills.java,v 1.1 2006/02/21 00:57:42 vauchers Exp $
 */
package pcgen.core;
	...
	public ArrayList getChoicesList(String bString)
	{
		...
		while (aTok.hasMoreTokens())
		{
			...
			else
			{
				...
				if ((aSkill != null) && !aSkill.isClassSkill(ownerClass))
				{
			...
			for (int index = 0, x = Globals.getSkillList().size(); index < x; ++index)
			{
				...
				//
				if (aSkill.isClassSkill(ownerClass))
				{
					continue;
				}
				...
				if (autoRank != 0)
				{
					...
					if (aSkill != null)
					{
						...
						if (aPC != null)
						{
							...
							aSkill.modRanks(1.0, ownerClass, true);
						}
					}
				}
			}
