 ...
 *
 * $Id: Utility.java,v 1.1 2006/02/21 00:57:42 vauchers Exp $
 */
package pcgen.core;
	...
	static boolean modChoices(PObject obj, ArrayList availableList, ArrayList selectedList, boolean process)
	{
		...
		else if ("SPELLLIST".equals(choiceType))
		{
			if (process && (aFeat != null))
			{
				...
				if (aList.size() > 1)
				{
					selectedValue = javax.swing.JOptionPane.showInputDialog(null,
		...
		else if ("SKILLLIST".equals(choiceType) || "CCSKILLLIST".equals(choiceType) ||
			"NONCLASSSKILLLIST".equals(choiceType))
		{
			...
			else  // if it was LIST
			{
				...
				{
					aSkill = (Skill) iter.next();
					if ("NONCLASSSKILLLIST".equals(choiceType) && (aSkill.costForPCClassList(aPC.getClassList()).intValue() == 1 ||
						aSkill.isExclusive()))
					{
			...
			while (aTok.hasMoreTokens())
			{
				...
				else if (aString.startsWith("WSize."))
				{
					...
					for (Iterator setIter = aPC.getWeaponProfList().iterator(); setIter.hasNext();)
					{
						...
						if ("Light".equals(sString) && wp.isWpLight() && !availableListContainsBString)
						{
							availableList.add(bString);
						}
						if ("1 handed".equals(sString) && wp.isWpOneHanded() && !availableListContainsBString)
						{
							availableList.add(bString);
						}
						if ("2 handed".equals(sString) && wp.isWpTwoHanded() && !availableListContainsBString)
						{
							availableList.add(bString);
						}
					}
