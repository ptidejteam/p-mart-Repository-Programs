 ...
 *
 * $Id: PCTemplateLoader.java,v 1.1 2006/02/21 00:57:49 vauchers Exp $
 */
...
final class PCTemplateLoader
{
	...
	public static void parseLine(PCTemplate template, String inputLine, File sourceFile, int lineNum) throws PersistenceLayerException
	{
		...
		while (colToken.hasMoreTokens())
		{
			...
			{
				template.setMoveRates(colString.substring(5));
			}
			else if (colString.startsWith("MOVEA:"))
			{
				template.setMoveRates(colString.substring(6));
				template.setMoveRatesFlag(1);
			}
			else if (colString.startsWith("MOVECLONE:"))
			{
				template.setMoveRates(colString.substring(10));
				template.setMoveRatesFlag(2);
			}
			...
			else
			{
				...
				for (iStat = 0; iStat < Globals.s_ATTRIBSHORT.length; ++iStat)
				{
					...
					{
						Globals.debugPrint("TEMPLATE: support for " + colString + " has been removed. Use ADD:BONUS|STAT instead");
