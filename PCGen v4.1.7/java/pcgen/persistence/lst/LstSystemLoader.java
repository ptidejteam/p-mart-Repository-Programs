 ...
 *
 * $Id: LstSystemLoader.java,v 1.1 2006/02/21 00:57:49 vauchers Exp $
 */
...
//
	...
	private void initCampaignFromList(List aArrayList, String aString) throws PersistenceLayerException
	{
		...
		//
		if (new File(Globals.customSkillFilePath(true)).exists())
		{
			tempGame = Globals.customSkillFilePath(false);
		...
		try
		{
			...
			while (newlineStr.hasMoreTokens())
			{
				...
				if (aLine.indexOf('\t') > 2)
				{
					...
					else if (nameString.indexOf(".COPY=") > -1)
					{
						...
						if (!isModItem)
						{
							...
							else
							{
								...
								if (anObj != null)
								{
									...
									Globals.errorPrint("Could not copy " + copyName + " to create " + nameString);
							}
							if (anObj != null)
								aList.add(anObj);
						}
...
//				throw new PersistenceLayerException(e, "ERROR:" + fileName + Constants.s_LINE_SEP + "error " + aLine + Constants.s_LINE_SEP + "Exception type:" + e.getClass().getName() + Constants.s_LINE_SEP + "Message:" + e.getMessage());
	...
	private void loadCustomItems()
	{
		...
		final BufferedReader br = Globals.getCustomEquipmentReader();
...
*/
	...
	private static void loadGameModeLoadInfo(GameMode gameMode, File f)
	{
		...
		{
			//Globals.errorPrint("loadGameModeLoadInfo: file not found: " + f.getPath(), ex);	// Ignore error--don't care if it's not there--means no encumberance rules
		}
		finally
		{
			try
	...
	private static GameMode loadGameModeMiscInfo(String modeName, File f)
	{
		...
		try
		{
			...
			try
			{
				for (int lineNum = 1; br != null; ++lineNum)
				{
					...
					GameModeLoader.parseMiscGameInfoLine(gameMode, aLine, f, lineNum);
				}
			}
			catch (Exception ex)
			{
		...
		{
			Globals.errorPrint("loadGameModeMiscInfo: file not found: " + f.getPath(), ex);
		}
		finally
		{
			try
