...
final class InfoSummary extends FilterAdapterPanel
{
	...
	protected RaceComboModel raceComboModel = null;		// Model for the race combo box.
	protected ClassComboModel classComboModel = null;	// Model for the race combo box.
	protected ClassModel pcClassTreeModel = null;		// Model for the pcClassTable.
	protected JTableEx pcClassTable;				// Contains the PC's current classes and levels
	...
	private TitledBorder statBorder;
	...
	private ActionListener levelCmdListener = new ActionListener()
	{
		public void actionPerformed(ActionEvent evt)
		{
			...
			PCClass pcClass = (PCClass) classComboModel.getSelectedItem();
			if (pcClass == null)
			{
				JOptionPane.showMessageDialog(null,
...
/////////////////////////
		...
		lblMonsterlHD.setText("Monster Hit Dice: ");
		c.anchor = GridBagConstraints.EAST;
		...
		lblHDModify.setText("Hit Dice to Add/Remove:");
		c.anchor = GridBagConstraints.EAST;
...
/////////////////////////
	...
	private void updateHD()
	{
		...
		if (aPC != null)
		{
			...
			if (monsterClass != null)
			{
				...
				if (aClass != null)
				{
					monsterHD += aClass.getLevel().intValue();
				}
			}
		}
			...
			if (!r.equals(aPC.getRace()))
			{
				...
				if (pnlHD.isVisible())
				{
					...
					if (monsterClass != null)
					{
						...
						if (aClass != null)
						{
							final int numLevels = aClass.getLevel().intValue() - aPC.getRace().getMonsterClassLevels();
							if (numLevels > 0)
							{
	...
	public void refreshDisplay()
	{
		...
		if (aPC.getLevelInfoSize() != 0)
		{
			...
			{
				classComboModel.setSelectedItem(lastSelection);
			}
		}
	...
	private void alignmentChanged()
	{
		...
		if (unqualified.length() > 0)
		{
			if (JOptionPane.showConfirmDialog(null, "This will change the following class(es) to ex-class(es):\n" + unqualified.toString(), Constants.s_APPNAME, JOptionPane.OK_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE) == JOptionPane.CANCEL_OPTION)
			{
				aPC.setAlignment(oldAlignment, false, true);
	 ...
	 */
	protected void createClassTreeModel()
	{
	 ...
	 */
	protected void createRaceComboModel()
	{
	 ...
	 */
	protected void createClassComboModel()
	{
	...
	final class ClassModel extends AbstractTableModel
	{
			...
			switch (columnIndex)
			{
				...
				case COL_CLASSNAME:
					if (aPC != null)
					{
						...
						if ((aClass == null) || (aClass.getLevel().intValue() == lvl))
						{
	...
	private void addMonsterHD(int direction)
	{
		int numHD = txtHD.getValue();
		if (numHD <= 0)
		{
			JOptionPane.showMessageDialog(null,
				"Number of hit dice must be positive.", Constants.s_APPNAME, JOptionPane.ERROR_MESSAGE);
			return;
		}
		numHD *= direction;
		...
		if (monsterClass != null)
		{
			...
			if (aClass != null)
			{
				...
				if (numHD < 0)
				{
					...
					if (pcClass != null)
					{
						currentHD += pcClass.getLevel().intValue();
					}
					//
					// Don't allow a number so big it causes us to drop below minimum level
					//
					System.err.println("minHD=" + minHD + "  currentHD=" + currentHD + "  numHD=" + numHD);
					if ((currentHD + numHD) < minHD)
					{
						numHD = minHD - currentHD;
						System.err.println("numHD modified to: " + numHD);
					}
					if ((pcClass == null) || (numHD == 0) || ((currentHD + numHD) < minHD))
					{
						JOptionPane.showMessageDialog(null,
							"Cannot lower hit dice any more.", Constants.s_APPNAME, JOptionPane.ERROR_MESSAGE);
						return;
					}
				}
