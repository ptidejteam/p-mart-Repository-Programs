...
// create available table of equipment
	...
	public boolean canAfford(Equipment base, Equipment selected, double newQty)
	{
...
//End LocationEditor classes
	 ...
	 */
	final class EquipmentModel extends AbstractTreeTableModel implements TreeTableModel
	{
		...
		public void setValueForItemInNodes(PObjectNode p, Equipment e, double f, int column)
		{
			if (p == null)
			{
				p = (PObjectNode) root;
...
/*					final float newQty = ((((Float)value).floatValue() > 0)
					...
					final float oldQty = (selectedEquipment == null) ? 0.0f : selectedEquipment.qty();
...
//Begin multicontainers code.
						...
						if (baseEquipment.acceptsChildren() && !tempHeaderParent)
						{
							if (oldQty == 0)
							{
								...
								baseEquipment.makeKitSelections();
							}
...
*/
				...
				case COL_CARRIED:
					...
					{
						double newValue = ((Float) value).doubleValue();
					...
					if (selectedEquipment != null)
					{
						...
						if (anInt >= EQUIPMENT_CONTAINED)
						{
							...
							else
							{
								...
								if (aParent.canContain(selectedEquipment) == 1)
								{
									double newValue = selectedEquipment.numberCarried().floatValue();
						...
						{
							setValueAt(new Float(selectedEquipment.qty()), node, COL_CARRIED);
						}
