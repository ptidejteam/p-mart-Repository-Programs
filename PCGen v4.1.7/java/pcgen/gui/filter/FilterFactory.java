...
public final class FilterFactory implements FilterConstants
{
	...
	public static boolean restoreFilterSettings(Filterable filterable)
	{
		...
		List customRemoved = preprocessFilterList("removed",
			SettingsHandler.retrieveFilterSettings(name + ".removed"));
...
*/
	...
	static PObjectFilter createCompoundFilter(PObjectFilter filter1,
	...
	static PObjectFilter createNamedFilter(PObjectFilter filter,
		String name,
		String description)
	{
...
final class StatusFilter extends AbstractPObjectFilter
{
	...
	{
		super(PropertyFactory.getString("in_filterStatus"),
			(isLoaded)
			? PropertyFactory.getString("in_filterCampLoad")
			: PropertyFactory.getString("in_filterCampNotLoad"));
...
final class AlignmentFilter extends AbstractPObjectFilter
{
	...
	public boolean accept(PObject pObject)
	{
		...
		if (pObject instanceof Deity)
		{
			...
			if (deityAlign.equals(Globals.getShortAlignmentAtIndex(alignment)) ||
				deityAlign.equals(Globals.getLongAlignmentAtIndex(alignment)))
			{
				return true;
			}
			return false;
...
final class PantheonFilter extends AbstractPObjectFilter
{
	PantheonFilter(String pantheon, int argDetailLevel)
	{
		...
		setCategory(PropertyFactory.getString("in_pantheon") +
			((detailLevel == LOW)
			? " (" + PropertyFactory.getString("in_general") + ")"
			: " (" + PropertyFactory.getString("in_specific") + ")"));
		setName(this.pantheon);
		setDescription((this.pantheon.equalsIgnoreCase(ALL))
			? PropertyFactory.getString("in_acceptPantAll")
			: PropertyFactory.getString("in_acceptPantOne") + pantheon + " " +
			PropertyFactory.getString("in_acceptPantTwo"));
	}
...
final class PCAlignmentFilter extends AbstractPObjectFilter
{
	...
	public String getName()
	{
		return super.getName() +
			" (" + Globals.getLongAlignmentAtIndex(Globals.getCurrentPC().getAlignment()) + ")";
	}
...
final class AutomaticFeatFilter extends AbstractPObjectFilter
{
	AutomaticFeatFilter()
	{
		super(PropertyFactory.getString("in_feats"),
			PropertyFactory.getString("in_Automatic"));
	}
...
final class NormalFeatFilter extends AbstractPObjectFilter
{
	...
	{
		super(PropertyFactory.getString("in_feats"),
			PropertyFactory.getString("in_Normal"));
	}
...
final class VirtualFeatFilter extends AbstractPObjectFilter
{
	...
	{
		super(PropertyFactory.getString("in_feats"),
			PropertyFactory.getString("in_Virtual"));
	}
...
final class AffordableFilter extends AbstractPObjectFilter
{
	AffordableFilter()
	{
		super(PropertyFactory.getString("in_miscel"),
			PropertyFactory.getString("in_Affordable"));
	}
...
final class NonMagicFilter extends AbstractPObjectFilter
{
	...
	{
		super(PropertyFactory.getString("in_miscel"),
			PropertyFactory.getString("in_Non-Magic"));
	}
...
final class PCSizeFilter extends AbstractPObjectFilter
{
	PCSizeFilter()
	{
		super(PropertyFactory.getString("in_size"),
			PropertyFactory.getString("PC"));
	}

	public String getName()
	{
		return super.getName() +
			" (" + Globals.getSizeAdjustmentAtIndex(Globals.getCurrentPC().sizeInt()).toString() + ")";
	}
	...
	public boolean accept(PObject pObject)
	{
		...
		if (pObject instanceof Equipment)
		{
			return ((Equipment) pObject).getSize().toUpperCase().equals(
				Globals.getSizeAdjustmentAtIndex(Globals.getCurrentPC().sizeInt()).getAbbreviation());
		}
...
final class TypeFilter extends AbstractPObjectFilter
{
	...
	{
		super(PropertyFactory.getString("in_type"), (capitalize)
...
final class RankFilter extends AbstractPObjectFilter
{
	...
	RankFilter(double min)
	{
		super(PropertyFactory.getString("in_skills"),
			PropertyFactory.getString("in_rank") + " > " + min);
		this.min = min;
	}
...
final class RankModifierFilter extends AbstractPObjectFilter
{
	...
	{
		super(PropertyFactory.getString("in_skills"),
			PropertyFactory.getString("in_rank") + " + " +
			PropertyFactory.getString("in_modifer") + " > " + min);
		this.min = min;
	}
...
final class UntrainedSkillFilter extends AbstractPObjectFilter
{
	...
	{
		super(PropertyFactory.getString("in_skills"),
			PropertyFactory.getString("in_untrained"));
	}
...
final class SizeFilter extends AbstractPObjectFilter
{
	...
	public boolean accept(PObject pObject)
	{
		...
		if (pObject instanceof Equipment)
		{
			...
			return aEquipSize.equals(Globals.getSizeAdjustmentAtIndex(size).getAbbreviation()) ||
				aEquipSize.equals(Globals.getSizeAdjustmentAtIndex(size).toString());
		}
		else if (pObject instanceof Race)
		{
			final String aRaceSize = ((Race) pObject).getSize();
			return aRaceSize.equals(Globals.getSizeAdjustmentAtIndex(size).getAbbreviation()) ||
				aRaceSize.equals(Globals.getSizeAdjustmentAtIndex(size).toString());
		}
