...
final class FilterParser
{
	...
	public static final String TOKEN_STARTGROUP = "(";
	public static final String TOKEN_ENDGROUP = ")";
	public static final char CHAR_TOKEN_STARTGROUP = '(';
	public static final char CHAR_TOKEN_ENDGROUP = ')';
	public static final char TOKEN_STARTFILTER = '[';
	public static final String TOKEN_ENDFILTER = "]";
...
//		Globals.debugPrint(" enforceStrongAssociationForNOT :\n");
//		for (Iterator it = enforceStrongAssociationForNOT(tokenList).iterator(); it.hasNext();)
//		{
//			Globals.debugPrint(" >>>" + (String)it.next() + "<<<\n");
//		}
	...
	private static List createTokenList(String parseString) throws FilterParseException
	{
		...
		StringTokenizer tokens = new StringTokenizer(parseString,
			" " +
			TOKEN_STARTFILTER + TOKEN_ENDFILTER +
			TOKEN_STARTGROUP + TOKEN_ENDGROUP,
			true);
		...
		if (braceCount > 0)
		{
			throw new FilterParseException(
				PropertyFactory.getString("in_missing") + " '" + TOKEN_ENDGROUP + "'.");
		}
		else if (braceCount < 0)
		{
			throw new FilterParseException(
				PropertyFactory.getString("in_missing") + " '" + TOKEN_STARTGROUP + "'.");
		}

		if (bracketCount > 0)
		{
			throw new FilterParseException(
				PropertyFactory.getString("in_missing") + " '" + TOKEN_ENDFILTER + "'.");
		}
		else if (bracketCount < 0)
		{
			throw new FilterParseException(
				PropertyFactory.getString("in_missing") + " '" + TOKEN_STARTFILTER + "'.");
		}
	...
	private static void checkTokens(List tokenList) throws FilterParseException
	{
		...
		for (Iterator it = tokenList.iterator(); it.hasNext();)
		{
			...
			{
				throw new FilterParseException(
					PropertyFactory.getString("in_malformed") + " " + token + ".");
			}
			...
			{
				throw new FilterParseException(
					PropertyFactory.getString("in_illegSeq") + " '" + lastToken + " " + token + "'.");
			}
			else if (sequenceError == -4)
			{
				throw new FilterParseException(
					PropertyFactory.getString("in_illegTok") + " '" + token + "'.");
			}
		...
		{
			throw new FilterParseException(
				PropertyFactory.getString("in_illegLastTok") + " '" + lastToken + "'.");
		}
	}
	...
	static boolean isLegalToken(String token)
	{
		...
		return s.equals(TOKEN_STARTGROUP) || s.equals(TOKEN_ENDGROUP) ||
			s.equals(TOKEN_AND) || s.equals(TOKEN_NOT) || s.equals(TOKEN_OR) ||
	...
	private static int isLegalTokenSequence(String token1, String token2)
	{
		...
		// missing operand
		if (s1.endsWith(TOKEN_ENDGROUP) && (s2.equals(TOKEN_NOT) ||
			(s2.length() > 0 && s2.charAt(0) == CHAR_TOKEN_STARTGROUP) || (s2.length() > 0 && s2.charAt(0) == TOKEN_STARTFILTER)))
		{
			return -2;
		}
		// missing operand
		if (s1.endsWith(TOKEN_ENDFILTER) && (s2.equals(TOKEN_NOT) ||
			(s2.length() > 0 && s2.charAt(0) == CHAR_TOKEN_STARTGROUP) || (s2.length() > 0 && s2.charAt(0) == TOKEN_STARTFILTER)))
		{
			return -2;
		}
		// illegal sequence
		if (s1.equals(TOKEN_AND) && !(s2.equals(TOKEN_NOT) ||
			(s2.length() > 0 && s2.charAt(0) == CHAR_TOKEN_STARTGROUP) || (s2.length() > 0 && s2.charAt(0) == TOKEN_STARTFILTER)))
		{
			return -3;
		}
		...
		// illegal sequence
		if (s1.equals(TOKEN_OR) && !(s2.equals(TOKEN_NOT) ||
			(s2.length() > 0 && s2.charAt(0) == CHAR_TOKEN_STARTGROUP) || (s2.length() > 0 && s2.charAt(0) == TOKEN_STARTFILTER)))
		{
			return -3;
		}
		// illegal first token
		if ("".equals(s1) && !(s2.equals(TOKEN_NOT) ||
			(s2.length() > 0 && s2.charAt(0) == CHAR_TOKEN_STARTGROUP) || (s2.length() > 0 && s2.charAt(0) == TOKEN_STARTFILTER)))
		{
			return -4;
		}
	...
	private PObjectFilter retrieveFilter(String filterName) throws FilterParseException
	{
		...
		for (int i = 0; i < filterList.length; i++)
		{
			for (Iterator it = filterList[i].iterator(); it.hasNext();)
			{
				...
				if (filterName.equals(TOKEN_STARTFILTER +
					filter.getCategory() +
