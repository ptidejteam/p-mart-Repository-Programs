...
class InfoDomain extends FilterAdapterPanel
{
	...
	private static final String[] s_domainColList =
		new String[]{
			PropertyFactory.getString("in_domains"),
			PropertyFactory.getString("in_sourceLabel")};
	...
	private final class ClassPopupMenu extends JPopupMenu
	{
		...
		{
			return Utility.createMenuItem(
				label,
				new AddClassActionListener(),
				PropertyFactory.getString("in_select"),
				(char) 0,
				accelerator,
				PropertyFactory.getString("in_selDeity"),
				"Add16.gif",
				true);
		}
		...
		private JMenuItem createRemoveMenuItem(String label, String accelerator)
		{
			return Utility.createMenuItem(
				label,
				new RemoveClassActionListener(),
				PropertyFactory.getString("in_select"),
				(char) 0,
				accelerator,
				PropertyFactory.getString("in_selDomain"),
				"Add16.gif",
				true);
		}
			...
			if (treeTable == deityTable)
			{
				...
				ClassPopupMenu
					...
					.add(
						Utility
					.createMenuItem(
						PropertyFactory.getString("in_editDeity"),
						new ActionListener()
						{
				...
				ClassPopupMenu
					...
					.add(
						Utility
					.createMenuItem(
						PropertyFactory.getString("in_createDeity"),
						new ActionListener()
						{
							public void actionPerformed(ActionEvent e)
							{
				...
				ClassPopupMenu
					...
					.add(
						Utility
					.createMenuItem(
						PropertyFactory.getString("in_delDeity"),
						new ActionListener()
						{
							public void actionPerformed(ActionEvent e)
							{
				...
				public void keyPressed(KeyEvent e)
				{
					...
					if (keyCode != KeyEvent.VK_UNDEFINED)
					{
						...
						for (int i = 0; i < menu.getComponentCount(); i++)
						{
							...
							{
								KeyStroke ks =
									((JMenuItem) menuComponent).getAccelerator();
	...
	private void hookupPopupMenu(JTableEx treeTable)
	{
		treeTable.addMouseListener(
			new ClassPopupListener(treeTable, new ClassPopupMenu(treeTable)));
	}
	...
	InfoDomain()
	{
		...
		setName(PropertyFactory.getString("in_domains"));
	...
	private void initComponents()
	{
		...
		Utility.setDescription(
			domainTable,
			PropertyFactory.getString("in_domainTableTip"));
		...
		deitySelect.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent evt)
			{
				...
				final int selectedRow =
					deitySorter.getRowTranslated(lsm.getMinSelectionIndex());
		...
		Utility.setDescription(
			domainSelect,
			PropertyFactory.getString("in_domainButTip"));
		...
		TitledBorder title1 =
			BorderFactory.createTitledBorder(
				etched,
				PropertyFactory.getString("in_deityInfo"));
		...
		Utility.setDescription(
			deityScroll,
			PropertyFactory.getString("in_infoScrollTip"));

		TitledBorder title2 =
			BorderFactory.createTitledBorder(
				etched,
				PropertyFactory.getString("in_domainInfo"));
		...
		aSplit =
			new FlippingSplitPane(
				FlippingSplitPane.HORIZONTAL_SPLIT,
				deityScroll,
				domainScroll);
	...
	private void formComponentShown(ComponentEvent evt)
	{
		...
		PCGen_Frame1.getStatusBar().setText(
			PropertyFactory.getString("in_statusBarDeity"));
		updateCharacterInfo();
		...
		if (!hasBeenSized)
		{
			...
			for (int i = 0; i < deityTable.getColumnCount(); i++)
			{
				...
				width =
					Globals.getCustColumnWidth(PropertyFactory.getString("in_deity"), i);
				if (width != 0)
					sCol.setPreferredWidth(width);
				sCol.addPropertyChangeListener(
					new ResizeColumnListener(
						deityTable,
						PropertyFactory.getString("in_deity"),
						i));
			}
	...
	private void setDomainInfoText(Domain aDomain)
	{
		...
		if (aDomain != null)
		{
			...
			if (aString.length() != 0)
			{
				infoText
					.append(" &nbsp;")
					.append(PropertyFactory.getString("in_domainGrant"))
					.append("</b>:")
					.append(aString);
			}
			...
			if (aString.length() != 0)
			{
				infoText
					.append(" &nbsp;<b>")
					.append(PropertyFactory.getString("in_requirements"))
					.append("</b>:")
					.append(aString);
			}
		}
	...
	private void setDeityInfoText(Deity aDeity)
	{
		if (aDeity != null)
		{
			StringBuffer infoText =
				new StringBuffer().append("<html><b>").append(aDeity.piSubString());
			infoText
				.append(" &nbsp;")
				.append(PropertyFactory.getString("in_descrip"))
				.append("</b>:")
				.append(aDeity.getDescription());
			...
			if (aString.length() != 0)
			{
				infoText
					.append(" &nbsp;<b>")
					.append(PropertyFactory.getString("in_deityFavWeap"))
					.append("</b>:")
					.append(aString);
			}
			...
			if (aString.length() != 0)
			{
				infoText
					.append(" &nbsp;<b>")
					.append(PropertyFactory.getString("in_deityHolyIt"))
					.append("</b>:")
					.append(aString);
			}
			infoText.append("</html>");
	...
	private void buildDomainLists()
	{
		...
		for (int i = 0; i < aPC.getCharacterDomainList().size(); i++)
		{
			final CharacterDomain aCD =
				(CharacterDomain) aPC.getCharacterDomainList().get(i);
	 ...
	 */
	private static final void addUnfilteredDomains(
		final ArrayList availDomainList,
		final Deity pcDeity)
	{
		...
		for (Iterator i = aPC.getClassList().iterator(); i.hasNext();)
		{
			...
			for (Iterator d = aClass.getAddDomains().iterator(); d.hasNext();)
			{
				...
				final StringTokenizer domainTok =
					new StringTokenizer(prestigeString, "|", false);
				...
				if (aClass.getLevel().intValue() >= level)
				{
					while (domainTok.hasMoreTokens())
					{
						...
						final Domain prestigeDomain =
							Globals.getDomainNamed(prestigeName);
	...
	private final void selectDomainIndex(int selectedRow)
	{
		...
		if (!addedDomain.qualifiesForDomain())
		{
			JOptionPane.showMessageDialog(
				null,
				PropertyFactory.getString("in_qualifyMess") + addedDomain.getName(),
				Constants.s_APPNAME,
		...
		for (int i = 0; i < characterDomains; i++)
		{
			...
			if (existingDomain == null || i == characterDomains - 1)
			{
				...
				if (existingDomain != null)
				{
					JOptionPane.showMessageDialog(
						null,
						PropertyFactory.getString("in_errorNoDomains") + " " + addedDomain.getName(),
						Constants.s_APPNAME,
						JOptionPane.INFORMATION_MESSAGE);
					return;
				}
	...
	private final void selectDeityIndex(int selectedRow)
	{
		...
		if (!aPC.canSelectDeity(aDeity))
		{
			...
			JOptionPane.showMessageDialog(
				null,
				PropertyFactory.getString("in_reqMess") + aDeity.getName() + ".",
				Constants.s_APPNAME,
				JOptionPane.INFORMATION_MESSAGE);
			lsm.clearSelection();
			return;
		...
		public Object getValueAt(int row, int column)
		{
			...
			if (deities != null && row >= 0 && row < deities.size())
			{
				...
				switch (column)
				{
					...
					case 0:
						...
						else
						{
							retVal =
								new StringBuffer()
								.append("<html>")
								.append(SettingsHandler.getPrereqFailColorAsHtml())
								.append(aDeity.piSubString())
								.append("</font></html>")
								.toString();
						}
						break;
					...
					default :
						Globals.errorPrint(
							PropertyFactory.getString("in_domIDEr1")
							+ " "
							+ column
							+ " "
							+ PropertyFactory.getString("in_domIDEr2"));
						break;
				}
			}
			return retVal;
		...
		public void mouseClicked(MouseEvent f)
		{
			...
			switch (f.getClickCount())
			{
				case (1):
					...
					{
						final Deity aDeity =
							(Deity) deityModel.getValueAt(selectedRow, -1);
		...
		public Object getValueAt(int row, int col)
		{
			...
			switch (col)
			{
				...
				case 0:
					...
					{
						retVal.append("<html><b>").append(aDomain.piSubString()).append(
							"*</b></html>");
					}
				...
				case 1:
					...
					{
						Globals.errorPrint(
							PropertyFactory.getString("in_errorMess"),
							exc);
					}
					break;
				default :
		...
		if (selectedRow >= 0)
		{
			...
			if (aDeity != null)
			{
				...
				else
				{
					JOptionPane.showMessageDialog(
						null,
						PropertyFactory.getString("in_domIDEr4"),
						Constants.s_APPNAME,
