...
public class PToolBar extends JToolBar
{
	...
	private void init(PCGen_Frame1 main)
	{
		...
		newItem = Utility.createButton(main.frameActionListener.newActionListener,
			"file.new",
			PropertyFactory.getString("in_newChaTip"),
			"New16.gif",
			false);
		add(newItem);

		openItem = Utility.createButton(main.frameActionListener.openActionListener,
			"file.open",
			PropertyFactory.getString("in_openChaTip"),
			"Open16.gif", true);
		add(openItem);

		closeItem = Utility.createButton(main.frameActionListener.closeActionListener,
			"file.close",
			PropertyFactory.getString("in_closeChaTip"),
			"Close16.gif",
			false);
		add(closeItem);

		saveItem = Utility.createButton(main.frameActionListener.saveActionListener,
			"file.save",
			PropertyFactory.getString("in_saveChaTip"),
			"Save16.gif",
			false);
		add(saveItem);
		...
		printPreviewItem = Utility.createButton(main.frameActionListener.printPreviewActionListener,
			"file.printpreview",
			PropertyFactory.getString("in_printPreviewTip"),
			"PrintPreview16.gif",
			false);
		add(printPreviewItem);

		printItem = Utility.createButton(main.frameActionListener.printActionListener,
			"file.print",
			PropertyFactory.getString("in_printChaTip"),
			"Print16.gif",
			false);
		add(printItem);
		...
		openFilters = new FilterToolTipButton(
			new ImageIcon(getClass().getResource("resource/Zoom16.gif")),
		...
		addKit = Utility.createButton(main.frameActionListener.addKitActionListener,
			"assign.kit",
			PropertyFactory.getString("in_addKits"),
			"Information16.gif",
			false);
		add(addKit);
		...
		preferencesItem = Utility.createButton(main.frameActionListener.preferencesActionListener,
			...
			PropertyFactory.getString("in_preferencesTip"),
			"Preferences16.gif",
			true);
		add(preferencesItem);
