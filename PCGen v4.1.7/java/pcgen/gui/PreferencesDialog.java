 ...
 *
 * $Id: PreferencesDialog.java,v 1.1 2006/02/21 00:57:39 vauchers Exp $
 */
...
//	private JCheckBox summaryTabShown = new JCheckBox("<html>Show <b>Summary</b> tab</html>");
	...
	private JTextField outputSheetTextDefault;
	private JButton outputSheetTextDefaultButton;
	...
	private void buildSettingsTreeAndPanel()
	{
		...
		JButton okButton = new JButton(PropertyFactory.getString("in_okay"));
	...
	private final class PrefsButtonListener implements ActionListener
	{
		public void actionPerformed(ActionEvent actionEvent)
		{
			...
			else if (source == outputSheetHTMLDefaultButton)
			{
				...
				fc.setCurrentDirectory(SettingsHandler.getHTMLOutputSheetPath());
			...
			else if (source == outputSheetPDFDefaultButton)
			{
				...
				fc.setCurrentDirectory(SettingsHandler.getPDFOutputSheetPath());
			...
			else if (source == outputSheetTextDefaultButton)
			{
				JFileChooser fc = new JFileChooser();
				fc.setDialogTitle(PropertyFactory.getString("in_Prefs_outputSheetTextDefaultTitle"));
				fc.setCurrentDirectory(SettingsHandler.getTextOutputSheetPath());
				fc.setSelectedFile(new File(SettingsHandler.getSelectedTextOutputSheet()));
				if (fc.showOpenDialog(getParent()) == JFileChooser.APPROVE_OPTION)
				{
					File newTemplate = fc.getSelectedFile();
					if (newTemplate.isDirectory() || (!newTemplate.getName().startsWith("csheet") && !newTemplate.getName().startsWith("psheet")))
					...
					else
					{
						if (newTemplate.getName().startsWith("csheet"))
						{
							SettingsHandler.setSelectedTextOutputSheet(newTemplate.getAbsolutePath());
							SettingsHandler.setTemplatePath(newTemplate.getParentFile().getParentFile());
						}
						else
						{
				...
				outputSheetTextDefault.setText(String.valueOf(SettingsHandler.getSelectedTextOutputSheet()));
			}
			...
			else
			{
				...
				if (SettingsHandler.getLookAndFeel() == laf.length)
				{
					...
					catch (Exception e) //This is what applySkin actually throws...
					{
						...
						SettingsHandler.setLookAndFeel(0);
						UIFactory.setLookAndFeel(0);
						JOptionPane.showMessageDialog(null, PropertyFactory.getString("in_Prefs_skinSetError") + e.toString(),
							in_pcgen, JOptionPane.ERROR_MESSAGE);
					}
				}
			}
		}
