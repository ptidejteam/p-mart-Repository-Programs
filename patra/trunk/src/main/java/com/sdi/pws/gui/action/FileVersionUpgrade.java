...
public class FileVersionUpgrade
extends AbstractAction
{
    ...
    public FileVersionUpgrade(JComponent aAppFrame, DatabaseHolder aHolder, Preferences aPrefs)
    {
        super(GuiUtil.getText("action.fileversionupgrade"));
        dbHolder = aHolder;
        ...
        {
            setEnabled(!Codec2.VERSION.equals(dbHolder.getCurrentDatabase().getVersion()));
        ...
        dbHolder.addPropertyChangeListener(new PropertyChangeListener()
        {
            public void propertyChange(PropertyChangeEvent aHolderEvt)
            {
                ...
                if (lNewDb != null)
                {
                    setEnabled(!Codec2.VERSION.equals(lNewDb.getVersion()));
    ...
    public void actionPerformed(ActionEvent e)
    {
        ...
        if(lDb != null)
        {
            if(prefs.hasPreference(Preferences.PREF_ACTIVEDEFAULTGROUP) && prefs.hasPreference(Preferences.PREF_DEFAULT_GROUP))
            {
                boolean lGroupEnabled = false;
                try{lGroupEnabled = prefs.getBoolPref(Preferences.PREF_ACTIVEDEFAULTGROUP);}catch(Exception eIgnore){;}
                final String lGroup = prefs.getPref(Preferences.PREF_DEFAULT_GROUP);
                if(lGroupEnabled) CodecUtil.upgradeVersion(lDb, lGroup);
                else CodecUtil.upgradeVersion(lDb);
            }
            else CodecUtil.upgradeVersion(lDb);
        }
    }
