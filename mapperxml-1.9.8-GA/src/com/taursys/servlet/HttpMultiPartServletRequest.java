 ...
 * a suffix of "_FileName" and "_ContentType". For uploaded files
 ...
 * </li>
 * <li><code>request.getParameter("myFile")</code> will return the String
 ...
 * </li>
 * <li><code>request.getParameterByteArray("myFile")</code> will return a
 * byte array of the file contents.
 * </li>
 * </ul>
 * For all uploaded files with a Content-Type OTHER THAN "text/..." the 
 * following parameters are available:
 * <ul>
 * <li>Example where input control's "name" attribute is "myPicture" and the
 * user selected "portrait.jpg" to upload.
 * </li>
 * <li><code>request.getParameter("myPicture")</code> will return the
 ...
 * </li>
 * <li><code>request.getParameterByteArray("myPicture")</code> will return a
...
public class HttpMultiPartServletRequest implements HttpServletRequest {
   ...
   * @throws IOException 
   */
  public void setupBoundary() throws MultiPartRequestContentException, IOException {
    if (request.getContentType() == null)
      throw new MultiPartRequestContentException("Unknown Content Type");
    String line = request.getContentType();
    StringTokenizer contentType = new StringTokenizer(
        line, " ;=");
    // Fetch boundary from contentType
    try {
      if (contentType.hasMoreTokens() && 
          contentType.nextToken().equals(MULTIPART_FORM_DATA)) {
        if (contentType.hasMoreTokens() && 
            contentType.nextToken().equals("boundary")) {
          boundary = contentType.nextToken();
          startBoundary = "--" + boundary;
          endBoundary = startBoundary + "--";
        } else {
          throw new MultiPartRequestContentException(
              "Content type is missing boundary attribute: contentType=" + 
          line);
        }
      } else {
        throw new MultiPartRequestContentException(
            "Content type is not multipart/form-data: contentType=" + 
          line);
      }
    } catch (NoSuchElementException ex) {
      throw new MultiPartRequestContentException(
          "Content type or boundary attribute is missing: contentType=" + 
          line);
    }
  }
  ...
  public String getParameter(String name) {
    ...
    if (values != null) {
      if (values[0] instanceof String) {
        ...
        return new String((byte[])values[0]);
      }
    }
    return null;
  }
    ...
    if (values != null) {
      ...
      for (int i = 0; i < results.length; i++) {
        if (values[i] instanceof String) {
          ...
          results[i] = new String((byte[])values[i]);
        }
      }
      return results;
    }
  ...
  public byte[] getParameterByteArray(String name) {
    if (name.endsWith(PARM_BYTE_ARRAY_SUFFIX)) {
      name = name.substring(0,name.length()-10);
    }
    Object[] values = (Object[])map.get(name);
    if (values != null && values[0] != null) {
      if (values[0] instanceof String) {
        return ((String)values[0]).getBytes();
      } else {
    ...
    try {
      while (true) {
        ...
        if (equalsSubArray(start, work, offset, size)
            ...
            offset -= 2;
            String results = new String(work, 0, offset);
            storeParameter(key, results);
            return;
        }
        offset += size;
        work[offset] = '\r';
  ...
  static public void main(String[] args) {
    try {
      ...
      rq.tokens = new StringTokenizer(testString,
          HttpMultiPartServletRequest.TOKEN_DELIM_DEFAULT, true);
