/**
 * FormRenderer - Renders subform to parent
 *
 * Copyright (c) 2002
...
import com.taursys.dom.DocumentAdapterException;
...
/**
 * FormRenderer is used by Subforms to inject their contents into
...
public class FormRenderer extends DocumentElementRenderer {
  ...
  protected void init() throws RenderException {
    ...
    if (sourceNode == null) {
      sourceId = form.getSourceId();
      if (sourceId != null) {
        sourceNode = da.getElementById(sourceId);
        if (sourceNode == null) {
  ...
  public void render() throws RenderException {
    ...
    if (getComponent().isVisible()) {
      ...
      if (sourceNode != null && getComponentNode() != null) {
        try {
          parentDocumentAdapter.importContents(da, sourceId, id);
        } catch (DocumentAdapterException e) {
          Debug.debug("Error during render." ,e);
        }
      }
    } else {
      removeSelf();
