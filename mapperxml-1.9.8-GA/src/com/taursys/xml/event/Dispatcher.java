...
public abstract class Dispatcher {
  ...
  protected Map map;
  private int index = 0;
  private Dispatcher parentDispatcher = null;
  ...
  protected String getParameter(String key, String defaultValue) {
    ...
    return (values == null || getParentIndex() >= values.length)
        ? defaultValue : values[getParentIndex()];
  }

  /**
   * Get the parent index value, else this Dispatcher's index value if no parent.
   * @return the parent index value, else this Dispatcher's index value if no parent.
   */  
  protected int getParentIndex() {
    if (parentDispatcher != null) {
      return parentDispatcher.getParentIndex();
    } else {
      return index;
    }
  }
   ...
   * @return this Dispatcher's index.
   */  
   ...
   * The parent dispatch primary use is to control the index
   * for the key/value map used by input type events.
   * @return The parent Dispatcher for this Dispatcher
   */
   ...
   * The parent dispatch primary use is to control the index
   * for the key/value map used by input type events.
   * @param dispatcher The parent Dispatcher for this Dispatcher
   */
