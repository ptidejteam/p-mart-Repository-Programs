...
public class ElementDelegateTest extends TestCase {
  ...
  public void testSetValueHolder2() throws ModelException {
    DocumentElement container = new DocumentElement();
    BoundDocumentElement field = new BoundDocumentElement();
    container.add(field);
    Attribute attribute = field.createBoundAttribute("style","lastName");
    TextNode text = field.createBoundTextNode("firstName");
    VOValueHolder holder = new VOValueHolder();
    holder.setValueObject(new TestVO("Phelan", "Marty"));
    field.setValueHolder(holder);
    ElementDelegate delegate = field.getElementDelegate();
    ...
    assertEquals("style", attribute.getAttributeName());
    assertEquals(attribute, components[0]);
    assertEquals(text, components[1]);
    assertEquals("Phelan", attribute.getText());
    assertEquals("Marty", text.getText());
  }
  
  public void testSetAttributeTextAutoCreate() throws ModelException {
    DocumentElement container = new DocumentElement();
    BoundDocumentElement field = new BoundDocumentElement();
    container.add(field);
    field.setAttributeText("style","color: red;");
    
    ElementDelegate delegate = field.getElementDelegate();
    Component[] components = delegate.getComponents();
    assertEquals(1, components.length);
    assertEquals(Attribute.class, components[0].getClass());
    Attribute attribute = (Attribute) components[0];
    assertEquals("style", attribute.getAttributeName());
    assertEquals("color: red;", attribute.getText());
  }
  
  public void testSetAttributeValueAutoCreate() throws ModelException {
    DocumentElement container = new DocumentElement();
    BoundDocumentElement field = new BoundDocumentElement();
    container.add(field);
    field.setAttributeValue("style","color: red;");
  ...
  public void testSetTextNodeTextAutoCreate() throws ModelException {
    DocumentElement container = new DocumentElement();
    BoundDocumentElement field = new BoundDocumentElement();
    container.add(field);
    field.setTextNodeText("Juneau");
    
    ElementDelegate delegate = field.getElementDelegate();
    Component[] components = delegate.getComponents();
    assertEquals(1, components.length);
    assertEquals(TextNode.class, components[0].getClass());
    TextNode text = (TextNode) components[0];
    assertEquals("Juneau", text.getText());
  }
  
  public void testSetTextNodeValueAutoCreate() throws ModelException {
    DocumentElement container = new DocumentElement();
    BoundDocumentElement field = new BoundDocumentElement();
    container.add(field);
    field.setTextNodeValue("Juneau");
