 ...
 * Contributor(s):   -;
 *
 * $Id: TimePeriodValuesCollectionTests.java,v 1.1 2007/10/10 20:52:50 vauchers Exp $
 *
 * Changes
 * -------
...
public class TimePeriodValuesCollectionTests extends TestCase {
    ...
    public void testGetDomainBoundsWithoutInterval() {
        ...
        //sv added arg to compile
        assertEquals(1500.0, r.getLowerBound(), 0);
        assertEquals(1500.0, r.getUpperBound(), 0);
        
        // check dataset with two time periods
        s1.add(new SimpleTimePeriod(1500L, 3000L), 2.0);
        r = dataset.getDomainBounds(false);
        assertEquals(1500.0, r.getLowerBound(), 0);
        assertEquals(2250.0, r.getUpperBound(), 0);  
    }
    ...
    public void testGetDomainBoundsWithInterval() {
        ...
        assertEquals(1000.0, r.getLowerBound(), 0);
        assertEquals(2000.0, r.getUpperBound(), 0);
        
        // check dataset with two time periods
        s1.add(new SimpleTimePeriod(1500L, 3000L), 2.0);
        r = dataset.getDomainBounds(true);
        assertEquals(1000.0, r.getLowerBound(), 0);
        assertEquals(3000.0, r.getUpperBound(), 0);  
    }
}
