...
public class Utils {
	...
	public static IFirstClassEntity createGhost(
		...
		try {
			...
			// Yann 2008/11/06: Duplication!
			// It could happen that I try to put twice the same ghost...
			// in this case, I return the existing one.
			Output.getInstance().normalOutput().print(
				"Trying to add twice the ghost ");
			Output.getInstance().normalOutput().println(anEntityName);
			return (IFirstClassEntity) anAbstractLevelModel
	...
	public static char[] extractPackageName(final char[] aJVMClassName) {
		if (ArrayUtils.indexOf(aJVMClassName, '.') == -1) {
			...
			return ArrayUtils.subarray(aJVMClassName, 0, ArrayUtils
				.lastIndexOf(aJVMClassName, '.'));
		}
	}
	...
	public static IFirstClassEntity searchForEnclosingEntity(
		...
		char[] enclosingEntityName =
			ArrayUtils.subarray(aMembreEntityName, 0, ArrayUtils.indexOf(
				aMembreEntityName,
				'$'));
		...
		while (ArrayUtils.indexOf(enclosingEntityName, '$') > 0) {
			final char[] enclosingMemberEntityName =
				ArrayUtils.subarray(enclosingEntityName, 0, ArrayUtils.indexOf(
					enclosingEntityName,
					'$'));
			IFirstClassEntity newMemberEnclosingEntity =
				(IFirstClassEntity) enclosingEntity
					.getConstituentFromName(enclosingMemberEntityName);

			// Yann 2006/02/08: Ghosts...
			// If I cannot find an entity, then it is a ghost...
			// Thus, I built recursively Ghost entities and
			// membre Ghost entities...
			...
			enclosingEntityName =
				ArrayUtils.subarray(enclosingEntityName, ArrayUtils.indexOf(
					enclosingEntityName,
					'$') + 1, enclosingEntityName.length);
		}
	...
	public static IFirstClassEntity searchForEntity(
		...
		if (Utils.isMemberEntity(anEntityName)) {
			...
			final char[] name =
				ArrayUtils.subarray(anEntityName, ArrayUtils.lastIndexOf(
					anEntityName,
					'$') + 1, anEntityName.length);
			...
			IFirstClassEntity membreEntity =
				(IFirstClassEntity) enclosingEntity
					.getConstituentFromID(anEntityName);
