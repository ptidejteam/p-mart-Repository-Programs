...
import padl.creator.relationship.ContainerRelationshipAnalyzer;
import padl.creator.relationship.Context;
import padl.creator.relationship.RelationshipAnalyzer;
import padl.creator.util.ExtendedFieldInfo;
import padl.creator.util.ExtendedMethodInfo;
import padl.creator.util.PackageCreator;
 ...
 */
abstract class AbstractClassFileCreator implements ICodeLevelModelCreator {
					...
					case JVMConstants.INVOKEVIRTUAL :
						{
							...
							if (indices != null) {
								...
								calles.add(className
										+ '.'
										+ nameAndType.substring(0, nameAndType
											.indexOf('.')));
							}
						}
						break;
	...
	public AbstractClassFileCreator(
		final ConstituentRepository aConstituentRepository,
		final String[] someClassFiles,
		final boolean recurseIntoDirectories,
		final boolean storeMethodInvocation) {

		this.constituentRepository = aConstituentRepository;
		...
		for (int i = 0; i < someClassFiles.length; i++) {
			...
			Output.getInstance().normalOutput().println(someClassFiles[i]);
	...
	public void create(final ICodeLevelModel aCodeLevelModel) {
		final ClassFile[] listOfEntities =
			this.constituentRepository.getEntities();
		final ClassFile[] listOfElements =
			this.constituentRepository.getElements();

		this.createPackages(
			aCodeLevelModel,
			this.listOfClassFiles,
			this.constituentRepository,
			listOfEntities);
	...
	private void createElements(
		...
		for (int x = 0; x < listOfElements.length; x++) {
			constituentName = constituentRepository.getElements()[x].getName();
			...
			try {
				...
				sortedElements[x] =
					((Integer) this.getClass().getMethod(
						buffer.toString(),
						new java.lang.Class[0]).invoke(this, new Object[0]))
						.intValue();
				buffer.setLength(0);
			}
			catch (final Throwable t) {
				t.printStackTrace(Output.getInstance().errorOutput());
			}
			finally {
		...
		for (int elementRecogniserNumber = 0; elementRecogniserNumber < listOfElements.length; elementRecogniserNumber++) {
			...
			while (iteratorOnEntities.hasNext()) {
				...
				if (!Utils.isAnonymousOrLocalEntity(currentClassName)
					...
					constituentName =
						constituentRepository.getElements()[sortedElements[elementRecogniserNumber]]
							.getName();
						...
						try {
							...
							this.getClass().getMethod(
								...
								new java.lang.Class[] { List.class,
										ICodeLevelModel.class }).invoke(
								this,
								new Object[] { listOfSubmittedElements,
										aCodeLevelModel });
						}
						catch (final Throwable t) {
							Output.getInstance().errorOutput().print("Error: ");
							Output
								.getInstance()
								.errorOutput()
								.println(
			...
			try {
				...
				if (!Utils.isAnonymousOrLocalEntity(entityID)
					...
					if (!Utils.isMemberEntity(entityID)) {
						...
						Utils.searchForEnclosingEntity(
							aCodeLevelModel,
							entityID).addConstituent(
							(IConstituentOfEntity) memberEntity);
						this.mapOfIDsEntities
							.put(entityDisplayID, memberEntity);
					}
				}
			}
			catch (final ModelDeclarationException mde) {
				Output.getInstance().warningOutput().println(mde.getMessage());
			}
		}
		...
		while (iterator.hasNext()) {
			...
			if (!Utils.isAnonymousOrLocalEntity(entityName)
				...
				if (firstClassEntity instanceof IClass
						&& !(firstClassEntity.equals(Factory
							.getInstance()
							.createHierarchyRoot()))) {
				...
				Utils.searchForPackage(
					...
					new PackageCreator() {
						public IPackage create(final char[] aName) {
							try {
								return Factory.getInstance().createPackage(
									aName);
							}
		...
		while (iterator.hasNext()) {
			...
			if (element instanceof ExtendedMethodInfo) {
				...
				try {
					...
					// No error can occur.
					// 2005/11/15 Stéphane: Actually, it does. 
					// Minimally, it should be logged, so we are aware 
					// that something odd is happening
					Output.getInstance().warningOutput().println(
						mde.getMessage());
					// Yann 2005/12/29: Sure, but...
					// Actually, the RelationshipAnalyser might already have created
					// a method in an entity if this method was called from another
					// entity. Thus, there would be an attempt to add a "duplicate"
					// element, which is legitimate but taken care of by the new condition.
	...
	private IParameter createParameter(
		final ICodeLevelModel aCodeLevelModel,
		char[] someParamType,
		boolean isCardinalityMany) throws ModelDeclarationException {
									...
									final IDelegatingMethod delegatingMethod =
										aCodeLevelModel
											...
											.createDelegatingMethod(
												...
												null);
									currentEntity
										.addConstituent(delegatingMethod);
									iterator.remove();
								}
								catch (final ModelDeclarationException mde) {
