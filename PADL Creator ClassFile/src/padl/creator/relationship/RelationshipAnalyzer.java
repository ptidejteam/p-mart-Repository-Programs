...
public class RelationshipAnalyzer {
	...
	private static void addRelationship(
		...
		try {
			...
			// Yann 2001/07/23: The following comment no longer applies:
			// The only error that can occur is a duplication
			// of the actor ID. It does not really matters, because
			// what is important is the existence of a link, not
			// the number of times this link exists.
			// Yann 2001/07/23: Exception should not occur.
			// No exception should occur (at least, no "Duplicate
			// ID") because of the unique ID that is appened to any
			// actor name before there are inserted into the instance of
			// Entity.
			// Yann 2002/07/31: The exception can now occur.
			// I don't modify the list of given constituents to
			// prevent duplicate calls, thus duplicate actor may be
			// created.
			// Yann 2004/08/02: Exception!
			// The exception does not occur "naturally" any more!
			// (<=> Code cleaner :-)
	...
	private static char[] extractReturnType(final String completeNameMethod) {
		String returnType =
			completeNameMethod.substring(completeNameMethod.indexOf(" ") + 1);
		returnType =
			returnType.substring(returnType.indexOf(".") + 1, returnType
				.indexOf(" "));
			...
			if (!isFieldEmpty
				...
				if (nextCouple[1].equals(String
					...
					entityDeclaringMethod =
						Utils.getEntityOrCreateGhost(
							this.codeLevelModel,
							nextCouple[1].substring(
								0,
								nextCouple[1].indexOf(" ")).toCharArray(),
							RelationshipAnalyzer.MapOfIDsEntities);
				}
				methodInvocation =
					new ExtendedMethodInvocation(this.codeLevelModel
						.getFactory(), currentEntity, currentMethod,
					// Yann 2012/02/02: Not sure!
						...
						RelationshipAnalyzer
							.computeMethodInvocationTypeThroughField(
								...
								calledMethod),
						//	RelationshipAnalyzer.computeMethodInvocationTypeDirect(
						//		currentMethod,
						//		calledMethod),
	...
	private IOperation createConstructor(
		final IFirstClassEntity anOriginEntity,
		final char[] anID) throws ModelDeclarationException {
	...
	private IOperation createOperation(final String methodData) {
		...
		if (calledMethod == null) {
			...
			Output.getInstance().errorOutput().print("Method name   :");
			Output.getInstance().errorOutput().println(usedMethodName);
			Output.getInstance().errorOutput().print('\t');
			Output.getInstance().errorOutput().print("Return type   :");
			Output.getInstance().errorOutput().println(returnType);
			Output.getInstance().errorOutput().print('\t');
			Output.getInstance().errorOutput().print("Declaring type: ");
			Output.getInstance().errorOutput().println(
			...
			if (methodInvocation.getTargetEntity() != null) {
				switch (methodInvocation.getType()) {
					...
					case IMethodInvocation.INSTANCE_CLASS :
						...
						RelationshipAnalyzer.addRelationship(methodInvocation
							.getOriginEntity(), relationship);
						break;
	...
	private IOperation searchForMethod(
		...
		if (!entityDeclaringMethod.doesContainConstituentWithID(usedMethodName)
			...
			try {
				...
				final char[] name =
					ArrayUtils.subarray(usedMethodName, 0, ArrayUtils.indexOf(
						usedMethodName,
						'('));
