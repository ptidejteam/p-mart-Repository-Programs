/*
 * (c) Copyright 2000-2002 Yann-Gaël Guéhéneuc, Ecole des Mines de Nantes and Object Technology International, Inc.
 * 
 * Use and copying of this software and preparation of derivative works based upon this software are permitted. Any copy of this software
 * or of any derivative work must include the above copyright notice of Yann-Gaël Guéhéneuc, this paragraph and the one after it.
 * 
 * This software is made available AS IS, and THE AUTHOR DISCLAIMS ALL WARRANTIES, EXPRESS OR IMPLIED, INCLUDING WITHOUT LIMITATION THE
 * IMPLIED WARRANTIES OF MERCHANTABILITY AND FITNESS FOR A PARTICULAR PURPOSE, AND NOT WITHSTANDING ANY OTHER PROVISION CONTAINED HEREIN,
 * ANY LIABILITY FOR DAMAGES RESULTING FROM THE SOFTWARE OR ITS USE IS EXPRESSLY DISCLAIMED, WHETHER ARISING IN CONTRACT, TORT (INCLUDING
 * NEGLIGENCE) OR STRICT LIABILITY, EVEN IF YANN-GAEL GUEHENEUC IS ADVISED OF THE POSSIBILITY OF SUCH DAMAGES.
 * 
 * All Rights Reserved.
 */
...
public final class LightMethodInvocationAnalyzer {
	...
	private void handleGetField(final Context context, final int offset) {
		String invocationSiteName =
			context
				...
				.get(
					ByteArray
						.getShortAtOffset(context.getRawCode(), offset + 1))
				.getAsJava();
		...
		final String fieldDeclaringClassName =
			invocationSiteName.substring(index + 1, invocationSiteName
				.lastIndexOf('.'));
	...
	private void handleInvokeVirtual(
		...
		if (context.getInvocationSiteName() == null) {
			String invocationSiteName =
				context.getConstantPool().getAsString(
					context.getConstantPool().get(
						ByteArray.getShortAtOffset(
							context.getRawCode(),
							offset + 1)).getIndices()[0]);
			invocationSiteName = invocationSiteName.replace('/', '.');
			context.setInvocationSiteName(invocationSiteName.toCharArray());
		}
		...
		if (!Utils.isSpecialMethod(methodName.toCharArray())) {
			...
			for (int i = 0; i < Constants.SETTERS_CARDINALITY_MANY.length; i++) {
				...
				if (methodName
					...
					// I keep the previously found invocation site.
					final String parameterType =
						methodName.substring(Math.max(
							methodName.indexOf('(') + 1,
							methodName.indexOf(',') + 2), methodName
							.indexOf(')'));
					if (!Util.isObjectModelRoot(parameterType.toCharArray())) {
						context.setInvocationSiteName(parameterType
							.toCharArray());
