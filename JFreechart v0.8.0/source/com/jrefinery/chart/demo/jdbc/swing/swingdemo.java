...
/**
 *  A Demostation of a swing application which display chart plots from a JDBC data source.
 */
...
public class swingdemo extends JFrame {
  ...
  protected JDBCChartAdapter chartData;
  ...
  protected void setupChart() {
    ...
    VerticalNumberAxis vnAxis1 = (VerticalNumberAxis) chart.getPlot().getAxis(Plot.VERTICAL);
  ...
  protected class RefreshChartDataThread extends Thread {
    ...
    public void run() {
      ...
      try {
        ...
        Plot chartPlot = chart.getPlot();
        Axis chartAxis = chartPlot.getAxis(Plot.HORIZONTAL);
        chartAxis.configure();
        chartAxis = chartPlot.getAxis(Plot.VERTICAL);
        chartAxis.configure();
      } catch (Exception e) {
        e.printStackTrace();
      }
  ...
  public void start() {
    ...
    if (conn != null) {
      setupPickLists();
      chartData = new JDBCChartAdapter(conn);
      setBusy(false);
    } else {
      if (isStandalone) {
        System.exit( 0 );
