 ...
 *
 * $Id: WaferMapRenderer.java,v 1.1 2007/10/10 19:21:54 vauchers Exp $
 *
 * Changes
 * -------
...
//    }
    ...
    private void makePaintIndex() {
        ...
        if (uniqueValues.size() <= paintLimit) {
    ...
    private void makePositionIndex(Set uniqueValues ) {
        int valuesPerColor = (int) Math.ceil((double) uniqueValues.size() / paintLimit);
        int count = 0; // assign a color for each unique value
        int paint = 0;
        for (Iterator i = uniqueValues.iterator(); i.hasNext();) {
