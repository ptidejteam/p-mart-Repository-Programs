 ...
 *
 * $Id: WaterfallBarRenderer.java,v 1.1 2007/10/10 19:21:54 vauchers Exp $
 *
 * Changes
 * -------
...
public class WaterfallBarRenderer extends BarRenderer 
    ...
    public RangeType getRangeType() {
        return RangeType.SERIES_CUMULATIVE;
    }
    ...
    public void drawItem(Graphics2D g2,
        ...
        if (orientation == PlotOrientation.HORIZONTAL) {
            ...
            rectWidth = Math.abs(j2dy1 - j2dy0);
        ...
        Paint seriesPaint = this.firstBarPaint;
        if (column == 0) {
            seriesPaint = this.firstBarPaint;
        }
        else if (column == categoryCount - 1) {
            seriesPaint = this.lastBarPaint;    
        } 
        else {
            if (valDiff < 0.0) {
                seriesPaint = this.negativeBarPaint;
            } 
            else if (valDiff > 0.0) {
                seriesPaint = this.positiveBarPaint;
            } 
            else {
                seriesPaint = this.lastBarPaint;
            }
