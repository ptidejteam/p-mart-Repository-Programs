...
public class Util {
	...
	public static void assertEquals(final int[] aFirstArray,
	...
	public static char[] computeFullyQualifiedName(
		...
		while (iterator.hasNext()) {
			...
			if (firstClassEntity.equals(anEntity)) {
				...
				final char[] fullyQualifiedName = Util
						.computeFullyQualifiedName(firstClassEntity, anEntity,
	...
	private static char[] computeFullyQualifiedName(
			...
			final IFirstClassEntity anEntity, final char[] aFullyQualifiedName) {

		final Iterator iterator = anEnclosingEntity
				.getIteratorOnConstituents(IFirstClassEntity.class);
		while (iterator.hasNext()) {
			final IFirstClassEntity firstClassEntity = (IFirstClassEntity) iterator
					.next();
			final char[] qualifiedName = new char[aFullyQualifiedName.length
					+ 1 + firstClassEntity.getName().length];
			System.arraycopy(aFullyQualifiedName, 0, qualifiedName, 0,
					aFullyQualifiedName.length);
			qualifiedName[aFullyQualifiedName.length] = '.';
			System.arraycopy(firstClassEntity.getName(), 0, qualifiedName,
			...
			if (firstClassEntity.equals(anEntity)) {
				...
				return Util.computeFullyQualifiedName(firstClassEntity,
						anEntity, qualifiedName);
			}
		}
	...
	public static char[] computeSimpleName(final char[] aFullyQualifiedName) {
		...
		if (index > 0) {
			simpleName = ArrayUtils.subarray(aFullyQualifiedName, index + 1,
					aFullyQualifiedName.length);
		} else {
			index = ArrayUtils.lastIndexOf(simpleName, '.');
			if (index > 0) {
				simpleName = ArrayUtils.subarray(simpleName, index + 1,
						simpleName.length);
			}
		}
	...
	private static int displayDirectoryContent(final int numberOfFiles,
			final String directory, final String fileExtension) {
		...
		for (int i = 0; i < list.length; i++) {
			...
			if (file.isDirectory()) {
				n = Util.displayDirectoryContent(n++, file.getAbsolutePath()
	...
	public static void displayDirectoryContent(final String directory,
	...
	public static IConstituent[] getArrayOfConstituents(
		...
		final IConstituent[] constituents = new IConstituent[aContainer
				.getNumberOfConstituents()];
		final Iterator iteratorOnEntities = aContainer
				.getIteratorOnConstituents();
		int count = 0;
		while (iteratorOnEntities.hasNext()) {
			final IConstituent constituent = (IConstituent) iteratorOnEntities
					.next();
	...
	public static IElement[] getArrayOfElements(final IFirstClassEntity anEntity) {
		...
		final IElement[] elements = new IElement[anEntity
				.getNumberOfConstituents()];
		final Iterator iteratorOnEntities = anEntity
				.getIteratorOnConstituents();
		int count = 0;
		while (iteratorOnEntities.hasNext()) {
	...
	public static IFirstClassEntity[] getArrayOfTopLevelEntities(
		...
		final IFirstClassEntity[] entities = new IFirstClassEntity[anAbstractModel
				.getNumberOfTopLevelEntities()];
		final Iterator iteratorOnEntities = anAbstractModel
				.getIteratorOnTopLevelEntities();
		int count = 0;
		while (iteratorOnEntities.hasNext()) {
			final IFirstClassEntity firstClassEntity = (IFirstClassEntity) iteratorOnEntities
					.next();
	...
	public static Iterator getFilteredConstituentsIterator(
			final IConstituent[] someConstituents, final int aSize,
			final IFilter aFilter) {

		return Util.getFilteredConstituentsList(someConstituents, aSize,
				aFilter).iterator();
	}
	...
	public static int getNumberOfArguments(final String methodSignature) {
		final String args = methodSignature.substring(methodSignature
				.indexOf('('));
		if (args.equals("()")) {
			return 0;
		} else {
	...
	public static List getPatternProperties(final Class clazz) {
		...
		try {
			final PropertyDescriptor[] basicProperties = Introspector
					.getBeanInfo(clazz, null).getPropertyDescriptors();
	...
	public static IConstituent[] getTypedConstituentsArray(
		...
		final List list = Util.getTypedConstituentsList(someConstituents,
				aSize, aConstituentType);
	...
	public static Iterator getTypedConstituentsIterator(
		...
		return Util.getTypedConstituentsList(someConstituents, aSize,
	...
	public static boolean isCollection(final char[] aTypeName) {
		...
		return aTypeDisplayName.equals("java.util.Collection")
				...
				|| aTypeDisplayName.equals("padl.creator.test.relationships.providers.VectorTest");
	}
	...
	public static boolean isEntityInheritingFrom(
		...
		final Iterator iterator = anInheritingEntity
		...
		while (iterator.hasNext()) {
			...
			if (firstClassEntity.equals(aPotentialInheritedEntity)) {
				...
				return Util.isEntityInheritingFrom(firstClassEntity,
