...
abstract class PtidejSolverDomainGenerator {
	...
	public static char[] convertToClaireIdentifier(
		...
		buffer.append(PtidejSolverDomainGenerator.PREFIX);
	...
	public static List directSubclassesOf(
		...
		for (int i = 0; i < pEntities.length; i++) {
			if (PtidejSolverDomainGenerator.isSub(
	...
	private static boolean isSub(
		...
		if (supers.size() > 0) {
			...
			while (iterator.hasNext()) {
				isSub |=
					PtidejSolverDomainGenerator.isSub(
	...
	private static boolean isSub(
		...
		if (pEntity instanceof IClass) {
			return PtidejSolverDomainGenerator.isSub(
				(IClass) pEntity,
				superPEntity);
		}
		return PtidejSolverDomainGenerator.isSub(
