/*
 * (c) Copyright 2002-2002 Yann-Gaël Guéhéneuc,
...
public class PtidejSolver2AC4DomainGenerator extends
		PtidejSolverDomainGenerator implements IWalker {
	...
	private void addInheritancePathCouple(
		...
		while (anIteratorOnSuperEntities.hasNext()) {
			...
			this.addInheritancePathCouple(origin, superEntity
				.getIteratorOnInheritedEntities());
			if (superEntity instanceof IClass) {
				this.addInheritancePathCouple(origin, ((IClass) superEntity)
	...
	private void close(final IAbstractModel p) {
		...
		for (int i = 0; i < this.listOfEntities.size(); i++) {
			...
			final char[] claireIdentifier =
				PtidejSolverDomainGenerator.convertToClaireIdentifier(id);
		...
		while (iterator.hasNext()) {
			...
			this.buffer.append(PtidejSolverDomainGenerator
	...
	public void visit(final IAggregation p) {
		if (PtidejSolver2AC4DomainGenerator.AGGREGATIONareCOMPOSITION) {
			this.addCouple(this.listOfEntities, this.enclosingEntity, p
				.getTargetEntity(), this.compositionCouples);
		}
		else {
			this.addCouple(this.listOfEntities, this.enclosingEntity, p
				.getTargetEntity(), this.aggregationCouples);
		}
	}
	public void visit(final IAssociation p) {
		this.addCouple(this.listOfEntities, this.enclosingEntity, p
			.getTargetEntity(), this.associationCouples);
	}
	...
	public void visit(final IContainerAggregation p) {
		if (PtidejSolver2AC4DomainGenerator.AGGREGATIONareCOMPOSITION) {
			...
			this.addCouple(this.listOfEntities, this.enclosingEntity, p
				.getTargetEntity(), this.containerCompositionCouples);
		}
		else {
			if (PtidejSolver2AC4DomainGenerator.AGGREGATIONareCONTAINERAGGREGATION) {
				this.addCouple(this.listOfEntities, this.enclosingEntity, p
			...
			this.addCouple(this.listOfEntities, this.enclosingEntity, p
				.getTargetEntity(), this.containerAggregationCouples);
		}
	}
	...
	public void visit(final ICreation p) {
		this.addCouple(this.listOfEntities, this.enclosingEntity, p
			.getTargetEntity(), this.creationCouples);
	}
	...
	public void visit(final IUseRelationship p) {
		this.addCouple(this.listOfEntities, this.enclosingEntity, p
			.getTargetEntity(), this.useCouples);
	}
}
