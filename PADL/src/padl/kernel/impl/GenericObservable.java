...
class GenericObservable implements IObservable, Serializable {
	...
	private List listOfModelListeners =
		new ArrayList(GenericContainerConstants.INITIAL_SIZE_GENERIC_OBSERVABLE);
				...
				while (iterator.hasNext()) {
					...
					if (listener != null) {
						try {
							...
							iae.printStackTrace(Output
								.getInstance()
								.errorOutput());
						}
						catch (final InvocationTargetException ite) {
							ite.printStackTrace(Output
								.getInstance()
								.errorOutput());
						}
					}
