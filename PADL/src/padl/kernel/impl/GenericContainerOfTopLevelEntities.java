...
import util.CharArrayComparator;
...
class GenericContainerOfTopLevelEntities implements Serializable {
	...
	// mechanism to fix this bug... Elegant? Performant?
	...
	void createMapOfIDsEntities(final char[] anID) {
		if (Arrays.equals(anID, Constants.FORBIDDEN_ID)
			...
			Output.getInstance().debugOutput().print(
				"(Re)Creating the maps of IDs and Entities (because of ");
			Output.getInstance().debugOutput().print(anID);
	...
	public void removeTopLevelEntityFromID(final char[] anID) {
		...
		// remove the entity from the appropriate contained, which then
