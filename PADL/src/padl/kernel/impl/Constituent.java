...
abstract class Constituent implements IConstituent {
	...
	public Constituent(final char[] anID) {
		if (anID == null) {
			throw new RuntimeException(new ModelDeclarationException(
					MultilingualManager.getString("ACTOR_ID_NULL",
							Constituent.class)));
		}
		this.id = anID;
		this.setName(anID);
	...
	protected void accept(final IVisitor visitor, final String methodName) {
		...
		if (!Constituent.cachedAcceptClassNames.containsKey(className)) {
			Constituent.cachedAcceptClassNames.put(className,
		...
		try {
			...
			// TODO understand the différence between forname and loadclass...
			final java.lang.Class[] argument = new java.lang.Class[] { visitor
					.getClass().getClassLoader().loadClass(acceptClassName) };
			...
			try {
				...
				final Method method = visitor.getClass().getMethod(methodName,
						argument);
	...
	protected final void updateWithIncomingPath(final char[] anIncomingPath) {
		// TODO: Replace with the use of an IModelListener, similar to the
		// Listener for top-level entities :-)
		final char[] newPath = new char[anIncomingPath.length + 1
				+ this.getPath().length];
		System.arraycopy(anIncomingPath, 0, newPath, 0, anIncomingPath.length);
		newPath[anIncomingPath.length] = this.getPathSymbol();
		System.arraycopy(this.getPath(), 0, newPath, anIncomingPath.length + 1,
				this.getPath().length);
	...
	public void setAbstract(final boolean aBoolean)
		...
		this.setVisibility(aBoolean ? (this.getVisibility() | Access.ACC_ABSTRACT)
	...
	public void setCodeLines(String[] code) throws ModelDeclarationException {
		if (this.isAbstract()) {
			throw new ModelDeclarationException(MultilingualManager.getString(
					"ELEM_CODE_DEF", Constituent.class));
		}
		this.codeLines = code;
	}
	...
	public void setPrivate(final boolean aBoolean) {
		try {
			this.setVisibility(aBoolean ? ((this.getVisibility() | Access.ACC_PRIVATE) & ~(Access.ACC_PUBLIC | Access.ACC_PROTECTED))
	...
	public void setProtected(final boolean aBoolean) {
		try {
			this.setVisibility(aBoolean ? ((this.getVisibility() | Access.ACC_PROTECTED) & ~(Access.ACC_PUBLIC | Access.ACC_PRIVATE))
	...
	public void setPublic(final boolean aBoolean) {
		try {
			this.setVisibility(aBoolean ? ((this.getVisibility() | Access.ACC_PUBLIC) & ~(Access.ACC_PRIVATE | Access.ACC_PROTECTED))
	...
	public void setStatic(final boolean aBoolean) {
		try {
			this.setVisibility(aBoolean ? (this.getVisibility() | Access.ACC_STATIC)
	...
	public void setVisibility(final int visibility)
		...
		if (this.getCodeLines() != null && Access.isAbstract(visibility)) {
			...
			throw new ModelDeclarationException(MultilingualManager.getString(
					"ELEM_ABSTRACT", Constituent.class));
		}
