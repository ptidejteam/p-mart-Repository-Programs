 ...
 * 05-Apr-2002 : Downgraded all servlet 2.3 specific calls to allow compilation and running under
 *               servlet 2.2
 *
 */
...
import com.jrefinery.chart.jdbc.*;
...
public class BaseImageServlet extends HttpServlet implements Constants {
  ...
  protected JFreeChart createChart(int type, String sql) {
    ...
    chartData = new JDBCChartAdapter(con_);
    ...
    try {
      ...
      VerticalNumberAxis vnAxis = (VerticalNumberAxis) chart.getPlot().getAxis(Plot.VERTICAL);
      vnAxis.setAutoRangeIncludesZero(false);
      vnAxis.setCrosshairVisible(false);
      vnAxis.configure();

      /// Customise the horizontal axis
      ValueAxis axis = (ValueAxis) chart.getPlot().getAxis(Plot.HORIZONTAL);
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      incZero = request.getParameter("zero").trim().toLowerCase().equals("true");
    } catch (Exception e) {
    }
    try {
    ...
    if (chart != null) {
      ...
      VerticalNumberAxis vnAxis = (VerticalNumberAxis) chart.getPlot().getAxis(Plot.VERTICAL);
      vnAxis.setAutoRangeIncludesZero(incZero);
      ...
      if (yTitle_ != null) {
        Axis chartVerticalAxis = chartPlot.getAxis(Plot.VERTICAL);
        chartVerticalAxis.setLabel(yTitle_);
      }

      Axis chartHorzAxis = chartPlot.getAxis(Plot.HORIZONTAL);
      //chartHorzAxis.s
      if (xTitle_ != null) {
        chartHorzAxis.setLabel(xTitle_);
      }
    ...
    try {
      ...
      switch (imageOutputType) {
        ...
        case RESPONSE_SVG:
          ...
          ctx.setComment("Generated by AAD Marine Science Support using Batik SVG Generator");
          //ctx.setSVGCanvasSize
