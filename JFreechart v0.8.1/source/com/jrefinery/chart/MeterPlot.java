 ...
 *
 * $Id: MeterPlot.java,v 1.1 2007/10/10 19:02:36 vauchers Exp $
 *
 * Changes
 * -------
...
public class MeterPlot extends Plot {
     ...
     *
     */
    public MeterPlot(Insets insets,
	...
	return (MeterDataset)chart.getDataset();
    }
	...
	double meterY = plotArea.getY()+gapVertical/2;
        ...
        double meterH = plotArea.getHeight()-gapVertical +
        				(meterAngle <= 180 ? plotArea.getHeight()/1.25 : 0);
    	...
    	MeterDataset data = (MeterDataset)chart.getDataset();
	if (data != null) {
            ...
            drawArc(g2, meterArea, dataMin, dataMax, Color.black, 1);
            drawTicks(g2, meterArea, dataMin, dataMax);
            drawArcFor(g2, meterArea, data, MeterPlot.FULL_DATA);
            drawArcFor(g2, meterArea, data, MeterPlot.NORMAL_DATA);
            drawArcFor(g2, meterArea, data, MeterPlot.WARNING_DATA);
            drawArcFor(g2, meterArea, data, MeterPlot.CRITICAL_DATA);
            drawTick(g2, meterArea, dataVal, true, Color.green, true);
    ...
    void drawArc(Graphics2D g2, Rectangle2D area, double minValue, double maxValue, Color color,
        ...
        Arc2D.Double arc = new Arc2D.Double(x, y, w, h, startAngle, extent,
                                            (outlineType > 0 ? Arc2D.PIE : Arc2D.OPEN));
        if (outlineType>0) {
	    g2.fill(arc);
        }
        else {
    ...
    void drawTick(Graphics2D g2, Rectangle2D meterArea, double value) {
        drawTick(g2, meterArea, value, false, null, false);
    }

    void drawTick(Graphics2D g2, Rectangle2D meterArea, double value, boolean label, Color color) {
        drawTick(g2, meterArea, value, label, color, false);
    }

    void drawTick(Graphics2D g2, Rectangle2D meterArea, double value,
                  boolean label, Color labelColor, boolean curValue) {
