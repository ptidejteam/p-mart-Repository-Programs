...
public final class Kit extends PObject implements Comparable
{
	...
	public void addKitSkills(List thingsToAdd, List warnings)
	{
		...
		if ((aList != null) && (aList.size() != 0))
		{
			...
			for (int i = 0, x = aList.size(); i < x; ++i)
			{
				...
				if (!passesPreReqToGainForList(aPC, this, kSkill.getPrereqs()))
				{
					continue;
				}
	...
	public void addKitFeats(List thingsToAdd, List warnings)
	{
		...
		if ((aList != null) && (aList.size() != 0))
		{
			...
			for (int i = 0, x = aList.size(); i < x; ++i)
			{
				...
				//
				if (!passesPreReqToGainForList(aPC, this, kFeat.getPrereqs()))
				{
					continue;
				}
	...
	public void addKitProfs(List thingsToAdd, List warnings)
	{
		...
		if ((aList != null) && (aList.size() != 0))
		{
			...
			for (int i = 0, x = aList.size(); i < x; ++i)
			{
				...
				if (!passesPreReqToGainForList(aPC, this, kProf.getPrereqs()))
				{
					continue;
				}
	...
	public void addKitGear(List thingsToAdd, List warnings)
	{
		...
		if ((aList != null) && (aList.size() != 0))
		{
			...
			for (int i = 0, x = aList.size(); i < x; ++i)
			{
				...
				if (!passesPreReqToGainForList(aPC, this, kGear.getPrereqs()))
				{
					continue;
				}
	...
	public void addKitSpells(List thingsToAdd, List warnings)
	{
		...
		if ((aList != null) && (aList.size() != 0))
		{
			...
			for (int i = 0, x = aList.size(); i < x; ++i)
			{
				...
				if (!passesPreReqToGainForList(aPC, this, kSpells.getPrereqs()))
				{
					continue;
				}
	...
	private String itemPassesPrereqs(String aString)
	{
		...
		if (passesPreReqToGainForList(Globals.getCurrentPC(), this, prereqList))
		{
			return itemName;
		}
		return null;
						...
						while (aTok.hasMoreTokens())
						{
							...
							if (aClass != null)
							{
								...
								if (!spells || ((baseStat != null) && (baseStat.length() > 0) && !baseStat.equalsIgnoreCase(Constants.s_NONE)))
								{
									...
									if (passesPreReqToGainForList(aPC, this, preR))
									{
										return aClass;
									}
								}
		...
		while (aTok.hasMoreTokens())
		{
			...
			else
			{
				...
				else
				{
					...
					al.add(tok);
					if (!passesPreReqToGainForList(aPC, null, al))
					{
						costFormula = null;
						continue;
					}
