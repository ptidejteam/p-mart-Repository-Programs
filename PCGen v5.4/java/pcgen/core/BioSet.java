...
public final class BioSet extends PObject
{
	...
	public double getBonusTo(PlayerCharacter pc, String aType, String aName)
	{
		...
		{
			final String bonus = tok.nextToken();
			if (bonus.startsWith("BONUS:"))
			{
				temporaryPObject.addBonusList(bonus.substring(6));
			}
		}
		return temporaryPObject.bonusTo(aType, aName);
	}
	...
	private String mapFind(Map argMap, final String argRegionName, final String argRaceName, final String addKey, final String altRaceName)
	{
		//
		// First check for region.racename.key
		//
		String r = (String) argMap.get(argRegionName + argRaceName + addKey);
		if (r != null)
		{
			return r;
		}
		...
		if (altRaceLength != 0)
		{
			r = (String) argMap.get(argRegionName + altRaceName + addKey);
			if (r != null)
			{
				return r;
			}
		...
		{
			r = (String) argMap.get(Constants.s_NONE + "." + argRaceName + addKey);
			if (r != null)
			{
				return r;
			}
			if (altRaceLength != 0)
			{
				r = (String) argMap.get(Constants.s_NONE + "." + altRaceName + addKey);
			}
		}
		return r;
	}
