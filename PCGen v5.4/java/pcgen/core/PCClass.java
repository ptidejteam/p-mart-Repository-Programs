 ...
 *
 * $Id: PCClass.java,v 1.1 2006/02/21 01:18:39 vauchers Exp $
 */
...
//			aClass.setSubClassString(getSubClassString());
	...
	{
		return (SettingsHandler.isBoolBypassClassPreReqs() || super.passesPreReqToGainForList(aPC, aObj, anArrayList));
	}
	...
	int getKnownForLevel(int pcLevel, int spellLevel, String bookName)
	{
		...
		// special case of LEVEL.All should only be applied
		// if they can cast spells of this level
		...
		if (!getKnownList().isEmpty())
		{
			...
			while (aTok.hasMoreTokens())
			{
				...
				if (iCount == spellLevel)
				{
					...
					if (SettingsHandler.bonusSpellsKnown() && (bonusSpell != null) && !bonusSpell.equals("0|0"))
					{
	...
	{
		int aNum = -1;
		if (iCasterLevel > getCastList().size())
		{
			iCasterLevel = getCastList().size();
		}
		if (iCasterLevel < 1)
		{
			// can't cast spells!
		...
		int iCount = 0;
		// List's start at 0 and go up, so 1st level is 0th element
		String aString = (String) getCastList().get(iCasterLevel - 1);
		StringTokenizer aTok = new StringTokenizer(aString, ",");
		while (aTok.hasMoreTokens())
		{
			aString = (String) aTok.nextToken();
	...
	public int getCastForLevel(int pcLevel, int spellLevel, String bookName, boolean includeAdj, boolean limitByStat)
	{
		...
		pcLevel += (int) aPC.getTotalBonusTo("PCLEVEL", name);
		pcLevel += (int) aPC.getTotalBonusTo("PCLEVEL", "TYPE." + getSpellType());
		// bonus LEVEL.All only applies if can cast spells of that level
		if (getNumFromCastList(pcLevel, spellLevel) >= 0)
		{
		...
		if (!getCastList().isEmpty())
		{
			...
			if (t < 0)
			{
				// they can't cast any spells of this level
				return 0;
			}
			total += (t * mult) + adj;
			Object bonusSpell = Globals.getBonusSpellMap().get(String.valueOf(spellLevel));
	...
	void addLevel(boolean argLevelMax, boolean bSilent)
	{
		...
		if (dnum > 0 && !aPC.getCharacterDomainList().isEmpty())
		{
			...
			{
				aCD = (CharacterDomain) i.next();
				// if the characterdomain comes from
				// this class, decrement total
				if (aCD.isFromPCClass(getName()))
				{
					--dnum;
				}
					...
					else if (adding && addNow && aName.length() > 0)
					{
						if (aPC.getCharacterDomainIndex(aName) == -1)
						{
							...
							if (aDomain != null)
							{
								...
								CharacterDomain aCD = aPC.getNewCharacterDomain();
	 ...
	 */
	public final void setHitPointList(Integer[] hps)
	{
		hitPointMap.clear();
	...
	{
		for (int i = 0; i < SystemCollections.getUnmodifiableCheckList().size(); ++i)
		{
			if (checkName.equalsIgnoreCase(SystemCollections.getUnmodifiableCheckList().get(i).toString()))
			{
	...
	{
		if (index >= 0 && index < SystemCollections.getUnmodifiableCheckList().size())
		{
			return checkBonus(SystemCollections.getUnmodifiableCheckList().get(index).toString(), includeBonus);
		}
