			...
			if (available)
			{
				...
				switch (mode)
				{
					...
					case GuiConstants.INFOSPELLS_VIEW_LEVEL:
						...
						// now add to the root node
						avaRoot.setChildren(ll);

						break; // end VIEW_LEVEL
					default:
						Logging.errorPrint("In InfoSpells.SpellModel.resetModel the mode " + mode + " is not handled (all spells).");
						break;
			...
			{ // selectedTable builder (it's a list of spellbooks)
				...
				switch (mode)
				{
						...
						for (int iBook = 0; iBook < bookList.size(); ++iBook)
						{
							...
							for (int iClass = 0; iClass < classList.size(); ++iClass)
							{
								...
								for (int iLev = 0; iLev <= 20; ++iLev)
								{
									...
									List charSpells = aClass.getCharacterSpell(null, bookName, iLev);
							...
							final List rList = aPC.getRace().getCharacterSpell(null, bookName, -1);
							if (rList.size() > 0)
							{
					...
					case GuiConstants.INFOSPELLS_VIEW_LEVEL:
						...
						// now add to the root node
						selRoot.setChildren(lb);

						break; // end VIEW_LEVEL
					default:
						Logging.errorPrint("In InfoSpells.SpellModel.resetModel the mode " + mode + " is not handled (spellbooks).");
						break;
