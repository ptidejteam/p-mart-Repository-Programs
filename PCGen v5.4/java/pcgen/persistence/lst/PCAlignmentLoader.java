 ...
 *
 * $Id: PCAlignmentLoader.java,v 1.1 2006/02/21 01:18:47 vauchers Exp $
 */
...
final class PCAlignmentLoader
{
	...
	public static void parseLine(PCAlignment alignment, String inputLine, URL sourceURL, int lineNum) throws PersistenceLayerException
	{
		if (alignment == null)
		{
			return;
		...
		final Iterator iter = SystemCollections.getUnmodifiableAlignmentList().iterator();
		while (iter.hasNext())
		{
			final PCAlignment testAlignment = (PCAlignment) iter.next();
			if (testAlignment.getName().equals(alignment.getName()) || testAlignment.getKeyName().equals(alignment.getKeyName()))
			{
