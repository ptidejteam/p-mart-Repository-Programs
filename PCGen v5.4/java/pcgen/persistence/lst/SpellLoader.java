...
final class SpellLoader
{
	...
	public static void parseLine(Spell obj, String inputLine, URL sourceURL, int lineNum) throws PersistenceLayerException
	{
		...
		while (colToken.hasMoreElements())
		{
			...
			{
				Logging.errorPrint("EFFECTS: tag deprecated - use DESC: instead in " + sourceURL.toString());
				obj.setDescription(colString.substring(8));
			}
			else if (colString.startsWith("EFFECTTYPE:"))
			{
				Logging.errorPrint("EFFECTTYPE: tag deprecated - use TARGETAREA: instead in " + sourceURL.toString());
...
//					+ " \"" + colString + "\"");
				Logging.errorPrint("Illegal spell info " 
					+ sourceURL.toString() + ":" + Integer.toString(lineNum) 
					+ " \"" + colString + "\"");
