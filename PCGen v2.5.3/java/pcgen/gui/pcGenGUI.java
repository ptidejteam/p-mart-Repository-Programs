...
public class pcGenGUI
{
	...
	public static void main(String[] args)
	{
		...
		final String partyArg = System.getProperty("pcgen.partymode");
		if (inFileName != null && templateName != null && outFileName != null)
		{
			partyMode = (partyArg != null);
	...
	public boolean loadPartyFromFile(File file, PCGen_Frame1 mainFrame)
	{
		...
		catch (IOException ex)
		{
			if (Globals.getUseGUI())
			{
				...
				System.out.println("Error loading party file.");
		}
		return false;
	}
		...
		try
		{
			...
			if (!Globals.displayListsHappy())
			{
				if (Globals.getUseGUI())
					...
					System.out.println("Insufficient campaign information to load character file.");
				return false;
			}
...
//  			br.close();
                        ...
                        if (Globals.getUseGUI())
                        {
                                for (java.util.Iterator it = ioHandler.getErrors().iterator(); it.hasNext();) {
                                        JOptionPane.showMessageDialog(null,
                                                                      "Error: " + (String)it.next(),
                                                                      Globals.s_APPNAME,
                                                                      JOptionPane.ERROR_MESSAGE);
                                }
                                for (java.util.Iterator it = ioHandler.getWarnings().iterator(); it.hasNext();) {
                                        JOptionPane.showMessageDialog(null,
                                ...
                                for (java.util.Iterator it = ioHandler.getMessages().iterator(); it.hasNext();) {
	...
	private void loadCampaignsForPC(BufferedReader br)
	{
		try
		{
			...
			{
				if (Globals.isLoadCampaignsWithPC())
				{
		...
		catch (IOException e)
		{
			if (Globals.getUseGUI())
			{
				...
				System.out.println("Error loading campaign data for character.");
		}
	}
}
