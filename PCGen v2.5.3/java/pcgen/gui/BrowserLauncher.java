...
public class BrowserLauncher
{
	...
	public static void openURL(String url) throws IOException
	{
		...
		{
			Globals.debugErrorPrint("Opening default system browser");
			...
			switch (jvm)
			{
				...
				case MRJ_3_1:
					try
					{
						Globals.debugErrorPrint("Invoking openURL on " + url);
		...
		{
			Globals.debugErrorPrint("Opening with browser " + Globals.getBrowserPath());
			// On MacOS X, one must use open -a to launch an app.
			if (jvm == MRJ_3_1 && Globals.getBrowserPath().getName().toLowerCase().endsWith(".app"))
			{
				Globals.debugErrorPrint("Executing open -a " + Globals.getBrowserPath().getAbsolutePath() + " " + url);
				Runtime.getRuntime().exec(new String[]{"open", "-a", Globals.getBrowserPath().getAbsolutePath(), url});
			}
			else
			{
				Globals.debugErrorPrint("Executing " + Globals.getBrowserPath().getAbsolutePath() + " " + url);
				Runtime.getRuntime().exec(new String[]{Globals.getBrowserPath().getAbsolutePath(), url});
			}
		}
