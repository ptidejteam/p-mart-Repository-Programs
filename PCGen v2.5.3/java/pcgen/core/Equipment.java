	...
	implements Serializable, EquipmentCollection, Comparable
{
	...
	private String size = Globals.s_SIZESHORT[Globals.SIZE_M];
//  	private String size = "";
	...
	public String getAltDamage()
	{
		...
		if (iMod > Constants.SIZE_C)
		{
			iMod = Constants.SIZE_C;
		}
		return adjustDamage(altDamage, Constants.s_SIZESHORT[iMod]);
	}
	...
	public static int getHandNum(String handDesc)
	{
		...
		{
			GuiFacade.showMessageDialog(null, "Unable to interpret hand setting: " + handDesc, "PCGen", JOptionPane.ERROR_MESSAGE);
			return 0;
		}
	}
	 ...
	 *@param  anObject  ???
	 */
	public boolean addHeaderChild(Object anObject)
	{
		return d_headerChildren.add((Equipment)anObject);
	}
	...
	public boolean removeHeaderChild(Object anObject)
	{
		if (d_headerChildren.remove(anObject))
		{
			//
			// re-key
			for (Iterator e = getHeaderChildren().iterator(); e.hasNext();)
			{
	 ...
	 */
	public Float getChildTypes(String aString)
	{
	 ...
	 */
	public Float getAcceptsTypes(String aString)
	{
	...
	public BigDecimal getCost()
	{
		...
		if ((iPlus != 0) || (altPlus != 0))
		{
			...
			for (int i = 0; i < 2; i++)
			{
				...
				aPlus = new BigDecimal(altPlus * altPlus);
			}
		}
	...
	{
		int mod = /*acMod.intValue() +*/ bonusTo("EQMARMOR", "AC", true) + bonusTo("COMBAT", "AC", true);
...
//			s_equipmentTypes.add(type);
	...
	public String getDamage()
	{
		...
		return adjustDamage(damage, Constants.s_SIZESHORT[iMod]);
	}
 ...
 */
		...
		if (aFlag)
		{
				...
				isShield() && aPC.canEquip("Shield"))
			{
				...
				if (isWeapon())
				{
					...
					{
						equipped = false;
						GuiFacade.showMessageDialog(null, "Cannot equip weapon - no valid weapon proficiency for " + getName() + " loaded.", "PCGen", JOptionPane.ERROR_MESSAGE);
				...
				{
					equipped = false;
					GuiFacade.showMessageDialog(null, "Your hands are too full. Check weapons/shields already equipped.", "PCGen", JOptionPane.ERROR_MESSAGE);
					return;
				}
				equipped = true;
			}
			...
			if (equipped == false)
			{
				GuiFacade.showMessageDialog(null, "Character cannot equip any more of this item type.", "PCGen", JOptionPane.INFORMATION_MESSAGE);
			}
		}
		else
		{
	...
	public void setModifiersAllowed(boolean modifiersAllowed)
	{
...
//		eq.setTypeString(typeString());
	...
	public final void setHand(int hand)
	{
		if (isWeapon())
		{
			...
			if (wp == null)
			{
				GuiFacade.showMessageDialog(null, "Cannot find a valid weapon prof for " + name + " loaded.", "PCGen", JOptionPane.ERROR_MESSAGE);
				return;
			}
			...
			if (wp.getType().equals("Natural"))
			{
				...
				{
					GuiFacade.showMessageDialog(null, "Can only place primary Natural weapon in Primary \"Hand\".", "PCGen", JOptionPane.INFORMATION_MESSAGE);
					return;
				}
				if (modifiedName().endsWith("Secondary") && hand != SECONDARY_HAND && hand != NEITHER_HAND)
				{
					GuiFacade.showMessageDialog(null, "Can only place secondary Natural weapon in Off \"Hand\".", "PCGen", JOptionPane.INFORMATION_MESSAGE);
					return;
				}
				if (modifiedName().endsWith("Primary") && isOnlyNaturalWeapon && hand != BOTH_HANDS && hand != NEITHER_HAND)
				{
					GuiFacade.showMessageDialog(null, "Can only place sole Natural weapon in both \"hands\".", "PCGen", JOptionPane.INFORMATION_MESSAGE);
					return;
				}
			}
			else
			{
				...
				{
					GuiFacade.showMessageDialog(null, "Cannot place light weapon in both hands.", "PCGen", JOptionPane.INFORMATION_MESSAGE);
					return;
				}
				if (wp.isTwoHanded() && hand != NEITHER_HAND && hand != BOTH_HANDS)
				{
					GuiFacade.showMessageDialog(null, "Two handed weapon must be in Neither or Both hands.", "PCGen", JOptionPane.INFORMATION_MESSAGE);
					return;
				}
			}
		}
	...
	public boolean save(BufferedWriter output)
	{
		...
		fa.write(output, "BASEITEM:" + baseItem);
		fa.write(output, "\tNAME:" + getName());
		fa.write(output, "\tSIZE:" + size);

		aString = getEqModifierString(true);			// key1.key2|assoc1|assoc2.key3.key4
		if (aString.length() > 0)
		{
			fa.write(output, "\tEQMOD:" + aString);
		}
		aString = getEqModifierString(false);			// key1.key2|assoc1|assoc2.key3.key4
		if (aString.length() > 0)
		{
			fa.write(output, "\tALTEQMOD:" + aString);
		}

		aString = getRawSpecialProperties();
		if (aString.length() > 0)
		{
			fa.write(output, "\tSPROP:" + aString);
		}
		fa.newLine(output);
		return true;
	}
	...
	public void load(String aLine)
	{
		...
		while (aTok.hasMoreTokens())
		{
			...
			{
				setName(aString.substring(5));
			}
			else if (aString.startsWith("SIZE:"))
			{
				newSize = aString.substring(5);
			}
			...
			{
				setSpecialProperties(aString.substring(6));
			}
		}
		resizeItem(newSize);
	}
	...
	public int bonusTo(String aType, String aName, boolean bPrimary)
	{
		...
		final String prefix = aType.toUpperCase() + "." + aName.toUpperCase() + ".";
		for (Iterator e = bonusMap.keySet().iterator(); e.hasNext();)
		{
	...
	public void removeEqModifier(EquipmentModifier eqMod, boolean bPrimary)
	{
		...
		// are deleted
		//
		if ((eq.associatedList.size() == 0) || (eq.getChoice(0, this) == 0))
		{
			eqModList.remove(eq);
	...
	public void addEqModifier(EquipmentModifier eqMod, boolean bPrimary)
	{
		...
		// are deleted
		//
		if (!bImporting && (eq.getChoice(1, this) == 0))
		{
			eqModList.remove(eq);
		}
		Globals.sortPObjectList(eqModList);
 ...
 */
	...
	private void adjustAcForSize(Equipment baseEq, String aSize)
	{
		if (isArmor())
		{
			...
			//
			for (int i = 0; i < baseEq.bonusList.size(); i++)
			{
				String aString = (String)baseEq.bonusList.get(i);
	...
	{
		if (!isWeapon() || (altDamage.length() == 0))
		{
			return altDamage;
		}
				...
				(valString.length() > 3 && (valString.endsWith("MIN") || valString.endsWith("MAX") || valString.endsWith("REQ"))))
			{
				...
				else
				{
					for (int j = 0; j < Globals.s_ATTRIBSHORT.length; j++)
					{
						...
						{
							//don't know why this was "99" when using the actual stat mod seems to be what the tag is really for - Lone Jedi
							//valString = "99";
							valString = String.valueOf(Globals.getCurrentPC().calcStatMod(j));
							break;
						}
					}
				}
	...
	private void setBase()
	{
		...
		{
			final EquipmentModifier aEqMod = (EquipmentModifier)e.next();
			String aString = aEqMod.getProficiency();
			if (aString.length() != 0)
			{
				profType = aEqMod;
				break;
...
//			if (eqMod.getItemType().contains("MAGIC"))
	...
	public Collection getContents()
	{
		...
		Equipment aEquip;
		for (Iterator it = getD_containedEquipment().iterator(); it.hasNext();)
		{
