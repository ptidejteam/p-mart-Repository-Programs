...
public class Names
{
	...
	private static final String TAB_CHARACTER = new String("\t");
	...
	private static final String NAMES_DIRECTORY = System.getProperty("user.dir") + File.separator +
		"system" + File.separator + "bio" + File.separator + "names" + File.separator;
	...
	private void parseFile(String fileName)
	{
		...
		try
		{
			...
			while (allTheLines.hasMoreTokens())
			{
				...
				{
//          System.out.println("found a comment: " + currentLine);
					continue;
				}
				if (currentLine.startsWith("[RULES]"))
				{
//          System.out.println("found the [RULES] tag...");
					inRulesSection = true;
					continue;
				}
				...
				if (inRulesSection)
				{
//          System.out.println("adding a rule def: " + currentLine);
				...
				{
//          System.out.println("found a new syllable : " + currentLine);
...
//        System.out.println("current line: " + currentLine);
		...
		catch (Exception exception)
		{
			...
			{
				System.out.println("ERROR:" + fileName + " error " + currentLine + " Exception type:" + exception.getClass().getName() + " Message:" + exception.getMessage());
	...
	private List buildTheRuleSyllableMapping()
	{
		...
		if (getInstance().getRuleDefinitions().length > 0)
		{
			for (int i = 0; i < getInstance().getRuleDefinitions().length; i++)
			{
				String rule = getInstance().getRuleDefinitions()[i];
...
//          System.out.println("adding syllable name: " + syllableName);
	...
	private String constructTheName(NameRule ruleToUse)
	{
		...
		{
			finalString += getRandomSyllableByName(ruleSyllables[i]);
		}
		return finalString;
	}
