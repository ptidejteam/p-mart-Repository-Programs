	...
	private void populateTables()
	{
		...
		if (i >= 0 && i < spellClassComboBox.getItemCount() && j >= 0 && j < 10)
		{
			...
			if (aClass != null)
			{
				...
				castableText.setText(aClass.getCastForLevelString(aClass.getLevel().intValue(), j, spellBookComboBox.getSelectedItem().toString()));
				for (e = aClass.spellList().iterator(); e.hasNext();)
				{
					aSpell = (Spell)e.next();
					int m = spellLevelComboBox.getSelectedIndex();
					String bName = (String)spellBookComboBox.getSelectedItem();
					if (aSpell.levelForClass(className, aClass.getName()) == spellLevelComboBox.getSelectedIndex() && aSpell.getSpellBooks().contains(bName))
					{
			...
			{
				aClass = Globals.getCurrentPC().getClassNamed("Cleric");
				knownText.setText("0");
