...
public class InfoMisc extends javax.swing.JPanel
{
	...
	private JEditorPane d_magicText;
	private JScrollPane d_moneyArea;
	private JEditorPane d_moneyText;
	...
	private void initComponents()
	{
		...
		d_magicText = new JEditorPane();
		d_moneyArea = new JScrollPane();
		d_moneyText = new JEditorPane();
	...
	private void sendDataToCore(java.awt.event.ComponentEvent evt)
	{
		PlayerCharacter pc = Globals.getCurrentPC();
		if (pc != null && d_shown)
		{
			String origText = (String)pc.getMiscList().get(0);
			String newText = d_moneyText.getText();
			if (!newText.equals(origText))
			{
				pc.getMiscList().set(0, newText);
				pc.setDirty(true);
			}
			origText = (String)pc.getMiscList().get(2);
			newText = d_magicText.getText();
			if (!newText.equals(origText))
			{
				pc.getMiscList().set(2, newText);
				pc.setDirty(true);
			}
		}
	}

	private void updateCoreMoney(java.awt.event.FocusEvent evt)
	{
		PlayerCharacter pc = Globals.getCurrentPC();
		if (pc != null)
		{
			...
			if (!newText.equals(origText))
			{
				pc.getMiscList().set(0, d_moneyText.getText());
				pc.setDirty(true);
			}
		}
	}

	private void updateCoreMagic(java.awt.event.FocusEvent evt)
	{
		PlayerCharacter pc = Globals.getCurrentPC();
		if (pc != null)
		{
			String origText = (String)pc.getMiscList().get(2);
	...
	private void getDataFromCore(java.awt.event.ComponentEvent evt)
	{
		requestDefaultFocus();
		PlayerCharacter pc = Globals.getCurrentPC();
		if (pc != null)
		{
			String currentMoney = (String)pc.getMiscList().get(0);
			String currentMagic = (String)pc.getMiscList().get(2);
