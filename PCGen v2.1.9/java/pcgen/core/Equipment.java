	...
	implements Serializable, EquipmentCollection, Comparable
{
	...
	public String toString()
	{
		String aString = name;
		if (hasHeaderParent)
			aString = aString + " -" + (headerParent.indexOfHeaderChild(this) + 1) + "- ";

		if (modifiedName.length() > 0)
			aString = aString + " (" + modifiedName + ")";
		return aString;
	}
...
//        		System.out.println("Inside insertChild");
	...
	{
		containerCapacityString = "";
		boolean comma = false;

		if (containerWeightCapacity.intValue() != -1)
		{
			containerCapacityString = containerCapacityString + containerWeightCapacity + " lbs";
			comma = true;
		}
		...
		for (Iterator e = d_acceptsTypes.keySet().iterator(); e.hasNext();)
		{
			if (comma)
			{
				containerCapacityString = containerCapacityString + ", ";
				comma = false;
			}
			aString = (String)e.next();
			...
			{
				containerCapacityString = containerCapacityString + ((Float)d_acceptsTypes.get(aString)).floatValue() + " ";
				containerCapacityString = containerCapacityString + aString;
				comma = true;
			}
	...
	public void updateContainerContentsString()
	{
		...
		if (acceptsChildren())
			containerContentsString = containerContentsString + getContainedWeight() + " lbs";

		Equipment anEquip = null;
		for (int e = 0; e < getChildCount(); e++)
		{
			anEquip = (Equipment)getChild(e);
			if (anEquip.getCarried().compareTo(new Float(0)) > 0)
				containerContentsString = containerContentsString + ", " + anEquip.numberCarried().toString() + " " + anEquip.toString();
	...
	public void setIsEquipped(boolean aFlag)
	{
		if (aFlag)
		{
				...
				typeStringContains("SHIELD") && aPC.canEquip("Shield"))
			{
				...
				if (handsInUse > aPC.getRace().GetHands())
				{
					equipped = false;
