...
//
	private int PcgWriteVersion = 1;	// Version of file we will write
	...
	{
		int move = getRace().movement(x).intValue(); // get racial base movement
		boolean isMedium = getRace().movement(x).intValue() == 30;
...

	{
		...
		for (int y = 0; y < inTmpl.getTemplates().size(); y++)
			addTemplate(Globals.getTemplateNamed((String)inTmpl.getTemplates().get(y)));
	}
	...
	public void removeTemplate(PCTemplate inTmpl)
	{
		if (inTmpl == null)
			return;
		for (int y = 0; y < inTmpl.getTemplates().size(); y++)
			removeTemplate(getTemplateNamed((String)inTmpl.getTemplates().get(y)));
		for (int x = 0; x < templateList.size(); x++)
	...
	{
		int iConMod = calcStatMod(Globals.CONSTITUTION);
	...
	public int getTotalBonusTo(String bonusType, String bonusName, boolean stacks)
	{
		...
		try
		{
			bonus = getClassBonusTo(bonusType, bonusName);
			bonus += getEquipmentBonusTo(bonusType, bonusName, stacks);
			bonus += getFeatBonusTo(bonusType, bonusName, stacks);
			bonus += getDomainBonusTo(bonusType, bonusName);
			bonus += getRace().bonusTo(bonusType, bonusName);

			if (getDeity() != null)
			{
				bonus += getDeity().bonusTo(bonusType, bonusName);
			}

			if (bonusType.startsWith("WEAPONPROF="))
			{
				bonus += getWeaponProfBonusTo(bonusType.substring(11), bonusName);
			}
		}
		catch (Exception exc)
		{
	...
	public int adjStats(int stat)
	{
		...
		total = getStats()[stat];
		statName = statNames.substring(stat * 3, stat * 3 + 3);
	...
	public void setRace(Race aRace)
	{
		...
		if (race != null)
		{
			StringTokenizer aTok = new StringTokenizer(getRace().getFeatList(), "|", false);
			while (aTok.hasMoreTokens())
			{
				String aString = (String)aTok.nextToken();
				if (aString.endsWith(")") && Globals.getFeatNamed(aString) == null)
				{
					String featName = aString.substring(0, aString.indexOf("(") - 1);
			...
			for (int x = 0; x < race.getTemplates().size(); x++)
			{
				removeTemplate(getTemplateNamed((String)race.getTemplates().get(x)));
			}
		}
		...
		if (race != null)
		{
			...
			while (aTok.hasMoreTokens())
			{
				...
				if (aString.endsWith(")") && Globals.getFeatNamed(aString) == null)
				{
					String featName = aString.substring(0, aString.indexOf("(") - 1);	//I think we want the first instance of it, in case of Weapon Focus(Longbow (Composite))
...
//						}
			...
			for (int x = 0; x < race.getTemplates().size(); x++)
			{
				addTemplate(Globals.getTemplateNamed((String)race.getTemplates().get(x)));
			}
		}
	}
	...
	public void changeSpecialAbilitiesForLevel(int level, boolean addIt, Collection aArrayList)
	{
		...
		for (Iterator e = aArrayList.iterator(); e.hasNext();)
		{
			...
			int thisInt = Integer.parseInt(aTok.nextToken());
...
/*					if (wp.isOneHanded())
	...
	public int getClassBonusTo(String type, String aName)
	{
		...
		for (Iterator e = classList.iterator(); e.hasNext();)
		{
			...
			if (aName.equals("AC"))
			{
				int j = aClass.bonusBasedOnStat(type, aName, aClass.getLevel().intValue());
				if (j >= 0 && j < 6)
					statBonus[j] = Math.max(statBonus[j], aClass.getBonusTo(type, aName, aClass.getLevel().intValue()));
				else
	...
	private int loadPcgVersion(BufferedReader br)
	{
		...
		try
		{
			...
			if (sVersion.equals("VERSION"))
			{
				...
				iVersion = new Integer(sVersion).intValue();
			}
			else
			{
	...
	public int replaceToken(String aString, BufferedWriter output)
	{
		try
		{
			...
			if (aString.startsWith("STAT"))
			{
				...
				fa.write(output, getRace().getName());
			...
			else if (aString.startsWith("MOVEMENT"))
			{
				if (aString.length() > 9)
				{
					...
					{
						if (race.movementType(x).toUpperCase().equals(moveType.toUpperCase()))
						{
			...
			//SPELLLISTCAST0.0 KNOWN0.0 BOOK0.0 TYPE0
			{
				...
				if (aClass != null)
				{
					...
					String castNum = aClass.getCastForLevelString(aClass.getLevel().intValue(), level, "Known Spells");
...
//            nextMode = 0;
	...
	private void changeDeity(boolean addIt)
	{
		...
		{
			ArrayList aArrayList = new ArrayList();
			StringTokenizer aTok = new StringTokenizer(getDeity().getSpecialAbility(), "|", false);
			while (aTok.hasMoreTokens())
	...
	{
		ArrayList aArrayList = new ArrayList();
		StringTokenizer aTok = new StringTokenizer((String)getMiscList().get(index), "\r\n", false);
		while (aTok.hasMoreTokens())
			aArrayList.add(aTok.nextToken());
		return aArrayList;
	}
	...
	private void saveVersionLine(FileAccess fa, BufferedWriter output)
	{
		...
		fa.write(output, new Integer(PcgWriteVersion).toString());
		fa.newLine(output);
	}
...
/* added 08/27/01 by TNC */
	...
	private void loadClassesLine(FileAccess fa, BufferedReader input)
	{
		...
		while (aTok.hasMoreTokens())
		{
			...
			int conBonus = calcStatMod(Globals.CONSTITUTION);
			for (int k = 0; k < l; k++)
			{
