/*
 * $Header: /home/cvs/cvsgelo/cvsroot/guehene-dpl-pcgen\040v2.1.9/java/pcgen/core/CharacterFeat.java,v 1.1 2006/02/20 21:45:59 vauchers Exp $
 */
package pcgen.core;
...
/*  public void setFeat(Feat _feat){ feat=_feat; } */
	...
	public void modChoices(boolean addIt)
	{
		...
		{
			title = "Spell Choice";
			PCClass aClass = aPC.getClassNamed("Wizard");
		...
		else if (choiceType.equals("WEAPONFOCUS"))
		{
			...
			availableList = (ArrayList)weaponFocusCharacterFeat.associatedList().clone();
			...
			while (aTok.hasMoreTokens())
			{
				...
				else if (aString.startsWith("TYPE."))
				{
							 ...
							 setIter.hasNext();)
					{
						...
						if (eq == null)
							continue;
						if (eq.typeString().lastIndexOf(aString.substring(5)) > -1 && !availableList.contains(wp.getName()))
		...
		else if (choiceType.startsWith("FEAT="))
		{
			...
			if (otherCharacterFeat != null)
				availableList = otherCharacterFeat.associatedList();
			selectedList = new ArrayList(associatedList);
		}
