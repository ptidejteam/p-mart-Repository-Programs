...
public class Skill extends PObject
{
	...
	private Float rank = new Float(0);
	...
	public Integer modifier()
	{
		...
		if (aPC == null || keyStat.equalsIgnoreCase("None"))
			return anInt;
		if (stat >= 0)
	...
	{
		int bonus = Globals.getCurrentPC().getTotalBonusTo("SKILLRANK", getName(), true);
		return new Float(rank.doubleValue() + bonus);
	}
	...
	public String modRanks(double rankMod, PCClass aClass)
	{
		...
		Float maxRank = Globals.getCurrentPC().getMaxRank(getName(), aClass);
		if (getTotalRank().doubleValue() >= maxRank.doubleValue() && rankMod >= 0.0 && !Globals.isBoolBypassMaxSkillRank())
	...
	{
		rank = aRank;
		if (aRank.doubleValue() == 0.0 && required == false)
			Globals.getCurrentPC().getSkillList().remove(this);
	}
