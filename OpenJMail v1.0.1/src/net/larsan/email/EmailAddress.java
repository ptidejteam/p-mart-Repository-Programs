...
public final class EmailAddress implements Serializable, Cloneable {
	...
	* characters 0 - 31, 34, 127 or spaces and must contain a "@" character,
	* an InvalidFormatException will be throw if the address provided to this constrcutor
    ...
    * "@" character) and an InvalidFormatException will be thrown if needed.
    */
    ...
    */
	...
	public boolean equals(EmailAddress address) { 
	    if((this.address).equalsIgnoreCase(address.getAddress())) return true;
	    else return false;
	}
    ...
    * characters 0 - 31, 34, 127 or spaces. It must however contain a "@" 
    ...
    */
	...
	public static boolean isValid(String address) { 
        ...
        if(address == null || address.length() == 0) return false;
            ...
            if(at == -1) flag = false; // doesn't exist
            else if(at == 0 || at == (address.length() - 1)) flag = false; // wrong position
            
            if(address.indexOf(" ") > -1) flag = false; // no spaces allowed
    ...
    public static EmailAddress parseAddress(String input) throws InvalidFormatException {
        ...
        while(str.hasMoreTokens()) {
            ...
            if(tmp.indexOf("@") > -1) { // this is an address part
                ...
                if(tmp.charAt(0) == '<') { // delete prececiding "<" and last ">" if they exist
        ...
        if(name.length() > 0 && name.charAt(0) == '\"') {
            ...
            while(name.charAt(name.length() - 1) == '\"' || name.charAt(name.length() - 1) == ' ') name.deleteCharAt(name.length() - 1);
