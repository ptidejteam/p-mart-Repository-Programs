 ...
 * CombinedPlot.java
 * -----------------
 * $Id: CombinedPlot.java,v 1.1 2007/10/10 18:54:39 vauchers Exp $
 *
...
public class CombinedPlot extends Plot implements HorizontalValuePlot, VerticalValuePlot  {
   ...
   * out horizontaly one after the other Each sub-plot should have a different
   ...
   */
  public CombinedPlot(Axis axis, int type) throws AxisNotCompatibleException, PlotNotCompatibleException {
   ...
   */
  public CombinedPlot(Axis horizontal, Axis vertical) throws AxisNotCompatibleException, PlotNotCompatibleException {
  ...
  public void draw(Graphics2D g2, Rectangle2D drawArea) {
    ...
    for (int i=0; i<n; i++) {
      ...
      if (type != OVERLAID) {
        ...
        if (type == VERTICAL) {
          maxAxisWidth = Math.max(maxAxisWidth, plot.getVerticalAxis().reserveWidth(g2, plot, tempArea));
        } else if (type == HORIZONTAL) {
          maxAxisHeight = Math.max(maxAxisHeight, plot.getHorizontalAxis().reserveHeight(g2, plot, tempArea));
        }
      }
    ...
    for (int i=0; i<n; i++) {
      ChartInfo chartInfo = (ChartInfo)charts.get(i);
      chartInfo.chart.draw(g2, subPlotArea[i]);
    }
  ...
  public void chartChanged(ChartChangeEvent event) {
    ...
    boolean adjust = (axisRangeSet[HORIZONTAL] || axisRangeSet[VERTICAL]);
    if (adjust) {
      if (axisRangeSet[HORIZONTAL]) {
        setAxisRangeSet(HORIZONTAL, false);
      }
      if (axisRangeSet[VERTICAL]) {
  ...
  protected void adjustPlotsAxis() {
    if (!isAdjusted) {
      try {
        ...
        // this should not happen, if it does, report it...
        System.out.println("CombinedPlot.adjustPlots(): " + e);
