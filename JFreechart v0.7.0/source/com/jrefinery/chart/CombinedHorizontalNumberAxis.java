 ...
 * ---------------------------------
 * $Id: CombinedHorizontalNumberAxis.java,v 1.1 2007/10/10 18:54:39 vauchers Exp $
 *
...
public class CombinedHorizontalNumberAxis extends HorizontalNumberAxis implements CombinableAxis {
   ...
   */
  public CombinedHorizontalNumberAxis(HorizontalNumberAxis axis,
                                      boolean visible) {
    super(axis.getLabel(),
          axis.getLabelFont(),
          axis.getLabelPaint(),
    ...
    if (visible) {
      ...
      if (showGridLines) {
        ...
        while (iterator.hasNext()) {
          ...
          float xx = (float)translatedValue(tick.getNumericalValue(), plotArea);
  ...
  public AxisRange getRange() {
    autoAdjustRange();
    return (new NumberAxisRange(getMinimumAxisValue(), getMaximumAxisValue()));
  }
  ...
  public void setRange(AxisRange range) {
    setAutoRange(false);
    setMinimumAxisValue((Number)range.getMin());
    setMaximumAxisValue((Number)range.getMax());
    if (visible) {
      HorizontalNumberAxis axis = (HorizontalNumberAxis)getParentAxis();
      axis.setAutoRange(false);
      axis.setMinimumAxisValue((Number)range.getMin());
      axis.setMaximumAxisValue((Number)range.getMax());
    }
  }
