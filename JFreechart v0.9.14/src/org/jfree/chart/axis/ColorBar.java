 ...
 *
 * $Id: ColorBar.java,v 1.1 2007/10/10 19:19:06 vauchers Exp $
 *
 * Changes
 * -------
...
public class ColorBar implements Cloneable, Serializable {
    ...
    public double draw(Graphics2D g2, double cursor,
        ...
        if (edge == RectangleEdge.TOP) {
            ...
            state = this.axis.draw(g2, cursor, reservedArea, colorBarArea, RectangleEdge.TOP);
        } 
        else if (edge == RectangleEdge.BOTTOM) {
            cursor = colorBarArea.getMaxY();
            state = this.axis.draw(g2, cursor, reservedArea, colorBarArea, RectangleEdge.BOTTOM);
        } 
        else if (edge == RectangleEdge.LEFT) {
            cursor = colorBarArea.getMinX();
            state = this.axis.draw(g2, cursor, reservedArea, colorBarArea, RectangleEdge.LEFT);
        } 
        else if (edge == RectangleEdge.RIGHT) {
            cursor = colorBarArea.getMaxX();
            state = this.axis.draw(g2, cursor, reservedArea, colorBarArea, RectangleEdge.RIGHT);
        }
    ...
    public void drawColorBar(Graphics2D g2, Rectangle2D colorBarArea, RectangleEdge edge) {
        ...
        if (RectangleEdge.isTopOrBottom(edge)) {
            ...
            while (xx <= colorBarArea.getMaxX()) {
                double value = this.axis.translateJava2DToValue(xx, colorBarArea, edge);
