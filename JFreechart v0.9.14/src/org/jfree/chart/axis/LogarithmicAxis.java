 ...
 *                   David M. O'Donnell;
 *
 * $Id: LogarithmicAxis.java,v 1.1 2007/10/10 19:19:06 vauchers Exp $
 *
 * Changes
 * -------
...
public class LogarithmicAxis extends NumberAxis {
    ...
    public List refreshTicks(Graphics2D g2, 
        ...
        if (RectangleEdge.isTopOrBottom(edge)) {
            ticks = refreshTicksHorizontal(g2, drawArea, dataArea, edge);
        }
        else if (RectangleEdge.isLeftOrRight(edge)) {
            ticks = refreshTicksVertical(g2, drawArea, dataArea, edge);
        }
        return ticks;
     ...
     * @return A list of ticks.
     */
    public List refreshTicksHorizontal(Graphics2D g2, Rectangle2D drawArea, Rectangle2D dataArea,
                                       RectangleEdge edge) {
        ...
        for (int i = iBegCount; i <= iEndCount; i++) {
            ...
            for (int j = 0; j < 10; ++j) {
                ...
                if (currentTickValue >= lowerBoundVal - SMALL_LOG_VALUE) {
                    ...
                    Rectangle2D tickLabelBounds = fm.getStringBounds(tickLabel, g2);
                    float x = 0.0f;
                    float y = 0.0f;
                    ...
                    Tick tick = new NumberTick(new Double(currentTickValue), tickLabel, x, y, 
                                               TextAnchor.BASELINE_LEFT, TextAnchor.CENTER, 0.0);
                    ticks.add(tick);
                }
            }
        }
        ...
        for (int i = iBegCount; i <= iEndCount; i++) {
            ...
            for (int j = 0; j < jEndCount; j++) {
                ...
                if (tickVal >= lowerBoundVal - SMALL_LOG_VALUE) {
                    ...
                    ticks.add(new NumberTick(new Double(tickVal), tickLabel, x, y, 
                                             TextAnchor.BASELINE_LEFT, TextAnchor.CENTER, 0.0));
