		...
		throws PersistenceLayerException
	{
		...
		if (loadFile)
		{
			try
			{
				...
				{
					convertedFileURL = Utility.fixFilenamePath(SettingsHandler.getPccFilesLocation().getAbsolutePath() + baseFileName);
					convertedFileURL = Utility.fileToURL(convertedFileURL);
				}
				// If it is a URL, don't append the path, and don't change the slashes
				else
				{
					convertedFileURL = Utility.fixURLPath(SettingsHandler.getPccFilesLocation().getAbsolutePath(), baseFileName);
				}
			}
