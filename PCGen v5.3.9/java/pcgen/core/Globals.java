...
//	}
	...
	/**
	 * @deprecated on 2003-08-08 as it is unused, will be removed sometime after 2003-08-22 unless this tag is replaced with an explanation for why this method should not be deleted.
	 * @param name
	 * @return
	 */
		...
		for (Iterator i = spellMap.keySet().iterator(); i.hasNext();)
		{
			...
			if (obj instanceof ArrayList)
			{
				for (Iterator j = ((ArrayList) obj).iterator(); j.hasNext();)
				{
					...
					if (aSpell.levelForKeyContains(aBuf.toString(), level) && aSpell.getType().indexOf(spellType) >= 0)
					{
						aList.add(aSpell);
					}
				}
