...
{
	LevelAbilitySpecial(PObject aowner, int aLevel, String aList)
	{
	...
	public List getChoicesList(String bString)
	{
		...
		final List aList = super.getChoicesList(bString);
	...
	void processToken(String aChoice, List aArrayList, String bString)
	{
		...
		if ("FEATLIST".equals(aChoice))
		{
			for (int index = 0; index < Globals.getFeatList().size(); ++index)
			{
				...
				if (PlayerCharacter.qualifiesForFeat(aFeat.getKeyName()) && (((!aPC.hasFeat(aChoice) && !aPC.hasFeatAutomatic(aChoice))) || aFeat.isMultiples()))
				{
		...
		else if (aChoice.startsWith("FEATTYPE=") || aChoice.startsWith("FEATTYPE."))
		{
			final String featType = aChoice.substring(9);
			aArrayList.addAll(aPC.getAvailableFeatNames(featType));
		}
		else if (aChoice.startsWith("FEAT=") || aChoice.startsWith("FEAT."))
		{
	...
	public void processChoice(List aArrayList, List selectedList, String eString)
	{
		...
		for (int i = 0; i < selectedList.size(); ++i)
		{
			...
			if (index >= Globals.getFeatList().size())
			{
				...
				{
					sa = new SpecialAbility(aString, "PCCLASS|" + owner.getName() + "|" + level());
				}
				else
				{
					sa = new SpecialAbility(aString);
				}
				owner.addSpecialAbilityToList(sa);
				owner.addSave(aString);
			}
		}
	}
}
