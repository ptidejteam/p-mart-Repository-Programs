		...
		else
		{
			...
			while (moves.countTokens() > 1)
			{
				...
				else if (tok.length() > 0)
				{
					...
					{
						Logging.errorPrint("Badly formed PRESTAT token: " + tok);
						newmove = 0;
					}
...
//			aString = aTok.nextToken();
			...
			while (aTok.hasMoreTokens())
			{
				...
				if ((tok.startsWith("TYPE=") || tok.startsWith("TYPE.")) && tag.startsWith("WEAPON"))
				{
					...
					while (bTok.hasMoreTokens())
					{
						...
						if (bList.size() == 0)
						{
							bList.addAll(Globals.getEquipmentOfType(Globals.getEquipmentList(), "Weapon." + bString, ""));
						}
						if (xList == null)
						{
							xList = new ArrayList();
				...
				{
					Equipment aEq = Globals.getEquipmentFromName(tok);
					if (aEq != null)
					{
	...
	public double bonusTo(String aType, String aName, Object obj, List aBonusList)
	{
		...
		if (!dontRecurse && (this instanceof Feat) && !SettingsHandler.isBoolBypassFeatPreReqs())
		{
	...
	private double calcBonus(String bString, String aType, String aName, String aTypePlusName, Object obj, int iTimes, BonusObj aBonusObj)
	{
		...
		else
		{
			...
			{
				//Should this be ignored?
				Logging.errorPrint("calcBonus NumberFormatException in BONUS: " + bString, e);
			}
		}

		final List bonusPreReqList = new ArrayList();
		String possibleBonusTypeString = aBonusObj.getTypeString();
		StringTokenizer pTok = new StringTokenizer(aBonusObj.getPrereqString());
...
//		}
	...
	public final boolean passesPreReqToGainForList(List argList)
	{
		...
		return passesPreReqToGainForList(aPC, null, argList);
	}
	...
	public final boolean passesPreReqToUseForList(List argList)
	{
		...
		return passesPreReqToUseForList(aPC, null, argList);
	}

	public final boolean passesPreReqToGainForList(PObject aObj, List anArrayList)
	{
		return passesPreReqToGainForList(Globals.getCurrentPC(), aObj, anArrayList);
	}

	private class PreReqState
	{
		PObject theObj;
		String preString;
		String aType;
	...
	private interface PreReqTests
	{
		boolean passesTests(PlayerCharacter aPC, PObject aObj, List argList, PreReqState preReqState);
	}

	private static class PreReqTestsToGain
		implements PreReqTests
	{
		private PreReqTests preReqTestsToUse;

		PreReqTestsToGain(PreReqTests preReqTestsToUse)
		{
			this.preReqTestsToUse = preReqTestsToUse;
		}

		public boolean passesTests(PlayerCharacter aPC, PObject aObj, List argList, PreReqState preReqState)
		{
			if (!preReqState.aType.startsWith("PRE"))
			{
				return preReqTestsToUse.passesTests(aPC, aObj, argList, preReqState);
			}

			String match = preReqState.aType.substring(3);
			boolean flag = true;

			if ("CLASSLEVELMAX".equals(match))
			{
				preReqState.handled = true;
				flag = passesPreClassLevelMax(preReqState.aList, aPC);
			}
			else if ("LEVELMAX".equals(preReqState.aType))
			{
				preReqState.handled = true;
				int preLevelMax;
				try
				{
					preLevelMax = Integer.parseInt(preReqState.aList);
				}
				catch (NumberFormatException nfe)
				{
					Logging.errorPrint("Badly formed PRELEVELMAX attribute: " + preReqState.aList);
					preLevelMax = 0;
				}
	...
	private static class PreReqTestsToUse
		implements PreReqTests
	{
		public boolean passesTests(PlayerCharacter aPC, PObject aObj, List argList, PreReqState preReqState)
		{
			...
			if (preReqState.aType.equals("PRETYPE"))
			{
				preReqState.handled = true;

				final StringTokenizer aTok = new StringTokenizer(preReqState.aList, ",|", true);
			...
			if ("RESTRICT".equals(preReqState.aType))
			{
				preReqState.handled = true;
				return passesRestrict(preReqState.aList, flag, aPC);
			}
			...
			if (match.equals("APPLY"))
			{
				flag = preReqState.theObj.passesPreApplied(aPC, aObj);
			}
			else if (match.equals("FEAT"))
			{
				StringTokenizer aTok = new StringTokenizer(preReqState.aList, "|");
				preReqState.aList = aTok.nextToken();
				flag = passesPreFeat(aTok, preReqState.aList, aPC != null ? aPC.aggregateFeatList() : null);
			}
			else if (match.equals("SKILL") || match.equals("SKILLMULT"))
			{
				flag = passesPreSkill(preReqState.aList, aPC);
			}
			else if (match.equals("SKILLTOT"))
			{
				flag = passesPreSkillTot(preReqState.aList, aPC);
			}
			else if (match.equals("CLASS"))
			{
				flag = passesPreClass(preReqState.aList, aPC);
			}
			else if (match.equals("SUBCLASS"))
			{
				final StringTokenizer dTok = new StringTokenizer(preReqState.aList, ",");
			...
			else if (match.equals("DEFAULTMONSTER"))
			{
				flag = (aPC.isMonsterDefault() == ("Y".equalsIgnoreCase(preReqState.aList)));
			}
			else if (match.equals("CLASSLEVELMAX"))
			{
				flag = passesPreClassLevelMax(preReqState.aList, aPC);
			}
			else if (match.equals("LEVELMAX"))
			{
				int preLevelMax;
				try
				{
			...
			else if (match.equals("LEVEL"))
			{
				int preLevel;
				try
				{
					preLevel = Integer.parseInt(preReqState.aList);
				}
				catch (NumberFormatException nfe)
				{
					Logging.errorPrint("Badly formed PRELEVEL attribute: " + preReqState.aList);
					preLevel = 1;
				}

				flag = (aPC.getTotalLevels() >= preLevel);
			}
			else if (match.equals("HD"))
			{
				flag = passesPreHD(preReqState.aList, aPC);
			}
			else if (match.equals("HP"))
			{
				int preHitPoints;
				try
				{
					preHitPoints = Integer.parseInt(preReqState.aList);
				}
				catch (NumberFormatException nfe)
				{
					Logging.errorPrint("Badly formed PREHP attribute: " + preReqState.aList);
					preHitPoints = 0;
				}
				flag = (aPC.hitPoints() >= preHitPoints);
			}
			else if ("RESTRICT".equals(preReqState.aType))
			{
				flag = passesRestrict(preReqState.aList, flag, aPC);
			}
			else if (match.equals("RACE"))
			{
				flag = passesPreRace(preReqState.aList, aPC);
			}
			else if (match.equals("TEMPLATE"))
			{
				flag = passesPreTemplate(aPC, preReqState.aList);
			}
			else if (match.equals("ATT"))
			{
				flag = false;
				...
				try
				{
					final int anInt = Integer.parseInt(preReqState.aList);
					flag = att >= anInt;
				}
				catch (NumberFormatException exc)
				{
					Logging.errorPrint("Badly formed PREATT attribute: " + preReqState.aList);
				}
			}
			else if (match.equals("UATT"))
			{
				flag = passesPreUAtt(preReqState.aList, aPC);
			}
			else if (match.startsWith("STAT"))
			{
				flag = preReqState.theObj.passesPreStat(match.substring(4), preReqState.aList, aPC);
			}
			else if (match.equals("SPELLTYPE"))
			{
				flag = preReqState.theObj.passesPreSpellType(preReqState.aList, aPC);
			}
			else if (match.equals("SPELLSCHOOL"))
			{
				flag = passesPreSpellSchool(preReqState.aList, aPC);
			}
			else if (match.equals("SPELLSCHOOLSUB"))
			{
				flag = passesPreSpellSchoolSub(preReqState.aList, aPC);
			}
			else if (match.equals("SPELL"))
			{
				flag = passesPreSpell(preReqState.aList, aPC);
			}
			else if (match.equals("SPELLBOOK"))
			{
				flag = false;
				for (Iterator ee = aPC.getClassList().iterator(); !flag && ee.hasNext();)
				{
					flag |= preReqState.aList.startsWith(((PCClass) ee.next()).getSpellBookUsed() ? "Y" : "N");
				}
			}
			else if (match.equals("SPELLCAST"))
			{
				flag = passesPreSpellCast(preReqState.aList, aPC, flag);
			}
			else if (match.equals("SA"))
			{
				flag = passesPreSA(preReqState.aList, aPC);
			}
			else if (match.equals("LANG"))
			{
				flag = passesPreLang(preReqState.aList, aPC);
			}
			else if (match.equals("WEAPONPROF"))
			{
				flag = passesPreWeaponProf(preReqState.aList, aPC);
			}
			else if (match.equals("ITEM"))
			{
				flag = passesPreItem(preReqState.aList, aPC);
			}
			else if (match.equals("EQUIP"))
			{
				flag = passesPreEquip(preReqState.aList, aPC);
			}
			else if (match.equals("EQUIPPRIMARY"))
			{
				flag = passesPreEquipPrimary(preReqState.aList, aPC);
			}
			else if (match.equals("EQUIPSECONDARY"))
			{
				flag = passesPreEquipSecondary(preReqState.aList, aPC);
			}
			else if (match.equals("EQUIPBOTH"))
			{
				flag = passesPreEquipBothHands(preReqState.aList, aPC);
			}
			else if (match.equals("EQUIPTWOWEAPON"))
			{
				flag = passesPreEquipTwoWeapon(preReqState.aList, aPC);
			}
			else if (match.equals("ARMORTYPE"))
			{
				flag = passesPreArmorType(preReqState.aList, aPC);
			}
			else if (match.startsWith("VAR"))
			{
				flag = passesPreVar(preReqState.aList, preReqState.aType, aPC);
			}
			else if (match.startsWith("VISION"))
			{
				flag = passesPreVision(preReqState.aList, aPC);
			}
			else if (match.equals("MULT"))
			{
				flag = preReqState.theObj.passesPreMult(aPC, aObj, preReqState.preString.substring(8));
			}
			else if (match.equals("DR"))
			{
				flag = passesPreDR(preReqState.aList, aPC);
			}
			else if (match.equals("GENDER"))
			{
				flag = aPC.getGender().startsWith(preReqState.aList);
			}
			else if (match.equals("CITY"))
			{
				flag = aPC.getResidence().equalsIgnoreCase(preReqState.aList);
			}
			else if (match.equals("BIRTHPLACE"))
			{
				flag = aPC.getBirthplace().equalsIgnoreCase(preReqState.aList);
			}
			else if (match.equals("REGION"))
			{
				// if the PCs full region starts with
				// preReqState.aList, then we don't
				// care about subregion but if the lst
				// specifies a subregion then this'll
				// attempt to match it, too
				flag = aPC.getFullRegion().startsWith(preReqState.aList);
			}
			else if (match.equals("DEITY"))
			{
				flag = passesPreDeity(preReqState.aList, aPC);
			}
			else if (match.equals("DEITYDOMAIN")) //for Mynex
			{
				flag = passesPreDeityDomain(preReqState.aList, aPC);
			}
			else if (match.equals("DEITYALIGN"))
			{
				flag = passesPreDeityAlign(preReqState.aList, aPC);
			}
			else if (match.equals("ALIGN"))
			{
				flag = passesPreAlign(preReqState.aList, aPC);
			}
			else if (match.startsWith("CHECK"))
			{
				flag = passesPreCheck(preReqState.aType, preReqState.aList, aPC);
			}
			else if (match.equals("DOMAIN"))
			{
				flag = passesPreDomain(preReqState.aList, aPC);
			}
			else if (match.equals("MOVE"))
			{
				flag = passesPreMove(aPC, flag, preReqState.aList);
			}
			// PREHANDSEQ, PREHANDSGT, PREHANDSGTEQ,
			// PREHANDSLT, PREHANDSLTEQ, PREHANDSNEQ
			else if (match.startsWith("HANDS"))
			{
				try
				{
					flag = doComparison(match.substring(5), aPC.getRace().getHands(), Integer.parseInt(preReqState.aList), aPC, "HANDS");
				}
				catch (NumberFormatException nfe)
				{
					Logging.errorPrint("Badly formed PREHANDS attribute: " + preReqState.aList);
				}
			}
			// PRELEGSEQ, PRELEGSGT, PRELEGSGTEQ,
			// PRELEGSLT, PRELEGSLTEQ, PRELEGSNEQ
			else if (match.startsWith("LEGS"))
			{
				try
				{
					flag = doComparison(match.substring(4), aPC.getRace().getLegs(), Integer.parseInt(preReqState.aList), aPC, "LEGS");
				}
				catch (NumberFormatException nfe)
				{
					Logging.errorPrint("Badly formed PRELEGS attribute: " + preReqState.aList);
				}
			}
			// PRESIZEEQ, PRESIZEGT, PRESIZEGTEQ,
			// PRESIZELT, PRESIZELTEQ, PRESIZENEQ
			else if (match.startsWith("SIZE"))
			{
				flag = preReqState.theObj.doComparison(match.substring(4), aPC.sizeInt(), Globals.sizeInt(preReqState.aList));
			}
			// PRESREQ, PRESRGT, PRESRGTEQ,
			// PRESRLT, PRESRLTEQ, PRESRNEQ
			else if (match.startsWith("SR"))
			{
				try
				{
					flag = preReqState.theObj.doComparison(match.substring(2), aPC.calcSR(false), Integer.parseInt(preReqState.aList));
				}
				catch (NumberFormatException nfe)
				{
					Logging.errorPrint("Badly formed PRESR attribute: " + preReqState.aList);
				}
			}
			// PREBASESIZEEQ, PREBASESIZEGT, PREBASESIZEGTEQ,
			// PREBASESIZELT, PREBASESIZELTEQ, PREBASESIZENEQ
			else if (match.startsWith("BASESIZE"))
			{
				flag = false;
				if ((aPC.getRace() != null) && !aPC.getRace().equals(Globals.s_EMPTYRACE))
				{
					final int iSize = Globals.sizeInt(preReqState.aList, -1);
					if (iSize < 0)
					{
						Logging.errorPrint("Invalid size '" + preReqState.aList + "' in PREBASESIZE");
					}
					else
					{
						flag = preReqState.theObj.doComparison(match.substring(8), aPC.racialSizeInt(), iSize);
					}
				}
			}
	...
	private static final PreReqTestsToUse preReqTestsToUse = new PreReqTestsToUse();
	private static final PreReqTestsToGain preReqTestsToGain = new PreReqTestsToGain(preReqTestsToUse);

	public boolean passesPreReqToGainForList(PlayerCharacter aPC, PObject aObj, List argList)
	{
		return passesPreReqForList(aPC, aObj, argList, preReqTestsToGain);
	}

	public boolean passesPreReqToUseForList(PlayerCharacter aPC, PObject aObj, List argList)
	{
		return passesPreReqForList(aPC, aObj, argList, preReqTestsToUse);
	}
	 ...
	 * </ul>
	 */
	private boolean passesPreReqForList(PlayerCharacter aPC, PObject aObj, List argList, PreReqTests preReqTests)
	{
		...
		PreReqState preReqState = new PreReqState();
		preReqState.theObj = this;
		...
		for (Iterator e = argList.iterator(); e.hasNext();)
		{
			...
			preReqState.preString = (String) e.next();
			preReqState.preString = preReqState.preString.toUpperCase();
			final StringTokenizer aaTok = new StringTokenizer(preReqState.preString, ":");
			preReqState.aType = aaTok.nextToken();
			if (aaTok.hasMoreTokens())
			{
				preReqState.aList = aaTok.nextToken();
			}
			else
			{
				preReqState.aList = "";
			...
			if (preReqState.aType.length() > 0 && preReqState.aType.charAt(0) == '!')
			{
				invertFlag = true;
				preReqState.aType = preReqState.aType.substring(1);
			}
	...
	public boolean passesPreMult(PlayerCharacter aPC, PObject aObj, String paramString)
	{
		...
		for (Iterator i = preList.iterator(); i.hasNext();)
		{
			...
			argList.add(aString);
			if (passesPreReqToGainForList(aPC, aObj, argList))
			{
				--qtdMult;
			}
			--qtdPreList;
	...
	public final String preReqHTMLStringsForList(PlayerCharacter aPC, PObject aObj, List aList, boolean includeHeader)
	{
		...
		for (Iterator e = aList.iterator(); e.hasNext();)
		{
			...
			if (aPC != null)
			{
				flag = passesPreReqToGainForList(aPC, aObj, newList);
			}
			else if (aObj == null)
			{
	...
	private static boolean passesPreFeat(StringTokenizer aTok, String aList, List aFeatList)
	{
		int number;
		final boolean flag;
		...
		{
			Logging.errorPrint("Exception in PREFEAT:" + aList + Constants.s_LINE_SEP + "Assuming 1 required", exceptn);
			number = 1;
			aTok = new StringTokenizer(aList, ",");
		}
	...
	private boolean passesPreApplied(PlayerCharacter aPC, PObject anObj)
	{
		if (anObj == null)
		{
	...
	private static boolean passesPreAlign(String aList, PlayerCharacter aPC)
	{
		...
		{
			flag = SystemCollections.getShortAlignmentAtIndex(aPC.getAlignment()).equals(aPC.getDeity().getAlignment());
		}
		return flag;
	}

	private static boolean passesPreCheck(String aType, String aList, PlayerCharacter aPC)
	{
		// PRECHECK:x,check.val,check.val
		// PRECHECKBASE:x,Fortitude.11
		boolean isBase = aType.endsWith("BASE");
		final StringTokenizer dTok = new StringTokenizer(aList, ",=.");
		String number = dTok.nextToken();
		int num;
		try
		{
			num = Integer.parseInt(number); // number we must match
		}
		catch (NumberFormatException nfe)
		{
			Logging.errorPrint("Badly formed PRECHECK: " + aList);
			num = 0;
		}
		while (dTok.hasMoreTokens())
		{
	...
	private static boolean passesPreDeityAlign(String aList, PlayerCharacter aPC)
	{
		...
		while (aTok.hasMoreTokens())
		{
			...
			if (aPC.getDeity() != null)
			{
				flag = (aPC.getDeity().getAlignment().equalsIgnoreCase(aString));
				if (flag)
				{
					break;
				}
	...
	private static boolean passesPreDomain(String aList, PlayerCharacter aPC)
	{
		...
		{
			Logging.errorPrint("Badly formed PREDOMAIN attribute: " + tok);
			number = 0;
		}
		while (aTok.hasMoreTokens() && number > 0)
		{
	...
	private static boolean passesPreItem(String aList, PlayerCharacter aPC)
	{
		...
		{
			Logging.errorPrint("Badly formed PREITEM attribute: " + tok);
			number = 0;
		}
		while (aTok.hasMoreTokens())
		{
	...
	private static boolean passesPreArmorType(String aList, PlayerCharacter aPC)
	{
		...
		{
			Logging.errorPrint("Badly formed PREARMORTYPE attribute: " + tok);
			number = 0;
		}
		while (aTok.hasMoreTokens())
		{
	...
	private static boolean passesPreEquipPrimary(String aList, PlayerCharacter aPC)
	{
		...
		{
			Logging.errorPrint("Badly formed PREEQUIPPRIMARY attribute: " + tok);
			number = 0;
		}
		final int equippedType = Equipment.EQUIPPED_PRIMARY;
		flag = (passesPreEquipHandleTokens(aTok, aPC, equippedType, number) <= 0);
		return flag;
	}
	...
	private static boolean passesPreEquipSecondary(String aList, PlayerCharacter aPC)
	{
		...
		final int equippedType = Equipment.EQUIPPED_SECONDARY;
		flag = (passesPreEquipHandleTokens(aTok, aPC, equippedType, number) <= 0);
		return flag;
	}
	...
	private static boolean passesPreEquipBothHands(String aList, PlayerCharacter aPC)
	{
		...
		{
			Logging.errorPrint("Badly formed PREEQUIPBOTH attribute: " + tok);
			number = 0;
		}
		final int equippedType = Equipment.EQUIPPED_BOTH;
		flag = (passesPreEquipHandleTokens(aTok, aPC, equippedType, number) <= 0);
		return flag;
	}
	...
	private static boolean passesPreEquipTwoWeapon(String aList, PlayerCharacter aPC)
	{
		...
		{
			Logging.errorPrint("Badly formed PREEQUIPTWOWEAPON attribute: " + tok);
			number = 0;
		}
		final int equippedType = Equipment.EQUIPPED_TWO_HANDS;
		flag = (passesPreEquipHandleTokens(aTok, aPC, equippedType, number) <= 0);
		return flag;
	}
	 ...
	 * @return
	 */
	private static int passesPreEquipHandleTokens(final StringTokenizer aTok, PlayerCharacter aPC, final int equippedType, int number)
	{
		while (aTok.hasMoreTokens())
		{
			if (aPC.getEquipmentList().isEmpty())
	...
	private static boolean passesPreSA(String aList, PlayerCharacter aPC)
	{
		...
		{
			Logging.errorPrint("Exception in PRESA:" + aList + Constants.s_LINE_SEP + "Assuming 1 required", exceptn);
			number = 1;
			aTok = new StringTokenizer(aList, ",");
		}
			...
			if (!bFound)
			{
				if (!aPC.getTemplateList().isEmpty())
				{
					for (Iterator e1 = aPC.getTemplateList().iterator(); e1.hasNext();)
					{
						...
						final List SAs = aTempl.getSpecialAbilityList(aPC.getTotalLevels(), aPC.totalHitDice());
	...
	private static boolean passesPreSpell(String aList, PlayerCharacter aPC)
	{
		...
		final List aArrayList = aPC.aggregateSpellList("Any", "", "", 0, 20);
	...
	private static boolean passesPreSpellCast(String aList, PlayerCharacter aPC, boolean flag)
	{
		...
		final StringTokenizer aTok = new StringTokenizer(aList, ",");
		final List classList = (ArrayList) aPC.getClassList().clone();
		PCClass aClass;
		while (aTok.hasMoreTokens())
		{
	...
	private static boolean passesPreSpellSchool(String aList, PlayerCharacter aPC)
	{
		...
		if (aTok.countTokens() < 3)
		{
			Logging.errorPrint("Badly formed PreSpellSchool tag: " + aList);
			return false;
		}
		...
		final List aArrayList = aPC.aggregateSpellList("Any", school, "A", minlevel, 20);
	...
	private static boolean passesPreSpellSchoolSub(String aList, PlayerCharacter aPC)
	{
		...
		if (aTok.countTokens() < 3)
		{
			Logging.errorPrint("Badly formed PreSpellSchoolSub tag: " + aList);
			return false;
		}
		...
		final List aArrayList = aPC.aggregateSpellList("Any", "No-Match", subSchool, minlevel, 20);
		flag = (aArrayList.size() >= number);
		return flag;
	}
	...
	private static boolean passesPreVar(String aList, String aType, PlayerCharacter aPC)
	{
		...
		while (aTok.hasMoreTokens() && flag)
		{
			...
			final float aTarget = aPC.getVariableValue(valString, "").floatValue();
			final float aVar;
		...
		while (aTok.hasMoreTokens())
		{
			...
			if (numberRequired == -1)
			{
				...
				catch (NumberFormatException nfe)
				{
					...
					Logging.errorPrint("Using \"" + aList + "\" old PREWEAPONPROF syntax. New syntax is PREWEAPONPROF:#,wp1,wp2,...,wpn");
			...
			else if (aString.startsWith("TYPE.") || aString.startsWith("TYPE="))
			{
				...
				for (Iterator e = aPC.getWeaponProfList().iterator(); e.hasNext();)
				{
					...
					{
						Equipment eq = Globals.getEquipmentNamed(profName);
						if (eq != null)
						{
	...
	//
	private static boolean passesPreClass(String aList, PlayerCharacter aPC)
	{
		final StringTokenizer aTok = new StringTokenizer(aList, ",");
		int numberRequired = 1;
		String aString = aTok.nextToken();
	...
	private static boolean passesPreClassLevelMax(String aList, PlayerCharacter aPC)
	{
		...
		{
			Logging.errorPrint("Badly formed passesPreClassLevelMax attribute: " + aList.substring(i + 1));
			preClass = 0;
		}
		final StringTokenizer aTok = new StringTokenizer(aList.substring(0, i), ",");
		while (aTok.hasMoreTokens())
		{
			String aString = aTok.nextToken();
			if ("SPELLCASTER".equals(aString))
	...
	private static boolean passesPreHD(String aList, PlayerCharacter aPC)
	{
	...
	private static boolean passesPreUAtt(String aList, PlayerCharacter aPC)
	{
		final boolean flag;
		int requiredValue;
		try
		...
		{
			Logging.errorPrint("Badly formed passesPreUAtt attribute: " + aList);
			requiredValue = 0;
		}
		int att = 0;
	...
	private static boolean passesPreRace(String aList, PlayerCharacter aPC)
	{
		boolean flag = false;
		final StringTokenizer aTok = new StringTokenizer(aList, "|");
		while (aTok.hasMoreTokens())
	...
	private static boolean passesPreDR(String aList, PlayerCharacter aPC)
	{
		final StringTokenizer aTok = new StringTokenizer(aList, ",");
		...
		final String aDR = aPC.calcDR();
		Map drMap = new HashMap();
		if (aDR != null)
		{
			final StringTokenizer bTok = new StringTokenizer(aDR, ";");
			while (bTok.hasMoreTokens())
			{
				final StringTokenizer cTok = new StringTokenizer(bTok.nextToken(), "/");
				final String aValue = cTok.nextToken();
				final String aType = cTok.nextToken();
				drMap.put(aType, aValue);
			}
		}
		while (aTok.hasMoreTokens())
		{
			final StringTokenizer bTok = new StringTokenizer(aTok.nextToken(), "=.");
			final String bType = bTok.nextToken();
			final int bValue;
			if (bTok.hasMoreTokens())
			{
				try
				{
					bValue = Integer.parseInt(bTok.nextToken());
				}
				catch (NumberFormatException nfe)
				{
					Logging.errorPrint("Badly formed passesPreDR value: " + aList);
					return false;
				}
			}
			else
	...
	private static boolean passesPreVision(String aList, PlayerCharacter aPC)
	{
		int number;
		boolean flag;
		...
		if (i >= 0)
		{
			try
			{
				range = Integer.parseInt(aList.substring(i + 1));
			}
			catch (NumberFormatException e)
			{
				Logging.errorPrint("Badly formed passesPreVision attribute: " + aList.substring(i + 1));
				range = 0;
			}
		}
		else
		...
		final String sString = aPC.getVision();
		while (aTok.hasMoreTokens() && number > 0)
		{
			String aString = aTok.nextToken();
			final StringTokenizer bTok = new StringTokenizer(aString, "(");
	...
	private static boolean passesPreSkill(String aList, PlayerCharacter aPC)
	{
		int number;
		int i = aList.lastIndexOf('=');
		int ranks;
		if (i >= 0)
		{
			...
			{
				Logging.errorPrint("Badly formed passesPreSkill attribute: " + aList.substring(i + 1));
				ranks = 0;
			}
		}
		else
		{
			Logging.errorPrint("passesPreSkill: bad prereq \"" + aList + "\"");
			return false;
		}
		...
		final List sList = (ArrayList) aPC.getSkillList().clone();
	...
	private static boolean passesPreSkillTot(String aList, PlayerCharacter aPC)
	{
		boolean flag;
		int i = aList.lastIndexOf('=');
		int ranks;
		...
		{
			Logging.errorPrint("Badly formed passesPreSkillTot attribute: " + aList.substring(i + 1));
			ranks = 0;
		}
		final StringTokenizer aTok = new StringTokenizer(aList.substring(0, i), ",");
	 ...
	 * @return boolean
	 */
	private boolean passesPreSpellType(String aList, PlayerCharacter aPC)
	{
		final StringTokenizer aTok = new StringTokenizer(aList, ",");
		if (aTok.countTokens() < 3)
		{
			Logging.errorPrint(getName() + ":badly formed PRESPELLTYPE: " + aList);
			return false;
		}
		...
		while (bTok.hasMoreTokens())
		{
			...
			List aArrayList = aPC.aggregateSpellList(castingType, "", "", minlevel, 20);
	...
	// PRESTATNEQ
	//
	// Of form: PRESTAT:1,INT=12,WIS=10
	//
	private final boolean passesPreStat(String compType, final String aList, final PlayerCharacter aPC)
	{
		StringTokenizer aTok = new StringTokenizer(aList, ",");
		String aString = aTok.nextToken();
		int matchesNeeded = 1;
		...
		while (aTok.hasMoreTokens())
		{
			...
			if (aString.length() < 3)
			{
				Logging.errorPrint("Badly formed PRESTAT token: " + aList);
				return false;
			}
			try
			{
				final int iStat = aPC.getStatList().getTotalStatFor(aString.substring(0, 3));
	...
	private static boolean passesPreTemplate(PlayerCharacter aPC, String aList)
	{
		if (aPC.getTemplateList().isEmpty())
		{
			return false;
		...
		while (aTok.hasMoreTokens())
		{
			...
			if (wildCard >= 0)
			{
				...
				for (Iterator ti = aPC.getTemplateList().iterator(); ti.hasNext();)
				{
			...
			else if (aPC.getTemplateNamed(templateName) != null)
			{
				return true;
			}
		}
...
//		return kitString;
	...
	{
		final StringTokenizer aTok = new StringTokenizer(kitString, "|");
	...
	static boolean modChoices(PObject obj, List availableList, List selectedList, boolean process)
	{
		availableList.clear();
		selectedList.clear();
		...
		if (aChoiceString.startsWith("WEAPONPROF|") || aChoiceString.startsWith("ARMORPROF|"))
		{
