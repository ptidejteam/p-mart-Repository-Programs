 ...
 *
 * @(#) $Id: EditorMainForm.java,v 1.1 2006/02/21 01:15:53 vauchers Exp $
...
//
	...
	private static final String[] tags = new String[]{
		...
		"PREDEITYALIGN", "PREDEITYDOMAIN", "PREDOMAIN", "PREEQUIP", "PREEQUIPBOTH", "PREEQUIPPRIMARY", "PREEQUIPSECONDARY", "PREEQUIPTWOWEAPON",
		"PREFEAT", "PREGENDER", "PREHANDSEQ", "PREHANDSGT", "PREHANDSGTEQ", "PREHANDSLT", "PREHANDSLTEQ", "PREHANDSNEQ", "PREHD", "PREHP",
		"PREITEM", "PRELANG", "PRELEVEL", "PRELEVELMAX", "PREMOVE", "PRERACE", "PRESA", "PRESIZEEQ", "PRESIZELT",
	...
	private void initComponentContents()
	{
		...
		switch (editType)
		{
			...
			case EditorConstants.EDIT_DEITY:
				...
				availableList.clear();
				selectedList.clear();
				for (e = SystemCollections.getUnmodifiableAlignmentList().iterator(); e.hasNext();)
				{
				...
				selectedList.clear();
				availableList.clear();
				final List raceList = ((Deity) thisPObject).getRaceList();
			...
			case EditorConstants.EDIT_RACE:
				...
				//
				moveGrantedTemplatesFromAvailableToSelected(((Race) thisPObject).getTemplateList(),
					selectedList, selectedList2, availableList);
				...
				if (thisPObject.getNumberOfMovementTypes() > 0)
				{
					for (int index = 0; index < thisPObject.getNumberOfMovementTypes(); index++)
					{
						final String aMove = MovementPanel.makeMoveString(thisPObject.getMovementType(index), thisPObject.getMovement(index), null, null);
				...
				for (e = Globals.getRaceMap().keySet().iterator(); e.hasNext();)
				{
					...
					bString = Globals.getBioSet().getTagForRace(aString, raceName, "HAIR");
					if (bString != null)
					{
					...
					bString =Globals.getBioSet().getTagForRace(aString, raceName, "EYES");
					if (bString != null)
					{
						availableList = Utility.split(bString, '|');
						for (Iterator colIt = availableList.iterator(); colIt.hasNext();)
					...
					bString = Globals.getBioSet().getTagForRace(aString, raceName, "SKINTONE");
					if (bString != null)
					{
						availableList = Utility.split(bString, '|');
						for (Iterator colIt = availableList.iterator(); colIt.hasNext();)
			...
			case EditorConstants.EDIT_SPELL:
				...
				Map lvlInfo = ((Spell) thisPObject).getLevelInfo();
				//
				// Initialize the contents of the available and selected domains lists
				//
				int iCount = 0;
			...
			case EditorConstants.EDIT_TEMPLATE:
				...
				//
				moveGrantedTemplatesFromAvailableToSelected(((PCTemplate) thisPObject).getTemplateList(),
					selectedList, selectedList2, availableList);
				...
				List featList = ((PCTemplate) thisPObject).feats(-1, -1);
				...
				if (thisPObject.getNumberOfMovementTypes() > 0)
				{
					for (int index = 0; index < thisPObject.getNumberOfMovementTypes(); index++)
					{
						final String aMove = MovementPanel.makeMoveString(thisPObject.getMovementType(index), thisPObject.getMovement(index), thisPObject.getMovementMult(index), thisPObject.getMovementMultOp(index));
						movementValues.add(aMove);
					}
				}
				pnlMovement.setSelectedList(movementValues);
				pnlMovement.setMoveRateType(thisPObject.getMoveRatesFlag());

				//
				// Populate the abilities panel
				//
				selectedList.clear();
				List abilitiesList = ((PCTemplate) thisPObject).getLevelStrings();
		...
		if (pnlWeapons != null)
		{
			...
			final List autoWeap = thisPObject.getWeaponProfAutos();
			if (autoWeap != null)
			{
		...
		switch (editType)
		{
			...
			case EditorConstants.EDIT_SKILL:
				...
				{
					for (e = ((Skill) thisPObject).getChoiceStringList().iterator(); e.hasNext();)
					{
		...
		final List autoList = thisPObject.getAutoArray();
		if (autoList != null)
		{
		...
		final List saList = thisPObject.getSpecialAbilityList();
	...
	private void parseAlignment(List availableList, List selectedList, String alignmentString, String qualifier)
	{
		for (int i = 0; i < alignmentString.length(); ++i)
		{
			...
			else if ((alignmentChar >= '0') && (alignmentChar <= '9'))
			{
				...
				availableList.remove(SystemCollections.getLongAlignmentAtIndex(idx));
				selectedList.add(SystemCollections.getLongAlignmentAtIndex(idx) + (qualifier == null ? "" : qualifier));
			}
		}
	}
...
*/
	...
	private void btnSaveActionPerformed()
	{
		...
		if (aString.length() == 0)
		{
			GuiFacade.showMessageDialog(null, PropertyFactory.getString("in_demMes1"), Constants.s_APPNAME, GuiFacade.ERROR_MESSAGE);
			return;
		}
		...
		switch (editType)
		{
			case EditorConstants.EDIT_DEITY:
				...
				for (int i = 0; i < sel.length; ++i)
				{
					...
					for (int align = 0; align < SystemCollections.getUnmodifiableAlignmentList().size(); ++align)
					{
						if (aString.equals(SystemCollections.getLongAlignmentAtIndex(align)))
						{
							if (qualifier != null)
							{
...
// TODO: check if all skills of one type are selected...maybe change to TYPE.blah?
	...
	private static boolean parseSynergyBonus(final BonusObj aBonus, List availableList, List selectedList)
	{
		...
		{
			final List bonusParts = Utility.split(aString, '|');
			//
