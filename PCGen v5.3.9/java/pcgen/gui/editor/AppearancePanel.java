 ...
 *
 * @(#) $Id: AppearancePanel.java,v 1.1 2006/02/21 01:15:53 vauchers Exp $
 */
...
final class AppearancePanel extends JPanel implements PObjectUpdater
{
	...
	public void updateData(PObject obj)
	{
		...
		Globals.getBioSet().addToUserMap(region, raceName, "HAIR:" + aString);

		sel = pnlEyeColor.getSelectedList();
		aString = EditUtil.delimitArray(sel, '|');
		Globals.getBioSet().removeFromUserMap(region, raceName, "EYES");
		Globals.getBioSet().addToUserMap(region, raceName, "EYES:" + aString);

		sel = pnlSkinTone.getSelectedList();
		aString = EditUtil.delimitArray(sel, '|');
		Globals.getBioSet().removeFromUserMap(region, raceName, "SKINTONE");
		Globals.getBioSet().addToUserMap(region, raceName, "SKINTONE:" + aString);
	}
	...
	public void updateView(PObject obj)
	{
		...
		aString = Globals.getBioSet().getTagForRace(region, raceName, "HAIR");
		if (aString != null)
		{
			pnlHairColor.setSelectedList(Utility.split(aString, '|'), true);
		}

		aString = Globals.getBioSet().getTagForRace(region, raceName, "EYES");
		if (aString != null)
		{
			pnlEyeColor.setSelectedList(Utility.split(aString, '|'), true);
		}

		aString = Globals.getBioSet().getTagForRace(region, raceName, "SKINTONE");
		if (aString != null)
		{
