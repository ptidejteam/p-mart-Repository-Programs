 ...
 *
 * @(#) $Id: AgePanel.java,v 1.1 2006/02/21 01:15:52 vauchers Exp $
 */
...
final class AgePanel extends JPanel implements PObjectUpdater
{
	...
	static final class AgeTableModel extends AbstractTableModel
	{
		...
		void reset(String region, String raceName)
		{
			...
			{
				// Populate the data
				dataValue = Globals.getBioSet().getTagForRace(region, raceName, "BASEAGE");
				if (dataValue != null)
				{
					StringTokenizer tok = new StringTokenizer(dataValue, "\t");
				...
				if (dataValue != null)
				{
					dataValue = Globals.getBioSet().getTagForRace(region, raceName, "AGEDIEROLL");
					StringTokenizer tok = new StringTokenizer(dataValue, "\t");
					for (Iterator iter = data.iterator(); iter.hasNext() && tok.hasMoreTokens();)
					{
						String value = tok.nextToken();
				...
				if (dataValue != null)
				{
					dataValue = Globals.getBioSet().getTagForRace(region, raceName, "MAXAGE");
					StringTokenizer tok = new StringTokenizer(dataValue, "\t");
					for (Iterator iter = data.iterator(); iter.hasNext() && tok.hasMoreTokens();)
					{
						String value = tok.nextToken();
		...
		void saveValues(String region, String raceName)
		{
			...
			{
				Object[] element = (Object[]) iter.next();
				Globals.getBioSet().addToUserMap(region, raceName, "BASEAGE:" + String.valueOf(element[1]));
				Globals.getBioSet().addToUserMap(region, raceName, "AGEDIEROLL:" + String.valueOf(element[2]));
				Globals.getBioSet().addToUserMap(region, raceName, "MAXAGE:" + String.valueOf(element[3]));
			}
		}
