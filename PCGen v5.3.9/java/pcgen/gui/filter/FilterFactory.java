...
public final class FilterFactory implements FilterConstants
{
	...
	public static void registerAllSpellFilters(FilterAdapterPanel fap)
	{
		if (spellFilters.size() == 0)
		{
			...
			for (Iterator it = SystemCollections.getUnmodifiableSchoolsList().iterator(); it.hasNext();)
			{
...
abstract class AlignmentFilter extends AbstractPObjectFilter
{
	...
	protected boolean passesAlignmentPrereqs(PObject pObject, int alignment)
	{
		...
		if ("allowed".equals(mode))
		{
			...
			for (int i = 1; i < SystemCollections.getUnmodifiableAlignmentList().size(); i++)
			{
		...
		for (int it = 0; it < pObject.getPreReqCount(); it++)
		{
			...
			{
				prealign = new StringBuffer(tmp.substring(9));
				break;
			}
		}
...
final class DeityAlignmentFilter extends AlignmentFilter
{
	...
	{
		super(PropertyFactory.getString("in_alignLabel"), SystemCollections.getLongAlignmentAtIndex(alignment));
	...
	public boolean accept(PObject pObject)
	{
		...
		if (pObject instanceof Deity)
		{
			...
			if (deityAlign.equals(SystemCollections.getShortAlignmentAtIndex(alignment)) || deityAlign.equals(SystemCollections.getLongAlignmentAtIndex(alignment)))
			{
				return true;
			}
			return false;
	...
	extends AlignmentFilter
{
	...
	{
		return super.getName() + " (" + SystemCollections.getLongAlignmentAtIndex(Globals.getCurrentPC().getAlignment()) + ")";
	}
...
final class PCSizeFilter extends AbstractPObjectFilter
{
	...
	public String getName()
	{
		return super.getName() + " (" + SystemCollections.getSizeAdjustmentAtIndex(Globals.getCurrentPC().sizeInt()).getName() + ")";
	}
	...
	public boolean accept(PObject pObject)
	{
		...
		if (pObject instanceof Equipment)
		{
			return ((Equipment) pObject).getSize().toUpperCase().equals(SystemCollections.getSizeAdjustmentAtIndex(Globals.getCurrentPC().sizeInt()).getAbbreviation());
		}
...
final class SizeFilter extends AbstractPObjectFilter
{
	...
	public boolean accept(PObject pObject)
	{
		...
		if (pObject instanceof Equipment)
		{
			...
			return aEquipSize.equals(SystemCollections.getSizeAdjustmentAtIndex(size).getAbbreviation()) || aEquipSize.equals(SystemCollections.getSizeAdjustmentAtIndex(size).getName());
		}
		else if (pObject instanceof Race)
		{
			final String aRaceSize = ((Race) pObject).getSize();
			return aRaceSize.equals(SystemCollections.getSizeAdjustmentAtIndex(size).getAbbreviation()) || aRaceSize.equals(SystemCollections.getSizeAdjustmentAtIndex(size).getName());
		}
