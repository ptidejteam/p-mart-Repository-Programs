	...
	private class FeatPopupMenu extends JPopupMenu
	{
		...
		private FeatPopupMenu(JTreeTable treeTable)
		{
			if ((this.treeTable = treeTable) == availableTable)
			{
				...
				}, "infoFeats.addFeat", (char) 0, "control EQUALS", "Add Feat", "Add16.gif", true));
			...
			else // selectedTable
			{
				...
				}, "infoFeats.removeFeat", (char) 0, "control MINUS", "Remove Feat", "Remove16.gif", true));
			}
		}
							...
							for (int i = 0; i < rootAsPObjectNode.getChildCount(); ++i)
							{
								if (aFeat.isType(rootAsPObjectNode.getChild(i).toString()))
								{
									...
									if (!available)
									{
										...
										{
											aFN.setItem(bFeat);
											aFN.setIsValid(bFeat.passesPreReqToGain());
										}
									}
									if (aFN.getItem() == null)
									{
					...
					while (fList.size() > 0 && loopmax-- > 0)
					{
						for (int i = 0; i < fList.size(); ++i)
						{
							...
							for (int j = 0; j < rootAsPObjectNode.getChildCount(); ++j)
							{
								...
								placed = placedThisFeatInThisTree(aFeat, po, Feat.preReqStringsForList(preReqList), 0, available);
								if (placed > 0)
								{
									break;
								}
