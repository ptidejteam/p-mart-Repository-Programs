...
public final class BrowserLauncher
{
	...
	static
	{
		...
		if (osName.startsWith("Mac OS"))
		{
			...
			try
			{
				...
				{
					// Assume that all 2.x
					// versions of MRJ work the
					// same.  MRJ 2.1 actually
					// works via Runtime.exec()
					// and 2.2 supports that but
					// has an openURL() method as
					// well that we currently
					// ignore.
					jvm = MRJ_2_1;
				}
				else if (version == 3.0)
				...
				{
					// Assume that all 3.1 and
					// later versions of MRJ work
					// the same.
					jvm = MRJ_3_1;
				}
				else
	...
	{
		GuiFacade.showMessageDialog(null, "Please set your browser in the Preferences", "PCGen", GuiFacade.ERROR_MESSAGE);
	}
		...
		switch (jvm)
		{
			case MRJ_2_0:
				try
				{
					...
					// Don't set browser =
					// appleEvent because then the
					// next time we call
					// locateBrowser(), we'll get
					// the same AppleEvent, to
					// which we'll already have
					// added the relevant
					// parameter. Instead,
					// regenerate the AppleEvent
					// every time.	There's
					// probably a way to do this
					// better; if any has any
	...
	public static void openURL(String url) throws IOException
	{
		...
		{
			String browserPath = SettingsHandler.getBrowserPath();
