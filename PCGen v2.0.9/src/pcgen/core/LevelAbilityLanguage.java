...
public class LevelAbilityLanguage extends LevelAbility
{

	LevelAbilityLanguage(PCClass aOwnerClass, int aLevel, String aList)
	{
	...
	{
		ArrayList aList = super.getChoicesList(bString.substring (9));
	...
	{
		if (!Globals.currentPC.languagesList().contains(aChoice))
	...
	void processChoice(ArrayList aArrayList, List selectedList)
	{
		for (int index = 0; index < selectedList.size(); index++)
			Globals.currentPC.addLanguage(selectedList.get(index).toString(), false);
	}
}
