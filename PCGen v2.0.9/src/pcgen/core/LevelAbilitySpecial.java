...
public class LevelAbilitySpecial extends LevelAbility
{
	  ...
	  PlayerCharacter aPC = Globals.currentPC;
 		if (aChoice.equals("FEATLIST"))
 		{
 			for (int index = 0; index < Globals.featList.size(); index++)
 			{
 				...
 				if (aPC.qualifiesForFeat(aFeat.keyName()) &&
 					(!aPC.hasFeat(aFeat.keyName()) ||
 					aFeat.multiples()))
 					aArrayList.add(Globals.featList.get(index).toString());
 			}
 		}
 		...
 		{
 			Feat aFeat = Globals.getFeatNamed(aChoice.substring(5));
 			if (aPC.qualifiesForFeat(aFeat.keyName()) &&
 				(!aPC.hasFeat(aFeat.keyName()) ||
 				aFeat.multiples()))
 				aArrayList.add(aFeat.keyName());
 		}
  ...
  void processChoice (ArrayList aArrayList, List selectedList)
  {
 		...
 		int index = 0;
 		for (index = 0; index < Globals.featList.size(); index++)
 			if (Globals.featList.get(index).toString().equals((String)selectedList.get(0).toString()))
 			{
 				aPC.modFeat(Globals.featList.get(index).toString(), true, false);
 				aPC.setFeats(aPC.feats() + 1);
 				break;
 			}
 		if (index >= Globals.featList.size())
 		{
 			aPC.specialAbilityList().add((String)selectedList.get(0).toString());
