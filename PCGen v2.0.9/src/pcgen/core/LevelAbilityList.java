...
public class LevelAbilityList extends LevelAbility
{
   ...
   */
	...
	{
 		StringTokenizer cTok = new StringTokenizer(aChoice, "[]", false);
 		aArrayList.add(cTok.nextToken());
 		while (cTok.hasMoreTokens())
 		{
 			String bTokString = cTok.nextToken();
 			if (bTokString.startsWith("CHOOSE:"))
 				aChoiceList.add(cnt + "|" + bTokString);
 			if (bTokString.startsWith("BONUS:"))
 				aBonusList.add(cnt + "|" + bTokString);
 		}
 		cnt++;
	}
  ...
  void processChoice (ArrayList aArrayList, List selectedList)
  {
 		for (int index = 0; index < selectedList.size(); index++)
 		{
 			...
 			for (Iterator e = aChoiceList.iterator(); e.hasNext();)
 			{
 				...
 				StringTokenizer bTok = new StringTokenizer(theChoice, "|", false);
 				if (theChoice.startsWith(cnt + "|"))
 				{
