...
public class Party extends PObject
{
  ...
  {
    Globals.pcList.trimToSize();
    FileInputStream aStream = null;
    PlayerCharacter holdPC = Globals.currentPC;
    try
    {
            ...
            else
            {
              if (bTok.hasMoreTokens() || flag)
              {
                ...
                if (aString.startsWith("FOR."))
                {
                  int x=0;
                  int j=0;
                  PStringTokenizer pTok = new PStringTokenizer(aString.substring(4),",","\\\\","\\\\");
                  ...
                  while (pTok.hasMoreTokens())
                  {
                    ...
                    switch(j++)
                    {
                    case 0: cMin = pcgen.util.Delta.decode(bString); break;
                    case 1: cMax = pcgen.util.Delta.decode(bString); break;
                    case 2: cStep = pcgen.util.Delta.decode(bString); break;
                    case 3: cString = bString; break;
                    case 4: cStartLineString = bString; break;
                    case 5: cEndLineString = bString; break;
                    case 6: existsOnly = !bString.equals("0"); break;
                    }
                  }
                  if (cMax.intValue()>=Globals.pcList.size() && existsOnly==true)
                    cMax = new Integer(Globals.pcList.size());
                  for(int k=cMin.intValue(); k<cMax.intValue(); k++)
                  {
                    ...
                    while(dString.length()>0)
                    {
                      ...
                      {
                        charNum = k;
                        if (charNum>=0 && charNum<Globals.pcList.size())
                        {
                          PlayerCharacter aPC = (PlayerCharacter)Globals.pcList.get(charNum);
