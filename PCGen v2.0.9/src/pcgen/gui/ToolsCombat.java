...
public class ToolsCombat extends JPanel
{
  ...
  private final String[] operations = {"Add", "Subtract"};
  ...
  private boolean inSWMode = Globals.starWarsMode;  //force a refresh the first time through refreshTable()
  ...
  {
    if (inSWMode != Globals.starWarsMode)
    {
      inSWMode = Globals.starWarsMode;
      initDamage();
      initAttributes();
    }
    setPreferredWidths();
  ...
  private void setPreferredWidths()
  {
    ...
    d_CombatTable.getColumnModel().getColumn(3).setPreferredWidth(50);   //size
    d_CombatTable.getColumnModel().getColumn(4).setPreferredWidth(85);   //HP or VP (current/max)
    d_CombatTable.getColumnModel().getColumn(5).setPreferredWidth(85);   //Flat-Footed AC or WP (current/max)
    d_CombatTable.getColumnModel().getColumn(6).setPreferredWidth(75);   //AC or Defense
    d_CombatTable.getColumnModel().getColumn(7).setPreferredWidth(75);   //Initiative Roll
    ...
    d_CombatTable.getColumnModel().getColumn(13).setPreferredWidth(75);  //Move
    ...
    d_RollInit.addActionListener(new ActionListener()
      {
        ...
        {
          d_InitRolls.clear();
          for (int count=0; count < Globals.pcList.size(); count++)
          {
            PlayerCharacter tempChar = (PlayerCharacter)Globals.pcList.get(count);
  ...
  {
    private final String[] dndColNameList = {"Name", "Class(es)", "Race", "Size", "HP (Cur/Max)", "Flat", "AC", "Init Roll", "Melee BAB", "Ranged BAB", "Fort Save", "Ref Save", "Will Save", "Move"};
    private final String[] swColNameList = {"Name", "Class(es)", "Race", "Size", "VP (Cur/Max)", "WP (Cur/Max)", "Defense", "InitRoll", "Melee BAB", "Ranged BAB", "Fort Save", "Ref Save", "Will Save", "Move"};
    private int lastRowCount = 0;

    public int getRowCount()
    {
      if (Globals.pcList.size() != lastRowCount)
      {
        lastRowCount = Globals.pcList.size();
        fireTableChanged(new TableModelEvent(this));
      }
      return Globals.pcList.size();
    }

    public int getColumnCount()
    {
      return Globals.starWarsMode ? swColNameList.length : dndColNameList.length;
    }

    public Class getColumnClass(int columnIndex)
    {
      if (columnIndex < 5 || ((columnIndex == 5) && Globals.starWarsMode) || columnIndex == 13)
      {
        return String.class;
      } else {
    ...
    {
      return Globals.starWarsMode ? swColNameList[columnIndex] : dndColNameList[columnIndex];
    }

    public Object getValueAt(int row, int column)
    {
      PlayerCharacter curChar = (PlayerCharacter)Globals.pcList.get(row);
      boolean isIncomplete = curChar.classList().isEmpty();  //if the character has no levels then it isn't complete enough for most stats
      switch (column)
      {
        case 0: //name
          return curChar.name();
        case 1: //class(es)
          ...
          Iterator classIter = curChar.classList().iterator();
          while (classIter.hasNext())
          {
            PCClass theClass = (PCClass)classIter.next();
            if (!theClass.name().startsWith("Domain"))
            {
        ...
        case 4: //HP or VP
          ...
          if (d_HitPoints.size() != Globals.pcList.size())
          {
            initDamage();
          }
          String hpDisplay = d_HitPoints.get(row) + "/" + curChar.hitPoints();
          return hpDisplay;
        case 5: //Flat-Footed AC or WP
          if (isIncomplete)
          {
            if (Globals.starWarsMode)
            {
          ...
          if (Globals.starWarsMode)
          {
            if (d_WoundPoints.size() != Globals.pcList.size())
            {
              initDamage();
            }
        ...
        case 6: //AC or Defense
          ...
          return Globals.starWarsMode ? curChar.defense() : new Integer(curChar.totalAC());
        case 7: //Initiative
          if (isIncomplete || d_InitRolls.isEmpty() || d_InitRolls.size() != Globals.pcList.size())
          {
            return new Integer(0);
          }  //if the character has no levels then don't bother getting data
        ...
        case 8: //Melee BAB
          ...
          return new Integer(curChar.baseAttackBonus(1));
        case 9: //Ranged BAB
          if (isIncomplete)
          {
            return new Integer(0);
          }  //if the character has no levels then don't bother getting data
          return new Integer(curChar.baseAttackBonus(2));
        case 10: //Fort Save
          if (isIncomplete)
          {
            return new Integer(0);
          }  //if the character has no levels then don't bother getting data
          return new Integer(curChar.getBonus(1, true));
        case 11: //Ref Save
          if (isIncomplete)
          {
            return new Integer(0);
          }  //if the character has no levels then don't bother getting data
          return new Integer(curChar.getBonus(2, true));
        case 12: //Will Save
          if (isIncomplete)
          {
            return new Integer(0);
          }  //if the character has no levels then don't bother getting data
          return new Integer(curChar.getBonus(3, true));
        case 13: //Move
          if (isIncomplete)
          {
            return "";
          }  //if the character has no levels then don't bother getting data
          String movelabel = curChar.race().movementType(0) + " " + curChar.movement(0) + "'";
          for(int x = 1; x < curChar.race().movements().length; x++)
            movelabel += ", " + curChar.race().movementType(x) + " " + curChar.movement(x) + "'";
          return movelabel;
      }
      return null;
    }
  ...
  private class ModifyListener implements ActionListener
  {
    public void actionPerformed(ActionEvent evt)
    {
      ...
      {
        int mod = d_Operation.getSelectedIndex() == 0 ? 1 : -1;
        ...
        if (target.equals("Init") && d_InitRolls.size() == Globals.pcList.size())
        {
          Integer oldValue = (Integer)d_InitRolls.get(row);
          newValue = oldValue.intValue() + modvalue;
          d_InitRolls.set(row, new Integer(newValue));
        } else if (target.equals("WP") && d_WoundPoints.size() == Globals.pcList.size()) {
          Integer oldValue = (Integer)d_WoundPoints.get(row);
          newValue = oldValue.intValue() + modvalue;
          d_WoundPoints.set(row, new Integer(newValue));
        } else if ((target.equals("HP") || target.equals("VP")) && d_HitPoints.size() == Globals.pcList.size()){  //HP or VP
