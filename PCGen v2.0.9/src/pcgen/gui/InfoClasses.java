...
public class InfoClasses extends JPanel
{
  ...
  private ArrayList displayClasses = Globals.classList;
  ...
  private void initComponents()
  {
    ...
    int[] cols = {0,1,2,3,4,5,6,8,9,10};
    allClassesTable.calcColumnWidths(cols);
    ...
    levelCap.setSelected(Globals.ignoreLevelCap);
    ...
    levelCap.addActionListener(new ActionListener()
    {
      public void actionPerformed(ActionEvent evt)
      {
        ...
        Globals.ignoreLevelCap = levelCap.isSelected();
      }
    });
    gridBagConstraints3 = new GridBagConstraints();
  ...
  {
    PCGen_Frame1.statusBar.setText("If the Max HP % is set to 0, random HP's will be generated, " +
      "otherwise the specified % of max HP will be used.");
  }

  private void tHpFocusLost(FocusEvent evt)
  {
    PlayerCharacter aPC = Globals.currentPC;
    if (aPC.totalLevels() < 1)
      return;
    aPC.setDirty(true);
    int i = tHp.getValue() - aPC.getTotalBonusTo("HP", "CURRENTMAX", true);
    int h = i % aPC.totalLevels();
    int k = 0;
    for (Iterator e1 = aPC.classList().iterator(); e1.hasNext();)
    {
      ...
      if (aClass.level().intValue() <= 0)
        continue;
      Integer[] hp = new Integer[aClass.hitPointList().length];
      for (int l = 0; l < hp.length; l++)
      {
        hp[l] = new Integer(i / aPC.totalLevels());
        if (k == 0)
        {
          k = 1;
          hp[l] = new Integer((i / aPC.totalLevels()) + h);
        }
      }
      aClass.setHitPointList(hp);
    }
  }

  private void tMaxHPPctFocusLost(FocusEvent evt)
  {
    Globals.hpPct = tMaxHPPct.getValue();
  }
  ...
  private void experienceFocusLost(FocusEvent evt)
  {
    ...
    PCClass aClass = (PCClass)Globals.currentPC.classList().get(0);
    Integer min = aClass.minExpForLevel(Globals.currentPC.totalLevels() + Globals.currentPC.race().LevelAdjustment());
    Integer anInt = new Integer(experience.getText());
    if (anInt.intValue() < min.intValue())
    {
      JOptionPane.showMessageDialog(null, "To be your level (" + new Integer(Globals.currentPC.totalLevels()).toString() +
        ") you must have at least " + min + " experience");
      experience.setText(min.toString());
      Globals.currentPC.setExperience(min);
      return;
    }
    min = aClass.minExpForLevel(Globals.currentPC.totalLevels() + 1 + Globals.currentPC.race().LevelAdjustment());
    if (anInt.intValue() >= min.intValue())
      JOptionPane.showMessageDialog(null, "You can advance a level with that much experience!");
    Globals.currentPC.setExperience(anInt);
  }
  ...
  private void buttonMinusActionListener(ActionEvent evt)
  {
    ...
    if (theClass != null)
    {
      Globals.currentPC.incrementClassLevel(-1, theClass);
    }
    pcChanged();
  }
  ...
  private void buttonPlusActionPerformed(ActionEvent evt)
  {
    ...
    {
      PCClass aClass = Globals.currentPC.getClassNamed(theClass.name());
      if (aClass == null || Globals.ignoreLevelCap || (!Globals.ignoreLevelCap && aClass.level().intValue() < aClass.maxLevel()))
        Globals.currentPC.incrementClassLevel(1, theClass);
      else
  ...
  public void pcChanged()
  {
    ...
    if (pc != null)
    {
      ...
      featCount.setText(Integer.toString(pc.feats()));
      skillCount.setText(Integer.toString(pc.skillPoints()));
      ...
      experience.setValue(pc.experience().intValue());
   ...
   *  It pulls its data straight from the Globals.classList Vector.
   */
  public class AllClassesTableModel extends AbstractTableModel
  {
    ...
    {
      prevGlobalClassCount = Globals.classList.size();
      displayClasses = new ArrayList(prevGlobalClassCount);

      switch (filterID)
      {
        case FILTER_ALL: // All
          for (Iterator it = Globals.classList.iterator(); it.hasNext();)
          {
    ...
    {
      if (prevGlobalClassCount != Globals.classList.size())
        updateFilter();
    ...
    {
      if (Globals.starWarsMode && columnIndex > 9)
        columnIndex += 2;
      else if (!Globals.starWarsMode && columnIndex > 7)
        columnIndex += 2;
    ...
    public Object getValueAt(int rowIndex, int columnIndex)
    {
      if (displayClasses != null)
      {
        PCClass c = (PCClass)displayClasses.get(rowIndex),
          pc = Globals.currentPC.getClassNamed(c.toString());
        if (pc != null)
        {
          c = pc;
        }
        ...
        switch (columnIndex)
        {
          ...
          case 2:
            return c.attackBonusType();

          case 3:
            return "1D" + c.hitDie();

          case 4:
            return c.fortitudeCheckType();

          case 5:
            return c.reflexCheckType();

          case 6:
            return c.willCheckType();

          case 7:
            return c.prestigeString();

          case 8:
            return (Globals.starWarsMode) ? c.defenseString() : c.spellType();

          case 9:
            return (Globals.starWarsMode) ? c.reputationString() : c.spellBaseStat();
  ...
  public class PCClassesTableModel extends AbstractTableModel
  {
    ...
    public int getRowCount()
    {
      if ((Globals.currentPC != null) && (Globals.currentPC.classList() != null))
      {
    ...
    {
      ArrayList result = (ArrayList)Globals.currentPC.classList().clone();
