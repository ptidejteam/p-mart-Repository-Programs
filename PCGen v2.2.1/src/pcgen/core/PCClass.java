		...
		for (Iterator e = bonusList.iterator(); e.hasNext();)
		{
			...
			while (bTok.hasMoreTokens())
			{
					...
					aName.equals(mname))
				{
					...
					{
						int j = aPC.getVariableValue(aString, "", "").intValue();
						i += j;
						aPC.SetBonusStackFor(j, bonusType);
					}
				}
			}
		}
...
//		int dieSizes[] = { 4, 6, 8, 10, 12 }; // Hit dice seem to be only valid in these ranges.
	...
	{
		return (specialtyList.size()>0 && getCastForLevel(pcLevel, spellLevel, bookName)>0)? "+1" : "";
	}
...
public String getSPForLevelString(int pcLevel, int spellLevel, String bookName)
  ...
  /********************************/
	...
	public int getCastForLevel(int pcLevel, int spellLevel, String bookName)
	{
		...
		if (stat < 10 + spellLevel && index != -2 && !name.startsWith("Domain"))
			return total;
		int adj = 0;
		...
		for (Iterator e1 = aPC.getClassList().iterator(); e1.hasNext();)
		{
			...
			if (getName().startsWith("Domain") && this!= aClass /*&& aClass.getBonusTo("DOMAIN","NUMBER",aClass.getLevel().intValue())>0*/) {
				spells = Math.max(spells,aClass.getCastForLevel(aClass.getLevel().intValue(), spellLevel));
			}
		}
		if (spells > 0)
			return knownSpellsFromSpecialty;
						...
						if (className.equals("Domain"))
						{
							...
							for (Iterator e2 = aPC.getDomainList().iterator(); e2.hasNext();)
							{
								...
								if (aSpell.levelForClass(aDomain.getName()) == i)
								{
									if (aString.length() > 0)
										aString = aString + ",";
									aString = aString + aDomain.getName() + ","+i;
								}
							}
						}
	...
	public void removeSpecialAbilityList(int aLevel, String aList)
	{
		Integer anInt = new Integer(aLevel);
		String aString = anInt.toString() + ":" + aList;
