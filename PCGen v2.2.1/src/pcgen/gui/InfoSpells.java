...
class InfoSpells extends JPanel
{
	...
	private void populateSpellClassComboBox()
	{
		...
		if (Globals.getCurrentPC() != null)
		{
			for (Iterator e = Globals.getCurrentPC().getClassList().iterator(); e.hasNext();)
			{
				...
				if (!aClass.getSpellType().equals("None") && !aClass.getName().equals("Domain"))
				{
					spellClassComboBox.addItem(aClass.getName());
				}
			}
			if (Globals.getCurrentPC().getDomainList().size() > 0)
			{
				String aString = Globals.getCurrentPC().domainClassName();
	...
	private void populateSpellBookComboBox()
	{
		...
		int k = 0;
		for (Iterator i = Globals.getCurrentPC().getSpellBooks().iterator(); i.hasNext();)
		{
	...
	private void populateTables()
	{
		...
		if (i >= 0 && i < spellClassComboBox.getItemCount() && j >= 0 && j < 10)
		{
			...
			if (spellBookComboBox.getSelectedItem().toString().equals("Known Spells"))
			{
				for (e = Globals.getSpellMap().values().iterator(); e.hasNext();)
				{
					...
					while (aTok.hasMoreTokens() && !addIt)
					{
						...
						addIt = aSpell.levelForClass(aString, aClass.getName()) == spellLevelComboBox.getSelectedIndex();
					}
					if (addIt)
					{
			...
			else
			{
				...
				{
					aSpell = (Spell)e.next();
					if (aSpell.levelForClass(className, aClass.getName()) == spellLevelComboBox.getSelectedIndex())
					{
						vectorA.addElement(aSpell);
					}
				}
			...
			if (aClass != null)
			{
				...
				int k = aClass.getKnownForLevel(aClass.getLevel().intValue(), j);
				anInt = new Integer(k);
				knownText.setText(anInt.toString());
				castableText.setText(String.valueOf(aClass.getCastForLevel(aClass.getLevel().intValue(), j, spellBookComboBox.getSelectedItem().toString()))+
					aClass.getBonusCastForLevelString(aClass.getLevel().intValue(), j, spellBookComboBox.getSelectedItem().toString()));
				...
				for (e = aClass.spellList().iterator(); e.hasNext();)
				{
					...
					if (aSpell.levelForClass(className, aClass.getName()) == m && aSpell.getSpellBooks().contains(bName))
					{
			...
			{
				aClass = Globals.getCurrentPC().getClassNamed("Domain");
				knownText.setText("0");
				int k = aClass.getCastForLevel(aClass.getLevel().intValue(), j);
				castableText.setText(String.valueOf(k));
			}
		}
	...
	private int getLevelsForSelectedFeats()
	{
		...
		while (i-- > 0)
		{
			Feat aFeat = Globals.getCurrentPC().getFeatNamed((String)featTable.getValueAt(cList[i], 0));
	...
	private void addSpellButton_clicked(ActionEvent evt)
	{
		...
		PCClass aClass = Globals.getCurrentPC().getClassNamed(className); //Null means domain.
		if (aClass == null || aClass.memorizesSpells() || (!aClass.memorizesSpells() && (tableB.getRowCount() < Integer.parseInt(knownText.getText()))))
		{
			...
			while (i-- > 0)
			{
				String aString = Globals.getCurrentPC().addSpell(spellClassComboBox.getSelectedItem().toString(), spellLevelComboBox.getSelectedIndex(),
	...
	private void delSpellButton_clicked(ActionEvent evt)
	{
		...
		while (i-- > 0)
		{
			String aString = Globals.getCurrentPC().delSpell(spellClassComboBox.getSelectedItem().toString(), spellLevelComboBox.getSelectedIndex(),
	...
	private void addBookButton_clicked(ActionEvent evt)
	{
		...
		if (Globals.getCurrentPC().addSpellBook(aString))
		{
			populateSpellBookComboBox();
			populateTables();
		}
	...
	private void delBookButton_clicked(ActionEvent evt)
	{
		Globals.getCurrentPC().setDirty(true);
		String aString = spellBookNameText.getText();
		if (Globals.getCurrentPC().delSpellBook(aString))
		{
			populateSpellBookComboBox();
			populateTables();
		}
