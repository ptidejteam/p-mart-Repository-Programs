	...
	public void pcChanged()
	{
		...
		if (aPC != null)
		{
			...
			for(int i=0;i<Globals.getCurrentPC().getDomainList().size();i++)
			{
				...
				if (aVector != null && (aVector.size()<=i || (aVector.elementAt(i) != null && !aVector.elementAt(i).equals(aDomain)))) {
			...
			if (aVector != null && aPC.getDomainMax() < aVector.size())
			{
				for (int i = aVector.size() - 1; i >= aPC.getDomainMax(); i--)
					aVector.removeElementAt(i);
			}
			else if (aVector != null && aPC.getDomainMax() > aVector.size())
			{
				...
				switch (column)
				{
					...
					case 4:
						...
						for (int i = 0; i < aDeity.getAlignments().length(); i++)
						{
							...
							int x = Integer.parseInt(aDeity.getAlignments().substring(i, i + 1));
							aString = aString + Globals.s_ALIGNSHORT[x];
						}
						retVal = aString;
						break;
					case 5:
			...
			switch (column)
			{
				case 0:
					...
					if (aString != null)
					{
						...
						if (!aDomain.qualifiesForDomain())
						{
							JOptionPane.showMessageDialog(null, "You do not qualify for this domain.", "PCGen", JOptionPane.INFORMATION_MESSAGE);
							return;
						}
						if (aDomain != null)
						{
