...
class InfoTraits extends JPanel
{
	...
	JTextField hairLengthText = new JTextField(Globals.getCurrentPC().getHairLength());
   ...
   * randomly generates traits/colors for certain fields. */
	private void randomTraits_click()
	{
		...
		int roll = 0;
		int colorNum = Globals.getColorList().size();
		int traitNum = Globals.getTraitList().size();
		Iterator e1 = null;
		String aString = null;
		if (eyeColorText.getText().equals(""))
		{
			roll = roller.nextInt();
			if (roll < 0) roll = -roll;
			roll = roll % Globals.getColorList().size() + 1;
			for (e1 = Globals.getColorList().iterator(); e1.hasNext();)
			{
		...
		if (personality1Text.getText().equals(""))
		{
			roll = roller.nextInt();
			if (roll < 0) roll = -roll;
			roll = roll % Globals.getTraitList().size() + 1;
			for (e1 = Globals.getTraitList().iterator(); e1.hasNext();)
			{
				aString = (String)e1.next();
				roll--;
				if (roll == 0)
