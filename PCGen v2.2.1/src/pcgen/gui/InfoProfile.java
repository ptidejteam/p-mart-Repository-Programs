...
class InfoProfile extends JPanel
{
	...
	public void startListeners()
	{
		...
		if (ageAdapter == null)
		{
			ageAdapter = new FocusAdapter()
			{
				...
				{
					Globals.getCurrentPC().setDirty(true);
					Globals.getCurrentPC().setAge(Delta.parseInt("0"+ageText.getText()));
				}
			};
		}
		...
		if (htAdapter == null)
		{
			htAdapter = new FocusAdapter()
			{
				...
				{
					Globals.getCurrentPC().setDirty(true);
					Globals.getCurrentPC().setHeight(Delta.parseInt("0"+htText.getText()));
				}
			};
		}
		...
		if (wtAdapter == null)
		{
			wtAdapter = new FocusAdapter()
			{
				...
				{
					Globals.getCurrentPC().setDirty(true);
					Globals.getCurrentPC().setWeight(Delta.parseInt("0"+wtText.getText()));
				}
			};
		}
	...
	public void refreshDisplay()
	{
		...
		{
			String movelabel = aPC.getRace().getMovementType(0) + " " + aPC.movement(0) + " feet";

			final Integer[] movements = aPC.getRace().getMovements();
			if(movements != null)
			{
				for (int x = 1; x < movements.length; x++)
					movelabel += ", " + aPC.getRace().getMovementType(x) + " " + aPC.movement(x) + " feet";
			} else {
