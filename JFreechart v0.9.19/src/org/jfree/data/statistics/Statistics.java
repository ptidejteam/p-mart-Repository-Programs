 ...
 *
 * $Id: Statistics.java,v 1.1 2007/10/10 19:34:50 vauchers Exp $
 *
...
public abstract class Statistics {
    ...
    /**
     * Calculates the median for a sublist within a list of values (Number objects) that are 
    ...
    public static double calculateMedian(final List values, final int start, final int end,
        ...
        if (copyAndSort) {
            ...
            for (int i = start; i <= end; i++) {
                System.out.println("About to set " + (i - start));
                working.set(i - start, values.get(i));  
            }
            Collections.sort(working); 
            result = calculateMedian(working, 0, working.size() - 1, false);
        }
        else {
