...
class EQFrame extends JFrame
{
	...
	static
	{
		...
		try
		{
			eqFrameProperties = ResourceBundle.getBundle("pcgen/gui/properities/LanguageBundle", currentLocale);
	...
	void handleQuit(boolean bCancelled)
	{
		if (!bCancelled)
		{
			Globals.setCustomizerLeftUpperCorner(getLocationOnScreen());
			Globals.setCustomizerDimension(getSize());
	...
	public EQFrame()
	{
		...
		final Dimension customizerDim = Globals.getCustomizerDimension();
		final Point customizerLoc = Globals.getCustomizerLeftUpperCorner();
		int x = -11;
		int y = -11;
