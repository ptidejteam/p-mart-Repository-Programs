...
//	private int PcgWriteVersion = 2;	// Version of file we will write
	...
	public void setSubRegion(String newSubRegion)
	{
	...
	public int sizeInt()
	{
		...
		if (race != null)
		{
			...
			for (Iterator e = getTemplateList().iterator(); e.hasNext();)
			{
				...
				if (template != null)
					iSize = Globals.sizeInt(template.getTemplateSize());
			}

			// Now check and see if a class has modiefied
			// the size of the character
			// with something like BONUS:SIZEMOD|NUMBER|+1
			int sizeMod = getTotalBonusTo("SIZEMOD", "NUMBER", true);
			iSize += sizeMod;
	...
	private int getModifierForSizeIncrease(String modName)
	{
		...
		for (int x = 0; x < race.sizesAdvanced(totalHitDice()); x++)
		{
			if (iSize < Constants.s_SIZESHORT.length)
			{
				mod += (int)Globals.sizeAdjustmentMultiplier(Constants.s_SIZESHORT[iSize], aList, modName);
				iSize += 1;
			}
		}
		return mod;
...
//		acmod += getTotalBonusTo("COMBAT", "AC", true);  now included in equipmentMod!
	...
	public void setAlignment(int index, boolean bLoading, boolean bForce)
	{
		...
		else
		{
			if ((bLoading) && (index != 9))
			{
				JOptionPane.showMessageDialog(null, "Invalid alignment. Setting to <none selected>", Constants.s_APPNAME, JOptionPane.INFORMATION_MESSAGE);
				alignment = 9;
			}
	...
	private void setMoveRates(PCTemplate inTmpl)
	{
		...
		if (movements != null)
		{
			...
			{
				getRace().setMoveRates(movementTypes[i], movements[i].intValue(), inTmpl.getMoveRatesFlag());
			}
		}
	}
 	...
 	public void doVision(String tmplVision, int flag)
	{
		...
		if (visionOutput.equals("") || visionOutput.equals(Constants.s_NONE)) visionOutput = "Normal";
	}
	...
	public void addTemplate(PCTemplate inTmpl, boolean addGold)
	{
		...
		subregion = inTmpl.getSubRegion();
		final String favoredClass = inTmpl.getFavoredClass();
	...
	public void removeTemplate(PCTemplate inTmpl)
	{
		if (inTmpl == null)
			return;
		if (subRace.equals(inTmpl.getSubRace())) subRace = Constants.s_NONE;
		if (region.equals(inTmpl.getRegion())) region = Constants.s_NONE;
		if (subregion.equals(inTmpl.getSubRegion())) subregion = Constants.s_NONE;
		final String favoredClass = inTmpl.getFavoredClass();
		if (favoredClass.length() != 0)
		{
			favoredClasses.remove(favoredClass);
		}
	...
	public void incrementClassLevel(int mod, PCClass aClass, boolean bSilent)
	{
		if (mod > 0)
		{
			...
			{
				JOptionPane.showMessageDialog(null, "Cannot increase Monster Hit Dice for this character beyond " + race.maxHitDiceAdvancement() + ". This characters current number of Monster Hit Dice is " + totalHitDice(), Constants.s_APPNAME, JOptionPane.INFORMATION_MESSAGE);
				return;
			}
		}
		...
		if (race != null)
		{
			...
			while (aTok.hasMoreTokens())
			{
				...
				else
				{
					...
					{
						JOptionPane.showMessageDialog(null, "Removing unknown feat: " + aString, Constants.s_APPNAME, JOptionPane.INFORMATION_MESSAGE);
					}
				}
			}
...
//						}
				...
				else
				{
					...
					{
						JOptionPane.showMessageDialog(null, "Adding unknown feat: " + aString, Constants.s_APPNAME, JOptionPane.INFORMATION_MESSAGE);
					}
				}
			}
			getAutoLanguages();
...
//					break;
	...
	public SortedSet getAutoWeaponProfs()
	{
		...
		for (e = getTemplateList().iterator(); e.hasNext();)
		{
...
System.out.println("bonusWeaponChoices="+bonusWeaponChoices);
		for (Iterator e = classList.iterator(); e.hasNext();)
		{
			final PCClass aClass = (PCClass)e.next();
	...
	public int handsFull()
	{
		...
		for (e = aArrayList.iterator(); e.hasNext();)
		{
			...
			if (eq.isEquipped())
			{
				...
				if (wp == null)
				{
					JOptionPane.showMessageDialog(null, "No entry in weapons.lst for " + eq.profName() + ". Weapons must be in that file to equip them.", Constants.s_APPNAME, JOptionPane.ERROR_MESSAGE);
...
*/
	...
	public boolean canEquip(String typeName)
	{
		...
		else if (typeName.equals("Weapon") || typeName.equals("Shield"))
		{
			...
			{
				JOptionPane.showMessageDialog(null, "Your hands are too full. Check weapons/shields already equipped.", Constants.s_APPNAME, JOptionPane.INFORMATION_MESSAGE);
				return false;
			}
			return true;
		}
		...
		try
		{
			...
			for (Iterator i = bonusMap.keySet().iterator(); i.hasNext();)
			{
				...
				if (aKey.startsWith(prefix))
				{
					...
					if (aKey.endsWith(".REPLACE"))
						aList.add(aKey);
					final int a =  Integer.parseInt((String)bonusMap.get(aKey));
					if (a>-999)
						bonus += a;
				}
			}
			...
			for (Iterator i = aList.iterator(); i.hasNext();)
			{
				...
				if (replaceKey.length() > 7)
				{
					...
					int replaceBonus = -999;
					if (aString !=null && !aString.startsWith("-999"))
					...
					if (aString != null && !aString.equals("-999"))
					{
						if (aBonus>-999)
							aBonus += Integer.parseInt(aString);
						else
					...
					if (aBonus>-999 && replaceBonus>-999)
	...
	public void setBonusStackFor(int bonus, String bonusType)
	{
		...
		else // stacking bonuses
		{
			...
			{
				int a = Integer.parseInt(aKey);
				if (a==-999)
					a=0;
				bonusMap.put(bonusType, String.valueOf(bonus + a));
			}
		}
	}
	...
	public ArrayList vFeatList()
	{
		...
		for (Iterator e = equipmentList.values().iterator(); e.hasNext();)
		{
			...
			if (aE.isEquipped())
			{
				...
				{
					final Feat aFeat = Globals.getFeatNamed(e1.next().toString());
					if (aFeat != null)
						vFeatList.add(aFeat);
				}
			}
...
//			loadForceLine(fa, input);
						...
						for (; ;)
						{
							...
							else
							{
								for (; ;)
								{
									...
									JOptionPane.showMessageDialog(null, "You MUST make a selection", Constants.s_APPNAME, JOptionPane.INFORMATION_MESSAGE);
								}
							}
			...
			if (bFixMade)
			{
				JOptionPane.showMessageDialog(null, "Fixed illegal value in hit points. Current character hit points: " + this.hitPoints() + " not " + oldHp, Constants.s_APPNAME, JOptionPane.ERROR_MESSAGE);
			}
	...
	private int loadPcgVersion(BufferedReader br)
	{
		...
		{
			JOptionPane.showMessageDialog(null, "Could not load campaign data from character file.", Constants.s_APPNAME, JOptionPane.ERROR_MESSAGE);
			e.printStackTrace();
		}
	...
	public int replaceToken(String aString, BufferedWriter output)
	{
		try
		{
				...
				aString.lastIndexOf('<') == -1 && aString.lastIndexOf('>') == -1)
			{
				...
				{
					if (region.equals(Constants.s_NONE))
						canWrite = false;
					return 0;
				}
			...
			else if (aString.equals("RACE"))
			{
				...
				if (subRace.equals(Constants.s_NONE))
				{
					fa.encodeWrite(output, tempRaceName);
				}
				else
				{
					fa.encodeWrite(output, tempRaceName + " (" + subRace + ")");
				}
			}
			...
			else if (aString.equals("WEIGHT"))
			{
				...
				fa.encodeWrite(output, region);
			else if (aString.equals("SUBREGION"))
				fa.encodeWrite(output, subregion);
			...
			else if (aString.startsWith("MOVEMENT"))
			{
				if ((race != null) && !race.equals(Globals.s_EMPTYRACE))
				{
					if (aString.length() > 9)
					{
						...
						for (int x = 0; x < race.getMovements().length; x++)
						{
					...
					{
						fa.encodeWrite(output, race.getMovementType(0) + " " + movement(0) + Globals.getAbbrMovementDisplay());
						for (int x = 1; x < race.getMovements().length; x++)
						{
							fa.encodeWrite(output, ", " + race.getMovementType(x) + " " + movement(x) + Globals.getAbbrMovementDisplay());
						}
					}
				}
			}
			else if (aString.startsWith("MOVE")) /* format : MOVE% prints out movename/move pair. MOVE%.NAME and MOVE%.RATE produce the appropriate parts.*/
			{
				if ((race != null) && !race.equals(Globals.s_EMPTYRACE))
				{
					...
					{
						fa.encodeWrite(output, race.getMovementType(moveIndex) + " " + movement(moveIndex) + Globals.getAbbrMovementDisplay());
					}
				}
			}
		...
		{
			exc.printStackTrace();
			JOptionPane.showMessageDialog(null, "Error replacing " + aString, Constants.s_APPNAME, JOptionPane.ERROR_MESSAGE);
			return 0;
		}
	}
...
//						if (isSecondaryWeapon(eq) && (eq != (Equipment)primaryWeapons.get(0)) && (strMod > 0))
...
//  		aArrayList.addAll(getEquipmentOfType("Shirt", subtype, equipped));
	...
	private void replaceTokenSpellMem(String aString, FileAccess fa, BufferedWriter output)
	{
		...
		if (aClass != null || classNum == -1)
		{
			...
			{
				if (bookNum == -1 || (aClass != null && aClass.getMemorizeSpells() == false))
				{
					bookName = Globals.getDefaultSpellBook();
				...
				if (aClass != null && aClass.getMemorizeSpells() == false)
					bookName = Globals.getDefaultSpellBook();
			}
			if (!bookName.equals(""))
			{
	...
	private void setDeity(String aString)
	{
		...
		if (!bFound)
		{
			if (!aString.equals(Constants.s_NONE))
			{
				...
				JOptionPane.showMessageDialog(null, "Deity not found: " + aString + s_CHECKLOADEDCAMPAIGNS, Constants.s_APPNAME, JOptionPane.ERROR_MESSAGE);
			}
		}
	}
...
// Very bad results if we don't write the invisible templates to the pcg file. :)
	...
	private void loadEquipmentLine(FileAccess fa, BufferedReader input)
	{
		...
		while (aTok.hasMoreTokens())
		{
			...
			else
			{
				...
				{
					JOptionPane.showMessageDialog(null, "Equipment not found: " + aName + " (" + eq.qty() + ")" + s_CHECKLOADEDCAMPAIGNS, Constants.s_APPNAME, JOptionPane.ERROR_MESSAGE);
				}
			}
		}
		...
		while (aTok.hasMoreElements())
		{
			...
			switch (i++)
			{
				case 0:
					...
					{
						JOptionPane.showMessageDialog(null, "Race not found: " + aString + s_CHECKLOADEDCAMPAIGNS, Constants.s_APPNAME, JOptionPane.ERROR_MESSAGE);
					}
					break;
				case 1:
					setAlignment(l, true);
				...
				default:
					...
					else
					{
						...
						if (x == 1)
						{
							JOptionPane.showMessageDialog(null, "Saved race (" + raceName + ") no longer has a HITDICE tag.", Constants.s_APPNAME, JOptionPane.ERROR_MESSAGE);
						}
					}
					break;
			}
				...
				default:
					...
					if (j >= 0)
					{
						...
						else
						{
							...
							{
								JOptionPane.showMessageDialog(null, "Domain not found: " + aString + s_CHECKLOADEDCAMPAIGNS, Constants.s_APPNAME, JOptionPane.ERROR_MESSAGE);
							}
						}
					}
					break;
	...
	private void handleSkillsLine(String skillsLine)
	{
		...
		while (skillTokenizer.hasMoreElements())
		{
			...
			else
			{
				...
				{
					JOptionPane.showMessageDialog(null, "Ranked skill not found: " + skillName + "(" + aFloat + ")" + s_CHECKLOADEDCAMPAIGNS, Constants.s_APPNAME, JOptionPane.INFORMATION_MESSAGE);
				}
			}
		}
	}
	...
	private void loadClassesLine(FileAccess fa, BufferedReader input)
	{
		...
		while (aTok.hasMoreTokens())
		{
			...
			{
				JOptionPane.showMessageDialog(null, "Class not found: " + aName + s_CHECKLOADEDCAMPAIGNS, Constants.s_APPNAME, JOptionPane.ERROR_MESSAGE);
			}
