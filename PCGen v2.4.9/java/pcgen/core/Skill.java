...
public class Skill extends PObject
{
	...
	public Integer modifier()
	{
		...
		Iterator typesForBonus = myType.iterator();
		while (typesForBonus.hasNext())
		{
			String singleType = (String)typesForBonus.next();
...
//loop through all current skill types checking for boni
	...
	public int statIndex(String aString)
	{
		...
		if (aString.equals("STR"))
			stat = Constants.STRENGTH;
		else if (aString.equals("DEX"))
			stat = Constants.DEXTERITY;
		else if (aString.equals("CON"))
			stat = Constants.CONSTITUTION;
		else if (aString.equals("INT"))
			stat = Constants.INTELLIGENCE;
		else if (aString.equals("WIS"))
			stat = Constants.WISDOM;
		else if (aString.equals("CHA"))
			stat = Constants.CHARISMA;
		return stat;
	}
