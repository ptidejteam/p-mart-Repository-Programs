...
public class Main {
	...
	public static void main(String args[]) {
		try {
			System.setOut(new PrintStream(
					new MultiChannelOutputStream(System.out,
							new FileOutputStream("Normal Output.txt", true))));
			System
					.setErr(new PrintStream(new MultiChannelOutputStream(
							System.err, new FileOutputStream(
									"Error Output.txt", true))));
	...
	public static void processSource(Context cx, String filename) {
		if (filename == null || filename.equals("-")) {
			...
			BufferedReader in = new BufferedReader(new InputStreamReader(global
					.getIn()));
			int lineno = 1;
				...
				if (script != null) {
					...
					if (result != Context.getUndefinedValue()) {
						try {
							...
							ToolErrorReporter.reportException(cx
									.getErrorReporter(), rex);
						}
					}
