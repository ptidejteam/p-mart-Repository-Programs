...
public abstract class AbstractField extends Parameter
  ...
  /**
   * Conditionally registers this component with various dispatchers.
   * For input, it will register with either ParameterDispatcher
   * (earlyInputNotify=true) or InputDispatcher (earlyInputNofity=false).
   * It will also register with RenderDispatcher.
   * Only registers if it NOT already registered and parent is not null
   */
  protected void lazyAddNotify() {
    if (!notifySet && parent != null) {
      notifySet = true;
      Dispatcher dispatcher;
      // Register for Input with either Parameter or Input dispatcher
      if (earlyInputNotify) {
        dispatcher = parent.getParameterDispatcher();
      } else {
        dispatcher = parent.getInputDispatcher();
      }
      if (dispatcher != null)
        dispatcher.addNotify(this);
      // Register with RenderDispatcher
      dispatcher = parent.getRenderDispatcher();
      if (dispatcher != null)
        dispatcher.addNotify(this);
  ...
  /**
   * Conditionally un-registers this component with various dispatchers.
   * For input, it will register with either ParameterDispatcher
   * (earlyInputNotify=true) or InputDispatcher (earlyInputNofity=false).
   * It will also register with RenderDispatcher.
  ...
  protected void lazyRemoveNotify() {
    if (notifySet && parent != null) {
      notifySet = false;
      Dispatcher dispatcher;
      // Un-register for Input with either Parameter or Input dispatcher
      if (earlyInputNotify) {
        dispatcher = parent.getParameterDispatcher();
      } else {
        dispatcher = parent.getInputDispatcher();
  ...
  /**
   * Sets flag for early input notification.  Normally this field registers to
   * receives its input notification from its Container's InputDispatcher.  If
   * earlyInputNofity is true, it will register with its Container's
   * ParameterDispatcher instead.  It will then be notified of input at the
  ...
  /**
   * Gets flag for early input notification.  Normally this field registers to
   * receives its input notification from its Container's InputDispatcher.  If
   * earlyInputNofity is true, it will register with its Container's
   * ParameterDispatcher instead.  It will then be notified of input at the
   * same time as Parameter components.
