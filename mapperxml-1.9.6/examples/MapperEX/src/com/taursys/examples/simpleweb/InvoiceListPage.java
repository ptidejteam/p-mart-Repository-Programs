...
public class InvoiceListPage extends ServletForm {
  ...
  private Document[] documents = new Document[4];
  ...
  private void jbInit() throws Exception {
    invoices.setValueObjectClass(com.taursys.examples.simpleweb.InvoiceVO.class);
  ...
  private void setupDocumentForStyle(Integer iStyle) throws Exception {
    ...
    if (documents[styleNo] == null) {
      DOMParser parser = new DOMParser();
      InputSource is = new InputSource(
          getClass().getResourceAsStream(
              "InvoiceListPage" + styleNo + ".html"));
      parser.parse(is);
      documents[styleNo] = parser.getDocument();
    }
    setDocument(documents[styleNo]);
  }
