...
{
  static final long serialVersionUID = -3992574725447249348L;
	...
	private JComboBox viewAvailComboBox  = new JComboBox();
	...
	public InfoFeats()
	{
		...
		Logging.debugPrint("FName=", aPC.getDisplayName());
		PObjectNode.resetPC();
		initComponents();
		initActionListeners();
	...
	private void initComponents()
	{
		...
		for (int i = 0; i < Globals.getFeatList().size(); ++i)
		{
			...
			final StringTokenizer aTok = 
					new StringTokenizer(bFeat.getType(), ".", false);
		...
		{
			Logging.debugPrint("chil=", cc[0].getChildCount());
		}
		...
		numFeatsField.addFocusListener(new FocusAdapter()
		{
			public void focusLost(FocusEvent evt)
			{
				if (numFeatsField.getText().length() > 0)
				{
					...
					{
						aPC.setDirty(true);
						aPC.setFeats(new Double(numFeatsField.getText()).doubleValue());
					}
				}
				else if (aPC != null)
				{
	...
	private void formComponentShown()
	{
		...
		if (!hasBeenSized)
		{
			...
			for (int i = 0; i < availableTable.getColumnCount(); ++i)
			{
				...
				aCol.addPropertyChangeListener(new ResizeColumnListener(availableTable,
																																"FeatAva", i));
			}
		}
		if (s > 0)
		...
		availableTable.getSelectionModel().addListSelectionListener(new ListSelectionListener()
		{
			public void valueChanged(ListSelectionEvent e)
			{
				if (!e.getValueIsAdjusting())
				{
					...
					Object temp =
						availableTable.getTree().getPathForRow(idx).getLastPathComponent();
					/////////////////////////
					if (temp == null)
					{
						JOptionPane.showMessageDialog(null, "Somehow, no feat was selected. Try again.", pcgen.core.Constants.s_APPNAME, JOptionPane.ERROR_MESSAGE);
						return;
					}
...
//			selectedSort.sortNodeOnColumn();
	...
	private void addFeat()
	{
		...
		if ((aFeat.getCost() > aPC.getFeats()) && !pcHasIt)
		{
		...
		private void resetModel(int mode, boolean available)
		{
			...
			switch (mode)
			{
				case GuiConstants.INFOFEATS_VIEW_TYPENAME:
					if (Logging.isDebugMode())
					{
						Logging.debugPrint(modelType + " children=" + InfoFeats.typeRoot.getChild(0).getChildCount());
					}
					setRoot(InfoFeats.typeRoot);
					if (Logging.isDebugMode())
					{
						Logging.debugPrint(modelType + " children=" + ((PObjectNode) super.getRoot()).getChild(0).getChildCount());
					}
					for (Iterator fI = Globals.getFeatList().iterator(); fI.hasNext();)
					{
						...
						*/
						...
						if (available && ((hasIt == HASFEAT_VIRTUAL) || 
						                  (hasIt == HASFEAT_NO) || aFeat.isMultiples()) || 
							...
							for (int i = 0; i < rootAsPObjectNode.getChildCount(); ++i)
							{
								if (aFeat.isType(rootAsPObjectNode.getChild(i).toString()))
								{
									...
									if (!available)
									{
										...
										final Feat bFeat = 
												aPC.getFeatNamed(featName, xlatInternalType(hasIt));
										if (bFeat != null)
										{
											aFN.setItem(bFeat);
				...
				case GuiConstants.INFOFEATS_VIEW_PREREQTREE:
					...
					else
					{
						...
						// make filters work ;-)
						Feat aFeat;
						for (Iterator it = aPC.aggregateFeatList().iterator(); 
								 it.hasNext();)
						{
							aFeat = (Feat) it.next();
							if (accept(aFeat))
					...
					while (fList.size() > 0 && loopmax-- > 0)
					{
						for (int i = 0; i < fList.size(); ++i)
						{
							...
							for (int j = 0; j < rootAsPObjectNode.getChildCount(); ++j)
							{
								...
								placed = 
										placedThisFeatInThisTree(
												aFeat, po, Feat.preReqStringsForList(preReqList), 0);
								if (placed > 0)
								{
									break;
								}
...
*/
		...
		private int placedThisFeatInThisTree(final Feat aFeat, PObjectNode po, 
																				 String preReqString, int level)
		{
			...
			while (true)
			{
				...
				if (a > -1)
				{
					...
					final StringTokenizer aTok = 
							new StringTokenizer(pString, ",\t", false);
					...
					else if (trychildren)
					{
						...
						{
							int j = placedThisFeatInThisTree(aFeat, po.getChild(i), 
																							 aString.toString(), level + 1);
							if (j == 2)
							{
								return 2;
							}
