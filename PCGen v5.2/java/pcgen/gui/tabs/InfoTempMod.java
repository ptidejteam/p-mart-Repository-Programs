...
public class InfoTempMod extends FilterAdapterPanel
{
	...
	private void setInfoLabelText(Object anObj)
	{
		...
		else if (aSpell != null)
		{
			...
			b.append(" &nbsp;<b>Level:</b>&nbsp; ").append(level);
	 ...
	 **/
	private void getBonusChoice(BonusObj newB, String aChoice)
	{
	...
	private final class TempModel extends AbstractTreeTableModel
	{
		 ...
		 **/
		private void resetModel(int mode, int argModelType)
		{
			...
			if (argModelType == MODEL_BONUS)
			{
				...
				switch (mode)
				{
					...
					default:
						Logging.errorPrint("In InfoTempMod.resetModel the mode " + mode + " is not supported.");
						break;
			...
			{ // targetTable builder
				...
				for (Iterator e = lastAvaObject.getBonusList().iterator(); e.hasNext();)
				{
					...
					aTok.nextToken(); //Throw token away. Wonder why?
					aTok.nextToken(); //Throw token away. Wonder why?
					aTok.nextToken(); //Throw token away. Wonder why?
					aString = aTok.nextToken();
	...
	private final class BonusModel extends AbstractTreeTableModel
	{
		...
		public Object getValueAt(Object node, int column)
		{
			...
			if (fn == null)
			{
				Logging.errorPrint("Somehow we have no active node when doing getValueAt in BonusModel");
				return null;
			}
			...
			if (fn.getItem() instanceof TempWrap)
			{
				...
				aTo = aTok.nextToken();
				aVal = aTok.nextToken();
				aVal = aPC.getVariableValue(aVal, "").toString();
