 ...
 *
 * TODO: AS IT IS ONLY USED TO PROVIDE TWO STATIC METHODS, IT SHOULD BE SEVERELY REFACTORED...
 *
 * @author     Jonas Karlsson <jujutsunerd@users.sourceforge.net>
 * @version    $Revision: 1.1 $
 */
public final class InfoPreview extends JPanel
{
	static final long serialVersionUID = 7621398326532595475L;
	...
	private final CsheetFilter psheetFilter = new CsheetFilter(1);
	...
	InfoPreview()
	{
		...
		btnBrowseTemplate.addActionListener(new ActionListener()
		{
			...
			{
				btnBrowseTemplate_actionPerformed();
			}
		});
		...
		btnPreviewInBrowser.addActionListener(new ActionListener()
		{
			/**
			 *  Anonymous event handler
			 *
			...
			public void actionPerformed(ActionEvent e)
			{
				btnPreviewInBrowser_actionPerformed();
			}
		});
		...
		addComponentListener(new ComponentAdapter()
		{
			public void componentShown(ComponentEvent evt)
			{
				...
				requestFocus();
				final String templateName = SettingsHandler.getSelectedCharacterHTMLOutputSheet();
		...
		if (new File(SettingsHandler.getSelectedCharacterHTMLOutputSheet()).exists())
		{
	 ...
	 *
	 */
	private void btnBrowseTemplate_actionPerformed()
	{
		...
		final int returnVal = fc.showOpenDialog(InfoPreview.this);

		if (returnVal == JFileChooser.APPROVE_OPTION)
		{
			SettingsHandler.setSelectedCharacterHTMLOutputSheet(fc.getSelectedFile().getPath());
			SettingsHandler.setTemplatePath(fc.getSelectedFile().getParentFile());
			txtSelectedTemplate.setText(SettingsHandler.getSelectedCharacterHTMLOutputSheet());
		}
		btnPreview.setEnabled(true);
		btnPreviewInBrowser.setEnabled(true);
	}
	 ...
	 *
	 */
	private void btnPreview_actionPerformed()
	{
	 ...
	 *
	 */
	private void btnPreviewInBrowser_actionPerformed()
	{
	...
	private static File getTempPreviewFile()
	{
		...
		final String template = SettingsHandler.getSelectedCharacterHTMLOutputSheet();
		if ((template == null) || (template.trim().length() == 0))
		{
			JOptionPane.showMessageDialog(null, "No HTML template specified in preferences.", "PCGen", JOptionPane.ERROR_MESSAGE);
			return null;
		}
  ...
  */
	...
	private void previewInternal()
	{
		...
		catch (IOException ex)
		{
			...
			Logging.errorPrint("Could not preview file. It probably looks ok in a browser though.", ex);
		}
	...
	private static void printToWriter(Writer w) throws IOException
	{
		...
		File template = new File(SettingsHandler.getSelectedCharacterHTMLOutputSheet());
