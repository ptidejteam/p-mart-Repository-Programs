...
public final class Equipment extends PObject implements Serializable, EquipmentCollection, Comparable
{
	...
	private List d_headerChildren = null;
	...
	{
		Logging.debugPrint("Entering seteDR.");
		Logging.debugPrint("Passed Value:", aString);
		try
		{
		...
		Logging.debugPrint("Exiting seteDR.");
		Logging.debugPrint("Exit value of eDR:", eDR.toString());
	}
	...
	public void collapseHeaderParent()
	{
		...
		equipped = lastHchild.isEquipped();
		location = lastHchild.getLocation();
	}
	...
	private void removeCommonFromList(final List altList, final List commonList, String errMsg)
	{
		for (int i = altList.size() - 1; i >= 0; --i)
		{
			...
			{
				Logging.debugPrint(errMsg, eqMod.getName());
			}
		}
	}
...
/*		Equipment anEquip = getContainedEquipment(childIndex);
	...
	private String pickChildType(SortedSet aTypeList, Float aQuant)
	{
		...
		if ((getChildType("Total").floatValue() + aQuant.floatValue()) <= acceptsType.floatValue())
		{
			...
			while (e.hasNext() && "".equals(canContain))
			{
				...
				{
					Logging.debugPrint("Testing ", aString);
				}
				...
				if (acceptsType(aString))
				{
					if (Logging.isDebugMode())
					{
						Logging.debugPrint("Accepts ", aString);
					}
	 ...
	 *@return             Description of the Return Value
	 */
	public boolean canHold(Map properties)
	{
		return true;
	}
	 ...
	 */
	public void updateProperties(Map properties, boolean additive)
	{
		// no action
	}
	...
	public BigDecimal getCost()
	{
		...
		if (modifierCosts.size() > 1)
		{
			...
			if (Logging.isDebugMode())
			{
				Logging.debugPrint("" + modifierCosts);
			}
		}
...
//	}
	...
	public void addToAltTypeList(String argAltType)
	{
		...
		final StringTokenizer aTok = new StringTokenizer(altType, ".", false);
		while (aTok.hasMoreTokens())
		{
			final String type = aTok.nextToken();
			addAltType(type);
	...
	private int parseCritMult(String aString)
	{
		if (aString.length() > 0 && aString.charAt(0) == 'x')
		{
			...
			{
				Logging.debugPrint("parseCritMult:" + getName() + ":" + aString);
			}
		}
		return -1;
	}
...
//	}
	 ...
	 *@param  weight    weight of item in taht list (for random generation)
	 */
	public void addTreasureList(String listName, int weight) {
		if(treasureList == null) {
			treasureList = new HashMap();
		}
		//Sigh, java 1.5 will make it no longer necessary to turn this into an Integer
		treasureList.put(listName, new Integer(weight));
	}
	
	/**
	 *  returns a hashmap of lists and weights for treasure generation for this item
	 *@return HashMap of String, Integer entries
	 */
	public Map getTreasureList() {
		return treasureList;
	}
	 ...
	 */
	private static final String getMagicBonus(final List eqModList)
	{
		for (Iterator e = eqModList.iterator(); e.hasNext();)
		{
			final EquipmentModifier eqMod = (EquipmentModifier) e.next();
...
///////////////////////////////
			...
			for (int idx = 0; idx < itemTypes.size(); ++idx)
			{
				...
				if (costExpr != null)
				{
					...
					if ((maxCost == null) || (thisCost.compareTo(maxCost) > 1))
					{
						if (Logging.isDebugMode())
						{
							Logging.debugPrint("Setting max cost=" + thisCost.toString());
						}
						maxCost = thisCost;
					}
				}
	...
	private BigDecimal evaluateCost(pcgen.util.PJEP myParser, final String costExpr, final String typeMatched)
	{
		if (Logging.isDebugMode())
		{
			Logging.debugPrint(typeMatched + ":" + costExpr);
		}
		...
		if (!myParser.hasError())
		{
			...
			if (Logging.isDebugMode())
			{
				...
				Logging.debugPrint("symbol table:" + st.toString());
				Logging.debugPrint("result=" + result.toString());
			}
			if (result != null)
			{
