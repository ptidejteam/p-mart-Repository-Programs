...
public class PObject extends Object implements Cloneable, Serializable, Comparable
{
	...
	protected HashMap spellLevelMap = null, preReqSpellLevelMap = null;
	...
	public void setMoveRates(String moveparse)
	{
		...
		if (moves.countTokens() == 1)
		{
			...
			Logging.debugPrint("single option in ", this.toString());
		...
		else
		{
			...
			while (moves.countTokens() > 1)
			{
				...
				Logging.debugPrint("multiple option in " + this.toString() + ":" + movementTypes[x] + " " + tok);

				if ((tok.length() > 0) && ((tok.charAt(0) == '*') || (tok.charAt(0) == '/')))
				{
					Logging.debugPrint("* or /");
	...
	public void setNaturalAttacks(PObject obj, String aString)
	{
		...
		int sizeInt = Globals.sizeInt(aSize);
		...
		{
			Logging.debugPrint("aString: "+ aString);
		}

		// This code is going to have problems with 'C' sized creatures
		// becuz they can't have a weapon larger than them
		final String bigSize = Globals.getSizeAdjustmentAtIndex(sizeInt).getAbbreviation();

		// Is this the right thing to do?
		// I think we need to replace old ones with "better" ones
		...
		while (attackTok.hasMoreTokens())
		{
			...
			//String eq = aTok.nextToken() + "\tTYPE:" + aTok.nextToken() + "\tWT:0\tCost:0\tSIZE:" + bigSize;
			...
			if (!attackName.equalsIgnoreCase(Constants.s_NONE))
			{
				...
				//eq.append(attackName).append("\tTYPE:").append(prof.getType()).append("\tWT:0\tCost:0\tSIZE:").append(bigSize);

				anEquip.setName(attackName);
				anEquip.setTypeInfo(prof.getType());
				anEquip.setWeight("0");
				anEquip.setSize(bigSize, true);
				...
				{
					Logging.errorPrint(obj.getName() + ": non-numeric value for number of attacks: '" + numAttacks + "'");
				}

				if (bonusAttacks > 0)
				{
					// only attack means:
					// one attack with one weapon.
					onlyOne = false;
				...
				if (aTok.hasMoreTokens())
				{
					...
					{
						Logging.errorPrint(obj.getName() + ": non-numeric value for hands required: '" + hString + "'");
					}
				}
				...
				if (Logging.isDebugMode())
				{
					Logging.debugPrint("Eq:" + Constants.s_LINE_SEP, eq.toString());
				}
	 ...
	 **/
	public void setNaturalWeapons(ArrayList aList)
	{
...
//			aString = aTok.nextToken();
		...
		for (Iterator i = autoArray.iterator(); i.hasNext();)
		{
			...
			while (aTok.hasMoreTokens())
			{
				...
				if (tok.startsWith("TYPE=") && tag.startsWith("WEAPON"))
				{
					...
					List listFromWPType = null;
					List listFromEquipmentType = null;
	...
	public boolean getSpellLevelforKey(String key, int levelMatch)
	{
		...
		{
			Logging.errorPrint("Badly formed SPELLLEVEL level tag: " + spellLevelMap.get(key));
			return false;
		}
		if ((levelMatch == -1 && levelInt >= 0) || (levelMatch >= 0 && levelInt == levelMatch))
		{
			...
			{
				return passesPreReqTestsForList((List) preReqSpellLevelMap.get(key));
			}
			return true;
		}
		return false;
		...
		for (Iterator iSpell = wSpellList.iterator(), iName = wNameList.iterator(); iSpell.hasNext() || iName.hasNext();)
		{
			...
			while (bTok.hasMoreTokens())
			{
				...
				while (cTok.hasMoreTokens())
				{
					...
					if (!spellLevelMap.containsKey(tagType + "|" + className + "|" + spellName))
					{
	...
	final double bonusTo(String aType, String aName, Object obj, List aBonusList)
	{
	...
	private double calcBonus(String bString, String aType, String aName, String aTypePlusName, Object obj, int iTimes)
	{
		...
		else
		{
			...
			catch (NumberFormatException e)
			{
				...
				Logging.errorPrint("NumberFormatException in BONUS: " + bString, e);
			}
		}

		final ArrayList bonusPreReqList = new ArrayList();
		...
		if (obj instanceof PlayerCharacter)
		{
			if (!passesPreReqTestsForList(bonusPreReqList))
			{
				return 0;
			}
		}
		...
		{
			if (!passesPreReqTestsForList((PObject) obj, bonusPreReqList))
			{
				return 0;
			}
		}
...
//		}
	 ...
	 * @return
	 */
	public final boolean passesPreReqTestsForList(List argList)
	{
		...
		return passesPreReqTestsForList(aPC, null, argList);
	}
	 ...
	 * </ul>
	 */
	public boolean passesPreReqTestsForList(PlayerCharacter aPC, PObject aObj, List argList)
	{
		...
		if (Logging.isDebugMode())
		{
			Logging.debugPrint("PreReq:", name);
		}
		...
		for (Iterator e = argList.iterator(); e.hasNext();)
		{
			...
			if ("Q".equals(aList))
			{
				...
				if (Logging.isDebugMode())
				{
					Logging.debugPrint("aList: ", aList);
				}
			}
			...
			if (aPC != null)
			{
				if (aPC.checkQualifyList(this.getName()) && !qValue)
				{
					if (Logging.isDebugMode())
					{
						Logging.debugPrint("In Qualify list:" + this + " -> Qualify list:" + aPC.getQualifyList());
					}
					qualifyValue = true;
				}
				else
				{
					Logging.debugPrint("Not In Qualify list:", this.toString());
				}
			}
			...
			if (aPC != null)
			{
				...
				else if ("PRELEVEL".equals(aType))
				{
					...
					{
						Logging.errorPrint("Badly formed PRELEVEL attribute: " + aList);
//						Assuming a 1 default
				...
				else if ("PRELEVELMAX".equals(aType))
				{
					...
					{
						Logging.errorPrint("Badly formed PRELEVELMAX attribute: " + aList);
						preLevelmax = 0;
					}
				...
				else if ("PREHP".equals(aType))
				{
					...
					{
						Logging.errorPrint("Badly formed PREHP attribute: " + aList);
						preHitPoints = 0;
					}
				...
				else if ("PREATT".equals(aType))
				{
					...
					{
						Logging.errorPrint("Badly formed PREATT attribute: " + aList);
					}
				}
				...
				else if (aType.startsWith("PREVISION"))
				{
					flag = passesPrereqVision(aList, aPC);
				}
				else if ("PREMULT".equals(aType))
				{
					flag = passesPrereqMult(aPC, aObj, preString.substring(8));
				}
				else if ("PREDR".equals(aType))
				{
					flag = passesPrereqDR(aList, aPC);
				}
				else if ("PREGENDER".equals(aType))
				{
					if (Logging.isDebugMode())
					{
						Logging.debugPrint("PREGENDER");
					}
				...
				else if ("PREBIRTHPLACE".equals(aType))
				{
					if (Logging.isDebugMode())
					{
						Logging.debugPrint("PREBIRTHPLACE");
					}
				...
				else if (aType.startsWith("PRECHECK"))
				{
					...
					{
						Logging.errorPrint("Badly formed PRECHECK attribute: " + tok);
						num = 0;
					}
					while (dTok.hasMoreTokens())
					{
				...
				else if (aType.startsWith("PREHANDS"))
				{
					...
					{
						Logging.errorPrint("Badly formed PREHANDS attribute: " + aList);
					}
				}
				//
				...
				else if (aType.startsWith("PRELEGS"))
				{
					...
					{
						Logging.errorPrint("Badly formed PRELEGS attribute: " + aList);
					}
				}
				//
				...
				else if (aType.startsWith("PRESR"))
				{
					...
					{
						Logging.errorPrint("Badly formed PRESR attribute: " + aList);
					}
				}
				//
				...
				else if (aType.startsWith("PREBASESIZE"))
				{
					...
					if ((aPC.getRace() != null) && !aPC.getRace().equals(Globals.s_EMPTYRACE))
					{
						...
						if (iSize < 0)
						{
							Logging.errorPrint("Invalid size '" + aList + "' in PREBASESIZE");
						}
						else
						{
				...
				{
					Logging.debugPrint("Prereq failed, unknown tag: ", aType);
					flag = false;
				}
			}
	...
	public boolean passesPrereqMult(PlayerCharacter aPC, PObject aObj, String paramString)
	{
		...
		{
			Logging.debugPrint("Incorrect PREMULT parameter. Assuming 1");
			qtdMult = 1;
		}
		int aIndex = paramString.indexOf(",");
		if (aIndex < 0)
		{
			Logging.debugPrint("PREMULT formatted incorrectly.");
			return false;
		}
		...
		for (int i = 0,nesting = 0,start = 0;i < aList.length();i++)
		{
			...
			{
				Logging.debugPrint("PREMULT Incorrect [] nesting.");
				return false;
			}
		}
		for (Iterator i = preList.iterator(); i.hasNext();)
		{
			...
			argList.add(aString);
			if (passesPreReqTestsForList(aPC, aObj, argList))
			{
				--qtdMult;
			}
			--qtdPreList;
	...
	public final void removeSave(String bonusString)
	{
		if (saveList != null)
		{
			...
			{
				Logging.errorPrint("removeBonus: Could not find bonus: " + bonusString + " in saveList.");
			}
		}
		else
		{
		...
		else if (aChoice.startsWith("WEAPONPROF")) // determine appropriate choices for chooser
		{
			...
			while (aTok.hasMoreTokens())
			{
				...
				if (aString.lastIndexOf('[') >= 0)
				{
					...
					if (Logging.isDebugMode())
					{
						Logging.debugPrint("bString=", bString);
					}
				}
...
//			flag = aPC.getDeity().allowsAlignment(alignment);
	...
	private static boolean passesPreDeityAlign(String aList, PlayerCharacter aPC)
	{
		...
		if (Logging.isDebugMode())
		{
			Logging.debugPrint("PREDEITYALIGN");
		}
	...
	private static boolean passesPreDeityDomain(String aList, PlayerCharacter aPC)
	{
		...
		if (Logging.isDebugMode())
		{
			Logging.debugPrint("PREDEITYDOMAIN");
		}
	...
	private static boolean passesPreArmorType(String aList, PlayerCharacter aPC)
	{
		...
		if (Logging.isDebugMode())
		{
			Logging.debugPrint("PREARMORTYPE");
		}
		...
		while (aTok.hasMoreTokens())
		{
			...
			for (Iterator e1 = aPC.getEquipmentList().iterator(); e1.hasNext();)
			{
				if (Logging.isDebugMode())
				{
					Logging.debugPrint("PREARMORTYPE A1");
				}
				...
				{ //not a TYPE string
					...
					else if (aString.indexOf("LIST") >= 0)
					{
						if (Logging.isDebugMode())
						{
							Logging.debugPrint("PREARMORTYPE A2");
						}
						...
						for (Iterator e2 = aPC.getArmorProfList().iterator(); e2.hasNext();)
						{
							...
							if (Logging.isDebugMode())
							{
								Logging.debugPrint("PREARMORTYPE ", aprof);
							}
	...
	private static boolean passesPreEquipPrimary(String aList, PlayerCharacter aPC)
	{
		...
		if (Logging.isDebugMode())
		{
			Logging.debugPrint("PREEQUIPPRIMARY");
		}
	...
	private static boolean passesPreEquipSecondary(String aList, PlayerCharacter aPC)
	{
		...
		if (Logging.isDebugMode())
		{
			Logging.debugPrint("PREEQUIPSECONDARY");
		}
	...
	private static boolean passesPreEquipBothHands(String aList, PlayerCharacter aPC)
	{
		...
		if (Logging.isDebugMode())
		{
			Logging.debugPrint("PREEQUIPBOTH");
		}
	...
	private static boolean passesPreEquipTwoWeapon(String aList, PlayerCharacter aPC)
	{
		...
		if (Logging.isDebugMode())
		{
			Logging.debugPrint("PREEQUIPTWOWEAPON");
		}
	...
	private static boolean passesPreSA(String aList, PlayerCharacter aPC)
	{
		...
		if (Logging.isDebugMode())
		{
			Logging.debugPrint("number of PreSA to Match: ", number);
		}
		while (aTok.hasMoreTokens())
		{
	...
	private static boolean passesPreSpellSchool(String aList, PlayerCharacter aPC)
	{
		...
		if (Logging.isDebugMode())
		{
			Logging.debugPrint("Spells=", aArrayList.size());
		}
	...
	private static boolean passesPreVar(String aList, String aType, PlayerCharacter aPC)
	{
		...
		while (aTok.hasMoreTokens() && flag)
		{
			...
			switch (i)
			{
				...
				default:
					Logging.errorPrint("In PObject.passesPrereqTestForLists the prevar type " + i + " is unsupported.");
					break;
			}
		}
		return flag;
	...
	private static boolean passesPreWeaponProf(String aList, PlayerCharacter aPC)
	{
		...
		if (Logging.isDebugMode())
		{
			Logging.debugPrint("PREWEAPONPROF");
		}
		...
		while (aTok.hasMoreTokens())
		{
			...
			if (numberRequired == -1)
			{
				...
				catch (NumberFormatException nfe)
				{
					...
					Logging.errorPrint("Using old PREWEAPONPROF syntax. New syntax is PREWEAPONPROF:#,wp1,wp2,...,wpn");
					Logging.debugPrint("Will use default for numberRequired: " + numberRequired, nfe);
				}
			}
	...
	private static boolean passesPrereqClass(String aList, PlayerCharacter aPC)
	{
		...
		{
			Logging.debugPrint("Will use default for numberRequired: " + numberRequired, exc);
		}
		...
		for (; ;)
		{
			...
			if (i >= 0)
			{
				...
				for (i = 0; i < classesToTest.size(); ++i)
				{
					...
					if (Logging.isDebugMode())
					{
						Logging.debugPrint(aString + "==" + Integer.toString(preClass) + " " + passes);
					}
					if (passes)
					{
						if (--numberRequired == 0)
	...
	private static boolean passesPrereqClassLevelMax(String aList, PlayerCharacter aPC)
	{
		...
		{
			Logging.errorPrint("Badly formed passesPrereqClassLevelMax attribute: " + aList.substring(i + 1));
			preClass = 0;
		}
	...
	private static boolean passesPrereqHD(String aList, PlayerCharacter aPC)
	{
		...
		{
			Logging.errorPrint("Badly formed passesPrereqClass/preHDMin attribute: " + tok);
			preHDMin = 1;
		}
		...
		{
			Logging.errorPrint("Badly formed passesPrereqClass/preHDMax attribute: " + tok);
			preHDMax = 1;
		}
	...
	private static boolean passesPrereqPreUAtt(String aList, PlayerCharacter aPC)
	{
		...
		{
			Logging.errorPrint("Badly formed passesPrereqPreUAtt attribute: " + aList);
			requiredValue = 0;
		}
		int att = 0;
		if (!aPC.getClassList().isEmpty())
		{
			for (Iterator e2 = aPC.getClassList().iterator(); e2.hasNext();)
			{
				...
				else
				{
					...
					{
						Logging.errorPrint("Badly formed passesPrereqPreUAtt attribute: " + s);
					}
				}
			}
		}
	...
	private static boolean passesPrereqDR(String aList, PlayerCharacter aPC)
	{
		final StringTokenizer aTok = new StringTokenizer(aList, ",", false);
		...
		{
			Logging.errorPrint("Badly formed passesPrereqDR/number of DRs attribute: " + tok);
			number = 1;
		}
		...
		while (aTok.hasMoreTokens())
		{
			...
			if (bTok.hasMoreTokens())
			{
				...
				{
					Logging.errorPrint("Badly formed passesPrereqDR value: " + aList);
					return false;
				}
			}
			else
	...
	private static boolean passesPrereqVision(String aList, PlayerCharacter aPC)
	{
		...
		if (i >= 0)
		{
			...
			{
				Logging.errorPrint("Badly formed passesPrereqVision attribute: " + aList.substring(i + 1));
				range = 0;
			}
		}
		else
		...
		{
			Logging.errorPrint("Badly formed passesPrereqVision/number of visions attribute: " + tok);
			number = 1;
		}
	...
	private static boolean passesPrereqSkill(String aList, PlayerCharacter aPC)
	{
		...
		if (i >= 0)
		{
			...
			{
				Logging.errorPrint("Badly formed passesPrereqSkill attribute: " + aList.substring(i + 1));
				ranks = 0;
			}
		}
		else
		{
			Logging.errorPrint("passesPrereqSkill: bad prereq \"" + aList + "\"");
			return false;
		}
		final StringTokenizer aTok = new StringTokenizer(aList.substring(0, i), ",", false);
		...
		{
			Logging.errorPrint("Badly formed passesPrereqSkill/number of skills attribute: " + tok);
			number = 1;
		}
	...
	private static boolean passesPrereqSkillTot(String aList, PlayerCharacter aPC)
	{
		...
		{
			Logging.errorPrint("Badly formed passesPrereqSkillTot attribute: " + aList.substring(i + 1));
			ranks = 0;
		}
		final StringTokenizer aTok = new StringTokenizer(aList.substring(0, i), ",", false);
	...
	private boolean passesPrereqSpellType(String aList, PlayerCharacter aPC)
	{
		...
		if (aTok.countTokens() == 2)
		{
			...
			{
				Logging.errorPrint("Badly formed passesPrereqSpellType/number attribute: " + tok);
				number = 0;
			}
			tok = aTok.nextToken();
			int minlevel;
			...
			{
				Logging.errorPrint("Badly formed passesPrereqSpellType/minLevel attribute: " + tok);
				minlevel = 0;
			}
		...
		{
			Logging.debugPrint(getName(), ":badly formed PRESPELLTYPE: " + aList);
		}
		return false;
	}
	...
	private final boolean passesPrereqStat(String compType, final String aList, final PlayerCharacter aPC)
	{
		...
		{
			Logging.errorPrint("Badly formed passesPrereqStat attribute: " + myTok);
		}
...
//		return kitString;
	...
	public LevelAbility addAddList(String aString)
	{
		return addAddList(0, aString);
	}
...
Logging.errorPrint("PObject addAddsForLevel");
	...
	private static boolean doComparison(String comparison, int iVal1, int iVal2, PlayerCharacter aPC, String tag)
	{
		...
		Logging.debugPrint("Prereq failed, unknown comparison: " + comparison);
		return false;
	}
	...
	{
		final List statList = Globals.getStatList();
...
// Please leave in - Byngl
	...
	static boolean modChoices(PObject obj, List availableList, List selectedList, boolean process)
	{
		...
		else if ("SALIST".equals(choiceType))
		{
			...
			PCGIOHandler.buildSALIST(choiceString, availableList, aBonusList);
			obj.addAssociatedTo(selectedList);
		}
				...
				else if (aString.startsWith("TYPE."))
				{
					...
					while(setIter.hasNext())
					{
						...
						if (eq == null)
						{
							...
							if (Logging.isDebugMode())
							{
								Logging.debugPrint("Prof Name: -" + wp.getName() + "- " + adding);
							}
		...
		else if ("FEATADD".equals(choiceType))
		{
			...
			while (aTok.hasMoreTokens())
			{
				...
				else
				{
					...
					if (aFeat == null)
					{
						Logging.debugPrint("Feat not found: ", featName);
						return false;
					}
