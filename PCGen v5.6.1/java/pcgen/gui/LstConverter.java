					...
					while (lineTokenizer.hasMoreTokens())
					{
						...
						if (bString.startsWith("PREFORT:"))
						{
							conversionWriter.write("PRECHECK:1," +
								SystemCollections.getUnmodifiableCheckList().get(0).toString().toUpperCase() +
								"=" + bString.substring(8));
							continue;
						}
						...
						{
							conversionWriter.write("PRECHECK:1," +
								SystemCollections.getUnmodifiableCheckList().get(1).toString().toUpperCase() +
						...
						{
							conversionWriter.write("PRECHECK:1," +
								SystemCollections.getUnmodifiableCheckList().get(2).toString().toUpperCase() +
								"=" + bString.substring(8));
							continue;
						}
						...
						else if (bString.startsWith("PREWEAPONPROF:"))
						{
							...
							{
								conversionWriter.write("PREWEAPONPROF:" + Integer.toString(p.countTokens() + 1) + "," + bString.substring(14));
								continue;
							}
						}
						switch (thisType)
						{
							case LstConverterConstants.CONVERT_RACETYPE: // race
								if ((!hasTagless && bString.startsWith("STATADJ")) || (field > 1 && field < 8))
								{
									...
									if (!"0".equals(bString))
									{
										bString = "BONUS:STAT|" + ((PCStat) SystemCollections.getUnmodifiableStatList().get(statNum)).getAbb() + "|" + bString;
										conversionWriter.write(bString);
									}
								}
								else if (field == 8)
...
//									FORTITUDECHECK has been replaced in v3.1.0
									...
									bString = "BONUS:CHECKS|BASE." + SystemCollections.getUnmodifiableCheckList().get(0).toString().toUpperCase() + "|" + bString;
								}
...
//									REFLEXCHECK has been replaced in v3.1.0
									...
									bString = getFormulaFor(1, bString);
									bString = "BONUS:CHECKS|BASE." + SystemCollections.getUnmodifiableCheckList().get(1).toString().toUpperCase() + "|" + bString;
								}
...
//									WILLPOWERCHECK has been replaced in v3.1.0
									...
									bString = getFormulaFor(1, bString);
									bString = "BONUS:CHECKS|BASE." + SystemCollections.getUnmodifiableCheckList().get(2).toString().toUpperCase() + "|" + bString;
								}
							...
							case LstConverterConstants.CONVERT_SKILLTYPE:  // skill
								...
								else if (bString.startsWith("SYNERGY:"))
								{
									...
									bString = "BONUS:SKILL|" + skillX + "|" + bonusToX + "|PRESKILL:1," + skillY + "=" + ranksY + "|TYPE=Synergy.STACK";
								}
								conversionWriter.write(bString);
								break;
			...
			switch (columnIndex)
			{
				...
				case 3:
					if (i.intValue() == 1)
					{
						...
						if (j == 0)
						{
							GuiFacade.showMessageDialog(null, "Set type to a known type before marking it to be converted.", "Oops!", GuiFacade.ERROR_MESSAGE);
							return;
						}
					}
					okList.set(rowIndex, i);
