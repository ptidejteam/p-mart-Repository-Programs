...
public abstract class FilterAdapterPanel extends JPanel implements Filterable
{
	 ...
	 */
	protected final boolean accept(PObject pObject)
	{
		if (pObject == null)
		{
			return false;
		...
		for (Iterator it = getSelectedFilters().iterator(); it.hasNext();)
		{
			...
			if ((mode == FilterConstants.MATCH_ALL) && !filter.accept(pObject))
			{
				return false;
			}
			else if ((mode == FilterConstants.MATCH_ALL_NEGATE) && !filter.accept(pObject))
			{
				return true;
			}
			else if ((mode == FilterConstants.MATCH_ANY) && filter.accept(pObject))
			{
				return true;
			}
			else if ((mode == FilterConstants.MATCH_ANY_NEGATE) && filter.accept(pObject))
			{
				return false;
			}
		}
