	...
	private void jbInit() throws Exception
	{
		...
		for (int i = 0; i < in_modeName.length; ++i)
		{
			...
			while (campaignIterator.hasNext())
			{
				...
				if (aCamp.canShowInMenu() && aCamp.isGameMode(((GameMode) SystemCollections.getUnmodifiableGameModeList().get(i)).getName()))
				{
					if (firstSubMenu == null)
					{
	...
	private final class CheckBoxListener implements ActionListener
	{
		public void actionPerformed(ActionEvent actionEvent)
		{
			...
			if (campaignNum >= 0)
			{
				...
				List gameModeList =
					((Campaign) campaigns.get(campaignNum)).getGameModeList();
