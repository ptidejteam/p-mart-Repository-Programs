	...
	private void launchProductWebsite(boolean avail, boolean isProductNotHelp)
	{
		...
		if (pon != null)
		{
			if (pon.getItem() instanceof Campaign)
			{
				...
				final String theURL = isProductNotHelp ? theCamp.getSourceWeb() : theCamp.getHelp();
				...
				else
				{
					...
					GuiFacade.showMessageDialog(null, "No "+type+" information found for Source: " + theCamp.getName(),
					    Constants.s_APPNAME, GuiFacade.WARNING_MESSAGE);
				}
			}
		}
		else
			...
			switch (mode)
			{
				case VIEW_PUBFMTSET: // by Publisher/Format/Setting/Product Name
					...
					while (fI.hasNext())
					{
						...
						if (!shouldDisplayThis(aCamp) || !aCamp.isGameMode(allowedModes))
						{
							continue;
						}
