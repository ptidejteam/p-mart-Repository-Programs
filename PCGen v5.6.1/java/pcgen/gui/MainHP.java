...
final class MainHP extends JPanel
{
	...
	private void incrementForRow(int iRow, int increment)
	{
		...
		{
			iRoll = iMax;
			GuiFacade.showMessageDialog(null, "Setting roll to maximum (" + iMax + ')', Constants.s_APPNAME, GuiFacade.ERROR_MESSAGE);
			increment = 0;
		}

		if ((iRoll + increment) < 1)
		{
			GuiFacade.showMessageDialog(null, "Roll must be at least the minimum (1)", Constants.s_APPNAME, GuiFacade.ERROR_MESSAGE);
		}
		else if ((iRoll + increment) > iMax)
		{
			GuiFacade.showMessageDialog(null, "Roll cannot exceed the maximum (" + iMax + ')', Constants.s_APPNAME, GuiFacade.ERROR_MESSAGE);
		}
		else
		{
			iRoll += increment;
