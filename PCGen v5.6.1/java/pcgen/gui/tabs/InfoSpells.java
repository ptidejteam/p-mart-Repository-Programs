...
public class InfoSpells extends FilterAdapterPanel
{
	...
	private void initComponents()
	{
		...
		primaryViewComboBox.addItem("Range");
		primaryViewComboBox.addItem("Duration");
		primaryViewComboBox.addItem("Type");
		...
		secondaryViewComboBox.addItem("Range");
		secondaryViewComboBox.addItem("Duration");
		secondaryViewComboBox.addItem("Type");
		...
		primaryViewSelectComboBox.addItem("Range");
		primaryViewSelectComboBox.addItem("Duration");
		primaryViewSelectComboBox.addItem("Type");
		...
		secondaryViewSelectComboBox.addItem("Range");
		secondaryViewSelectComboBox.addItem("Duration");
		secondaryViewSelectComboBox.addItem("Type");
...
//			SettingsHandler.setSpellsTab_SelectedListMode(secondaryViewSelectMode);
	...
	private List getInfoFromNode(PObjectNode fNode)
	{
		...
		{
			GuiFacade.showMessageDialog(null, "only possible when sorted by class and level", Constants.s_APPNAME, GuiFacade.ERROR_MESSAGE);
		...
		spLevel = ((SpellInfo) fNode.getItem()).getActualLevel();
			...
			{
				CharacterDomain cd = aPC.getCharacterDomainForDomain(theOwner.getName());
	...
	private void addSpellToTarget(PObjectNode fNode, String bookName)
	{
		...
		final String aString = aPC.addSpell(cs, featList, className, bookName, spLevel, spLevel);
	...
	private void metamagicButton()
	{
		...
		if (!(fNode.getItem() instanceof SpellInfo))
		{
			GuiFacade.showMessageDialog(null, "Unable to metamagic this spell", Constants.s_APPNAME, GuiFacade.ERROR_MESSAGE);
			return;
		}
		...
		{
			GuiFacade.showMessageDialog(null, "Unable to metamagic spells for "+spellA.getOwner().getName(), Constants.s_APPNAME, GuiFacade.ERROR_MESSAGE);
			return;
		}
		...
		private void resetModel(int primaryMode, int secondaryMode, boolean available)
		{
			...
			if (!available) // !available = selected list of spells (right-side of tab)
			{
				...
				for (Iterator iBook = bookList.iterator(); iBook.hasNext();)
				{
					...
					spellList.addAll(aPC.getRace().getCharacterSpell(null, bookName, -1));
				}
					...
					for (int pindex = 0 ; pindex < primaryNodes.length; pindex++)
					{
						...
						if (firstPass)
						{
							switch (secondaryMode)
							{
								...
								break;
								case GuiConstants.INFOSPELLS_VIEW_RANGE:   // By Range
									secondaryNodes = getRangeNodes();
								break;
								case GuiConstants.INFOSPELLS_VIEW_DURATION:   // By Duration
									secondaryNodes = getDurationNodes();
								break;
								case GuiConstants.INFOSPELLS_VIEW_TYPE:   // By Duration
									secondaryNodes = getTypeNodes();
								break;
							}
						}
						else
						...
						for (int sindex = 0 ; sindex < secondaryNodes.length; sindex++)
						{
							...
							if (aClass != null && iLev > -1 && spellMatch && (aClass instanceof PCClass))
							{
								...
								spellMatch = spell.levelForKeyContains(theObject.getSpellKey(), theLevel);
							}
