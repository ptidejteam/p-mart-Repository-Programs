...
public class PObjectNode implements Cloneable, ResetableListIterator
{
	...
	public String toString()
	{
		...
		if (item instanceof SpellInfo)
		{
			...
			final Spell theSpell = spellA.getSpell();

			final StringBuffer val = new StringBuffer(80);

			if (!theSpell.passesPreReqToGain() || 
				((spellA.getOwner() instanceof PCClass) && ((PCClass)spellA.getOwner()).isProhibited(theSpell)))
			{
				// indicates to LabelTreeCellRenderer to change text color
				// to a user-preference (default is red)
				Color aColor = Color.red;
				...
				val.append("|"+aColor.getRGB()).append("|");
			}
			// first, set the name
			val.append(theSpell.piSubString());	// gets name of spell
			...
			{
				//val.append(" [").append(theSpell.getDescriptor(", ")).append(']');
