...
final class PreferencesDialog extends JDialog
{
	...
	private void applyOptionValuesToControls()
	{
		...
		unitSetType.setSelectedIndex(Globals.getSelectedUnitSet());
	...
	private void setOptionsBasedOnControls()
	{
		...
		Globals.selectUnitSet((String) unitSetType.getSelectedItem());
		...
		PCGen_Frame1.forceUpdate_InfoDescription();
	}
	...
	private JPanel buildDisplayOptionsPanel()
	{
		...
		Utility.buildConstraints(c, 0, ++line, 2, 1, 0, 0);
		label = new JLabel(in_showFeatDescription + ": ");
		gridbag.setConstraints(label, c);
		displayOptsPanel.add(label);
		Utility.buildConstraints(c, 2, line, 1, 1, 0, 0);
		gridbag.setConstraints(featDescriptionShown, c);
		displayOptsPanel.add(featDescriptionShown);
	...
	private JPanel buildLanguagePanel()
	{
		...
		gridbag.setConstraints(label, c);
		langPanel.add(label);
		Utility.buildConstraints(c, 1, 5, 2, 1, 0, 0);
		final int unitSetCount = Globals.getUnitSetCount();
		unitSetNames = new String[unitSetCount];
		for (int i = 0; i < unitSetCount; ++i)
		{
			unitSetNames[i] = Globals.getUnitSet(i).getName();
		}
...
////////////////////
	...
	private JPanel buildOutputPanel()
	{
		...
		outputPanel.add(label);
		Utility.buildConstraints(c,1,10,1,1,0,0);
		skillChoice.setModel(
			new DefaultComboBoxModel(
				new String[] {
					in_skillChoiceNone,
					in_skillChoiceUntrained,
					in_skillChoiceAll }));
		skillChoice.setSelectedIndex(SettingsHandler.getIncludeSkills());
