...
final class PCGVer2Creator implements IOConstants
{
	...
	private void appendAutoSortLines(StringBuffer buffer)
	{
		...
		buffer.append(LINE_SEP);
		buffer.append(TAG_AUTOSORTSKILLS).append(':');
		buffer.append(aPC.isAutoSortSkills() ? "Y" : "N");
		buffer.append(LINE_SEP);
	}
	...
	private void appendOutputSheetsLines(StringBuffer buffer)
	{
		if (SettingsHandler.getSaveOutputSheetWithPC())
		{
			...
			buffer.append(EntityEncoder.encode(SettingsHandler.getSelectedCharacterHTMLOutputSheet()));
			buffer.append(LINE_SEP);
			buffer.append(TAG_PDFOUTPUTSHEET).append(':');
			buffer.append(EntityEncoder.encode(SettingsHandler.getSelectedCharacterPDFOutputSheet()));
			buffer.append(LINE_SEP);
		}
	}
	...
	private void appendHeightLine(StringBuffer buffer)
	{
		...
		buffer.append(aPC.getHeightInInches());
		buffer.append(LINE_SEP);
	}
	...
	private void appendWeightLine(StringBuffer buffer)
	{
		...
		buffer.append(aPC.getWeightInPounds());
		buffer.append(LINE_SEP);
	}
	...
	private void appendAlignmentLine(StringBuffer buffer)
	{
		...
		if (Globals.getGameModeAlignmentText().length() != 0)
		{
			...
			buffer.append(SystemCollections.getShortAlignmentAtIndex(aPC.getAlignment()));
			buffer.append(LINE_SEP);
		}
	}
...
//							aSource.substring(aSource.lastIndexOf('|') + 1)));
		...
		for (Iterator i = aPC.getLevelInfo().iterator(); i.hasNext();)
		{
			...
			if (aClass != null)
			{
				...
				appendSpecials(buffer, specials.get(aClass.getKeyName() + TAG_SPECIALTY + lvl), TAG_SPECIALTIES, TAG_SPECIALTY, lvl);
				appendSpecials(buffer, specials.get(aClass.getKeyName() + TAG_SA + lvl), TAG_SPECIALABILITIES, TAG_SA, lvl);
				if (lvl == 0)
				{
					appendSpecials(buffer, specials.get(aClass.getKeyName() + TAG_SA + (lvl-1)), TAG_SPECIALABILITIES, TAG_SA, -1);
				}
				//
			...
			int sp = pcl.getSkillPointsGained(false);
			if (sp != 0)
			{
				buffer.append('|').append(TAG_SKILLPOINTSGAINED).append(":" + sp);
			}

			sp = pcl.getSkillPointsRemaining(false);
			if (sp != 0)
			{
...
//			buffer.append(')');
	...
	private static void appendSpecials(StringBuffer buffer,
		List specials, String tag_group, String tag_item, int lvl)
	{
