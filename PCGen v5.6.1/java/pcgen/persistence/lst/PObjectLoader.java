...
public final class PObjectLoader
{
	 ...
	 * Private since instances need never be created
	 * and  API methods are public and static
	 */
	...
	/**
	 * This method parses a line in an LST file containing the source
	 * information into the map form used by a PObject.
	 * @param sourceLine String LST formatted source information line
	...
	/**
	 * This method parses a Tag and its value from an LST formatted file
	 * (or other source, such as an LST editor)
	 * It applies the value of the tag to the provided PObject.
	 *
	 ...
	 * (or other source, such as an LST editor)
	 * It applies the value of the tag to the provided PObject
	 * If a level is given, the tag value is applied to the
	 * object at the specified level [as appropriate for the tag]
	 * A level of -9 or lower is treated as "at all levels."
	 *
	 * @param obj PObject which the tag will be applied to
	 * @param aTag String tag and value to parse
	...
	private static boolean parseCTagLevel(PObject obj, String aTag, int anInt)
	{
		...
		else if (aTag.startsWith("CHANGEPROF:"))
		{
	...
	private static boolean parseTTagLevel(PObject obj, String aTag, int anInt)
	{
		...
		{
			obj.setTempDescription(pcgen.io.EntityEncoder.decode(aTag.substring(9)));
		}
		else
		{
			return false;
	...
	private static boolean parseVTagLevel(PObject obj, String aTag, int anInt)
	{
		...
		{
			obj.addVirtualFeat(aTag.substring(6));
		}
	...
	private static List createSpellList(final String sourceLine)
	{
		...
		// Search for pre-reqs.  Tokenize by "|" and look for strings starting with PRE
		StringBuffer preTagBuffer = new StringBuffer();
		StringTokenizer st = new StringTokenizer(sourceLine, "|");
		while (st.hasMoreTokens())
		{
			String token = st.nextToken();
			if (token.startsWith("PRE")) //check as mixed case.  Wouldn't want to mistake Prestidigitation as a PRE tag.
			{
		...
		if (spellSrcBuffer.length() > 0) //This one has better be or why are we here!
		{
			spellSrc = spellSrcBuffer.substring(0, spellSrcBuffer.length()-1);
		}
		
		//The following is deprecated but will remain here for backwards compatability
		//In the future the "[]" notation to delimite PRExxx tags for the SPELL tag should not be used
