 ...
 *
 * $Id: Skill.java,v 1.1 2006/02/21 01:33:15 vauchers Exp $
 */
...
public final class Skill extends PObject
{
	...
	public Integer modifier()
	{
		...
		/*
		// This is should be handled in the data files
		if (Globals.checkRule("SYS_CIP"))
		{
		...
		*/
		...
		if (aCheck != ACHECK_NONE)
		{
			...
			if ((aCheck == ACHECK_WEIGHT) && Globals.checkRule("SYS_WTPSK"))
			{
			...
			{
				if ((aCheck != ACHECK_NONPROF) && Globals.checkRule("SYS_LDPACSK"))
				{
					...
					switch (load)
					{
						case Constants.MEDIUM_LOAD:
							minBonus = (aCheck == ACHECK_DOUBLE) ? -6 : -3;
							break;

						case Constants.HEAVY_LOAD:
						case Constants.OVER_LOAD:
							minBonus = (aCheck == ACHECK_DOUBLE) ? -12 : -6;
							break;
				...
				for (Iterator e = itemList.iterator(); e.hasNext();)
				{
					...
					*/
					...
					{
						maxBonus += 2 * eq.acCheck().intValue();
					}
				}
			}
	...
	public void setZeroRanks(PCClass aClass)
	{
		...
		final String aResp = modRanks(-curRankCost, aClass, false);
		if (aResp.length() != 0)
		{
			// error or debug? XXX
	...
	private double modRanks2(double g, int idx, String bSkill)
	{
		...
		if (!aPC.isImporting())
		{
			if (choiceString.length() > 0 && !Utility.doublesEqual(g, 0) && !Utility.doublesEqual(curRank, (int) newRank))
			{
