	...
	int getChoice(int pool, Equipment parent, boolean bAdd)
	{
		...
		while (!forEqBuilder && aTok.hasMoreTokens())
		{
			...
			else if (aString.startsWith("TYPE=") || aString.startsWith("TYPE."))
			{
				...
				{
					for (Iterator e = EquipmentList.getEquipmentList().iterator(); e.hasNext();)
					{
...
*/
	...
	public List getActiveBonuses()
	{
		...
		{
			BonusObj aBonus = (BonusObj) ab.next();
			// Don't need a passesPreReq check as it wouldn't have
			// been applied if it didn't already pass
	...
	public BigDecimal addItemCosts(String bonusType, int qty, Equipment parent)
	{
		double val = 0;
		for (Iterator ab = getBonusList().iterator(); ab.hasNext();)
		{
			...
			if (aBonus.getBonusName().equals(bonusType))
			{
				...
				while (aTok.hasMoreTokens())
				{
					String sub_type = aTok.nextToken();
				...
				if (meetsAll)
					val += bonusTo(bonusType, typeString, parent);
			}
		}
