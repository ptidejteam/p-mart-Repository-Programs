 ...
 *
 * $Id: Feat.java,v 1.1 2006/02/21 01:33:15 vauchers Exp $
 */
...
public final class Feat extends PObject implements HasCost
{
	...
	{
		if (SettingsHandler.useFeatBenefits() && benefit != null && benefit.length() > 1)
		{
			return benefit;
		}
	...
	{
		return aPC.getVariableValue(cost, "").doubleValue();
	}
	...
	/**
	 * Bypass normal prerequisite checks if BypassFeatPreReqs is true
	 * Otherwise, use the prerequisite checks from the parent object.
	 */
	public boolean passesPreReqToGainForList(PlayerCharacter aPC, PObject aObj, List argList)
	{
		return (Globals.checkRule("FEATPRE") || PrereqHandler.passesPreReqToGainForList(this, aPC, aObj, argList));
	}
	...
	void modAdds(boolean addIt)
	{
		...
		// BUG: currently adds 2 items from the list. --- arcady 10/12/2001
		{
			...
			Logging.debugPrint("WARNING: ADD:"+addType+"|"+addSec+" not handled for feat "+name);
		...
		if (!isImporting)
		{
			for (Iterator e = templates.iterator(); e.hasNext();)
			{
				...
				if (templateName.length() != 0)
				{
					...
					Globals.getCurrentPC().addTemplateNamed(templateName);
				}
			}
		}
		return newTemplates;
