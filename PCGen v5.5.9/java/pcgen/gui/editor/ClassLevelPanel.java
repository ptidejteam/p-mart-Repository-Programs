 ...
 *
 * @(#) $Id: ClassLevelPanel.java,v 1.1 2006/02/21 01:28:20 vauchers Exp $
 */
...
*/		tagList.setSelectedIndex(0);
		...
		addBtn.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent evt)
			{
				...
				{
					Object selectedValue = GuiFacade.showInputDialog(null, "Enter the value for " + tag, Constants.s_APPNAME, GuiFacade.INFORMATION_MESSAGE, null, null, "");
		...
		if ((x >= 0) && (x < levelTagList.size()))
		{
			...
			{ // this is a pre-existing one that needs to be removed
				...
				if (!bRemoved)
				{
					...
					GuiFacade.showMessageDialog(null, "This tag " + lt.getTag() + ":" + lt.getValue() + " needs to be hand-deleted from customClasses.lst", Constants.s_APPNAME, GuiFacade.ERROR_MESSAGE);
					return;
				}
			}
	...
	public void updateView(PObject po)
	{
		...
		while (flag)
		{
			String src = obj.getSpellListItemAsString(index++);
			if (src != null)
			{
				int y = src.indexOf('|');
...
System.err.println("lev="+lev+" src="+src);
...
//		}
	...
	public void updateData(PObject po)
	{
		...
		{
			Logging.errorPrint(
					"Invalid PObject passed to ClassLevelPanel.updateData()!");
			return;
		}
		...
		try
		{
			...
			if( sourceFile==null )
			{
				...
				sourceFile = new URL(
						"file:"
						+ CustomData.customClassFilePath(true)).toString();
				obj.setSourceFile(sourceFile);
				obj.setSourceCampaign(customCampaign);
			}

			// Create the custom source entry
			CampaignSourceEntry tempSource = new CampaignSourceEntry(
					customCampaign, sourceFile);
