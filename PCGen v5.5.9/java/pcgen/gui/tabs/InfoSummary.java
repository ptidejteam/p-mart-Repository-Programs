 ...
 *
 * $Id: InfoSummary.java,v 1.1 2006/02/21 01:28:27 vauchers Exp $
 */
 ...
 */
public final class InfoSummary extends FilterAdapterPanel
{
	private static final String NONABILITY = "Cannot modify a nonability";
	private static final String STAT_UPPER_LIMIT = "Cannot raise stat above "; // intentionally a fragment
	private static final String IN_PURCHASE_MODE = " in Purchase Mode"; // intentionally a fragment
	private static final String STAT_LOWER_LIMIT = "Cannot lower stat below "; // intentionally a fragment
	...
	private ActionListener levelCmdListener = new ActionListener()
	{
		public void actionPerformed(ActionEvent evt)
		{
			...
			if (numLevels <= 0)
			{
				GuiFacade.showMessageDialog(null, "Number of levels must be positive.", Constants.s_APPNAME, GuiFacade.ERROR_MESSAGE);
				return;
			}
			PCClass pcClass = (PCClass) classComboBox.getSelectedItem();
			if (pcClass == null)
			{
				GuiFacade.showMessageDialog(null, "You must select a class.", Constants.s_APPNAME, GuiFacade.ERROR_MESSAGE);
				return;
			}
			if (!pcClass.isQualified())
			{
				GuiFacade.showMessageDialog(null, "You are not qualified to take the class " + pcClass.getName() + ".", Constants.s_APPNAME, GuiFacade.ERROR_MESSAGE);
				return;
			}
	...
	{
		return alignmentStrings = SystemCollections.getAlignmentListStrings(true);
	}
	...
	private void initComponents()
	{
		...
		TableColumn col = statTable.getColumnModel().getColumn(STAT_COLUMN);
		int width = Globals.getCustColumnWidth("AbilitiesS", STAT_COLUMN);
		if (width == 0)
		{
		...
		col.addPropertyChangeListener(new ResizeColumnListener(statTable, "AbilitiesS", STAT_COLUMN));
		statTable.setColAlign(STAT_COLUMN, SwingConstants.LEFT);

		col = statTable.getColumnModel().getColumn(BASE_COLUMN);
		width = Globals.getCustColumnWidth("AbilitiesS", BASE_COLUMN);
		if (width == 0)
		{
		...
		col.addPropertyChangeListener(new ResizeColumnListener(statTable, "AbilitiesS", BASE_COLUMN));
		statTable.setColAlign(BASE_COLUMN, SwingConstants.CENTER);

		col = statTable.getColumnModel().getColumn(RACE_COLUMN);
		width = Globals.getCustColumnWidth("AbilitiesS", RACE_COLUMN);
		if (width == 0)
		{
		...
		col.addPropertyChangeListener(new ResizeColumnListener(statTable, "AbilitiesS", RACE_COLUMN));
		statTable.setColAlign(RACE_COLUMN, SwingConstants.CENTER);

		col = statTable.getColumnModel().getColumn(OTHER_COLUMN);
		width = Globals.getCustColumnWidth("AbilitiesS", OTHER_COLUMN);
		if (width == 0)
		{
			col.setPreferredWidth(20);
		}
		...
		col.addPropertyChangeListener(new ResizeColumnListener(statTable, "AbilitiesS", OTHER_COLUMN));
		statTable.setColAlign(OTHER_COLUMN, SwingConstants.CENTER);

		col = statTable.getColumnModel().getColumn(TOTAL_COLUMN);
		width = Globals.getCustColumnWidth("AbilitiesS", TOTAL_COLUMN);
		if (width == 0)
		{
			col.setPreferredWidth(20);
		}
		...
		col.addPropertyChangeListener(new ResizeColumnListener(statTable, "AbilitiesS", TOTAL_COLUMN));
		statTable.setColAlign(TOTAL_COLUMN, SwingConstants.CENTER);

		col = statTable.getColumnModel().getColumn(MOD_COLUMN);
		width = Globals.getCustColumnWidth("AbilitiesS", MOD_COLUMN);
		if (width == 0)
		{
			col.setPreferredWidth(20);
		}
		...
		col.addPropertyChangeListener(new ResizeColumnListener(statTable, "AbilitiesS", MOD_COLUMN));
		statTable.setColAlign(MOD_COLUMN, SwingConstants.CENTER);
...
/////////////////////////
	...
	private void statTableMouseClicked(MouseEvent evt)
	{
		...
		if (selectedStat < 0 || selectedStat >= Globals.s_ATTRIBSHORT.length)
		{
			...
			case INC_COLUMN:
				{
					...
					final int pcPlayerLevels = aPC.getTotalPlayerLevels();
					...
					if (aPC.isNonAbility(selectedStat))
					{
						...
						{
							GuiFacade.showMessageDialog(null, NONABILITY, Constants.s_APPNAME, GuiFacade.ERROR_MESSAGE);
						}
					}
					else if (stat >= aStat.getMaxValue())
					{
						if (!SettingsHandler.isExpertGUI())
						{
							GuiFacade.showMessageDialog(null, STAT_UPPER_LIMIT + Integer.toString(aStat.getMaxValue()), Constants.s_APPNAME, GuiFacade.ERROR_MESSAGE);
						}
					}
					else if ((pcPlayerLevels < 2) && (stat >= SettingsHandler.getPurchaseScoreMax()) && isPurchaseMode)
					{
						if (!SettingsHandler.isExpertGUI())
						{
							GuiFacade.showMessageDialog(null, STAT_UPPER_LIMIT + SettingsHandler.getPurchaseScoreMax() + IN_PURCHASE_MODE, Constants.s_APPNAME, GuiFacade.ERROR_MESSAGE);
						}
					}
					else
					{
			...
			case DEC_COLUMN:
				{
					...
					if (stat <= aStat.getMinValue())
					{
						if (!SettingsHandler.isExpertGUI())
						{
							GuiFacade.showMessageDialog(null, STAT_LOWER_LIMIT + Integer.toString(aStat.getMinValue()), Constants.s_APPNAME, GuiFacade.ERROR_MESSAGE);
						}
					}
					...
					else if ((pcPlayerLevels < 2) && (stat <= minPurchaseScore) && isPurchaseMode)
					{
						if (!SettingsHandler.isExpertGUI())
						{
							GuiFacade.showMessageDialog(null, STAT_LOWER_LIMIT + minPurchaseScore + IN_PURCHASE_MODE, Constants.s_APPNAME, GuiFacade.ERROR_MESSAGE);
						}
					}
					else
					{
		...
		if (makeChange)
		{
			final int preIncHpMod = (int)aPC.getStatBonusTo("HP", "BONUS");
			...
			if ((int) aPC.getStatBonusTo("HP", "BONUS") != preIncHpMod)
			{
	...
	private void updatePool(boolean checkPurchasePoints)
	{
		...
		// monster levels XXX
		int pcPlayerLevels = aPC.getTotalPlayerLevels();
		...
		if (SettingsHandler.isPurchaseStatMode())
		{
			...
			poolText.setText(bString + " / " + availablePool);
			...
			if (checkPurchasePoints)
			{
				...
				//
				if ((pcPlayerLevels < 2) && (availablePool > 0) && (usedStatPool > availablePool))
				{
					GuiFacade.showMessageDialog(null, "You have exceded the maximum points of " + availablePool + " as specified by the method \"" + SettingsHandler.getPurchaseModeMethodName() + "\"", Constants.s_APPNAME, GuiFacade.INFORMATION_MESSAGE);
				}
			}
	...
	private void updateRace()
	{
		if (raceComboBox.getSelectedItem() != null)
		{
			...
			if (!r.equals(oldRace))
			{
				...
				PCGen_Frame1.forceUpdate_PlayerTabs();
				PCGen_Frame1.forceUpdate_InfoRace();
				...
				final boolean updateEquip = (((oldRace != null) && (oldRace.getMonsterClass(false) != null)) || (r.getMonsterClass(false) != null));
				if (updateEquip)
				{
	...
	private void refreshDisplay()
	{
		...
		if (alignmentStrings.length != SystemCollections.getUnmodifiableAlignmentList().size()) // - 1 DRH
		{
			rebuild = true;
		}
		else
		{
			String[] al = SystemCollections.getAlignmentListStrings(true);
		...
		//
		if ((aPC.getTotalLevels() == 0) || SettingsHandler.isMonsterDefault())  // new PC?
		{
			classComboBox.setSelectedItem(null);
		}
		else if (aPC.getLevelInfoSize() != 0)
		{
		...
		{
			GuiFacade.showMessageDialog(null, "What kind of class is this!?.  (Please report this as a bug.)", Constants.s_APPNAME, GuiFacade.ERROR_MESSAGE);
		}
		...
		enableControls(!alignmentComboBox.isVisible() || align != SystemCollections.getIndexOfAlignment(Constants.s_NONE));
		startListeners();
	}
	...
	private void alignmentChanged()
	{
		...
		if (unqualified.length() > 0)
		{
			if (GuiFacade.showConfirmDialog(null, "This will change the following class(es) to ex-class(es):" + Constants.s_LINE_SEP + unqualified.toString(), Constants.s_APPNAME, GuiFacade.OK_CANCEL_OPTION, GuiFacade.QUESTION_MESSAGE) == GuiFacade.CANCEL_OPTION)
			{
				aPC.setAlignment(oldAlignment, false, true);
		...
		needsUpdate = true;
		updateCharacterInfo();
		enableControls(newAlignment != SystemCollections.getIndexOfAlignment(Constants.s_NONE));
	}
	...
	private void setInfoLabelText(Race aRace)
	{
		...
		if ((aRace != null) && !aRace.getName().startsWith("<none"))
		{
			...
			if (aRace.getNumberOfMovements() > 0)
			{
				...
				movelabel.append(' ').append(Globals.displayDistanceInUnitSet(Globals.convertDistanceToUnitSet(aRace.getMovement(0).intValue())));
				...
				for (int i = 1; i < aRace.getNumberOfMovements(); ++i)
				{
					...
					movelabel.append(' ').append(Globals.displayDistanceInUnitSet(Globals.convertDistanceToUnitSet(aRace.getMovement(i).intValue())));
					movelabel.append(Globals.getDistanceUnit());
				}
	...
	private void setStatLabelText()
	{
		...
		if (aPC != null)
		{
			if (Globals.getGameModeACText().length() != 0)
			{
				...
				statBuf.append("<i>Total</i>: ").append(aPC.getACTotal());
			...
			{
				statBuf.append("<b>Total AC:</b> ").append((int) aPC.getTotalBonusTo("COMBAT", "AC"));
				statBuf.append("<br>");
			}
			...
			{
				statBuf.append(" <b>").append(Globals.getGameModeAltHPAbbrev()).append("</b>: ").append(aPC.altHP());
			}
			statBuf.append("<br>");
			...
			for (int z = 0; z < SystemCollections.getUnmodifiableCheckList().size(); ++z)
			{
				...
				statBuf.append(" <i>").append(SystemCollections.getUnmodifiableCheckList().get(z).toString()).append("</i>: ").append(Delta.toString(bonus));
			}
	...
	private void addClass(PCClass theClass, int levels)
	{
		if (Globals.getGameModeAlignmentText().length() != 0)
		{
			...
			{
				GuiFacade.showMessageDialog(null, "You must select an Alignment before adding classes.", Constants.s_APPNAME, GuiFacade.ERROR_MESSAGE);
				return;
			}
		}
		...
		{
			GuiFacade.showMessageDialog(null, "Maximum level is " + theClass.getMaxLevel(), Constants.s_APPNAME, GuiFacade.INFORMATION_MESSAGE);
			return;
		}

		// Check with the user on their first level up
		if (aPC.getTotalLevels() == 0 && levels > 0)
		{

			if (SettingsHandler.isPurchaseStatMode()
				&& SettingsHandler.getPurchaseModeMethodPool() > getUsedStatPool())
			{
					int proceed =
						GuiFacade.showConfirmDialog(
							this,
							PropertyFactory.getString("in_sumPoolWarning"),
							PropertyFactory.getString("in_sumLevelWarnTitle"),
							GuiFacade.YES_NO_OPTION,
							GuiFacade.WARNING_MESSAGE);
					if (proceed != GuiFacade.YES_OPTION)
					{
						return;
					}
			}
			else if (SettingsHandler.isShowWarningAtFirstLevelUp())
			{
				final JCheckBox shouldDisplay =
					new JCheckBox(
						PropertyFactory.getString("in_sumAbilitiesWarningCheckBox"),
						true);
				shouldDisplay.addItemListener(new ItemListener()
				{
					public void itemStateChanged(ItemEvent evt)
					{
						SettingsHandler.setShowWarningAtFirstLevelUp(
							shouldDisplay.isSelected());
					}
				});
				JPanel msgPanel =
					GuiFacade.buildMessageLabelPanel(
						PropertyFactory.getString("in_sumAbilitiesWarning"),
						shouldDisplay);
	...
	private final class StatTableModel extends AbstractTableModel
	{
		...
		public Object getValueAt(int rowIndex, int columnIndex)
		{
			...
			final String aStat = Globals.s_ATTRIBSHORT[rowIndex];
			switch (columnIndex)
			{
				case BASE_COLUMN:
					if (currentPC.isNonAbility(rowIndex))
					{
						return "*";
					}
					return new Integer(currentPC.getStatList().getBaseStatFor(aStat));
				case RACE_COLUMN:
					if (currentPC.isNonAbility(rowIndex))
					{
						return "*";
					...
					int rBonus = (int) currentPC.getRaceBonusTo("STAT", aStat);
				...
				case OTHER_COLUMN:
					...
					int iRace = (int) currentPC.getRaceBonusTo("STAT", aStat);
					return new Integer(currentPC.getStatList().getTotalStatFor(aStat) - currentPC.getStatList().getBaseStatFor(aStat) - iRace);
				case TOTAL_COLUMN:
					if (currentPC.isNonAbility(rowIndex))
					{
						return "*";
					}
					return new Integer(currentPC.getStatList().getTotalStatFor(aStat));
				case MOD_COLUMN:
					if (currentPC.isNonAbility(rowIndex))
					{
						return new Integer(0);
					}
					return new Integer(currentPC.getStatList().getStatModFor(aStat));
				case INC_COLUMN:
		...
		public void setValueAt(Object obj, int rowIndex, int columnIndex)
		{
			if ((rowIndex >= 0) && (rowIndex < aPC.getStatList().getStats().size()) && (columnIndex == 1))
			{
				...
				else if ((pcPlayerLevels < 2) && SettingsHandler.isPurchaseStatMode())
				{
					final int maxPurchaseScore = SettingsHandler.getPurchaseScoreMax();
					if (statVal > maxPurchaseScore)
					{
						GuiFacade.showMessageDialog(null, STAT_UPPER_LIMIT + maxPurchaseScore + IN_PURCHASE_MODE, Constants.s_APPNAME, GuiFacade.ERROR_MESSAGE);
						return;
					}
	...
	private static final class ClassModel extends AbstractTableModel
	{
			...
			switch (columnIndex)
			{
				...
				case COL_CLASSNAME:
					if (currentPC != null)
					{
						PCClass aClass = null;
						int lvl = 0;
						for (int idx = 0; idx < currentPC.getLevelInfoSize(); ++idx)
						{
							final String classKeyName = currentPC.getLevelInfoClassKeyName(idx);
							aClass = currentPC.getClassKeyed(classKeyName);
							if ((aClass != null) && !shouldDisplayThis(aClass))
							{
								continue;
							}
							if (rowIndex-- == 0)
							{
								...
								lvl = currentPC.getLevelInfoClassLevel(idx);
								break;
							}
						}
				...
				case COL_SRC:
					if (currentPC != null)
					{
						...
						for (int idx = 0; idx < currentPC.getLevelInfoSize(); ++idx)
						{
							final String classKey = currentPC.getLevelInfoClassKeyName(idx);
							aClass = currentPC.getClassKeyed(classKey);
							if ((aClass != null) && !shouldDisplayThis(aClass))
							{
								continue;
							}
	...
	public void addMonsterHD(int direction)
	{
		...
		if (numHD <= 0)
		{
			GuiFacade.showMessageDialog(null, "Number of hit dice must be positive.", Constants.s_APPNAME, GuiFacade.ERROR_MESSAGE);
			return;
		}
		numHD *= direction;
		...
		if (monsterClass != null)
		{
			...
			if (aClass != null)
			{
				...
				if (numHD < 0)
				{
					...
					{
						GuiFacade.showMessageDialog(null, "Cannot lower hit dice any more.", Constants.s_APPNAME, GuiFacade.ERROR_MESSAGE);
						return;
					}
				}
