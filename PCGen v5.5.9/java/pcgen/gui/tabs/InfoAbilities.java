...
public final class InfoAbilities extends JPanel
{
	...
	private static AddSpecialAbility asa;
	...
	private void addSpecialAbility()
	{
		...
		asa = new AddSpecialAbility(aPC, this);
/*		Object selectedValue = GuiFacade.showInputDialog(null, PropertyFactory.getString("in_iaSpeAbTip"), Constants.s_APPNAME, GuiFacade.INFORMATION_MESSAGE, null, null, "");
...
*/
	...
	public void updateCharacterInfo()
	{
		...
		List specialAbilities = aPC.getSpecialAbilityTimesList();
	...
	private void racialLanguageSelectPressed()
	{
		if (aPC != null)
		{
			...
			for (Iterator i = Globals.getLanguageList().iterator(); i.hasNext();)
			{
				...
				if (aLang != null)
				{
					if (PrereqHandler.passesPreReqToGainForList(aLang, aPC, null, aLang.getPreReqList()))
					{
			 ...
			 * */
			if (aPC.getTotalLevels() >1)
			{
				if (Globals.checkRule("INTBONUSLANG")) //$NON-NLS-1$
				{
	...
	private List getOptionalWeaponProficiencies()
	{
		if (aPC != null)
		{
			...
			final List pcDomains = aPC.getCharacterDomainList();
	...
	private void weaponSelectPressed()
	{
		if (aPC != null)
		{
			...
			{
				GuiFacade.showMessageDialog(null, PropertyFactory.getString("in_iaNoOptProfs"), Constants.s_APPNAME, GuiFacade.INFORMATION_MESSAGE);
				return;
			}
			...
			for (; ;)
			{
				...
				else
				{
					...
					for (; ;)
					{
						Object selectedValue = GuiFacade.showInputDialog(null, PropertyFactory.getString("in_iaMultiChoice1") + Constants.s_LINE_SEP + PropertyFactory.getString("in_iaMultiChoice2"), Constants.s_APPNAME, GuiFacade.INFORMATION_MESSAGE, null, bonusCategory.toArray(), bonusCategory.get(selIdx));
						if (selectedValue != null)
						{
				...
				else
				{
					...
					if (profBonusObject instanceof PCClass)
					{
						...
						((PCClass) profBonusObject).getChoices("WEAPONPROF|1|" + pcgen.core.utils.Utility.join(profWeapons, "[WEAPONPROF]|") + PropertyFactory.getString("in_proficiency"), new ArrayList());
					}
					else if (profBonusObject instanceof Race)
					{
						profWeapons = ((Race) profBonusObject).getWeaponProfBonus();
						((Race) profBonusObject).getChoices("WEAPONPROF|1|" + pcgen.core.utils.Utility.join(profWeapons, "[WEAPONPROF]|") + PropertyFactory.getString("in_proficiency"), new ArrayList());
					}
					else if (profBonusObject instanceof PCTemplate)
					{
						profWeapons = ((PCTemplate) profBonusObject).getWeaponProfBonus();
						((PCTemplate) profBonusObject).getChoices("WEAPONPROF|1|" + pcgen.core.utils.Utility.join(profWeapons, "[WEAPONPROF]|") + PropertyFactory.getString("in_proficiency"), new ArrayList());
					}
				}
