...
public class InfoDomain extends FilterAdapterPanel
{
	...
	private final class ClassPopupMenu extends JPopupMenu
	{
		...
		ClassPopupMenu(JTableEx treeTable)
		{
			if (treeTable == deityTable)
			{
				 ...
				 */
				ClassPopupMenu.this.add(createAddMenuItem(PropertyFactory.getString("in_select"), "shortcut EQUALS"));
				this.addSeparator();
			...
			else // domainTable
			{
				 ...
				 * get "control PLUS" to function on standard US keyboard with Windows 98
				 */
				ClassPopupMenu.this.add(createRemoveMenuItem(PropertyFactory.getString("in_select"), "shortcut EQUALS"));
			}
		}
	}
	...
	private void buildDomainLists()
	{
		...
		domTotal.setText(Integer.toString(aPC.getMaxCharacterDomains()));
		// use star (*) to identify which are chosen in the table
		domChosen.setText(Integer.toString(aPC.getCharacterDomainUsed()) + "*");
	 ...
	 */
	private static final void addUnfilteredDomains(final List availDomainList, final Deity pcDeity)
	{
		availDomainList.clear();
	...
	private final void selectDomainIndex(int selectedRow)
	{
		...
		{
			GuiFacade.showMessageDialog(null, "Not allowed to choose a domain", Constants.s_APPNAME, GuiFacade.INFORMATION_MESSAGE);
			return;
		}
		...
		{
			GuiFacade.showMessageDialog(null, PropertyFactory.getString("in_qualifyMess") + addedDomain.getName(), Constants.s_APPNAME, GuiFacade.INFORMATION_MESSAGE);
			return;
		}

		CharacterDomain aCD = aPC.getCharacterDomainForDomain(addedDomain.getName());
		if (aCD == null)
		{
		...
		if (aPC.getCharacterDomainUsed() >= aPC.getMaxCharacterDomains())
		{
			GuiFacade.showMessageDialog(null, "No more domains allowed", Constants.s_APPNAME, GuiFacade.INFORMATION_MESSAGE);
			return;
		}
		...
		{
			addedDomain.setIsLocked(true);
			aCD.setDomain(addedDomain);
	...
	{
		Object value = deityModel.getValueAt(selectedRow, -1);
		// This happens when you select <None>
		if (!(value instanceof Deity))
		{
			return;
		}

		final Deity aDeity = (Deity) value;
		if (aDeity == null)
		{
			return;
		}
		...
		{
			final ListSelectionModel lsm = deityTable.getSelectionModel();
			GuiFacade.showMessageDialog(null, PropertyFactory.getString("in_reqMess") + aDeity.getName() + ".", Constants.s_APPNAME, GuiFacade.INFORMATION_MESSAGE);
			lsm.clearSelection();
			return;
		}
		...
		{
			final int areYouSure = GuiFacade.showConfirmDialog(null, PropertyFactory.getString("in_confDomLost1") + " " + aDeity.getName() + System.getProperty("line.separator") + PropertyFactory.getString("in_confDomLost2"), Constants.s_APPNAME, GuiFacade.OK_CANCEL_OPTION);
			if (areYouSure != GuiFacade.OK_OPTION)
			{
				return;
			}
		}
	...
	private static final class DeityModel extends AbstractTableModel
	{
		...
		/**
		 * Return the value of a grid cell by using the
		 * information from the global deity list.
		 *
		...
		public Object getValueAt(int row, int col)
		{
			...
			switch (col)
			{
				...
				case 0:
					...
					else if (!aDomain.qualifiesForDomain())
					{
	...
	private void deleteDeityButtonClick()
	{
		...
		if (selectedRow >= 0)
		{
			...
			if (aDeity != null)
			{
				...
				{
					final int areYouSure = GuiFacade.showConfirmDialog(null, PropertyFactory.getString("in_delDeity") + aDeity.getName() + "?", Constants.s_APPNAME, GuiFacade.OK_CANCEL_OPTION);
					if (areYouSure != GuiFacade.OK_OPTION)
					{
						return;
					}
				...
				{
					GuiFacade.showMessageDialog(null, PropertyFactory.getString("in_domIDEr4"), Constants.s_APPNAME, GuiFacade.ERROR_MESSAGE);
				}
			}
		}
	}
