...
public final class Kit extends PObject implements Comparable
{
	...
	public void addKitSkills(List thingsToAdd, List warnings)
	{
		...
		if ((aList != null) && (aList.size() != 0))
		{
			...
			for (int i = 0, x = aList.size(); i < x; ++i)
			{
				...
				if (!PrereqHandler.passesPreReqToGainForList(this, aPC, this, kSkill.getPrereqs()))
				{
					continue;
				}
	...
	public void addKitFeats(List thingsToAdd, List warnings)
	{
		...
		if ((aList != null) && (aList.size() != 0))
		{
			...
			for (int i = 0, x = aList.size(); i < x; ++i)
			{
				...
				//
				if (!PrereqHandler.passesPreReqToGainForList(this, aPC, this, kFeat.getPrereqs()))
				{
					continue;
				}
	...
	public void addKitProfs(List thingsToAdd, List warnings)
	{
		...
		if ((aList != null) && (aList.size() != 0))
		{
			...
			for (int i = 0, x = aList.size(); i < x; ++i)
			{
				...
				if (!PrereqHandler.passesPreReqToGainForList(this, aPC, this, kProf.getPrereqs()))
				{
					continue;
				}
	...
	public void addKitGear(List thingsToAdd, List warnings)
	{
		...
		if ((aList != null) && (aList.size() != 0))
		{
			...
			for (int i = 0, x = aList.size(); i < x; ++i)
			{
				...
				if (!PrereqHandler.passesPreReqToGainForList(this, aPC, this, kGear.getPrereqs()))
				{
					continue;
				}
				...
				Equipment eq = EquipmentList.getEquipmentNamed(eqName);
				if (eq == null)
				{
				...
				if (tryResize)
				{
					eq.resizeItem(SystemCollections.getSizeAdjustmentAtIndex(aPC.sizeInt()).getAbbreviation());
				}
				if (eqMods != null)
				{
					for (int j = 0; j < eqMods.size(); ++j)
					{
						...
						if (idxPipe < 0)
						{
							eqMod = EquipmentList.getQualifiedModifierNamed(eqModName, eqTypeList);
						}
						else
						{
							eqMod = EquipmentList.getQualifiedModifierNamed(eqModName.substring(0, idxPipe), eqTypeList);
						}
						if (eqMod == null)
						{
	...
	public void addKitSpells(List thingsToAdd, List warnings)
	{
		...
		if ((aList != null) && (aList.size() != 0))
		{
			...
			for (int i = 0, x = aList.size(); i < x; ++i)
			{
				...
				if (!PrereqHandler.passesPreReqToGainForList(this, aPC, this, kSpells.getPrereqs()))
				{
					continue;
				}
	...
	private String itemPassesPrereqs(String aString)
	{
		...
		if (PrereqHandler.passesPreReqToGainForList(this, Globals.getCurrentPC(), this, prereqList))
		{
			return itemName;
		}
		return null;
						...
						while (aTok.hasMoreTokens())
						{
							...
							if (aClass != null)
							{
								...
								if (!spells || ((baseStat != null) && (baseStat.length() > 0) && !baseStat.equalsIgnoreCase(Constants.s_NONE)))
								{
									...
									if (PrereqHandler.passesPreReqToGainForList(this, aPC, this, preR))
									{
										return aClass;
									}
								}
		...
		while (aTok.hasMoreTokens())
		{
			...
			else
			{
				...
				else
				{
					...
					al.add(tok);
					if (!PrereqHandler.passesPreReqToGainForList(this, aPC, null, al))
					{
						costFormula = null;
						continue;
					}
