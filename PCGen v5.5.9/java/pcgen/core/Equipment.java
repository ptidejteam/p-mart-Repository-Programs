...
*/
	...
	public String getSpecialProperties()
	{
		...
		// To allow SPROP:xxx|varName
		String sprop = specialProperties;
		final int pos_pipe = sprop.indexOf('|');
		final int pos_perc = sprop.indexOf('%');
		if ((pos_pipe >= 0) && (pos_perc < 0))
		{
			Logging.errorPrint("Bad SPROP: tag '" + sprop + "'");
		}
		else
		{
			if (pos_pipe > 0 && pos_perc > 0)
			{
				StringTokenizer pipeTok = new StringTokenizer(sprop.substring(pos_pipe + 1),"|");
				StringTokenizer perTok = new StringTokenizer(sprop.substring(0, pos_pipe), "%", true);
				StringBuffer newAbility = new StringBuffer();
				while (perTok.hasMoreTokens())
				{
					String perString = perTok.nextToken();
					if ("%".equals(perString))
					{
						...
						final int sInt = getVariableValue(pipeTok.nextToken(), "", "", true).intValue();
						newAbility.append(sInt);
					}
					else
					{
						newAbility.append(perString);
					}
				}
				sprop = newAbility.toString();
			}
		}
		sp.append(sprop);
...
// --Recycle Bin STOP (7/17/03 11:56 PM)
	...
	public BigDecimal getCost()
	{
		...
		for (Iterator e = eqModifierList.iterator(); e.hasNext();)
		{
			...
			c = c.add(aEqMod.addItemCosts("ITEMCOST", iCount, typeList()));
		}
	...
	private void updateContainerCapacityString()
	{
		...
		if (containerWeightCapacity.intValue() != -1)
		{
			tempStringBuffer.append(containerWeightCapacity).append(' ').append(Globals.getWeightUnit());
			comma = true;
		}
	...
	public void updateContainerContentsString()
	{
		...
		{
			tempStringBuffer.append(getContainedWeight(true)).append(' ').append(Globals.getWeightUnit());
		} else {
...
/*		SizeAdjustment saSize = Globals.getSizeAdjustmentNamed(getSize());
		...
		return Globals.convertWeightToUnitSet(aWeight);
	}
	...
	private String getDamage(boolean bPrimary)
	{
		...
		return adjustDamage(baseDamage, SystemCollections.getSizeAdjustmentAtIndex(iMod).getAbbreviation());
	}
...
//	}
	 ...
	 */
	public String profName(final int hands)
	{
	 ...
	 *
	 * @return	List of virtual feats this item has
	 */
	...
	public double bonusTo(String aType, String aName, Object anObj, boolean bPrimary)
	{
		...
		if (Globals.checkRule("SYS_35WP"))
		{
			...
			super.bonusTo(aType, aName, this, aList);
		}
	...
	public Float getVariableValue(String aString, String src, String subSrc, boolean bPrimary)
	{
		...
		for (int i = 0; i < aString.length(); ++i)
		{
			...
			if (i == aString.length() - 1 || (delimiter.lastIndexOf(aString.charAt(i)) > -1 && valString.length() > 1) || (valString.length() > 3 && (valString.endsWith("MIN") || valString.endsWith("MAX") || valString.endsWith("REQ"))))
			{
				...
				else if ("WT".equals(valString))
				{
					...
					else
					{
						...
						{
							Float unitWeight = new Float(weightInPounds / getBaseQty());
							valString = unitWeight.toString();
						}
						else
						{
							valString = String.valueOf(weightInPounds);
						}
				...
				// This is just to see if it's a double that has
				// already been parsed
				// If not, try to parse it on the PC level
				try
				{
				...
				{
					valString = Globals.getCurrentPC().getVariableValue(valString, src).toString();
				}
				try
				{
	...
	private void cleanTypes()
	{
		...
		while (aTok.hasMoreTokens())
		{
			...
			{
				if (aString.equalsIgnoreCase(SystemCollections.getSizeAdjustmentAtIndex(i).getName()))
				{
					break;
				}
			}
