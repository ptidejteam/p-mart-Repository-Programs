 ...
 *
 * $Id: PCClass.java,v 1.1 2006/02/21 01:27:59 vauchers Exp $
 */
...
//			aClass.setSubClassString(getSubClassString());
	...
	void addLevel(boolean argLevelMax, boolean bSilent)
	{
		...
		if (!aPC.hasDomainSource("PCClass", getName(), newLevel))
		{
			...
			{
				aPC.addDomainSource("PCClass", getName(), newLevel, 1);
			}
		}
...
/*				if (total % 3 == 2)
	...
	{
		List aList = new LinkedList();
		if (vFeatMap.containsKey(String.valueOf(aLevel)))
		{
			aList.addAll((List) vFeatMap.get(String.valueOf(aLevel)));
		}
		super.addVirtualFeat(aString, aList);
		vFeatMap.put(String.valueOf(aLevel), aList);
	}
