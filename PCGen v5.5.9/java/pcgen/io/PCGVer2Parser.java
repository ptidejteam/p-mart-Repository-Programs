...
final class PCGVer2Parser implements PCGParser, IOConstants
{
	...
	private void parseHeightLine(String line)
	{
		try
		{
			aPC.setHeightInInches(Integer.parseInt(line.substring(TAG_HEIGHT.length() + 1)));
		}
		catch (NumberFormatException nfe)
		{
	...
	private void parseWeightLine(String line)
	{
		try
		{
			aPC.setWeightInPounds(Integer.parseInt(line.substring(TAG_WEIGHT.length() + 1)));
		}
		catch (NumberFormatException nfe)
		{
	...
	private void parseFeatLine(String line)
	{
		...
		if (it.hasNext())
		{
			...
			//Using non-aggregate because when using aggregate, we get clones
			//of the PCs actual feats, which don't get saved or preserved
			aFeat = aPC.getFeatNonAggregateKeyed(featKey);
			if (aFeat != null)
			{
	...
	private void parseVFeatLine(String line)
	{
		...
		if (it.hasNext())
		{
			...
			aPC.addVirtualFeat(featKey, aPC.getVirtualFeatList());
			aFeat = PlayerCharacter.getFeatNamedInList(aPC.getVirtualFeatList(), featKey);
	...
	private void parseEquipmentLine(String line)
	{
		...
		if (aEquip == null)
		{
			...
			//
			aEquip = EquipmentList.getEquipmentKeyed(itemKey);
			if (aEquip != null)
			{
				// standard item
			...
			if (line.indexOf(TAG_CUSTOMIZATION) >= 0)
			{
				...
				for (Iterator it = tokens.getElements().iterator(); it.hasNext();)
				{
					...
					if (TAG_CUSTOMIZATION.equals(element.getName()))
					{
						...
						{
							final Equipment aEquip2 = EquipmentList.getEquipmentKeyed(baseItemKey);
							if (aEquip2 != null)
							{
	...
	private void parseEquipSetTempBonusLine(String line)
	{
		...
		{
			final String message =
				"Illegal EquipSetTempBonus line ignored: " + line + Constants.s_LINE_SEP +
				"Error: " + pcgpex.getMessage();
			warnings.add(message);
			return;
		}
