...
public class ExperienceAdjusterModel extends Observable {
	 ...
	 */
	public void addExperienceToParty(int groupExperience) {
		LogUtilities.inst().logMessage(ExperienceAdjusterPlugin.LOG_NAME, "Party Awarded " + groupExperience + " Total Experience Split as:");
		for (int i = 0; i < party.size(); i++) {
			Combatant cbt = ((ExperienceListItem)party.get(i)).getCombatant();
			cbt.setXP(cbt.getXP() + (groupExperience/party.size()));
			LogUtilities.inst().logMessage(ExperienceAdjusterPlugin.LOG_NAME, cbt.getName() + ": " + groupExperience/party.size());
		}
	}
	...
	public void updatePartyExperience() {
		...
		experience = new Double(experience * multiplier).intValue();
