...
public class RandomNamePlugin extends GMBPlugin {
	...
	private JMenuItem nameHelpItem = new JMenuItem();
	...
	public void initMenus() {
		...
		nameHelpItem.setMnemonic('R');
		nameHelpItem.setText("Random Name Generator");
		nameHelpItem.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(java.awt.event.ActionEvent evt) {
				helpMenuItem(evt);
			}
		});
		GMBus.send(new HelpMenuItemAddMessage(this, nameHelpItem));
	}
	...
	public void handleMessage(GMBMessage message) {
		if(message instanceof StateChangedMessage) {
			if(GMGenSystemView.getTabPane().getSelectedComponent() instanceof NameGenPanel) {
				nameHelpItem.setEnabled(false);
				nameToolsItem.setEnabled(false);
			}
			else {
				nameHelpItem.setEnabled(true);
