 ...
 *
 * $Id: Feat.java,v 1.1 2006/02/21 01:00:27 vauchers Exp $
 */
 ...
 */
public final class Feat extends PObject
{
	///////////////////////////////////////
	// Static properties
	...
	///////////////////////////////////////
	// Methods - Accessors
	public int getLevelsPerRepIncrease()
	{
	 ...
	 */
	boolean matchesType(String featType)
	{
	...
	void modAdds(boolean addIt)
	{
		...
		else if ("FORCEPOINT".equals(addType))
		{
			if (featType != FEAT_AUTOMATIC)
			{
				...
				if (addIt)
				{
					final int X = aPC.getRawFPoints() + addPoints;
					aPC.setFPoints(X);
				}
				else
				{
	...
	boolean modChoices(boolean addIt, ArrayList availableList, ArrayList selectedList, boolean process)
	{
		...
		if (specialAbilityList != null)
		{
			...
			for (Iterator it = specialAbilityList.iterator(); it.hasNext();)
			{
				...
				if (idx >= 0)
				{
					...
					sa = new SpecialAbility(sb.toString(), sa.getSource(), sa.getDesc());
				}
				aList.add(sa);
			}
		}
	...
	public String getPCCText()
	{
		...
		if (getLevelsPerRepIncrease() != 0)
		{
			txt.append("\tREP:").append(getLevelsPerRepIncrease());
		}
