 ...
 *
 * $Id: PCTemplate.java,v 1.1 2006/02/21 01:00:27 vauchers Exp $
 */
...
//	private String kit = "";
	...
	public final Integer[] getMovementsMult()
	{
	...
	public String getPCCText()
	{
		...
		if (movements != null && movements.length > 0)
		{
			...
			for (int index = 0; index < movements.length; index++)
			{
				...
				if (movementTypes[index] != null && movementTypes[index].length() > 0)
				{
					txt.append(movementTypes[index]).append(",");
				}
				if (movementMultOp[index].length() > 0)
				{
					txt.append(movementMultOp[index]).append(movementsMult[index]);
				}
				else
				{
