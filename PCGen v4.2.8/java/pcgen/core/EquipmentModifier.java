 ...
 *
 * $Id: EquipmentModifier.java,v 1.1 2006/02/21 01:00:27 vauchers Exp $
 */
package pcgen.core;
...
public final class EquipmentModifier extends PObject
{
	...
	private static ArrayList getSpellMetafeats(String listEntry)
	{
	...
	public String toString()
	{
		...
		if (namingOption == NAMINGOPTION_SPELL)
		{
			if (getAssociatedCount() > 0)
			{
				...
				final ArrayList metaFeats = getSpellMetafeats(listEntry);
	...
	String replaceArmorType(ArrayList aTypes)
	{
	...
	int getChoice(int pool, Equipment parent, boolean bAdd)
	{
		...
		while (aTok.hasMoreTokens())
		{
			...
			else if (aString.startsWith("TYPE="))
			{
				...
				{
					Globals.errorPrint("Unknown option in CHOOSE '" + aString + "'");
				}
			}
...
*/
	...
	{
		final ArrayList myBonusList = new ArrayList(super.getBonusList());
	...
	ArrayList getVFeatList()
	{
		if (vFeatList != null)
		{
			...
			{
				ArrayList vFeats = new ArrayList();
