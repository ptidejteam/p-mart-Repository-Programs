 ...
 *
 * $Id: Equipment.java,v 1.1 2006/02/21 01:00:27 vauchers Exp $
 */
...
public final class Equipment extends PObject implements Serializable, EquipmentCollection, Comparable
{
	...
	// The item is held in neither hand and equipped for Rapid Shot
	public static final int EQUIPPED_RAPIDSHOT = 5;
	public static final String EQUIPPED_RAPIDSHOT_STR = "Rapid Shot";
	...
	private Integer defBonus = new Integer(0);
	 ...
	 * Primarily of interest for modern and futuristic games.
	 ...
	 * Primarily of interest for futuristic games.
	 * Defaults to empty string.
	 * /
	...
	*/
	...
	static
	{
		...
		locationStringList[EQUIPPED_RAPIDSHOT] = EQUIPPED_RAPIDSHOT_STR;
	 ...
	 *  Made public visible 02 Dec 2002 for Bug #586332 -- sage_sam
	 *
	 ...
	 */
	public static String getLocationName(int handNumber)
	{
		if ((handNumber < 0) || (handNumber > locationStringList.length))
		{
			return locationStringList[0];
		}
		else
		{
			return locationStringList[handNumber];
		}
	}
	...
	/**
	 *  Gets the defBonus attribute of the Equipment object
	 *
	 *@return    The defBonus value
	 */
	public Integer getDefBonus()
	{
		int dbon = defBonus.intValue() + (int) bonusTo("EQMARMOR", "DEFBONUS", true);
		if (dbon > 100)
		{
			dbon = 100;
		}
	...
	/**
	 *  Sets the defBonus attribute of the Equipment object
	 *
	 *@param  aString  The new defBonus value
	 */
	public void setDefBonus(String aString)
	{
		try
		{
			defBonus = Delta.decode(aString);
		}
		catch (NumberFormatException ignore)
		{
			// ignore
...
//	}
	...
	{
		return ((location >= EQUIPPED_NEITHER) && (location <= EQUIPPED_RAPIDSHOT));
	}
	...
	{
		return (o != null) && (o instanceof Equipment) && ((o == this) || this.keyName.equals(((Equipment) o).keyName));
	}
	...
	ArrayList getVFeatList()
	{
		...
		ArrayList eqModList = getEqModifierList(true);
		if (eqModList.size() != 0)
		{
	...
	public void setIsEquipped(boolean aFlag, int hand)
	{
		...
		if (aFlag)
		{
			if (hand == EQUIPPED_RAPIDSHOT)
			{
				location = EQUIPPED_RAPIDSHOT;
			}
			...
			else if ((isWeapon() && aPC.canEquip("Weapon")) || (isShield() && aPC.canEquip("Shield")))
			{
				...
				if (isWeapon())
				{
					...
					if (location == EQUIPPED_RAPIDSHOT)
					{
						// no extra hands in use
						hand = EQUIPPED_RAPIDSHOT;
					}
			...
			else if (location == EQUIPPED_RAPIDSHOT)
			{
				hand = EQUIPPED_RAPIDSHOT;
			}
			else
	...
	public final void setLocation(int newLocation)
	{
		...
		if (isWeapon())
		{
			...
			else
			{
				...
				// Make sure a two-handed weapon is not equipped to a single hand
				if (((newLocation == EQUIPPED_PRIMARY) || (newLocation == EQUIPPED_SECONDARY) || (newLocation == EQUIPPED_TWO_HANDS)) && Globals.isWeaponTwoHanded(aPC, this, wp))
				{
		...
		if (newLocation == EQUIPPED_RAPIDSHOT)
		{
	...
	{
		final ArrayList eqModList = getEqModifierList(bPrimary);
		final EquipmentModifier eq = getEqModifierKeyed(eqMod.getKeyName(), bPrimary);
		if (eq == null)
		{
			return;
		}
	 ...
	 *
	 *  TODO: Why does this exist? Cut-n-paste code should be refactored away.
	 *
	 *@param  aString  ???
	 *@param  src      ???
	 *@param  subSrc   ???
	...
	public String getItemNameFromModifiers()
	{
		...
		if (baseEquipment != null)
		{
			...
			{
				final int idx = list1.indexOf(e.next());
				if (idx >= 0)
				{
					list1.remove(idx);
				}
			...
			{
				final int idx = list2.indexOf(e.next());
				if (idx >= 0)
				{
					list2.remove(idx);
				}
		...
		final String desc1 = getNameFromModifiers(list1, magic1, "");
		String magic2 = "";
		String desc2 = "";
		if (isDouble())
		{
		...
		final StringBuffer common = new StringBuffer(getNameFromModifiers(comn, magic1 + magic2, eqMaster));
