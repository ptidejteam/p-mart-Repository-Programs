 ...
 *
 * $Id: PObject.java,v 1.1 2006/02/21 01:00:27 vauchers Exp $
 */
package pcgen.core;
...
public class PObject extends Object implements Cloneable, Serializable, Comparable
{
	...
	protected ArrayList variableList = null;
	...
	/**
	 * TODO:This one is getting a bit too 'hacky' and should be rewritten.
	 * @param bonus
	 * @param bonusType
	 */
	final void setBonusStackFor(double bonus, String bonusType)
	{
		...
		// e.g. "COMBAT.AC.Dodge"
		if (bonusType != null && aTok.countTokens() > 2 && !bonusType.endsWith(".STACK"))
		{

			String aString = aTok.nextToken(); // should be bonus category e.g. "COMBAT" XXX Is the intention that this be thrown away?
			aString = aTok.nextToken(); // should be bonus name e.g. "AC" XXX Is the intention that this be thrown away?
	...
	{
		final StringBuffer src = new StringBuffer(source.length());
		//
		// Strip out the desired type
		//
		if (aSource.endsWith(".CLEAR"))
		{
			if (aSource.length() > 6)
			{
				aSource = aSource.substring(0, aSource.length() - 6);
				final StringTokenizer aTok = new StringTokenizer(source, "|", false);
				while (aTok.hasMoreTokens())
				{
					final String aString = aTok.nextToken();
					if (!aString.startsWith(aSource))
					{
						if (src.length() != 0)
						{
							src.append('|');
	...
	private String returnSourceInForm(int sourceDisplay, boolean includePage)
	{
		...
		while (aTok.hasMoreTokens())
		{
			...
			if (aString.startsWith("SOURCEPAGE:"))
			{
				...
				pageString.append(aString.substring(11));
			}
			switch (sourceDisplay)
			{
				...
				case Constants.SOURCELONG:
					if (aString.startsWith("SOURCELONG:") || aString.startsWith("SOURCE:"))
					{
						...
						{
							pageString.setLength(0);
							pageString.append(aString.substring(7));
						}
					}
					break;
...
//					Globals.errorPrint("In PObject.getSource the sourceDisplay " + sourceDisplay + " is unsupported.");
		...
		if (includePage && pageString.length() > 0 && sourceDisplay != Constants.SOURCEWEB)
		{
			if (sString.length() > 0)
			{
				sString.append(", ").append(pageString.toString());
			}
			else
			{
	...
	{
		return returnSourceInForm(Constants.SOURCEPAGE, true);
	}
...
System.err.println("2=" + aBonus.toString());
		...
		{
			System.err.println("error in BONUS: " + aString);
		}
		return aBonus;
	}
*/
