 ...
 *
 * $Id: PCGVer2Creator.java,v 1.1 2006/02/21 01:00:32 vauchers Exp $
 */
...
*/
		...
		//
		for (int i = 0; i < aPC.getLevelInfoSize(); ++i)
		{
			final String classKeyName = aPC.getLevelInfoClassKeyName(i);
			final int lvl = aPC.getLevelInfoClassLevel(i) - 1;
			...
			ArrayList statList = aPC.getLevelInfoModifiedStats(i, true);
			if (statList != null)
			{
			...
			statList = aPC.getLevelInfoModifiedStats(i, false);
			if (statList != null)
			{
				for (int j = 0; j < statList.size(); ++j)
				{
...
//			buffer.append(')');
	...
	private void appendExperienceLine(StringBuffer buffer)
	{
		...
		buffer.append(aPC.getXP()).append(':');
		buffer.append(aPC.getNextLevelXP());
		buffer.append(LINE_SEP);
	}
...
/*
			...
			buffer.append('|');
			buffer.append(TAG_LOCATION).append(':');
			buffer.append(EntityEncoder.encode(aEquip.getLocationString()));
*/

/*
			if (aEquip.isContainer() && (aEquip.getContents().size() > 0))
			{
				buffer.append('|');
...
//                                                  buffer.append(LINE_SEP);
	...
	private static String createComment(String s)
	{
		...
		String work = new String(s + LINE_SEP);
		work = work.replace('\t', ' ');
		work = work.replace('\f', ' ');
