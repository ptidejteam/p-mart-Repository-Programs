 ...
 *
 * @(#) $Id: EditorMainForm.java,v 1.1 2006/02/21 01:00:36 vauchers Exp $
 */
	...
	private void intComponentContents()
	{
		...
		switch (editType)
		{
			...
			case EditorConstants.EDIT_FEAT:
				...
				pnlMainTab.setVisible(((Feat) thisPObject).isVisible());
				pnlMainTab.setMultiples(((Feat) thisPObject).isMultiples());
				pnlMainTab.setStacks(((Feat) thisPObject).isStacks());
				pnlMainTab.setCost(((Feat) thisPObject).getCost());
				pnlMainTab.setSpellLevels(((Feat) thisPObject).getAddSpellLevel());
				pnlMainTab.setRepIncrease(((Feat) thisPObject).getLevelsPerRepIncrease());
			...
			case EditorConstants.EDIT_LANGUAGE:
				...
				//
				for (int i = ((Language) thisPObject).getMyTypeCount(); i > 0;)
				{
					aString = ((Language) thisPObject).getMyType(--i);
					if (!aString.equals(Constants.s_CUSTOM))
					{
						selectedList.add(aString);
						availableList.remove(aString);
			...
			case EditorConstants.EDIT_RACE:
				...
				pnlMainTab.setMonsterClassList(availableList);

				pnlMainTab.setBonusFeats(thisRace.getBonusInitialFeats());
				pnlMainTab.setBonusSkillPoints(thisRace.getBonusSkillsPerLevel());
				pnlMainTab.setCR(thisRace.getCR());
				pnlMainTab.setDisplayName(thisRace.getDisplayName());
				pnlMainTab.setHands(thisRace.getHands());
				pnlMainTab.setHitDiceAdvancement(thisRace.getHitDiceAdvancement(), thisRace.isAdvancementUnlimited());
				pnlMainTab.setLegs(thisRace.getLegs());
				pnlMainTab.setLevelAdjustment(thisRace.getLevelAdjustmentFormula());
				pnlMainTab.setMonsterClass(thisRace.getMonsterClass(false));
				pnlMainTab.setMonsterLevel(thisRace.getMonsterClassLevels(false));
				pnlMainTab.setRaceSize(thisRace.getSize());
				pnlMainTab.setReach(thisRace.getReach());
				pnlMainTab.setSkillMultiplier(thisRace.getInitialSkillMultiplier());
				pnlMainTab.setHitDiceNumber(thisRace.hitDice(false));
				pnlMainTab.setHitDiceSize(thisRace.getHitDiceSize(false));
			...
			case EditorConstants.EDIT_SPELL:
				//pnlMainTab.setDescriptionText(thisPObject.getDescription());	// don't want PI here
				//pnlMainTab.setDescIsPI(thisPObject.getDescIsPI());
				pnlMainTab.setSpellInfo((Spell) thisPObject);

				Hashtable lvlInfo = ((Spell) thisPObject).getLevelInfo();
				//
				// Initialize the contents of the available and selected domains lists
				//
				int iCount = 0;
			...
			case EditorConstants.EDIT_TEMPLATE:
				...
				//
				for (int i = ((PCTemplate) thisPObject).getMyTypeCount(); i > 0;)
				{
					aString = ((PCTemplate) thisPObject).getMyType(--i);
					if (!aString.equals(Constants.s_CUSTOM))
					{
						selectedList.add(aString);
						availableList.remove(aString);
				...
				pnlMainTab.setIsRemovable(((PCTemplate) thisPObject).isRemovable());
				pnlMainTab.setGenderLock(((PCTemplate) thisPObject).getGenderLock());
				pnlMainTab.setVisible(((PCTemplate) thisPObject).isVisible());
				pnlMainTab.setSubRegion(((PCTemplate) thisPObject).getSubRegion());
				pnlMainTab.setSubRace(((PCTemplate) thisPObject).getSubRace());
				pnlMainTab.setBonusSkillPoints(((PCTemplate) thisPObject).getBonusSkillsPerLevel());
				pnlMainTab.setNonProficiencyPenalty(((PCTemplate) thisPObject).getNonProficiencyPenalty());
				pnlMainTab.setBonusFeats(((PCTemplate) thisPObject).getBonusInitialFeats());
				pnlMainTab.setCR(((PCTemplate) thisPObject).getCR(-1, -1));
				pnlMainTab.setLevelAdjustment(((PCTemplate) thisPObject).getLevelAdjustmentFormula());
				pnlMainTab.setTemplateSize(((PCTemplate) thisPObject).getTemplateSize());
				...
				movementsMult = ((PCTemplate) thisPObject).getMovementsMult();
				...
				if (movementTypes != null)
				{
					for (int index = 0; index < movementTypes.length; index++)
					{
						final String aMove = MovementPanel.makeMoveString(movementTypes[index], movements[index], movementsMult[index], movementMultOp[index]);
						movementValues.add(aMove);
					}
				}
				pnlMovement.setSelectedList(movementValues);
		...
		switch (editType)
		{
			...
			case EditorConstants.EDIT_SPELL:
				pnlQClasses = new QualifiedAvailableSelectedPanel("in_demLevel", null, new EditorAddFilter()
				{
					...
					public Object decode(Object anObj)
					{
						return decodeDomainEntry(pnlQClasses, (String) anObj);
					}
				},
				null);
				pnlQDomains = new QualifiedAvailableSelectedPanel("in_demLevel", null, new EditorAddFilter()
				{
					...
					public Object decode(Object anObj)
					{
						return decodeDomainEntry(pnlQDomains, (String) anObj);
					}
				},
				null);
				break;
	...
	private void btnSaveActionPerformed(ActionEvent evt)
	{
		...
		switch (editType)
		{
			case EditorConstants.EDIT_DEITY:
				...
				thisPObject.setDescription(pnlMainTab.getDescriptionText());
				thisPObject.setDescIsPI(pnlMainTab.getDescIsPI());
			...
			case EditorConstants.EDIT_FEAT:
				thisPObject.setDescription(pnlMainTab.getDescriptionText());
				thisPObject.setDescIsPI(pnlMainTab.getDescIsPI());
				((Feat) thisPObject).setMultiples(pnlMainTab.getMultiples() ? "Y" : "N");
				((Feat) thisPObject).setStacks(pnlMainTab.getStacks() ? "Y" : "N");
				((Feat) thisPObject).setVisible(pnlMainTab.getVisible());
				((Feat) thisPObject).setCost(pnlMainTab.getCost());
				((Feat) thisPObject).setAddSpellLevel(pnlMainTab.getSpellLevels());
				((Feat) thisPObject).setLevelsPerRepIncrease(new Integer(pnlMainTab.getRepIncrease()));
			...
			case EditorConstants.EDIT_RACE:
				Race thisRace = (Race) thisPObject;
				thisRace.setBonusInitialFeats(pnlMainTab.getBonusFeats());
				thisRace.setBonusSkillsPerLevel(pnlMainTab.getBonusSkillPoints());
				thisRace.setCR(pnlMainTab.getCR());
				thisRace.setDisplayName(pnlMainTab.getDisplayName());
				thisRace.setHands(pnlMainTab.getHands());
				thisRace.setHitDiceAdvancement(pnlMainTab.getHitDiceAdvancement());
				thisRace.setAdvancementUnlimited(pnlMainTab.getHitDiceAdvancementUnlimited());
				thisRace.setLegs(pnlMainTab.getLegs());
				thisRace.setLevelAdjustment(pnlMainTab.getLevelAdjustment());
				thisRace.setMonsterClass(pnlMainTab.getMonsterClass());
				thisRace.setMonsterClassLevels(pnlMainTab.getMonsterLevel());
				thisRace.setSize(pnlMainTab.getRaceSize());
				thisRace.setReach(pnlMainTab.getReach());
				thisRace.setInitialSkillMultiplier(pnlMainTab.getSkillMultiplier());
				thisRace.setHitDice(pnlMainTab.getHitDiceNumber());
				thisRace.setHitDiceSize(pnlMainTab.getHitDiceSize());
			...
			case EditorConstants.EDIT_TEMPLATE:
				...
				thisPCTemplate.setRemovable(pnlMainTab.getIsRemovable());
				thisPCTemplate.setGenderLock(pnlMainTab.getGenderLock());
				thisPCTemplate.setVisible(pnlMainTab.getVisible());
				thisPCTemplate.setSubRegion(pnlMainTab.getSubRegion());
				thisPCTemplate.setSubRace(pnlMainTab.getSubRace());
				thisPCTemplate.setBonusSkillsPerLevel(pnlMainTab.getBonusSkillPoints());
				thisPCTemplate.setBonusInitialFeats(pnlMainTab.getBonusFeats());
				thisPCTemplate.setCR(pnlMainTab.getCR());
				thisPCTemplate.setLevelAdjustment(pnlMainTab.getLevelAdjustment());
				thisPCTemplate.setNonProficiencyPenalty(pnlMainTab.getNonProficiencyPenalty());
				thisPCTemplate.setMoveRates(pnlMovement.getMoveValues());
				thisPCTemplate.setMoveRatesFlag(pnlMovement.getMoveRateType());
				thisPCTemplate.setTemplateSize(pnlMainTab.getTemplateSize());
...
// TODO: check if all skills of one type are selected...maybe change to TYPE.blah?
		...
		else if (editType == EditorConstants.EDIT_DOMAIN)
		{
	...
	//
	private String decodeDomainEntry(QualifiedAvailableSelectedPanel pnl, String entry)
	{
		final int idx = entry.indexOf('=');
		if (idx >= 0)
		{
