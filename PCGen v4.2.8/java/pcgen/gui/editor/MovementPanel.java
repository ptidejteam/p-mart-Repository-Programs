 ...
 *
 * @(#) $Id: MovementPanel.java,v 1.1 2006/02/21 01:00:36 vauchers Exp $
 */
...
final class MovementPanel extends JPanel
{
	...
	private JComboBox cmbRateType;
	private JComboBox cmbMoveType;
	...
	private void initComponents(boolean isMoveOnly)
	{
		...
		cmbRateType = new JComboBox();
		cmbMoveType = new JComboBox(new String[]{"Walk", "Fly", "Swim", "ALL"});
		cmbMoveType.setEditable(true);
		rdbAdjustType = new JRadioButton[]{new JRadioButton("Add/Set to"), new JRadioButton("Multiply by"), new JRadioButton("Divide by")};
	...
	private void btnAddActionPerformed(JList lst, JButton btn)
	{
		...
		if (moveType == null || moveType.length() == 0 || (!rdbAdjustType[0].isSelected() && !rdbAdjustType[1].isSelected() && !rdbAdjustType[2].isSelected()) || txtMoveAmount.getText().trim().length() == 0)
		{
			JOptionPane.showMessageDialog(null, "You must enter a movement type, adjustment and amount.", Constants.s_APPNAME, JOptionPane.ERROR_MESSAGE);
			return;
		}
	 ...
	 * values.
	 *
	 * @param argSelected An arraylist of string movement values. eg Walk,*2 or
	 * Fly,40
	 */
	 ...
	 * @return String The movement value string.
	 */
	Integer[] getMoveRates()
	{
		Object selected[] = getSelectedList();
		Integer types[] = new Integer[selected.length];
		for (int index = 0; index < selected.length; index++)
		{
			final String moveString = (String) selected[index];
			final int idx = moveString.indexOf(",");
			types[index] = new Integer(moveString.substring(idx+1));
		}
		return types;
	}
	 ...
	 * @param moveRate The rate of the movemement, or the amount by
	 * which the move should be multiplied.
	 * @return String The resulting string eg Walk,*3 or Fly,40
	 */
	static String makeMoveString(String rateType, String movementMultOp, String moveRate)
	{
		StringBuffer buffer = new StringBuffer();
	 ...
	 * @return String The resulting string eg Walk,*3 or Fly,40
	 */
	static String makeMoveString(String rateType, Integer moveRate, Integer multAmount, String movementMultOp)
	{
		StringBuffer buffer = new StringBuffer();
		buffer.append(rateType).append(',');
		if ("*".equals(movementMultOp) || "/".equals(movementMultOp))
	 ...
	 */
	static String[] splitMoveString(String moveString)
	{
