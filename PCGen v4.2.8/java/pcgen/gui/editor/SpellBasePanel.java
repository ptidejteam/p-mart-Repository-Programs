 ...
 *
 * @(#) $Id: SpellBasePanel.java,v 1.1 2006/02/21 01:00:36 vauchers Exp $
 */
...
public class SpellBasePanel extends BasePanel
{
	...
	private JComboBox cmbSavingThrow;
	private JList lstItems;
	private JComboBox cmbSpellRes;
	...
	private void intComponentContents()
	{
		...
		cmbSchool.setModel(new DefaultComboBoxModel(Globals.getSchoolsList().toArray()));
		...
		ArrayList subschools = new ArrayList(10);
		subschools.add("(None)");
	...
	private void initComponents()
	{
		...
		cmbComponents = new JComboBox();
		lblCastingTime = new JLabel();
		cmbCastingTime = new JComboBox();
		lblRange = new JLabel();
		cmbRange = new JComboBox();
		...
		cmbSavingThrow = new JComboBox();
		lblSpellRes = new JLabel();
		cmbSpellRes = new JComboBox();
		lblCost = new JLabel();
		txtCost = new DecimalNumberField(0.0, 6);
		lblSchool = new JLabel();
		cmbSchool = new JComboBox();
		lblSubschool = new JLabel();
		cmbSubschool = new JComboBox();
		lblItems = new JLabel();
	...
	{
		pnlDescription.setText(aSpell.getDescription());	// don't want PI here
		pnlDescription.setDescIsPI(aSpell.getDescIsPI());
		cmbComponents.setSelectedItem(aSpell.getComponentList());
		cmbCastingTime.setSelectedItem(aSpell.getCastingTime());
		cmbRange.setSelectedItem(aSpell.getRange());
		cmbTarget.setSelectedItem(aSpell.getTarget());
		cmbDuration.setSelectedItem(aSpell.getDuration());
		cmbSavingThrow.setSelectedItem(aSpell.getSaveInfo());
		cmbSpellRes.setSelectedItem(aSpell.getSpellResistance());
		cmbSchool.setSelectedItem(aSpell.getSchool());
		cmbSubschool.setSelectedItem(aSpell.getSubschool());
		if (cmbSubschool.getSelectedIndex() < 0)
		{
			cmbSubschool.setSelectedIndex(0);
		}
		txtCost.setValue(aSpell.getCost().doubleValue());
		txtXpCost.setValue(aSpell.getXPCost());
	...
	private boolean canCreateItem(final String itemType, final String items)
	{
