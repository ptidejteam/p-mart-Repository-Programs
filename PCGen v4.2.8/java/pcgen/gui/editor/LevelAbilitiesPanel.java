 ...
 *
 * @(#) $Id: LevelAbilitiesPanel.java,v 1.1 2006/02/21 01:00:36 vauchers Exp $
 */
...
final class LevelAbilitiesPanel extends JPanel
{
	...
	private JComboBox cmbLevelType;
	private JComboBox cmbAbilityType;
	...
	private void initComponents()
	{
		...
		cmbLevelType = new JComboBox(levelTypes);
		cmbAbilityType = new JComboBox(abilityTypes);
	...
	private void btnAddActionPerformed(JList lst, JButton btn)
	{
		...
		if (levelType == null || levelType.length() == 0 || abilityType == null || abilityType.length() == 0 || txtLevel.getText().trim().length() == 0 || txtAbility.getText().trim().length() == 0)
		{
			JOptionPane.showMessageDialog(null, "You must enter a level type, level, ability type and ability details.", Constants.s_APPNAME, JOptionPane.ERROR_MESSAGE);
			return;
		}
	 ...
	 */
	static String[] splitAbilityString(String abilityString)
	{
