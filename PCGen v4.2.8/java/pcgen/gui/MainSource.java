...
public class MainSource extends FilterAdapterPanel
{
	...
	public MainSource()
	{
		...
		{
			JOptionPane.showMessageDialog(null, "Error in MainSource whilst initialising:\n " + e.toString() + "\n" + "PCGen may not operate correctly as a result of this error. ", Constants.s_APPNAME, JOptionPane.ERROR_MESSAGE);
			Globals.errorPrint("Error initialising MainSource: " + e.toString(), e);
		}
		initActionListeners();
	...
	{
		final ArrayList allowedModes = Globals.getAllowedGameModes();
	...
	private void loadCampaigns_actionPerformed(ActionEvent e)
	{
		...
		Globals.debugPrint("WeaponProfs=", Globals.getWeaponProfSize());
		Globals.debugPrint("ArmorProfs=", Globals.getArmorProfListSize());
	}
	...
	private void unloadAllCampaigns_actionPerformed(ActionEvent e)
	{
		...
		{
			JOptionPane.showMessageDialog(this, "PC's are not closed in debug mode.  " + "Please be aware that they may not function correctly " + "until campaign data is loaded again.", Constants.s_APPNAME, JOptionPane.WARNING_MESSAGE);
		}
		else
		{
			parent.closeAllPCs();
			if (parent.getBaseTabbedPane().getTabCount() > PCGen_Frame1.FIRST_CHAR_TAB) // All non-player tabs will be first
			{
				JOptionPane.showMessageDialog(this, "Can't unload campaigns until all PC's are closed.", Constants.s_APPNAME, JOptionPane.INFORMATION_MESSAGE);
				return;
			}
		}
	...
	public void updateLoadedCampaignsUI()
	{
		...
		//That code has since dissappeared, so I'll reinstate it along with
		// the code that came to take its place
		...
		if (pon != null)
		{
			if (pon.getItem() instanceof Campaign)
			{
				...
				if (!theURL.equals(""))
				{
					...
					{
						Globals.errorPrint("Could not open browser to " + theURL, ioEx);
						JOptionPane.showMessageDialog(null, "Could not open browser to " + theURL, Constants.s_APPNAME, JOptionPane.ERROR_MESSAGE);
					}
				}
				else
				{
					JOptionPane.showMessageDialog(null, "No web information found for Source: " + theCamp.getName(), Constants.s_APPNAME, JOptionPane.WARNING_MESSAGE);
				}
			}
		}
		else
		{
			JOptionPane.showMessageDialog(null, "Please select a source.", Constants.s_APPNAME, JOptionPane.ERROR_MESSAGE);
		}
	}
...
//  				}
		...
		availableTable.getSelectionModel().addListSelectionListener(new ListSelectionListener()
		{
			public void valueChanged(ListSelectionEvent e)
			{
				if (!e.getValueIsAdjusting())
				{
					...
					{
						JOptionPane.showMessageDialog(null, "No campaign selected. Try again.", Constants.s_APPNAME, JOptionPane.ERROR_MESSAGE);
						return;
					}
	 ...
	 */
	final class CampaignModel extends AbstractTreeTableModel implements TreeTableModel
	{
