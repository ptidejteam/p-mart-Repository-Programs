...
public final class InfoSkillsSorters
{
	...
	public static boolean keystatsMatch (PObjectNode node, Skill skill)
	{
		return node.toString().equals(skill.getMyType(0));
	}

	public static boolean costsMatch (PObjectNode node, Skill skill, InfoSkills tab)
	{
	...
	public static class GenericSubtypeName_Final extends FinalSorter
	{
		...
		public boolean nodeGoHere(PObjectNode node, Skill skill)
		{
			...
			{
				if (node.toString().equals(it.next())) return true;
			}
		...
		{
			// children
			for (Iterator it = node; it.hasNext(); )
			{
				PObjectNode child = (PObjectNode)it.next();

				// grandchildren
				for (Iterator jt = child; jt.hasNext(); )
				{
					PObjectNode[] grandchildren = ((PObjectNode)jt.next()).getChildren();
					...
					{
						if (grandchildren[i].getChildCount() == 1)
						{
							grandchildren[i] = grandchildren[i].getChild(0);
						}
					}
