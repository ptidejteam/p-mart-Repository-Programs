...
*/
	...
	{
		final int[] expandedRows = availableTable.getExpandedRows();
		createModelAvailable();
//		if (availableSort != null)
//			availableSort.sortNodeOnColumn();
	...
	{
		final int[] expandedRows = selectedTable.getExpandedRows();
		createModelSelected();
//		if (selectedSort != null)
...
// we can only be here if the PC can add the feat
	 ...
	 */
	private final class FeatModel extends AbstractTreeTableModel implements TreeTableModel
	{
				...
				case GuiConstants.INFOFEATS_VIEW_TYPENAME:
					...
					for (Iterator fI = Globals.getFeatList().iterator(); fI.hasNext();)
					{
						...
						*/
						...
						if (available && ((hasIt == HASFEAT_VIRTUAL) || (hasIt == HASFEAT_NO) || aFeat.isMultiples()) || (!available && (hasIt != HASFEAT_NO) && accept(aFeat)))
						{
					...
					while (fList.size() > 0 && loopmax-- > 0)
					{
						for (int i = 0; i < fList.size(); ++i)
						{
							...
							for (int j = 0; j < rootAsPObjectNode.getChildCount(); ++j)
							{
								...
								placed = placedThisFeatInThisTree(aFeat, po, Feat.preReqStringsForList(preReqList), 0);
								if (placed > 0)
								{
									break;
								}
			...
			while (true)
			{
				...
				if (a > -1)
				{
					...
					else if (trychildren)
					{
						...
						if (preReqString.length() > b + len && preReqString.substring(b + len).charAt(0) == ',')
						{
							++len;
						}
