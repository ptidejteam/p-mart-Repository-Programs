...
final class InfoClasses extends FilterAdapterPanel
{
	...
	private JButton adjXP2 = new JButton(PropertyFactory.getString("in_adjXP"));
	 ...
	 */
	private final class ClassModel extends AbstractTreeTableModel implements TreeTableModel
	{
			...
			switch (mode)
			{
				...
				case GuiConstants.INFOCLASS_VIEW_TYPE_NAME: // type/name
					...
					for (; fI.hasNext();)
					{
						...
						{
							if ((!added && i == rootAsPObjectNode.getChildCount() - 1) || aClass.isType(rootAsPObjectNode.getChildren()[i].getItem().toString()))
							{
								PObjectNode aFN = new PObjectNode();
				...
				default:
					Globals.errorPrint(PropertyFactory.getString("in_clICEr1") + " " + mode + " " + PropertyFactory.getString("in_clICEr2"));
					break;
			}
			PObjectNode rootAsPObjectNode = (PObjectNode) root;
		...
		private void addSubClassesTo(PObjectNode aFN, PCClass aClass)
		{
			if (aClass.getSubClassList() != null && !aClass.getSubClassList().isEmpty())
			{
				for (Iterator sI = aClass.getSubClassList().iterator(); sI.hasNext();)
				{
					...
					aSN.setParent(aFN);
					aSN.setItem(sClass);
					aSN.setIsValid(sClass.passesPreReqTests());
					aFN.addChild(aSN);
				}
			}
		}
		...
		public String getColumnName(int column)
		{
			switch (column)
			{
				...
				default:
					Globals.errorPrint(PropertyFactory.getString("in_clICEr3") + " " + column + " " + PropertyFactory.getString("in_clICEr2"));
					break;
			}
			return "";
		}
		...
		public Class getColumnClass(int column)
		{
			switch (column)
			{
				...
				case COL_SRC:
					return String.class;
				default:
					Globals.errorPrint(PropertyFactory.getString("in_clICEr4") + " " + column + " " + PropertyFactory.getString("in_clICEr2"));
					break;
			}
			return String.class;
		}
		...
		public Object getValueAt(Object node, int column)
		{
			...
			switch (column)
			{
				case COL_NAME: // Name
					...
					{
						Globals.errorPrint(PropertyFactory.getString("in_clICEr5"));
						return "";
					}
				...
				default:
					Globals.errorPrint(PropertyFactory.getString("in_clICEr6") + " " + column + " " + PropertyFactory.getString("in_ICEr2"));
					break;
	...
	private class ClassPopupMenu extends JPopupMenu
	{
		...
		ClassPopupMenu(JTreeTable treeTable)
		{
			if (treeTable == availableTable)
			{
				 ...
				 */
				ClassPopupMenu.this.add(createAddMenuItem(PropertyFactory.getString("in_add1"), "control EQUALS"));
			}
			...
			else // selectedTable
			{
				 ...
				 */
				ClassPopupMenu.this.add(createAddMenuItem(PropertyFactory.getString("in_add1"), "control EQUALS"));
				ClassPopupMenu.this.add(createRemoveMenuItem(PropertyFactory.getString("in_remove1"), "control MINUS"));
			}
		}
	}
	...
	InfoClasses()
	{
		...
		setName(PropertyFactory.getString("in_class"));
	...
	private void initComponents()
	{
		...
		Utility.setDescription(rightButton, PropertyFactory.getString("in_clAddTip"));
		...
		Utility.setDescription(leftButton, PropertyFactory.getString("in_clRemoveTip"));
		...
		txtNextLevel.addFocusListener(new FocusAdapter()
		{
			private boolean isProcessingTxtNextLevelFocusLost = false;
			...
			public void focusLost(FocusEvent evt)
			{
				...
				// for some reason this gets processed twice, want to ignore the second (and subsequent)
				//
				if (!isProcessingTxtNextLevelFocusLost)
				{
					isProcessingTxtNextLevelFocusLost = true;
					txtNextLevelFocusLost(evt);
					isProcessingTxtNextLevelFocusLost = false;
				}
			}
		});
	...
	private void initActionListeners()
	{
		...
		adjXP.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent evt)
			{
				String selectedValue = JOptionPane.showInputDialog(null, PropertyFactory.getString("in_clEnterXP"), Constants.s_APPNAME, JOptionPane.QUESTION_MESSAGE);
...
//						experienceFocusLost(null); // force xp messages as neccessary
					...
					{
						JOptionPane.showMessageDialog(null, PropertyFactory.getString("in_clInvalidNum"), Constants.s_APPNAME, JOptionPane.ERROR_MESSAGE);
						return;
					}
				}
			}
		});
		adjXP2.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent evt)
			{
				String selectedValue = JOptionPane.showInputDialog(null, PropertyFactory.getString("in_clEnterXP"), Constants.s_APPNAME, JOptionPane.QUESTION_MESSAGE);
				if (selectedValue != null)
				{
					try
					{
						int x = Integer.parseInt(selectedValue) + aPC.getNextLevelXP();
						if (maybeSetNextLevelExperience(x))
						{
							txtNextLevel.setValue(x);
						}
					}
					catch (NumberFormatException e)
					{
	...
	private static boolean maybeSetNextLevelExperience(int nextXP)
	{
		final PlayerCharacter aPC = Globals.getCurrentPC();
		aPC.setNextLevelXP(nextXP);
		return true;
	}
...
// 			return;
	...
	private void txtNextLevelFocusLost(FocusEvent evt)
	{
		maybeSetNextLevelExperience(txtNextLevel.getValue());
	}
	...
	private void updateCharacterInfo()
	{
		...
		//if (aString.length() != 0)
		if (Globals.getGameModeShowClassDefense() == true)
		{
		...
		lDefense.setText(Integer.toString(aPC.classDefense()));

		updateChecks();

		lWP.setText(Integer.toString(aPC.woundPoints()));
		updateXP(); // race changes effective XP
		lReputation.setText(Integer.toString(aPC.reputation()));
		needsUpdate = false;
	}
	...
	private void setInfoLabelText(PCClass aClass, PObjectNode pn)
	{
		...
		if (aClass != null)
		{
			...
			{
				b.append(" <b>").append(PropertyFactory.getString("in_defense")).append("</b>:").append(aClass.defenseString());
			}
			if (Globals.getGameModeReputationText().length() != 0)
			{
				b.append(" <b>").append(PropertyFactory.getString("in_reputation")).append("</b>:").append(aClass.getReputationString());
			}
		...
		availableTable.getSelectionModel().addListSelectionListener(new ListSelectionListener()
		{
			public void valueChanged(ListSelectionEvent e)
			{
				if (!e.getValueIsAdjusting())
				{
					...
					{
						lastClass = null;
						JOptionPane.showMessageDialog(null, PropertyFactory.getString("in_clNoClass"), Constants.s_APPNAME, JOptionPane.ERROR_MESSAGE);
						return;
					}
	...
	private void addClass(ActionEvent evt, int levels)
	{
		if (Globals.getGameModeAlignmentText().length() != 0)
		{
			...
			{
				JOptionPane.showMessageDialog(null, PropertyFactory.getString("in_clSelAlign"), Constants.s_APPNAME, JOptionPane.ERROR_MESSAGE);
				return;
			}
		}
		...
		//
		// TODO:
		// If attempting to add a different subclass (eg. Evoker to an Illusionist) warn the user
		// However, adding a level of the base class (i.e. Wizard to Illusionist) should still be okay
		//

		// Fix this logic -- it looks like you might slip past
		// the 20 cap with a monster PC?  XXX --bko
		if (levels < 0 || aClass == null || SettingsHandler.isIgnoreLevelCap() || (!SettingsHandler.isIgnoreLevelCap() && aClass.getLevel() < aClass.getMaxLevel()))
		{
		...
		{
			JOptionPane.showMessageDialog(null, PropertyFactory.getString("in_clMaxLvl"), Constants.s_APPNAME, JOptionPane.INFORMATION_MESSAGE);
			return;
		}
				...
				if (!hasClothes)
				{
					...
					if (selectedClothes.size() != 0)
					{
						...
						if (eq != null)
						{
							...
							{
								Globals.errorPrint(PropertyFactory.getString("in_clEqEr"));
							}
						}
					}
				}
	...
	private void updateXP()
	{
		...
		{
			Globals.debugPrint(PropertyFactory.getString("in_clUpXP") + " " + aPC);
		}
		if (aPC == null)
		{
			return;
		...
		if (Globals.isDebugMode())
		{
			Globals.debugPrint(PropertyFactory.getString("in_clUpXP") + " ", experience.getValue());
		}
		txtNextLevel.setValue(aPC.getNextLevelXP());
	}
	...
	private void initSEPanel(JPanel sep)
	{
		...
		//
		// Didn't really want this button, but without it the current value in txtNextLevel
		// gets transferred to a newly created character.
		//
		gbc = new GridBagConstraints();
		gbc.gridy = 1;
		pnlXP.add(adjXP2, gbc);
