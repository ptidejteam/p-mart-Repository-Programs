...
public final class FilterDialogFactory implements FilterConstants
{
 ...
 * @author Thomas Behr
 */
final class FilterSelectDialog extends JDialog implements ActionListener, FilterConstants
{
...
//                  saveButton.setPreferredSize( buttonDimension );
		...
		optionPanel.setBorder(BorderFactory.createTitledBorder(BorderFactory.createCompoundBorder(BorderFactory.createCompoundBorder(BorderFactory.createLoweredBevelBorder(), BorderFactory.createRaisedBevelBorder()), BorderFactory.createEmptyBorder(0, 5, 2, 5)), PropertyFactory.getString("in_options"), TitledBorder.LEFT, TitledBorder.TOP, UIManager.getFont("Label.font")));
...
//  		mainPanel.add(rightButtonPanel, 1, 2, 1, 1);
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		else if (src.equals(saveButton))
		{
			...
			{
				// show warning
				JOptionPane.showMessageDialog(null, PropertyFactory.getString("in_filterErP1") + Constants.s_LINE_SEP + PropertyFactory.getString("in_filterErP2"), PropertyFactory.getString("in_filterErWarn"), JOptionPane.ERROR_MESSAGE);
				return;
			}
			...
			{
				filter = FilterFactory.createCompoundFilter((PObjectFilter) selectedModel.get(0), (PObjectFilter) selectedModel.get(1), operand);
			...
			{
				filter = FilterFactory.createNamedFilter(filter, filterNameDialog.getName(), filterNameDialog.getDescription());
			}
...
final class GBLPanel extends JPanel
{
	 ...
	 *
	 */
	protected void add(JComponent comp, int row, int col, int width, int height)
	{
 ...
 * @author Thomas Behr
 */
final class FilterCustomDialog extends JDialog implements ActionListener, FilterConstants
{
	private int maxSelected;
 ...
 * @author Thomas Behr
 */
final class FilterNameDialog extends JDialog implements ActionListener, FilterConstants
{
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		else
		{
			...
			{
				JOptionPane.showMessageDialog(null, PropertyFactory.getString("in_filterEr2P1") + " '" + name + "'!", PropertyFactory.getString("in_filterErWarn"), JOptionPane.ERROR_MESSAGE);
			}
			else
			{
				int index = indexOfIllegalChar(name);
				JOptionPane.showMessageDialog(null, PropertyFactory.getString("in_filterEr3P1") + " '" + name.charAt(index) + "' " + PropertyFactory.getString("in_filterEr3P2"), PropertyFactory.getString("in_filterErWarn"), JOptionPane.ERROR_MESSAGE);
	...
	private static int indexOfIllegalChar(String s)
	{
		...
		{
			if (!Character.isJavaIdentifierStart(chars[i]) && !Character.isWhitespace(chars[i]) && !((chars[i] == '(') || (chars[i] == ')')))
			{
				return i;
			}
		}
 ...
 * @author Thomas Behr
 */
final class FilterEditorDialog extends JDialog implements ActionListener, FilterConstants
{
	private ArrayList customFilters;
	private HashMap customFiltersOrigin;
...
//                  contentPanel.add(allButtonPanel, BorderLayout.SOUTH);
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		else if (src.equals(applyButton))
		{
			if (filterable != null)
			{
				storeFilters(filterable.getAvailableFilters(), filterable.getSelectedFilters(), filterable.getRemovedFilters());
		...
		else if (src.equals(cancelButton))
		{
			if (filterable != null)
			{
				unStoreFilters(filterable.getAvailableFilters(), filterable.getSelectedFilters(), filterable.getRemovedFilters());
			}
		...
		else if (src.equals(createButton))
		{
			...
			try
			{
				filter = (new FilterParser(new List[]{filterable.getAvailableFilters(), filterable.getSelectedFilters(), filterable.getRemovedFilters(), customFilters})).parse(editorArea.getText());
			}
			catch (FilterParseException ex)
			{
				filter = null;
				Globals.errorPrint("Error in FilterDialogFactory::actionPerformed", ex);
				JOptionPane.showMessageDialog(null, ex.getMessage(), PropertyFactory.getString("in_error"), JOptionPane.ERROR_MESSAGE);
			}
			if (filter != null)
			{
				// possible naming
