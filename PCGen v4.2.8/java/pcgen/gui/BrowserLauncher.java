 ...
 * <br>Jonas Karlsson added pcgen-specific support for setting browser path (updated to be based on BrowserLauncher 1.4b1)
 *
...
final class BrowserLauncher
{
	...
	private static Object locateBrowser()
	{
		...
		switch (jvm)
		{
			...
			case OTHER:
			default:
				// getenv is depreacated in JDK1.3, but
				// Sun is insane to do this.
				browser = java.lang.System.getProperty("BROWSER", "netscape");
				break;
		}
		return browser;
	}
	...
	public static void openURL(String url) throws IOException
	{
		...
		{
			Globals.debugPrint(PropertyFactory.getString("in_BLEr4"));
			Object browser = locateBrowser();
			if (browser == null)
			{
				throw new IOException(PropertyFactory.getString("in_BLEr5") + ": " + errorMessage);
			}
			...
			switch (jvm)
			{
				...
				case MRJ_3_1:
					try
					{
						Globals.errorPrint(PropertyFactory.getString("in_BLEr11") + " " + url);
		...
		{
			Globals.errorPrint(PropertyFactory.getString("in_BLEr15") + " " + SettingsHandler.getBrowserPath());
			// On MacOS X, one must use open -a to launch an app.
			if (jvm == MRJ_3_1 && SettingsHandler.getBrowserPath().getName().toLowerCase().endsWith(".app"))
			{
				Globals.errorPrint(PropertyFactory.getString("in_BLEr16") + " " + SettingsHandler.getBrowserPath().getAbsolutePath() + " " + url);
				Runtime.getRuntime().exec(new String[]{"open", "-a", SettingsHandler.getBrowserPath().getAbsolutePath(), url});
			}
			else
			{
				Globals.errorPrint(PropertyFactory.getString("in_BLEr17") + " " + SettingsHandler.getBrowserPath().getAbsolutePath() + " " + url);
				Runtime.getRuntime().exec(new String[]{SettingsHandler.getBrowserPath().getAbsolutePath(), url});
			}
		}
	...
	private static native int ICLaunchURL(int instance, byte[] hint, byte[] data, int len, int[] selectionStart, int[] selectionEnd);
}
