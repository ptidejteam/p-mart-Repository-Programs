...
class JTableEx extends JTable
{

	JTableEx(TableModel tm)
	{
		this(tm, null, null);
	}
	...
	JTableEx(TableModel tm, TableColumnModel tcm, ListSelectionModel lsm)
	{
		super(tm, tcm, lsm);
	}
	...
	public final void setOptimalColumnWidths(int[] columns)
	{
		...
		{
			Globals.errorPrint("Bad parameters passed to setOptimalColumnWidth.");
			return;
		}
		...
		for (int i = 0; i < columns.length; i++)
		{
			try
			{
				...
				if (r != null)
				{
					for (int row = rowCount - 1; row >= 0; --row)
					{
						...
						if (value != null)
						{
							final Component c = r.getTableCellRendererComponent(this, value, false, false, row, columnIndex);
			...
			{
				Globals.errorPrint("Exception JTableEx.setOptimalColumnWidths:" + i + ":" + columns.length + ":" + columncount + Constants.s_LINE_SEP + "Exception type:" + e.getClass().getName() + Constants.s_LINE_SEP + "Message:" + e.getMessage());
			}
		}
