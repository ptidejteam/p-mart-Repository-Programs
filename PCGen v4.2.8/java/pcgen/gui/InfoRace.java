...
class InfoRace extends FilterAdapterPanel
{
	...
	private class RacePopupMenu extends JPopupMenu
	{
		...
		{
			return Utility.createMenuItem(label, new AddRaceActionListener(), "Select", '\0', accelerator, PropertyFactory.getString("in_irSelRaceTip"), "Add16.gif", true);
		}

		RacePopupMenu()
		{
			RacePopupMenu.this.add(createAddMenuItem(PropertyFactory.getString("in_select"), "control EQUALS"));
		}
	}
	...
	private void initComponents()
	{
		...
		pnlHD.setLayout(new FlowLayout());
		...
		btnAddHD.addActionListener(new ActionListener()
		{
			...
			{
				Globals.getRootFrame().addMonsterHD(1);
				updateHD();
			}
		});
		...
		TitledBorder title1 = BorderFactory.createTitledBorder(etched, PropertyFactory.getString("in_irRaceInfo"));
...
//		avaLabel.setPreferredSize(new Dimension(150, 20));
	...
	{
		requestFocus();
		Globals.getRootFrame().getStatusBar().setText(PropertyFactory.getString("in_irSelectRace"));
		updateCharacterInfo();
		int width;
		int s = splitPane.getDividerLocation();
	...
	private void removeTemplate(ActionEvent evt)
	{
		...
		{
			JOptionPane.showMessageDialog(null, PropertyFactory.getString("in_irNotRemovable"), Constants.s_APPNAME, JOptionPane.ERROR_MESSAGE);
			return;
		}
	 ...
	 **/
	private final class RaceModel extends AbstractTreeTableModel implements TreeTableModel
	{
		// this is the root node
		private PObjectNode raceRoot;

		// list of column names
		private final String[] raceNameList = {"Name", "Stat Adjustments", "PreReqs", "Size", "Speed", "Vision", "Favored Class", "Lvl Adj"};
			...
			switch (column)
			{
				...
				case COL_MOVE:
					...
					if (race.getMovements() != null)
					{
						...
						{
							movelabel.append(", ").append(race.getMovementType(i)).append(' ').append(race.getMovement(i)).append(Globals.getAbbrMovementDisplay());
						}
				...
				case COL_CLASS:
					...
					sRet = (!".".equals(race.getFavoredClass())) ? race.getFavoredClass() : "Various";
					break;
	...
	private final class AllTemplatesTableModel extends AbstractTableModel
	{
		...
		private final String[] ALL_TEMPLATES_COLUMN_NAMES = new String[]{"Q", "Name", "Lvl Adj", "Modifiers", "Prereqs", "Source"};
