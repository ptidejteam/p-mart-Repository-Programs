...
class InfoSpells extends FilterAdapterPanel
{
	...
	private final class MainMM extends JPanel
	{
	...
	private static int getDC(PCClass aClass, int level)
	{
		...
		DC += level + a + aPC.getTotalBonusTo("STAT", aClass.getName(), true) / 2 + aPC.getTotalBonusTo("SPELL", "DC", true);
		return DC;
	}
...
//Globals.errorPrint("MMaddSB:MMaddSpell: " + className + ":" + spLevel + ":" + aSpell.getName() + ":" + bookName + ":feats:"+selFeatList.size());
	 ...
	 **/
	private final class SpellModel extends AbstractTreeTableModel implements TreeTableModel
	{
 ...
 */
	...
	private final class MMFrame extends JFrame
	{
		MainMM mainMM = null;
		...
		private MMFrame()
		{
			...
			MMFrame.this.setSize(screenWidth >> 1, screenHeight >> 1);
