...
//  	private PrinterJob printerJob = PrinterJob.getPrinterJob();
	...
	private void setDefaultTemplateSelection()
	{
		...
		{
			tempSel = SettingsHandler.getSelectedPDFOutputSheet();
		}
...
//		buttonPanel.add(templatePathButton);
	...
	private void export()
	{
		...
		for (int loop = 0; loop < pcExports.length; loop++)
		{
			...
			SettingsHandler.setSelectedPDFOutputSheet(fcExport.getSelectedFile().getAbsolutePath());
...
//			Party.print(template, bw);
		...
		{
			SettingsHandler.setSelectedPDFOutputSheet(template.getAbsolutePath());
