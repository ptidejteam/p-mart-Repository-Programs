 ...
 *
 * $Id: GameModeLoader.java,v 1.1 2006/02/21 01:00:36 vauchers Exp $
 */
...
final class GameModeLoader
{
	public static void parseMiscGameInfoLine(GameMode gameMode, String aLine, File aFile, int lineNum) throws PersistenceLayerException
	{
		...
		else if (aLine.startsWith("DEFENSEABBREV:"))
		{
			gameMode.setDefenseAbbrev(aLine.substring(14));
		}
		...
		else if (aLine.startsWith("SHOWTAB:"))
		{
			...
			if (aLine.startsWith("DOMAINS|"))
			{
				gameMode.setShowDomainTab(aLine.charAt(8) == 'Y');
			}
			if (aLine.startsWith("FEATS|"))
			{
				gameMode.setShowFeatTab(aLine.charAt(6) == 'Y');
			}
			else if (aLine.startsWith("SPELLS|"))
			{
		...
		else if (aLine.startsWith("WOUNDPOINTSABBREV:"))
		{
			gameMode.setWoundPointsAbbrev(aLine.substring(18));
		}
