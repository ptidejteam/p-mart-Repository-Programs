...
public class InfoClasses extends JPanel
{
	...
	private static final String[] ALL_CLASSES_COLUMN_NAMES = new String[]{"Q", "Name", "BAB", "HD",
																																				"Fort", "Reflex", "Will", "PreReqs", "Defense", "Reputation", "Spell Type",
...
//		jPanel1.add(tHp, gridBagConstraints2);
	...
	private void buttonMinusActionListener(ActionEvent evt)
	{
		...
		if (theClass != null)
		{
			aPC.incrementClassLevel(-1, theClass);
		}
		pcChanged();
	}
	...
	private void buttonPlusActionPerformed(ActionEvent evt)
	{
		...
		{
			PCClass aClass = aPC.getClassNamed(theClass.getName());
...
//			tHp.setValue(pc.hitPoints());
		...
		else if (Globals.isSidewinderMode())
		{
			...
			TableColumn col = allClassesTable.getColumnModel().getColumn(8);
			col.setHeaderValue("AC");
			col = allClassesTable.getColumnModel().getColumn(9);
			col.setHeaderValue("N/A");
		}
		else //all other game modes, specifically D&D
		{
			jButtonHP.setText("HP");
			lMaxHPPct.setText("Max HP %");
	 ...
	 *  It pulls its data straight from the Globals.getClassList() ArrayList.
	 */
		...
		public void setFilter(int filterID)
		{
			...
			switch (filterID)
			{
				case FILTER_ALL: // All
					...
					{
						final PCClass pcClass = (PCClass)it.next();
						if (pcClass.isVisible())
				...
				case FILTER_QUALIFIED: // Qualified
					...
					{
						final PCClass pcClass = (PCClass)it.next();
						if (pcClass.isVisible() && pcClass.isQualified())
							displayClasses.add(pcClass);
					}
					break;
				...
				case FILTER_BASE: // Base
					...
					{
						final PCClass pcClass = (PCClass)it.next();
						if (pcClass.isVisible() && !pcClass.isPrestige() && (!pcClass.isMonster() || pcClass.isQualified()))
							displayClasses.add(pcClass);
					}
					break;
				...
				case FILTER_PRESTIGE: // Prestige
					...
					{
						final PCClass pcClass = (PCClass)it.next();
						if (pcClass.isVisible() && pcClass.isPrestige())
							displayClasses.add(pcClass);
					}
					break;
				...
				case FILTER_PC: // All
					...
					{
						final PCClass pcClass = (PCClass)it.next();
						if (pcClass.isVisible() && pcClass.isPC())
							displayClasses.add(pcClass);
					}
					break;
				...
				case FILTER_NPC: // All
					...
					{
						final PCClass pcClass = (PCClass)it.next();
						if (pcClass.isVisible() && pcClass.isNPC())
							displayClasses.add(pcClass);
					}
					break;
				...
				case FILTER_MONSTER: // All
					...
					{
						final PCClass pcClass = (PCClass)it.next();
//						if (pcClass.isVisible() && pcClass.isMonster() && pcClass.isQualified())
						if (pcClass.isVisible() && pcClass.isMonster())
							displayClasses.add(pcClass);
					}
					break;
		...
		public String getColumnName(int columnIndex)
		{
			if (Globals.isStarWarsMode() && columnIndex > 9)
				columnIndex += 2;
			else if (Globals.isSidewinderMode() && columnIndex > 9)
				columnIndex += 2;
			else if (!Globals.isStarWarsMode() && !Globals.isSidewinderMode() && columnIndex > 7)
				columnIndex += 2;
		...
		public Object getValueAt(int rowIndex, int columnIndex)
		{
			...
			{
				PCClass aClass = (PCClass)displayClasses.get(rowIndex);
				final PCClass pcClass = Globals.getCurrentPC().getClassNamed(aClass.toString());
				if (pcClass != null)
				{
					aClass = pcClass;
				}
				...
				{
					case 0:
						return aClass.isQualified() ? "Y" : "N";

					case 1:
						return aClass.toString();

					case 2:
						return aClass.getAttackBonusType();

					case 3:
						return "1D" + aClass.getHitDie();

					case 4:
						return aClass.getFortitudeCheckType();

					case 5:
						return aClass.getReflexCheckType();

					case 6:
						return aClass.getWillCheckType();

					case 7:
						return aClass.preReqHTMLStrings();

					case 8:
						return (Globals.isStarWarsMode() || Globals.isSidewinderMode()) ? aClass.defenseString() : aClass.getSpellType();

					case 9:
						return (Globals.isStarWarsMode() || Globals.isSidewinderMode()) ? aClass.getReputationString() : aClass.getSpellBaseStat();
	...
	public class PCClassesTableModel extends AbstractTableModel
	{
		...
		{
			final PlayerCharacter aPC = Globals.getCurrentPC();
			if ((aPC != null) && (aPC.getClassList() != null))
			{
		...
		public Object getValueAt(int rowIndex, int columnIndex)
		{
			...
			if ((aPC != null) && (aPC.getClassList() != null))
			{
				final PCClass c = (PCClass)filterDomain().get(rowIndex);
				switch (columnIndex)
				{
					case 0:
