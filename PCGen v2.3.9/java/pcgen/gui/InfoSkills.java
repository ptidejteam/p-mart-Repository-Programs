...
public class InfoSkills extends JPanel
{
	...
	private void initComponents()
	{
		...
		final PlayerCharacter aPC = Globals.getCurrentPC();
		maxSkillRank = new JLabel(new Integer(aPC.totalLevels() + aPC.totalHitDice() + 3).toString());
		maxCrossSkillRank = new JLabel(new Integer((aPC.totalLevels() + aPC.totalHitDice() + 3) / 2).toString());
	...
	private void lowerSkillActionPerformed(ActionEvent evt)
	{
		...
		final Skill aSkill = getSelectedSkill();
		if (aSkill == null)
			return;
		modRank(aSkill, -1.0);
	}
	...
	private void raiseToMaxActionPerformed(ActionEvent evt)
	{
		...
		if (maxRank > aSkill.getRank().doubleValue() || Globals.isBoolBypassMaxSkillRank())
		{
			...
			//
			// Drop fraction if dealing with class skill
			if (cost == 1)
	...
	private void populateSkillModel()
	{
		...
		for (Iterator i = Globals.getSkillList().iterator(); i.hasNext();)
		{
			...
			if (bSkill != null)
				aSkill = bSkill;
			final int skillViewIndex = skillView.getSelectedIndex();
			boolean addIt = false;
			switch (skillViewIndex)
			{
				...
				case 4: //Untrained
					...
					break;
				case 5: //Rank > 0
					// was Math.floor, but why would we not want 0.5 rank skills for this filter? (merton_monk)
					addIt = aSkill.getTotalRank().doubleValue() > 0.0;
					break;
				case 6: //Rank + modifier > 0
					// was Math.floor, but why would we truncate the value instead of doing a straight comparison? (merton_monk)
					addIt = aSkill.getTotalRank().doubleValue() + aSkill.modifier().doubleValue() > 0.0;
					break;
				default:
