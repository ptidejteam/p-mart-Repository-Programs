...
*/
		...
		if (i >= 0 && i < spellClassComboBox.getItemCount() && j >= 0 && j < 10)
		{
			...
			else
			{
				...
				{
					aSpell = (Spell)e.next();
					if (aSpell.levelForClass(className, aClass.getName()).lastIndexOf("," + String.valueOf(spellLevelComboBox.getSelectedIndex())) > -1)
						vectorA.addElement(aSpell);
				}
			}
			if (aClass != null)
			{
				knownText.setText(String.valueOf(aClass.getKnownForLevel(aClass.getLevel().intValue(), j)));
