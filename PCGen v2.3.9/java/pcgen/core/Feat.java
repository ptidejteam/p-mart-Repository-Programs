/*
 * @(#) $Id: Feat.java,v 1.1 2006/02/20 23:47:11 vauchers Exp $
 *
	...
	public void parseLine(String inputLine, File sourceFile, int lineNum)
	{
		...
		for (col = 0; col < colMax; col++)
		{
			...
			else if ((len > 14) && aCol.startsWith("ADDSPELLLEVEL:"))
			{
				...
				{
					JOptionPane.showMessageDialog(null, "Bad addSpellLevel " + aCol, Globals.s_APPNAME, JOptionPane.ERROR_MESSAGE);
				}
			}
			...
			else if (aCol.startsWith("REP"))
			{
				...
				{
					JOptionPane.showMessageDialog(null, "Bad level per value " + aCol, Globals.s_APPNAME, JOptionPane.ERROR_MESSAGE);
				}
			}
...
//				System.out.println("This is:" + this);
				addToQualifyListing(aCol.substring(8));
			}
			else if (aCol.startsWith("VISIBLE:"))
			{
				if (aCol.substring(8).startsWith("Export"))
				{
					featVisible = 2; // output, no display: character sheet only
				}
				else if (aCol.substring(8).startsWith("No"))
				{
					featVisible = 0; // no Output, no display: Stealth
				}
				else if (aCol.substring(8).startsWith("Display"))
				{
					featVisible = 3; // Display only, no output: DisplayOnly
				}
				else
				{
					featVisible = 1; // default. Display and output: Yes
				}
			}
...
*/		//removed because depended on additonal types in (nonexistant) eq
		...
		else if (choiceType.equals("SPELLCLASSES"))
		{
			...
			for (iter = aPC.getClassList().iterator(); iter.hasNext();)
			{
				...
				if (!aClass.getSpellBaseStat().equals(Globals.s_NONE))
