...
//	}
	...
	private void setAlignments(String newAlign)
	{
		...
		{
			bString = Globals.commaDelimit(newAlign);
...
// Not to sure what I can get rid of and still have it work.
	...
	{
		return getCastForLevel(pcLevel, spellLevel, "Known Spells", true);
	}
	...
	public int getCastForLevel(int pcLevel, int spellLevel, String bookName, boolean includeAdj)
	{
		...
		if (stat < 10 + spellLevel && index != -2)
			return total;
		int adj = 0;
		if (includeAdj && !bookName.equals("Known Spells") && (specialtyList.size() > 0 || aPC.getCharacterDomainList().size() > 0))
		{
			Spell aSpell = null;
	...
	public String getUdamForLevel(int aLevel, boolean includeCrit, boolean includeStrBonus)
	{
		...
		aDamage = Globals.adjustDamage(aDamage, Globals.s_SIZESHORT[Globals.SIZE_M], Globals.s_SIZESHORT[iSize]);
		...
		if (includeStrBonus && aPC.adjStats(Globals.STRENGTH) / 2 > 5)
			aString.append("+");
		if (includeStrBonus && aPC.adjStats(Globals.STRENGTH) / 2 != 5)
			aString.append(aPC.calcStatMod(Globals.STRENGTH));
		if (includeCrit)
		{
	...
	public void setLevel(Integer newLevel)
	{
		if (newLevel.intValue() >= 0)
			level = newLevel;
		final PlayerCharacter aPC = Globals.getCurrentPC();
		if (newLevel.intValue() != 0 && knownSpellsList.size() > 0)
		{
	...
	public void doMinusLevelMods(PlayerCharacter aPC, int oldLevel)
	{
		...
		modDomainsForLevel(oldLevel, false);
	...
	public void addLevel(boolean levelMax, boolean bSilent)
	{
					...
					if (!bSilent)
					{
						if (Globals.isSkillIncrementBefore())
						{
							...
							Object selectedValue = JOptionPane.showInputDialog(null,
								...
								Globals.s_ATTRIBLONG[Globals.INTELLIGENCE] + " here may award more skill points.\n\nCurrent Stats:\n" + sStats + "\n",
		...
		{
			spMod = getSkillPoints() + aPC.getRace().getBonusSkillsPerLevel() + aPC.calcStatMod(Globals.INTELLIGENCE) + aPC.getTotalBonusTo("SKILLPOINTS", "NUMBER", true);
			if (spMod < 1)
				spMod = 1;
		}
		if (total == 1)
	...
	private void checkForSubClass()
	{
		...
		if (showAdditionalChoices)
		{
			...
			if (!selectedList.isEmpty())
			{
				final String theChoice = (String)selectedList.get(0);
...
// what????										cost = new Integer(0);
	...
	private void setGoldString(final String aString)
	{
		goldString = aString;
	}
	...
	private void setSpecialsString(final String aString)
	{
	...
	private void setDefenseString(final String aString)
	{
...
/*Prestige7*/ 	case 10:
	...
	private void setReputationString(final String aString)
	{
	...
	private void setExClass(final String aString)
	{
		exClass = aString;
	}
	...
	private void setAttackCycle(final String aString)
	{
		attackCycle = aString;
	}
		...
		for (col = 0; col < colMax; col++)
		{
			...
			switch (option)
			{
				...
				default:
					...
					{
						if (aInt - 1 < castList.size())
							getCastList().set(aInt - 1, colString.substring(5));
						else
							castList.add(colString.substring(5));
					}
					...
					{
						setExClass(colString.substring(8));
					}
					else if (colString.startsWith("EXCHANGELEVEL:"))
					{
						setLevelExchange(colString.substring(14));
					}
