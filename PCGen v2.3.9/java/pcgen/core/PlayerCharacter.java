...
//	private int PcgWriteVersion = 2;	// Version of file we will write
	...
	private String subRace = Globals.s_NONE;
	private String region = Globals.s_NONE;
	...
	{
		for (Iterator i = weaponProfList.iterator(); i.hasNext();)
		{
	...
	public boolean checkQualifyList(String qualifierItem)
	{
		...
		if (qualifyArrayList.contains(qualifierItem))
			return true;
		else
			return false;
	}
	...
	public int getStat(int idx)
	{
		...
		{
			if ((idx == Globals.HONOR) && Globals.isHackMasterMode())
			{
				for (idx = 0; idx < Globals.s_ATTRIBLONG.length; idx++)
				{
					if (idx != Globals.HONOR)
					{
						stat += stats[idx];
					}
				}
	...
	{
		String critterType = "Humanoid";
		if (race != null)
			critterType = race.getType();
	...
	public int movement(int x)
	{
		...
		boolean isMedium = getRace().getSize().equals("M");
		...
		move += getTotalBonusTo("MOVE", "TYPE=" + getRace().getMovementType(x).toUpperCase(), true);
	...
	public int touchAC()
	{
		return makeAcMod() + getRace().getStartingAC().intValue() + naturalArmorModForSize();
	}
	...
	{
		int initmod = calcStatMod(Globals.DEXTERITY) + getRace().getInitMod().intValue();
	...
	public String getAttackString(int index, int bonus)
	{
		...
		for (int i = 0; i < classList.size(); i++)
		{
			...
			final int b = aClass.baseAttackBonus(index);
	...
	public void setAlignment(int index, boolean bLoading, boolean bForce)
	{
		...
		else
		{
			if ((bLoading) && (index != 9))
			{
				JOptionPane.showMessageDialog(null, "Invalid alignment. Setting to <none selected>", Globals.s_APPNAME, JOptionPane.INFORMATION_MESSAGE);
				alignment = 9;
			}
	...
	private void doVision(String tmplVision, int flag)
	{
		...
		else if (flag == 1 && !tmplVision.equals(Globals.s_NONE))
		{
			visionOutput += ", " + tmplVision;
		}
		else
		...
		if (visionOutput.equals("") || visionOutput.equals(Globals.s_NONE)) visionOutput = "Normal";
	}

	public void addToQualifyList(PObject fromThere)
	{
		final String tempQualifyList = fromThere.getQualifyString();
		final StringTokenizer aaTok = new StringTokenizer(tempQualifyList, "|", false);
	...
	public void cutFromQualifyList(PObject fromThere)
	{
		...
		while (aaTok.hasMoreTokens())
		{
			final String aaaString = aaTok.nextToken();
	...
	public void addTemplate(PCTemplate inTmpl, boolean addGold)
	{
		...
		String tempSubRace = inTmpl.getSubRace();
		if (!tempSubRace.equals(Globals.s_NONE)) subRace = tempSubRace;
		region = inTmpl.getRegion();
	...
	public void removeTemplate(PCTemplate inTmpl)
	{
		if (inTmpl == null)
			return;
		if (subRace.equals(inTmpl.getSubRace())) subRace = Globals.s_NONE;
		if (region.equals(inTmpl.getRegion())) region = Globals.s_NONE;
	...
	public void incrementClassLevel(int mod, PCClass aClass, boolean bSilent)
	{
		if (mod > 0)
		{
			...
			{
				JOptionPane.showMessageDialog(null, "Cannot increase Monster Hit Dice for this character beyond " + race.maxHitDiceAdvancement() + ". This characters current number of Monster Hit Dice is " + totalHitDice(), Globals.s_APPNAME, JOptionPane.INFORMATION_MESSAGE);
				return;
			}
		}
	...
	public void makeIntoExClass(PCClass aClass)
	{
		...
		{
			JOptionPane.showMessageDialog(null, exc.getMessage(), Globals.s_APPNAME, JOptionPane.ERROR_MESSAGE);
		}
		...
		if (race != null)
		{
			...
			while (aTok.hasMoreTokens())
			{
				...
				else
				{
					...
					{
						JOptionPane.showMessageDialog(null, "Removing unknown feat: " + aString, Globals.s_APPNAME, JOptionPane.INFORMATION_MESSAGE);
					}
				}
			}
...
//						}
				...
				else
				{
					...
					{
						JOptionPane.showMessageDialog(null, "Adding unknown feat: " + aString, Globals.s_APPNAME, JOptionPane.INFORMATION_MESSAGE);
					}
				}
			}
			getAutoLanguages();
				...
				while (aStrTok.hasMoreTokens())
				{
					...
					else
					{
						if (thisString.endsWith("(SPECIALS)"))
						{
							...
							String cString = "1";
							String dString = "2";
							final String name = sa.getName() + " (" + sa.getDesc() + ")";
							if (sa != null)
							{
								if ((addIt == false) && !aDesc.equals(""))
								{
									cString = new String(eString);
									dString = new String(name);
									for (int i = 0; i < 10; i++)
									{
								...
								{
									getSpecialAbilityList().add(name);
								}
							}
						}
						else if (addIt)
	...
	public boolean isSpellCaster(int minLevel)
	{
		for (Iterator e1 = classList.iterator(); e1.hasNext();)
		{
			...
			if (!aClass.getSpellType().equals(Globals.s_NONE) && aClass.getLevel().intValue() >= minLevel)
				return true;
		}
		return false;
	}
	...
	public boolean isSpellCastermax(int maxLevel)
	{
		...
		{
			final PCClass aClass = (PCClass)e1.next();
			if (!aClass.getSpellType().equals(Globals.s_NONE) && aClass.getLevel().intValue() <= maxLevel)
				return true;
		}
		return false;
	}
	...
	public int modFeat(String featName, boolean addIt, boolean addAll)
	{
		...
		String subName = "";
		// see if our choice (featList), auto (autoFeatList) or virtual (vFeatList) 
		// has this feat
		Feat aFeat = getFeatNamed(featName);
		if (addIt && aFeat!=null) {
		// if we're adding the feat and we only have it in our virtual list
		// then pretend we never had the feat
			if (!hasFeat(featName) && !hasFeatAutomatic(featName))
				aFeat=null;
		}
		...
		if (Globals.getWeaponTypes().contains(choiceType))
		{
			...
			j += weaponProfs.size() - (int)(aFeat.getAssociatedList().size() * aFeat.getCost());
...
//          break;
	...
	public SortedSet getBonusLanguages()
	{
		...
		for (e = getRace().getLanguageBonus().iterator(); e.hasNext();)
		{
			...
			if (aLang.equals("ALL"))
			{
				bonusLangs.addAll(Globals.getLanguageSetNames());
			}
			else
	...
	public boolean isProficientWith(Equipment eq)
	{
		if (eq.isWeapon())
		{
			...
			return (wp != null && hasWeaponProfNamed(wp.getName()));
		}
		else if (eq.isArmor())
		{
	...
	public int handsFull()
	{
		...
		for (e = aArrayList.iterator(); e.hasNext();)
		{
			...
			if (eq.isEquipped())
			{
				...
				if (wp == null)
				{
					JOptionPane.showMessageDialog(null, "No entry in weapons.lst for " + eq.profName() + ". Weapons must be in that file to equip them.", Globals.s_APPNAME, JOptionPane.ERROR_MESSAGE);
					if (Globals.isDebugMode())
					{
...
/*					if (wp.isOneHanded())
	...
	public boolean canEquip(String typeName)
	{
		...
		else if (typeName.equals("Weapon") || typeName.equals("Shield"))
		{
			...
			{
				JOptionPane.showMessageDialog(null, "Your hands are too full. Check weapons/shields already equipped.", Globals.s_APPNAME, JOptionPane.INFORMATION_MESSAGE);
				return false;
			}
			return true;
		}
...
//			bonusName = bonusName.toUpperCase();
	...
	public int getSkillBonusTo(String type, String aName)
	{
		...
		for (Iterator e = skillList.iterator(); e.hasNext();)
		{
			...
			{
				//
				// Don't add bonuses included in the skill with the same name
				// as it gets added on elsewhere
				//
...
//		int x = 0,j = 0;
/*		No longer necessary since stacking and prereqs are now functional. Leaving this in
		temporarily just in case... merton_monk@yahoo.com 12/12/01
		if (aName.equals("AC"))
		{
...
/*			No longer necessary due to stacking and pre-reqs now funcitoning
			if (aName.equals("AC"))
			{
...
/*		if (aName.equals("AC"))
		{
			...
			for (int i = 0; i < 7; i++)
				tempVal += statBonus[i];
			bonus += tempVal;
			setBonusStackFor(tempVal, type + "." + aName + "."); // this value stacks
		}
*/
		return bonus;
	}
...
/*P classlevel/5 + (classlevel +3)/5 Wheel of Time method. */
				case 11:
	...
	public String addSpell(String className, int spellLevel, ArrayList aFeatList, String spellName, String bookName)
	{
		...
		if (aSpell == null && bookName.equals("Known Spells"))
		...
		if ((!aClass.getMemorizeSpells() && listNum >= known) ||
			...
			for (Iterator e1 = aClass.getFeatAutos().iterator(); e1.hasNext();)
			{
				...
				if (aFeat == null && aString.endsWith(")"))
				{
					...
					if (aFeat != null)
					{
						...
						cFeat.getAssociatedList().add(subName);
					}
	...
	public ArrayList aggregateFeatList()
	{
		...
		for (Iterator e = vFeatList().iterator(); e.hasNext();)
		{
			...
			else if (aFeat.isMultiples())
			{
				...
				for (Iterator e1 = aFeat.getAssociatedList().iterator(); e1.hasNext();)
				{
					Object anObject = e1.next();
					if (!bFeat.getAssociatedList().contains(anObject))
						bFeat.getAssociatedList().add(anObject);
				}
			}
		}
		for (Iterator e = featAutoList().iterator(); e.hasNext();)
...
//			loadForceLine(fa, input);
						...
						for (; ;)
						{
							...
							else
							{
								for (; ;)
								{
									...
									JOptionPane.showMessageDialog(null, "You MUST make a selection", Globals.s_APPNAME, JOptionPane.INFORMATION_MESSAGE);
								}
							}
			...
			if (bFixMade)
			{
				JOptionPane.showMessageDialog(null, "Fixed illegal value in hit points. Current character hit points: " + this.hitPoints() + " not " + oldHp, Globals.s_APPNAME, JOptionPane.ERROR_MESSAGE);
			}
	...
	private int loadPcgVersion(BufferedReader br)
	{
		...
		{
			JOptionPane.showMessageDialog(null, "Could not load campaign data from character file.", Globals.s_APPNAME, JOptionPane.ERROR_MESSAGE);
			e.printStackTrace();
		}
	...
	private FORNode parseFORs(StringTokenizer tokens)
	{
		...
		while (tokens.hasMoreTokens())
		{
			...
			if (line.startsWith("|FOR"))
			{
				...
				/**
				 * bug fix:
				 * need to check if there are at least 2 tokens;
				 * else we do not have a FOR-loop,
				 * but a regular token like |FORCEPOINTS| or |FORTITUDE|
				 *
				 * author: Thomas Behr 30-12-01
				 */
	...
	private FORNode parseFORs(String forLine, StringTokenizer tokens)
	{
		...
		boolean exists = forVars.nextToken().charAt(0) == '1';
			...
			for (int y = 0; y < node.children().size(); y++)
			{
				if (node.children().get(y) instanceof FORNode)
				{
					...
					for (Iterator ivar = loopVariables.keySet().iterator(); ivar.hasNext();)
					{
						...
						for (int i = 0; i < minString.length() - aString.length(); i++)
							if (minString.substring(i, i + aString.length()).equals(aString))
							{
								String cString = minString.substring(i + aString.length());
								minString = minString.substring(0, i) + loopVariables.get(aString) + cString;
							}
						for (int i = 0; i < maxString.length() - aString.length(); i++)
							if (maxString.substring(i, i + aString.length()).equals(aString))
							{
								String cString = maxString.substring(i + aString.length());
								maxString = maxString.substring(0, i) + loopVariables.get(aString) + cString;
							}
						for (int i = 0; i < stepString.length() - aString.length(); i++)
							if (stepString.substring(i, i + aString.length()).equals(aString))
							{
								String cString = stepString.substring(i + aString.length());
								stepString = stepString.substring(0, i) + loopVariables.get(aString) + cString;
							}
					}
				...
				else
				{
					...
					for (Iterator ivar = loopVariables.keySet().iterator(); ivar.hasNext();)
					{
						...
						if (anObject == null)
							continue;
						String aString = anObject.toString();
						for (int i = 0; i < lineString.length() - aString.length(); i++)
							if (lineString.substring(i, i + aString.length()).equals(aString))
							{
								String cString = lineString.substring(i + aString.length());
								lineString = lineString.substring(0, i) + loopVariables.get(aString) + cString;
							}
					}
		...
		else
		{
			for (int y = 0; y < node.falseChildren().size(); y++)
			{
				if (node.falseChildren().get(y) instanceof FORNode)
				{
					...
					boolean exists = existsOnly;
					existsOnly = nextFor.exists();
					loopFOR(nextFor, getVariableValue(nextFor.min(), "", "").intValue(), getVariableValue(nextFor.max(), "", "").intValue(), getVariableValue(nextFor.step(), "", "").intValue(), output, fa);
					existsOnly = exists;
					loopVariables.remove(nextFor.var());
				}
	...
	public boolean evaluateExpression(String expr)
	{
		for (Iterator ivar = loopVariables.keySet().iterator(); ivar.hasNext();)
		{
			...
			if (anObject == null)
				continue;
			String aString = anObject.toString();
			for (int i = 0; i < expr.length() - aString.length(); i++)
				if (expr.substring(i, i + aString.length()).equals(aString))
				{
					String cString = expr.substring(i + aString.length());
					expr = expr.substring(0, i) + loopVariables.get(aString) + cString;
				}
		}
	...
	public int replaceToken(String aString, BufferedWriter output)
	{
		try
		{
				...
				aString.lastIndexOf('<') == -1 && aString.lastIndexOf('>') == -1)
			{
				...
				{
					if (region.equals(Globals.s_NONE))
						canWrite = false;
					return 0;
				}
			...
			else if (aString.equals("RACE"))
			{
				...
				if (tempRaceName.equals(Globals.s_NONE)) tempRaceName = getRace().getName();

				if (subRace.equals(Globals.s_NONE))
				{
				...
				else
				{
					...
					for (Iterator e = templateList.iterator(); e.hasNext();)
					{
						...
						// --- arcady.
						{
							...
							/* added by Emily Smirle (Syndaryl) Nov 12, 2001, fixed by Eric Statz (Immediately thereafter)*/
							{
							...
							else if (aLabel.equals("SR"))
							{
								...
								/*fa.write(output, new Integer(aTemplate.getSR(totalLevels(), totalHitDice(), getSize())).toString());*/
							}
							else if (aLabel.equals("CR"))
							{
								fa.write(output, Integer.toString(aTemplate.getCR(totalLevels(), totalHitDice(), getSize())));
								/*fa.write(output, new Integer(aTemplate.getCR(totalLevels(), totalHitDice(), getSize())).toString());*/
							}
							/* end added by Emily Smirle (Syndaryl) Nov 12, 2001*/
							else
							{
			...
			else if (aString.startsWith("CLASS"))
			{
				...
				for (Iterator e = classList.iterator(); e.hasNext();)
				{
					...
					if (i == -1 || cmp == 1)
					{
						...
						if (cmp < 2)
						{
							if (aClass.getSubClassName().equals(Globals.s_NONE) || aClass.getSubClassName().equals(""))
			...
			else if (aString.equals("HITDICE"))
			{
				...
				/**
				 * bug fix:
				 * error occured, if character had no class, e.g. monsters,
				 * due to it.next() although it.size() == 0
				 *
				 * author: Thomas Behr 01-01-02
				 */
				PCClass aClass;
				String aaClassLevel;
				...
				//
				int temp = calcStatMod(Globals.CONSTITUTION);
			...
			/**
                         * author: Thomas Behr 25-12-01
			 */
			else if (aString.equals("INITIATIVEMISC"))
			{
				final int initiativeMisc = initiativeMod() - calcStatMod(Globals.DEXTERITY);
					...
					for (int x = 0; x < getRace().getMovements().length; x++)
					{
						if (race.getMovementType(x).toUpperCase().equals(moveType.toUpperCase()))
						{
							...
							if (aString.equals("RATE"))
								fa.write(output, "" + movement(x) + Globals.getAbbrMovementDisplay());
						}
					}
				}
				else
				{
					fa.write(output, race.getMovementType(0) + " " + movement(0) + Globals.getAbbrMovementDisplay());
					for (int x = 1; x < getRace().getMovements().length; x++)
						fa.write(output, ", " + race.getMovementType(x) + " " + movement(x) + Globals.getAbbrMovementDisplay());
				}
			}
			...
			{ /* added Nov. 15, 2001 by Emily Smirle (Syndaryl) */
				StringTokenizer aTok = new StringTokenizer(aString, ".");
				...
				if (aTok.hasMoreTokens())
				{
					...
					{
						fa.write(output, race.getMovementType(moveIndex));
					}
					else if (fString.equals("RATE"))
					{
						fa.write(output, movement(moveIndex) + Globals.getAbbrMovementDisplay());
					}
				}
				else
				{
					fa.write(output, race.getMovementType(moveIndex) + " " + movement(moveIndex) + Globals.getAbbrMovementDisplay());
				}

			}/* end added Nov. 15, 2001 by Emily Smirle (Syndaryl) */
			else if (aString.equals("SIZE"))
			{
				fa.write(output, getSize());
			}
			else if (aString.equals("SIZELONG")) /* added Nov. 12, 2001 by Emily Smirle (Syndaryl) */
			{
				String sizeLong = "";
				char sizeShort = getSize().charAt(0);
				switch (sizeShort)
				{
					case 'D':
						fa.write(output, "Diminutive");
						break;
					case 'F':
						fa.write(output, "Fine");
						break;
					case 'T':
						fa.write(output, "Tiny");
						break;
					case 'S':
						fa.write(output, "Small");
						break;
					case 'M':
						fa.write(output, "Medium");
						break;
					case 'L':
						fa.write(output, "Large");
						break;
					case 'H':
						fa.write(output, "Huge");
						break;
					case 'G':
						fa.write(output, "Gigantic");
						break;
					case 'C':
						fa.write(output, "Collosal");
						break;
				}
			}/* end added Nov. 12, 2001 by Emily Smirle (Syndaryl) */
			...
			//SPELLLISTCAST0.0 KNOWN0.0 BOOK0.0 TYPE0
			{
				...
				if (aClass != null)
				{
					final int stat = getStatFromAbbrev(aClass.getSpellBaseStat());
					int knownNum = 0;
			...
			else if (aString.equals("TOTAL.WEIGHT"))
			{
				...
				fa.write(output, totalWeight.toString() + " " + Globals.getWeightDisplay());
			}
			else if (aString.equals("TOTAL.VALUE"))
			{
				Float totalValue = totalValue();
				fa.write(output, totalValue.toString() + " " + Globals.getCurrencyDisplay());
			}
		...
		{
			exc.printStackTrace();
			JOptionPane.showMessageDialog(null, "Error replacing " + aString, Globals.s_APPNAME, JOptionPane.ERROR_MESSAGE);
			return 0;
		}
	}
		...
		if (eq != null)
		{
			if (tempString.equals("LONGNAME"))
			{
				...
				fa.write(output, eq.getWeight().toString());
                        /**
                         * author: Thomas Behr 04-01-02
                         */
			else if (tempString.equals("TOTALWT"))
				fa.write(output, Float.toString(eq.qty().floatValue()*eq.getWeight().floatValue()));
			...
			else if (tempString.equals("SIZELONG")) /* added Nov 12, 2001 by Emily Smirle */
			{
			}
	...
	private void replaceTokenLanguages(String aString, FileAccess fa, BufferedWriter output)
	{
		...
		else
		{
			...
			for (Iterator setIter = getLanguagesList().iterator(); setIter.hasNext();)
			{
				if (c > 0)
					fa.write(output, ", ");
				fa.write(output, (String)setIter.next().toString());
				c++;
			}
		}
	}
	...
	private void replaceTokenSkillListMods(FileAccess fa, BufferedWriter output)
	{
		...
		for (Iterator e = getSkillList().iterator(); e.hasNext();)
		{
			...
			int modSkill = -1;
			if (aSkill.keyStat().compareToIgnoreCase(Globals.s_NONE) != 0)
			{
	 ...
	 *
         * @author: Thomas Behr 01-01-02
	 */
	private int acModFromDexterity()
	{
	...
	private int acModFromClass()
	{
		...
		acmod += getClassBonusTo("COMBAT", "AC");
                return acmod;
        }
	 ...
	 *
         * @author: Thomas Behr 01-01-02, 03-01-02
	 */
	private int acModFromAbilities()
	{
                int acmod = 0;

		acmod += acModFromDexterity();
		acmod += acModFromClass();
		acmod += getFeatBonusTo("COMBAT", "AC", true);
//  		acmod += getTemplateBonusTo("COMBAT", "AC", true);
//  		acmod += getRace().bonusTo("COMBAT", "AC");
		acmod += getSkillBonusTo("COMBAT", "AC");
		return acmod;
	}
	 ...
	 */
        private int acModFromNaturalBySource() 
        {
                int acmod = 0;

                // get natural boni to ac by race
                acmod += _calculateACBonusByType( "Natural" , getRace() );

                // get natural ac for templates
                for ( Iterator it = templateList.iterator(); it.hasNext(); ) {
			acmod += ( (PCTemplate) it.next() ).getNaturalArmor(totalLevels(), totalHitDice(), getSize());
                }
	 ...
	 * @author: Thomas Behr 03-01-02
	 */
        private int acModFromNatural() 
        {
                int acmod = 0;
                
                // get all boni to ac of TYPE=Natural
                acmod += _calculateACBonusByType( "Natural" , null );
         ...
         *
         * @param:  String type     - bonus type; this is case sensitive!!
         * @param:  PObject pObject - if this is null,
         *                            the total bonus of the specified type
         *                            will be calculated,
         *                            else only the bonus from the specified pObject
         *
         * @author: Thomas Behr 04-01-02
         */
        private int _calculateACBonusByType( String type , PObject pObject )
        {
                int acmod = 0;

                // get total bonus of this type
                if ( pObject == null ) {
                        getTotalBonusTo( "COMBAT" , "AC" , true );
                        String value = (String) bonusMap.get("COMBAT.AC."+type.toUpperCase());
                        if ( value != null ) {
                                acmod += Integer.parseInt(value);
                        }
                }
	 ...
	 *
         ...
         *
         * @author: Thomas Behr 30-12-01, 04-01-02
	 */
	private void replaceTokenAC(String aString, FileAccess fa, BufferedWriter output)
	{
		...
		for (; i < tokens.length; i++)
		{
			...
			{
				ac += acModFromAbilities() * mult;
			}
			else if (tokens[i].equals("ARMOR"))
			{
				ac += acModFromArmor() * mult;
			}
			else if (tokens[i].equals("CLASS"))
			{
				ac += acModFromClass() * mult;
			}
			else if (tokens[i].equals("DEFLECTION"))
			{
				ac += _calculateACBonusByType( "Deflection" , null ) * mult;
			}
			else if (tokens[i].equals("DEX"))
			{
				ac += acModFromDexterity() * mult;
			}
			else if (tokens[i].equals("DODGE"))
			{
				ac += _calculateACBonusByType( "Dodge" , null ) * mult;
			}
			...
			{
                                ac += acModFromNatural() * mult;
			}
			else if (tokens[i].equals("NATURAL_BYSOURCE"))
			{
                                ac += acModFromNaturalBySource() * mult;
			}
			else if (tokens[i].equals("SHIELD"))
			{
				ac += acModFromShield() * mult;
			}
	...
	private void replaceTokenProhibitedList(FileAccess fa, BufferedWriter output)
	{
		for (Iterator iter = classList.iterator(); iter.hasNext();)
		{
			...
			if (aClass.getLevel().intValue() > 0)
			{
				if (!aClass.getProhibitedString().equals(Globals.s_NONE))
	...
	private ArrayList getExpandedWeapons()
	{
		...
		for (int idx = 0; idx < weapList.size(); idx++)
		{
			...
			else if (equip.isMelee() && equip.isRanged())
			{
				...
				if (eqm.whatHand() == Equipment.TWOWEAPON_HANDS)
				{
					eqm.setHand(Equipment.PRIMARY_HAND);
				}
				eqm.setName(appendToName(eqm.getName(), "Thrown"));	// Add "Thrown" to the name of the weapon
				setProf(equip, eqm);
				weapList.add(idx + 1, eqm);
				idx += 1;				// Skip the item we just added
			}
		}
		return weapList;
	}
	...
	private void replaceTokenWeapon(String aString, FileAccess fa, BufferedWriter output)
	{
		...
		if (eq != null)
		{
			...
			{
				int dbl = getTotalBonusTo("WEAPONPROF=" + eq.profName(), "CRITRANGEDOUBLE", true);
			...
			{
				fa.write(output, eq.getRange().toString() + Globals.getAbbrMovementDisplay());
			}
			...
			else if (aString.startsWith("HIT") || aString.startsWith("TOTALHIT"))
			{
				...
				if (wp == null || !hasWeaponProfNamed(wp.getName()))
...
//				final int magichit = eq.bonusTo("WEAPON", "TOHIT") + eq.bonusTo("WEAPONPROF=" + eq.profName(), "TOHIT");
			...
			else if (aString.endsWith("DAMAGE"))
			{
				...
				int bonus = 0;
				final int strMod = calcStatMod(Globals.STRENGTH);
				...
				if (!aString.startsWith("BASE"))
				{
					...
					{
						if (isSecondaryWeapon(eq) && (eq != primaryWeapon) && (strMod > 0))
						{
							bInt = new Integer(bInt.intValue() + (strMod / 2));
						}
						else
						{
...
//					bonus = eq.bonusTo("WEAPON", "DAMAGE");
					bonus = eq.getBonusToDamage(true);
					for (Iterator ei = eq.typeArrayList().iterator(); ei.hasNext();)
					...
					{
						if (wp.getType().equals("Natural") && eq.isOnlyNaturalWeapon() && eq.modifiedName().endsWith("Primary"))
						{
							bonus += strMod / 2;
						}
					}
				...
				{
//					bonus = eq.bonusTo("WEAPON", "DAMAGE");
					bonus = eq.getBonusToDamage(false);
					for (Iterator ei = eq.typeArrayList().iterator(); ei.hasNext();)
					{
						bonus += getTotalBonusTo("DAMAGE", "TYPE=" + ei.next().toString(), true);
					}
					...
					if (strMod > 0)
					{
						bonus += strMod / 2;	// only get half strength bonus
					}
					else
					{
						bonus += strMod;
	...
	/**
	 * select items, which improve AC
         *
	 * @author: Thomas Behr 25-12-01, 06-01-02
	 */
		...
		int len, FileAccess fa, BufferedWriter output)
	{
		...
		if (item >= aArrayList.size() - 1 && existsOnly)
		{
			len = 0;
			noMoreItems = true;
		}
...
//                  if ( tempString.equals( "ARMOR" ) ) {
//                          tempString = aTok.nextToken();

//                          if ( tempString.startsWith( "ALL" ) ) {
//                                  index = Integer.parseInt( tempString.substring( 3 ) );
//                                  return _replaceTokenArmorSuit( index , aTok.nextToken() , 3 , len , fa , output );
//                          } elseif ( tempString.startsWith( "EQUIPPED" ) ) {
//                                  index = Integer.parseInt( tempString.substring( 8 ) );
//                                  return _replaceTokenArmorSuit( index , aTok.nextToken() , 1 , len , fa , output );
//                          } else if ( tempString.startsWith( "NOT_EQUIPPED" ) ) {
//                                  index = Integer.parseInt( tempString.substring( 12 ) );
//                                  return _replaceTokenArmorSuit( index , aTok.nextToken() , 2 , len , fa , output );
//                          }
//                          else if ( tempString.startsWith( "SUIT" ) ) {
//                                  property = aTok.nextToken();
//                                  if ( property.startsWith( "ALL" ) ) {
//                                          index = Integer.parseInt( property.substring( 3 ) );
//                                          property = aTok.nextToken();
//                                          equipped = 3;
//                                  } else if ( property.startsWith( "EQUIPPED" ) ) {
//                                          index = Integer.parseInt( property.substring( 8 ) );
//                                          property = aTok.nextToken();
//                                          equipped = 1;
//                                  } else if ( property.startsWith( "NOT_EQUIPPED" ) ) {
//                                          index = Integer.parseInt( property.substring( 12 ) );
//                                          property = aTok.nextToken();
//                                          equipped = 2;
//                                  } else {
//                                          index = Integer.parseInt( tempString.substring( 4 ) );
//                                  }
//                                  return _replaceTokenArmorSuit( index , property , equipped , len , fa , output );
//                          }
//                          else if ( tempString.startsWith( "SHIELD" ) ) {
//                                  property = aTok.nextToken();
...
//                                          equipped = 2;
//                                  } else {
//                                          index = Integer.parseInt( tempString.substring( 6 ) );
//                                  }
//                                  return _replaceTokenArmorShield( index , property , equipped , len , fa , output );
//                          }
//                          else if ( tempString.startsWith( "ITEM" ) ) {
//                                  property = aTok.nextToken();
//                                  if ( property.startsWith( "EQUIPPED" ) ) {
//                                          index = Integer.parseInt( property.substring( 8 ) );
//                                          property = aTok.nextToken();
...
//                                          index = Integer.parseInt( tempString.substring( 4 ) );
//                                  }
//                                  return _replaceTokenArmorItem( index , property , equipped , len , fa , output );
//                          }
//                  } else {
//                          index = Integer.parseInt( tempString.substring( 5 ) );
//                          return _replaceTokenArmor( index , aTok.nextToken() , 3 , len , fa , output );
//                  }
				...
				if (spellNumber == -1 && aSpell != null)
				{
					...
					else if (bSpell != null)
					{
						...
						else if (aLabel.startsWith("BONUSSPELL"))
						{
							...
							if (bSpell.isInSpecialty(aClass.getSpecialtyList(), aClass.getName(), spellLevel))
							{
								for (Iterator ip = getCharacterDomainList().iterator(); ip.hasNext();)
								{
									CharacterDomain aCD = (CharacterDomain)ip.next();
									if (aCD.getDomain() != null && aCD.getDomainSource().startsWith("PCClass|" + aClass.getName()))
	...
	private int replaceTokenSkill(String aString, int len, FileAccess fa, BufferedWriter output)
	{
		...
		if (aSkill != null)
		{
			if ((cmp == 5 || cmp == 6) && aSkill.keyStat().equals(Globals.s_NONE))
				fa.write(output, "n/a");
			else
				switch (cmp)
				{
	...
	private void printFeat(int numberPos, String aString, ArrayList anArrayList, FileAccess fa, BufferedWriter output)
	{
		...
		final int j = aString.lastIndexOf(".");
		int i = -1;
		if (j == -1)
	...
	public Float getVariableValue(String aString, String src, String subSrc)
	{
		...
		for (int i = 0; i < aString.length(); i++)
		{
				...
				(valString.length() > 3 && (valString.endsWith("MIN") || valString.endsWith("MAX") || valString.endsWith("REQ"))))
			{
				...
				if (valString.length() > 0 && getStatFromAbbrev(valString) > -1)
				{
					final int stat = getStatFromAbbrev(valString);
					valString = Integer.toString(calcStatMod(stat));
					if (Globals.isDebugMode())
						System.out.println("MOD=" + valString);
				}
				else if (valString.length() == 8 && getStatFromAbbrev(valString.substring(0, 3)) > -1 && valString.endsWith(".BASE"))
				{
					if (Globals.isDebugMode())
						System.out.print("STAT=" + valString.substring(0, 3));
					final int stat = getStatFromAbbrev(valString.substring(0, 3));
					valString = Integer.toString(calcUnAdjStatMod(stat));
					if (Globals.isDebugMode())
						System.out.println(" BASE=" + valString);
				}
				else if (valString.length() == 8 && getStatFromAbbrev(valString.substring(0, 3)) > -1 && valString.endsWith("SCORE"))
				{
					final int stat = getStatFromAbbrev(valString.substring(0, 3));
				...
				else if (valString.startsWith("CLASSLEVEL="))
				{
					...
					if (valString.length() > 11)
						aClass = getClassNamed(valString.substring(11));
					else
						System.out.println("Error! Cannot determine CLASS!");
					if (aClass != null)
						valString = aClass.getLevel().toString();
				...
				else if (valString.equals("SIZEMOD"))
				{
					...
					valString = String.valueOf(Globals.loadTypeForStrength(adjStats(Globals.STRENGTH), totalWeight()));
...
//            nextMode = 0;
				...
				{
//					JOptionPane.showMessageDialog(null, "Math error determining value for " + aString + " " + src + " " + subSrc + "(" + valString + ")", Globals.s_APPNAME, JOptionPane.ERROR_MESSAGE);
				}
				mode = nextMode;
				nextMode = 0;
				valString = "";
		...
		for (stat = 0; stat < Globals.s_ATTRIBLONG.length; stat++)
		{
			...
			switch (method)
			{
				case 0:
					...
					break;
				case 1:
					roll = RollingMethods.roll(4, 6, 0, 3, 0);		// roll 4d6, keep 3 highest, add 0 to total
					break;
				case 2:
					roll = RollingMethods.roll(3, 6, 0, 3, 0);		// roll 3d6, unmodified
					break;
				case 3:
					roll = RollingMethods.roll(5, 6, 0, 3, 0);		// roll 5d6, keep 3 highest, add 0 to total
					break;
				case 4:
					roll = RollingMethods.roll(4, 6, 0, 3, 1);		// 4d6, reroll 1's, keep 3 highest, add 0 to total
					break;
				case 5:
					roll = RollingMethods.roll(4, 6, 0, 3, 2);		// 4d6, reroll 1's and 2's, keep 3 highest, add 0 to total
					break;
				case 6:
					roll = RollingMethods.roll(3, 6, 5, 3, 0);		// roll 3d6, keep 3 highest, add 5 to total
					break;
				case 7:
					int[] keep = { 1, 3, 4 }; // 0-indexed
					roll = RollingMethods.roll(5, 6, keep, 0); // roll 5d6, drop lowest and middle, add 0 to total
					break;
			}
	...
	{
		Race aRace = (Race)Globals.getRaceMap().get(aString);
		setRace(aRace);
	}
	...
	private void setDeity(String aString)
	{
		...
		if (!bFound)
		{
			if (!aString.equals(Globals.s_NONE))
			{
				setDeity(Globals.s_NONE);
				JOptionPane.showMessageDialog(null, "Diety not found: " + aString + s_CHECKLOADEDCAMPAIGNS, Globals.s_APPNAME, JOptionPane.ERROR_MESSAGE);
			}
		}
	}
...
//				fa.write(output, eq.getKeyName());
				...
				if (!eq.getSize().equals(eq.getBaseSize()) || (eqMod1.length() != 0) || (eqMod2.length() != 0))
				{
					fa.write(output, ";" + eq.getSize() + ";");
					if (eqMod1.length() != 0)
					{
					...
					{
						fa.write(output, Globals.s_NONE);
					}
					fa.write(output, ";");
					if (eqMod2.length() != 0)
					{
	...
	private void saveMiscLine(FileAccess fa, BufferedWriter output)
	{
		fa.write(output, escapeColons2(eyeColor) + " :" + escapeColons2(skinColor) + " :" + escapeColons2(hairColor) + " :" +
			escapeColons2(hairStyle) + " :" + escapeColons2(speechTendency) + " :" + escapeColons2(phobias) + " :" +
...
/* added 08/27/01 by TNC */
	...
	private void loadEquipmentLine(FileAccess fa, BufferedReader input)
	{
		...
		while (aTok.hasMoreTokens())
		{
			...
			if (((sized.length() != 0) && !eq.getSize().equals(sized)) || (eq.getEqModifierList(true).size() + eq.getEqModifierList(false).size() != 0))
			{
				eq.resizeItem(sized);
			...
			{
				JOptionPane.showMessageDialog(null, "Equipment not found: " + aName + " (" + eq.qty() + ")" + s_CHECKLOADEDCAMPAIGNS, Globals.s_APPNAME, JOptionPane.ERROR_MESSAGE);
			}
		}
		...
		while (aTok.hasMoreElements())
		{
			...
			switch (i++)
			{
				case 0:
					...
					{
						JOptionPane.showMessageDialog(null, "Race not found: " + aString + s_CHECKLOADEDCAMPAIGNS, Globals.s_APPNAME, JOptionPane.ERROR_MESSAGE);
					}
					break;
				case 1:
					setAlignment(l, true);
				...
				default:
					if (hitPointList != null)
					{
						...
						if (aRace != null)
						{
							if (x == aRace.hitDice())
							{
								aRace.setHitPointList(hitPointList);
								return;
							}
						}
					}
					else
					{
						...
						if (x == 1)
						{
							JOptionPane.showMessageDialog(null, "Saved race (" + raceName + ") no longer has a HITDICE tag.", Globals.s_APPNAME, JOptionPane.ERROR_MESSAGE);
						}
					}
					break;
			}
				...
				default:
					...
					if (j >= 0)
					{
						...
						else
						{
							...
							{
								JOptionPane.showMessageDialog(null, "Domain not found: " + aString + s_CHECKLOADEDCAMPAIGNS, Globals.s_APPNAME, JOptionPane.ERROR_MESSAGE);
							}
						}
					}
					break;
	...
	private void handleSkillsLine(String skillsLine)
	{
		...
		while (skillTokenizer.hasMoreElements())
		{
			...
			else
			{
				...
				{
					JOptionPane.showMessageDialog(null, "Ranked skill not found: " + skillName + "(" + aFloat + ")" + s_CHECKLOADEDCAMPAIGNS, Globals.s_APPNAME, JOptionPane.INFORMATION_MESSAGE);
				}
			}
		}
	}
	...
	private void loadClassesLine(FileAccess fa, BufferedReader input)
	{
		...
		while (aTok.hasMoreTokens())
		{
			...
			{
				JOptionPane.showMessageDialog(null, "Class not found: " + aName + s_CHECKLOADEDCAMPAIGNS, Globals.s_APPNAME, JOptionPane.ERROR_MESSAGE);
			}
			...
			{
				aString = aTok.nextToken();
				if ((getStatFromAbbrev(aString.toUpperCase()) > -1) || aString.equalsIgnoreCase(Globals.s_NONE) || aString.equalsIgnoreCase("Any") || aString.equalsIgnoreCase("SPELL"))
				{
					if (aClass != null)
					{
	...
	private void loadStatsLine(FileAccess fa, BufferedReader input)
	{
		...
		{
			JOptionPane.showMessageDialog(null, "Number of Stats for character is " + statCount +
				". PCGen is currently using " + Globals.s_ATTRIBLONG.length + ". Cannot load character."
				, Globals.s_APPNAME, JOptionPane.ERROR_MESSAGE);
			return;
		}
	...
	private int getStatFromAbbrev(String sAbbrev)
	{
		for (int stat = 0; stat < Globals.s_ATTRIBSHORT.length; stat++)
		{
			if (sAbbrev.equalsIgnoreCase(Globals.s_ATTRIBSHORT[stat]))
				return stat;
		}
		return -1;
	}
