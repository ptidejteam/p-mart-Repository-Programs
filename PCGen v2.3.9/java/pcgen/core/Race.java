			...
			if (i >= 0)
			{
				switch (i)
				{
					...
					case 3:
						...
						total = min + RollingMethods.roll(diceNum, diceSide) + bonus;
						aPC.setHeight(total);
...
//				System.out.println("aString: " + aString);
		...
		char aChar = Globals.s_SIZESHORT[sizeInt].charAt(0);			// This code is going to have problems with 'C' sized creatures, 'cuz they can't have a weapon larger than them
		...
		while (attackTok.hasMoreTokens())
		{
			...
			eq.append((String)aTok.nextToken()).append("\tTYPE:").append((String)aTok.nextToken()).append("\tWT:0\tCost:0\tSIZE:").append(aChar);
...
//				System.out.println("Eq:\n" + eq);
			...
			anEquip.parseLine(eq.toString(), sourceFile, lineNum);
	...
	public void rollHp()
	{
		...
		final Integer maxHp = new Integer(hp);
...
//		aRace.type = type;
	...
	public void parseLine(String inputLine, File sourceFile, int lineNum)
	{
		...
		for (int col = 0; col < colMax; col++)
		{
			...
			else if ((col < 7) || aString.startsWith("STATADJ"))
			{
				int iStat = col;
				if (col > 7)
				{
					iStat = Integer.parseInt(aString.substring(7, 8));
				...
				if ((iStat > 0) && (iStat <= Globals.s_ATTRIBLONG.length))
				{
					...
					{
						setStatMod(iStat, 0);
						nonability[iStat] = true;
					}
					else
					{
						setStatMod(iStat, Integer.parseInt(aString));
						nonability[iStat] = false;
					}
				}
			}
			else if (col == 7)
			...
			else if (aString.startsWith("RACENAME:"))
			{
				raceOutputName = aString.substring(9);
			}
...
// Pass into PREALIGN instead
				preReqArrayList.add("PREALIGN:" + Globals.commaDelimit(aString.substring(3)));
...
// HitDieLock
			...
			else if (aString.startsWith("CCSKILL"))
			{
				setCcSkillList(aString.substring(8));
			}
			...
			else if (aString.startsWith("HITDICEADVANCEMENT"))
			{
				...
				for (int x = 0; x < hitDiceAdvancement.length; x++)
				{
					...
					if (temp.startsWith("*"))
						unlimitedAdvancement = true;
					if (unlimitedAdvancement)
			...
			else if (aString.startsWith("MONSTERCLASS:"))
			{
				StringTokenizer mclass = new StringTokenizer(aString.substring(13), ":");
				if (mclass.countTokens() != 2)
					JOptionPane.showMessageDialog(null, "Illegal Monster Class info " + sourceFile.getName() + ":" + Integer.toString(lineNum) + " \"" + aString + "\"", "PCGen", JOptionPane.ERROR_MESSAGE);
				monsterClass = mclass.nextToken();
				monsterClassLevels = Integer.parseInt(mclass.nextToken());
			}
			...
			{
				setType(aString.substring(Globals.s_TAG_TYPE.length()));
			}
