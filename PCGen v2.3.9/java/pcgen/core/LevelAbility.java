	...
	public void process()
	{
		...
		if (aArrayList.size() > 0 && type != 7)
		{
			...
			if (Globals.isDebugMode())
				System.out.println("choice: " + c.getSelectedList() + eString);
	...
	void processChoice(ArrayList aArrayList, List selectedList, String aString)
	{
		...
		switch (type)
		{
			case 4:
				...
				{
					final String zString = new StringBuffer().append(zText).append(selectedList.get(index)).append(")").append(aString).toString();
			...
			case 6:
				if (selectedList.size() > 0)
				{
					...
					while (dnum > 0)
					{
						...
						Globals.getCurrentPC().getCharacterDomainList().add(aCD);
						dnum--;
					}
				}
				break;
