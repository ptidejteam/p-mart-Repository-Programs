...
public class PObject extends Object implements Cloneable, Serializable
{
	...
	{
		sourceFileIndex = Globals.saveSourceFile(sourceFile);
	}
	...
	public String getSource()
	{
		...
		{
			return Globals.savedSourceFile(sourceFileIndex);
		}

		return Globals.savedSource(sourceIndex) + source;
	}
	...
	protected void setBaseQty(String aString)
	{
	...
	public boolean parseTagLevel(String aTag, int anInt)
	{
		...
		{
			final String src = Globals.getCurrentSource();
			if (src.length() > 0)
			{
				setSource(Globals.saveSource(src), aTag.substring(7));
			}
			else
			{
					...
					if (aPC.getDeity() != null)
					{
						...
						while (bTok.hasMoreTokens())
						{
							...
							if (adding)
							{
								...
								final String skipAToken = cTok.nextToken();
	...
	public boolean passesPreReqTestsForList(PObject aObj, ArrayList anArrayList)
	{
		...
		for (Iterator e = anArrayList.iterator(); e.hasNext();)
		{
			...
			if (aPC != null)
			{
				...
				{
					if (Globals.isDebugMode())
						System.out.println("In Qualify list:" + this + " -> Qualify list:" + aPC.getQualifyList());
					qualifyValue = true;
				}
//				else
//				{
...
//			If any class is over in level this should return false.
				...
				else if (aType.startsWith("PREVAR"))
				{
					...
					while (aTok.hasMoreTokens() && flag)
					{
						...
						final float bFloat = aPC.getVariable(varName, true, true, "", "").floatValue();
						switch (i)
						{
							case 0:
								flag = (aFloat < bFloat);
				...
				else if (aType.equals("PREFORT"))
				{
					...
					final int fort = aPC.getBonus(1, true) + aPC.calcStatMod(Globals.CONSTITUTION);
					flag = (fort >= nextInt);
				}
				else if (aType.equals("PREWILL"))
				{
					final int nextInt = Integer.parseInt(aList);
					final int will = aPC.getBonus(3, true) + aPC.calcStatMod(Globals.CONSTITUTION);
					flag = (will >= nextInt);
				}
				else if (aType.equals("PREREFLEX"))
				{
					final int nextInt = Integer.parseInt(aList);
					final int ref = aPC.getBonus(2, true) + aPC.calcStatMod(Globals.CONSTITUTION);
					flag = (ref >= nextInt);
				}
			...
			if (Globals.isDebugMode())
				System.out.println(aType + ":" + aList + "=" + flag + " Qvalue:" + Qvalue + " qualifyValue:" + qualifyValue);
		...
		for (Iterator e = anArrayList.iterator(); e.hasNext();)
		{
			...
			else if (aType.equals("PREALIGN"))
			{
				...
				while (aTok.hasMoreTokens())
				{
					...
					if ((raceNumber >= 0) && (raceNumber < Globals.s_ALIGNLONG.length))
					{
						if (i++ > 0)
							pString.append(",");
					...
					{
						JOptionPane.showMessageDialog(null, "Invalid alignment: " + raceNumber, Globals.s_APPNAME, JOptionPane.INFORMATION_MESSAGE);
					}
				}
			}
			else
