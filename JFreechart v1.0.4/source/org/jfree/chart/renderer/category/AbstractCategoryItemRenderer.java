 ...
 *
 * $Id: AbstractCategoryItemRenderer.java,v 1.1 2007/10/10 20:46:51 vauchers Exp $
 *
 * Changes:
 * --------
...
public abstract class AbstractCategoryItemRenderer extends AbstractRenderer
    ...
    public void drawRangeMarker(Graphics2D g2,
        ...
        if (marker instanceof ValueMarker) {
            ...
            if (orientation == PlotOrientation.HORIZONTAL) {
                rect = new Rectangle2D.Double(Math.min(start2d, end2d), 
                        dataArea.getMinY(), Math.abs(end2d - start2d), 
                        dataArea.getHeight());
            }
            else if (orientation == PlotOrientation.VERTICAL) {
                rect = new Rectangle2D.Double(dataArea.getMinX(), 
                        Math.min(start2d, end2d), dataArea.getWidth(), 
                        Math.abs(end2d - start2d));
            }
            ...
            if (im.getOutlinePaint() != null && im.getOutlineStroke() != null) {
                ...
                if (orientation == PlotOrientation.VERTICAL) {
                    ...
                    Line2D line = new Line2D.Double(x0, y0, x0, y1);
                    g2.setPaint(im.getOutlinePaint());
                    g2.setStroke(im.getOutlineStroke());
                    g2.draw(line);
