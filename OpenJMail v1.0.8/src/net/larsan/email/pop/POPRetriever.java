...
public class POPRetriever {
    ...
    */
    ...
    /**
    * Send "UIDL" command on a single message. Returns a MessageMark where the public variable
    * "number" contains the number of the message and "uidl" it's uidl.
    */

    public MessageMark uidl(int number) throws POPException, IOException {
        write("UIDL ");
        writeln(number);
        String response = consume();
        return parseUIDLMarkString(response);
    }
    
    
    /**
    * Set UIDL command. Returns a MessageMark array where each memebr represents
    * a single message and containing it's message number and UIDL.
    */
    
    public MessageMark[] uidl() throws POPException, IOException {
        ...
        while((tmp = br.readLine()) != null) {
            try {
                // parse the line and add to buffer
                list.add(parseUIDLMarkString(tmp));
            } catch(POPException e) { }
        }
    ...
    */
    ...
    private String consume(boolean multiline) throws POPException, IOException {
        ...
        // delete the first three charcters, ie: "+OK" - we already know that
        return (answer.substring(3)).toString().trim();
    ...
    // tokenize a strign into message marks containing UIDs
    private MessageMark parseUIDLMarkString(String tmp) throws POPException {        
        StringTokenizer st = new StringTokenizer(tmp, " ");
        try {
            int n = Integer.parseInt(st.nextToken());
