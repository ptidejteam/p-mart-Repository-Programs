 ...
 *                   Arnaud Lelievre;
 *
 * $Id: PiePlot.java,v 1.1 2007/10/10 19:29:15 vauchers Exp $
 *
...
public class PiePlot extends Plot implements Cloneable, Serializable {
    ...
    public static final Paint DEFAULT_OUTLINE_PAINT = Color.lightGray;
    ...
    private PieItemLabelGenerator labelGenerator;
    ...
    private PieItemLabelGenerator toolTipGenerator;

    /** The URL generator. */
    private PieURLGenerator urlGenerator;

    /** Log4j logging. */
    static Logger logger = Logger.getLogger(PiePlot.class);
    ...
    /**
     * Sets the paint used to fill a section of the pie and sends a {@link RendererChangeEvent} to
    ...
    /**
     * Sets the stroke used to fill a section of the pie and sends a {@link RendererChangeEvent} to
     * all registered listeners.
     *
     * @param section  the section index (zero-based).
     * @param stroke  the stroke (<code>null</code> permitted).
     ...
     */
    public PieItemLabelGenerator getLabelGenerator() {
     ...
     */
    public void setLabelGenerator(PieItemLabelGenerator generator) {
    ...
    /**
     * Returns the margin between the edge of the pie and the link point.
     * 
    ...
    /**
     * Sets the link paint and sends a {@link PlotChangeEvent} to all registered listeners.
     * 
    ...
    public PiePlotState initialise(Graphics2D g2,
        ...
        state.setTotal(DatasetUtilities.getPieDatasetTotal(plot.getDataset()));
    ...
    public void draw(Graphics2D g2, Rectangle2D plotArea, PlotState parentState,
        ...
        if (logger.isInfoEnabled()) {
            logger.info("Entering draw() method, plot area = " + plotArea.toString());   
        }
    ...
    protected void drawPie(Graphics2D g2, 
        ...
        double gapHorizontal = plotArea.getWidth() 
            * (this.interiorGap + this.labelGap + this.maximumLabelWidth + this.labelLinkMargin);
        ...
        state.setLinkWRadius(linkArea.getWidth() / 2.0);
        state.setLinkHRadius(linkArea.getHeight() / 2.0);       
        ...
        if ((this.dataset != null) && (this.dataset.getKeys().size() > 0)) {
            ...
            double totalValue = DatasetUtilities.getPieDatasetTotal(this.dataset);
                ...
                if (state.getInfo() != null) {
                    ...
                    if (entities != null) {
                        ...
                        if (this.toolTipGenerator != null) {
                            tip = this.toolTipGenerator.generateToolTip(dataset, key);
                        }
                        String url = null;
                        if (this.urlGenerator != null) {
                            url = this.urlGenerator.generateURL(dataset, key, pieIndex);
                        }
                        PieSectionEntity entity = new PieSectionEntity(
                            arc, dataset, pieIndex, section, key, tip, url
                        );
        ...
        if (this.labelGenerator != null) {
            ...
            for (int i = 0; i < leftKeys.getItemCount(); i++) {   
                TextBlock block = TextUtilities.createTextBlock(
                    this.labelGenerator.generateSectionLabel(
                        this.dataset, leftKeys.getKey(i)
                    ), 
                    this.labelFont, this.labelPaint, maxLabelWidth, new G2TextMeasurer(g2)
                );
                TextBox label = new TextBox(block);
                double theta = Math.toRadians(leftKeys.getValue(i).doubleValue());
                double baseY = state.getPieCenterY() - Math.sin(theta) * state.getLinkHRadius();
                double hh = label.getHeight(g2);
                ...
                distributor1.addPieLabelRecord(
                    new PieLabelRecord(
                        leftKeys.getKey(i), theta, baseY, label, hh,
                        lGap / 2.0 + lGap / 2.0 * -Math.cos(theta), 
                        0.9 + getExplodePercent(this.dataset.getIndex(leftKeys.getKey(i)))
                    )
                );
            }
            distributor1.distributeLabels(linkArea.getMinY(), linkArea.getHeight());
            for (int i = 0; i < distributor1.getItemCount(); i++) {
                drawLeftLabel(g2, state, distributor1.getPieLabelRecord(i));
            }

            // draw the right labels...
            PieLabelDistributor distributor2 = new PieLabelDistributor(rightKeys.getItemCount());
            for (int i = 0; i < rightKeys.getItemCount(); i++) {
                TextBlock block = TextUtilities.createTextBlock(
                    this.labelGenerator.generateSectionLabel(this.dataset, rightKeys.getKey(i)),
                    this.labelFont, this.labelPaint, maxLabelWidth, new G2TextMeasurer(g2)
                );
                TextBox label = new TextBox(block);
                double theta = Math.toRadians(rightKeys.getValue(i).doubleValue());
                double baseY = state.getPieCenterY() - Math.sin(theta) * state.getLinkHRadius();
                double hh = label.getHeight(g2);
                if (logger.isDebugEnabled()) {
                    logger.debug("Label height = " + hh);   
                ...
                distributor2.addPieLabelRecord(
                    new PieLabelRecord(
                        rightKeys.getKey(i), theta, baseY, label, hh,
                        lGap / 2.0 + lGap / 2.0 * Math.cos(theta), 
                        0.9 + getExplodePercent(this.dataset.getIndex(rightKeys.getKey(i)))
                    )
                );
            }
     ...
     */
    public void drawLeftLabel(Graphics2D g2, PiePlotState state, PieLabelRecord record) {
        ...
        double elbowX = state.getPieCenterX() + Math.cos(theta) * state.getLinkWRadius();
        double elbowY = state.getPieCenterY() - Math.sin(theta) * state.getLinkHRadius();
     ...
     * @param record  the label record.
     */
    public void drawRightLabel(Graphics2D g2, PiePlotState state, PieLabelRecord record) {
        double theta = record.getAngle();
        double linkX = state.getPieCenterX() 
                       + Math.cos(theta) * state.getPieWRadius() * record.getLinkPercent();
        double linkY = state.getPieCenterY() 
    ...
    public boolean equals(Object obj) {
        ...
        if (super.equals(obj) && obj instanceof PiePlot) {
            ...
            boolean b21 = (this.maximumLabelWidth == plot.maximumLabelWidth);
            boolean b22 = (this.labelGap == plot.labelGap);
            boolean b23 = (this.labelLinkMargin == plot.labelLinkMargin);
            boolean b24 = ObjectUtils.equal(this.labelLinkPaint, plot.labelLinkPaint);
            boolean b25 = ObjectUtils.equal(this.labelLinkStroke, plot.labelLinkStroke);
            boolean b26 = ObjectUtils.equal(this.toolTipGenerator, plot.toolTipGenerator);
            boolean b27 = ObjectUtils.equal(this.urlGenerator, plot.urlGenerator);
            boolean b28 = (this.minimumArcAngleToDraw == plot.minimumArcAngleToDraw);
