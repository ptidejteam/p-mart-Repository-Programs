 ...
 * Contributor(s):   -;
 *
 * $Id: CategoryAxis.java,v 1.1 2007/10/10 19:29:22 vauchers Exp $
 *
...
public class CategoryAxis extends Axis implements Cloneable, Serializable {
    ...
    /** Log4j logging. */
    static Logger logger = Logger.getLogger(CategoryAxis.class);
    ...
    public AxisState draw(Graphics2D g2, 
        ...
        if (logger.isInfoEnabled()) {
            logger.info("Entering draw() method, cursor = " + cursor);
        }
    ...
    protected AxisState drawCategoryLabels(Graphics2D g2,
        ...
        if (logger.isInfoEnabled()) {
            logger.info("Entering drawCategoryLabels() method, cursor = " + state.getCursor());
        }
        if (state == null) {
    ...
    protected double calculateTextBlockWidth(TextBlock block, 
        ...
        Dimension2D size = block.calculateDimensions(g2);
