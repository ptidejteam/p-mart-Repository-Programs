 ...
 *                   Nicolas Brodu;
 *
 * $Id: TextTitle.java,v 1.1 2007/10/10 19:29:24 vauchers Exp $
 *
...
public class TextTitle extends Title implements Serializable, Cloneable {
    ...
    /** Log4j logging. */
    static Logger logger = Logger.getLogger(TextTitle.class);
    ...
    public float getPreferredWidth(Graphics2D g2, float height) {
        ...
        if (this.text != null && !this.text.equals("")) {
            ...
            Dimension2D d = title.calculateDimensions(g2);
            result = (float) getSpacer().getAdjustedWidth(d.getWidth());
        }
    ...
    public float getPreferredHeight(Graphics2D g2, float width) {
        ...
        if (this.text != null && !this.text.equals("")) {
            g2.setFont(this.font);
            TextBlock title = TextUtilities.createTextBlock(
                this.text, this.font, this.paint, width, new G2TextMeasurer(g2)
            );
            Dimension2D d = title.calculateDimensions(g2);
    ...
    public void draw(Graphics2D g2, Rectangle2D area) {
        if (logger.isInfoEnabled()) {
            logger.info("Drawing title to area " + area.toString());
        }
        if (this.text.equals("")) {
            return;
        }
