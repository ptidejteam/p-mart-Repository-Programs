...
//				FileAccess.encodeWrite(output, aPC.getSubRegion());
			...
			else if (aString.startsWith("TEMPLATE"))
			{
				...
				replaceTokenFollowers(aString, fa, output);
			}
...
/*				Float aFloat = new Float(aPC.multiclassXPMultiplier().floatValue() * 100.0);
			...
			{
				replaceTokenBaseMovement(aString, fa, output);
			}
			...
			{
				FileAccess.write(output, Integer.toString(aPC.getTotalBonusTo("CLASS", "DEFENSE", true)));
			}
...
//				aPC.doVision(aPC.getVision(), 2);
	 ...
	 */
	private void replaceTokenBaseMovement(String aString, FileAccess fa, BufferedWriter output)
	{
		if ((aPC.getRace() != null) && !aPC.getRace().equals(Globals.s_EMPTYRACE))
		{
	...
	private void replaceTokenFollowers(String aString, FileAccess fa, BufferedWriter output)
	{
		...
		if (!followers.isEmpty()){ /* if it's empty, do nothing */
	...
	public String replaceTokenEq(String aString)
	{
		...
		if (aString.startsWith("EQTYPE"))
		{
			...
			{
				aList = aPC.getEquipmentOfTypeInOutputOrder(aType, 3);
			}
		}
		else
		{
