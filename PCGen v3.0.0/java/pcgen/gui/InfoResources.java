...
public class InfoResources extends FilterAdapterPanel
{
	...
	protected FollowerModel availableModel = null;  // available Model
	protected FollowerModel selectedModel = null;   // selected Model
	protected JTreeTable availableTable;       // available table
	protected JTreeTable selectedTable;        // selected table
	...
	private JSplitPane topSplit = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT);
	private JSplitPane botSplit = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT);
	private JSplitPane centerSplit = new JSplitPane(JSplitPane.VERTICAL_SPLIT);
	...
	protected JComboBox viewModeBox = new JComboBox();
	private final JLabel sortLabel = new JLabel("Sort");
	protected JComboBox viewSortBox = new JComboBox();
	...
	private class resPopupMenu extends JPopupMenu
	{
		...
		resPopupMenu(JTreeTable treeTable)
		{
			...
			{
				resPopupMenu.this.add(createAddMenuItem("Add to List", "control EQUALS"));
			}
			else // selectedTable
			{
...
//Globals.errorPrint("--done-- focusGained: "+aPC.getName());
	...
	private void createMasterView()
	{
		...
		sButton.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent evt)
			{
				...
				topSplit.setOrientation(splitOrientation);
	...
	private void initActionListeners()
	{
		...
		addComponentListener(new ComponentAdapter()
		{
			public void componentResized(ComponentEvent e)
			{
				...
				if (i > 0)
					SettingsHandler.setPCGenOption("InfoEquipping.topSplit", i);
				i = botSplit.getDividerLocation();
				if (i > 0)
					SettingsHandler.setPCGenOption("InfoEquipping.botSplit", i);
				i = centerSplit.getDividerLocation();
				if (i > 0)
					SettingsHandler.setPCGenOption("InfoEquipping.centerSplit", i);
			}
		});
	...
	private void setFollowerInfo(Object obj)
	{
		...
		if (obj instanceof PlayerCharacter)
		{
			...
			if (Globals.isDndMode())
				b.append("<b>HP</b>: ").append(mPC.hitPoints());
			b.append("<br>");
			int bonus = mPC.getBonus(PlayerCharacter.ATTACKBONUS, true);
	 ...
	 **/
	private String followerStatBlock(Follower aF, PlayerCharacter newPC)
	{
		StringBuffer b = new StringBuffer();
		...
		{
			b.append("<b>").append(Globals.s_ATTRIBSHORT[i]).append("</b>: ").append(newPC.adjStats(i)).append(" ");
		}
		b.append("<br>");

		if (Globals.isStarWarsMode() || Globals.isWheelMode())
		{
			b.append("<b>Defense</b>  ");
		}
		else
		{
			b.append("<b>AC</b>  ");
		}
		b.append("<i>Total</i>: ").append(newPC.totalAC());
		...
		b.append("<br>");
		int bonus = newPC.getBonus(PlayerCharacter.ATTACKBONUS, true);
		b.append(" <b>BAB</b>: ").append((bonus >= 0)?"+":"").append(bonus);
		b.append("<br>");
		if (Globals.isStarWarsMode())
		{
			b.append(" <b>VP</b>: ").append(newPC.hitPoints());
			b.append(" <b>WP</b>: ").append(newPC.woundPoints());
		}
		else if (Globals.isSidewinderMode())
		{
			b.append(" <b>Vigor</b>: ").append(newPC.hitPoints());
			b.append(" <b>Body Points</b>: ").append(newPC.woundPoints());
		}
		else
		{
			b.append(" <b>HP</b>: ").append(newPC.hitPoints());
		}
		b.append("<br>");

		b.append("<b>Saves</b>: ");
		bonus = newPC.getBonus(PlayerCharacter.FORTBONUS, true);
		int statMod = newPC.calcStatMod(Constants.CONSTITUTION);
		b.append(" <i>Fort</i>: ").append((bonus >= 0)?"+":"").append(bonus);
		if (statMod != 0) b.append(" (").append((bonus + statMod >= 0)?"+":"").append(bonus + statMod).append(")");
		bonus = newPC.getBonus(PlayerCharacter.REFBONUS, true);
		statMod = newPC.calcStatMod(Constants.DEXTERITY);
		b.append(" <i>Ref</i>: ").append((bonus >= 0)?"+":"").append(bonus);
		if (statMod != 0) b.append(" (").append((bonus + statMod >= 0)?"+":"").append(bonus + statMod).append(")");
		bonus = newPC.getBonus(PlayerCharacter.WILLBONUS, true);
		statMod = newPC.calcStatMod(Constants.WISDOM);
		b.append(" <i>Will</i>: ").append((bonus >= 0)?"+":"").append(bonus);
		if (statMod != 0) b.append(" (").append((bonus + statMod >= 0)?"+":"").append(bonus + statMod).append(")");
			...
			public void valueChanged(ListSelectionEvent e)
			{
				if (!e.getValueIsAdjusting())
				{
						...
						!(fN.getItem() instanceof String))
					{
						...
						if (fN.getItem() instanceof Follower)
						{
							loadButton.setText("Load from File");
	...
	private void formComponentShown(ComponentEvent evt)
	{
		...
		{
			hasBeenSized = true;
			top = SettingsHandler.getPCGenOption("InfoResources.topSplit", (int)(this.getSize().getWidth() * 7 / 10));
			bot = SettingsHandler.getPCGenOption("InfoResources.botSplit", (int)(this.getSize().getWidth() - 300));
			cent = SettingsHandler.getPCGenOption("InfoResources.centerSplit", (int)(this.getSize().getHeight() - 100));
...
//Globals.errorPrint("IR  --done-- updateCharacterInfo");
	...
	private void addButton(ActionEvent evt)
	{
		...
		if (aPC.getFileName().equals(""))
		{
			JOptionPane.showMessageDialog(null,
				"You must save the current character first",
		...
		if ((fNode.getItem() instanceof Race))
		{
			...
			Object nValue = JOptionPane.showInputDialog(null,
				"Please enter a name for new " + target + ":",
				Constants.s_APPNAME,
			...
			newPC.setStats(new int[]{10, 10, 10, 10, 10, 10});
			...
			//Globals.getRootFrame().getCharacterPane().setSelectedIndex(0);
	...
	private void delButton(ActionEvent evt)
	{
		...
		{
			aPC.delEquipment((Equipment)fNode.getItem());
		}
		else
		{
			return;
	...
	private void loadButton(ActionEvent evt)
	{
		...
		if (fNode.getItem() instanceof Follower)
		{
			...
			((PCGen_Frame1)Globals.getRootFrame()).loadPCFromFile(file);
	 ...
	 **/
	public class FollowerModel extends AbstractTreeTableModel implements TreeTableModel
	{
		...
		protected int modelType = MODEL_AVAILABLE;
		...
		private final int SEL_COL_NAME = 10;
		private final int SEL_COL_TYPE = 11;
		private final int SEL_COL_FILE = 12;
		// view modes for Famliar tables
		public final int VIEW_TYPE = 0;
		public final int VIEW_NAME = 1;

		// column positions for Item tables
		private final int ITEM_COL_TYPE = 0;
		private final int ITEM_COL_NAME = 1;
		private final int ITEM_COL_SRC = 2;
		// view modes for Item tables
		public final int ITEM_VIEW_TYPE = 0;
		public final int ITEM_VIEW_NAME = 1;

		// column positions for Vehicle tables
		private final int VEHICLE_COL_TYPE = 0;
		private final int VEHICLE_COL_NAME = 1;
		private final int VEHICLE_COL_SRC = 2;
		// view modes for Vehicle tables
		public final int VEHICLE_VIEW_TYPE = 0;
		public final int VEHICLE_VIEW_NAME = 1;

		/**
		 * Creates a FollowerModel
		 **/
		public FollowerModel(int mode, boolean available)
		{
			super(null);
			...
			//
			avaNameList = new String[]
			{
				"Type/Name", "Size", "Speed", "Vision", "Alignment", "Type/Race", "Source"
			};

			selNameList = new String[]
			{
				"Type/Name", "Type/Race", "File Name"
			};
		 ...
		 **/
		public void resetModel(int mode, boolean available)
		{
			...
			for (Iterator iClass = aPC.getClassList().iterator(); iClass.hasNext();)
			{
				...
				for (Iterator iComp = Globals.getCompanionModList().iterator(); iComp.hasNext();)
				{
					...
					if ((aComp.getClassMap().containsKey(aClass.getName())) &&
						(!selectedList.contains(aComp.getType())))
		 ...
		 **/
		public int adjustColumnConst(int column)
		{
			// available table
		...
		public Object getValueAt(Object node, int column)
		{
			...
			switch (column)
			{
				...
				case COL_TYPE:
					if (race != null)
						if (race.getDisplayName().equals("None"))
							sRet = race.getType();
						else
