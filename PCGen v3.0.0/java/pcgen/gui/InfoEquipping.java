...
//Globals.debugPrint("setInfoLabelText: "+eqI.getName()+":");
	...
	private void setCombatLabelText(PCClass aClass)
	{
		if (aClass != null)
		{
//Globals.debugPrint("setCombatLabelText: "+aClass.getName());
...
//Globals.debugPrint("fCS:before:updateCharacterInfo");
		...
		{
			hasBeenSized = true;
			s = SettingsHandler.getPCGenOption("InfoEquipping.splitPane", (int)(this.getSize().getWidth() * 7 / 10));
			t = SettingsHandler.getPCGenOption("InfoEquipping.bsplit", (int)(this.getSize().getHeight() - 101));
			u = SettingsHandler.getPCGenOption("InfoEquipping.asplit", (int)(this.getSize().getWidth() - 408));
...
//Globals.debugPrint("------------------addEquipButton---------------- ");
	...
	private void addEquipSetButton(ActionEvent evt)
	{
		...
		Globals.debugPrint("addEquipSetButton:aString: " + aString);

		if (aString.equals(selectedEquipSet))
		{
			Globals.debugPrint("addESB: Set already exists: aString: " + aString + " == " + selectedEquipSet);
			return;
		}
		EquipSet pcSet = aPC.getEquipSetByName(aString);
		if (pcSet != null)
		{
			Globals.debugPrint("addESB: Set already exists: " + aString);
			return;
		}
	...
	{
		String aString = equipSetTextField.getText();
		Globals.debugPrint("delEquipSetButton:aString: " + aString);

		EquipSet eSet = aPC.getEquipSetByName(aString);

		if (eSet == null)
		{
			Globals.debugPrint("delEquipSetButton: No EquipSet named: " + aString);
			return;
		}
