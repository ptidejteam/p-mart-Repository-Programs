...
public class PCGen_Frame1 extends JFrame
{
	...
	private static class WaitCursorEventQueue extends EventQueue
	{
		private int delay;
	...
	static class PopupListener extends MouseAdapter
	{
		...
		public void mousePressed(MouseEvent e)
		{
			...
			switch (tabCount)
			{
			case 3: // one PC tab only -- no shifting
				pcPopupMenu.setShiftType(PCPopupMenu.SHIFT_NONE);
				break;
			case 4: // two PC tabs -- support swapping
				pcPopupMenu.setShiftType(index == 2 ? PCPopupMenu.SHIFT_RIGHT : PCPopupMenu.SHIFT_LEFT);
				break;
	 ...
	 * ("Campaign") and {@link #tabDMTools} ("GM Tools")
	 * panels.
	 ...
	 * tabs in <code>baseTabbedPane</code>. i.e. <br />
	 * <code>baseTabbedPane.getComponentAt(x) == characterList.elementAt(x - 2)</code>
	 **/
//	private static ArrayList characterList = new ArrayList();
	...
	/**
	 * The "GM Tools" tab on the main application screen.
	 * Contains <code>toolsCombat and toolsNotes</code>.
	 **/
	...
	{
		setTitle("PCGen - " + Globals.getGameMode() + " Campaign");
	}
	...
	private void jbInit() throws Exception
	{
		...
		baseTabbedPane.addTab("GM Tools", tabDMTools);
	...
	void preferencesItem_actionPerformed(ActionEvent event)
	{
		...
		int result;
		switch (result = PreferencesDialog.show(this))
		{
	...
	void moveTab(int oldIndex, int newIndex)
	{
		...
		PlayerCharacter aPC = (PlayerCharacter)pcList.get(oldIndex - 2);
		pcList.remove(oldIndex - 2);
		pcList.add(newIndex - 2, aPC);
	...
	{
		int index = popupListener.getTabIndex();
		moveTab(index, index == 2 ? (baseTabbedPane.getTabCount() - 1) : (index - 1));
	}

	void shiftRightPopupItem_actionPerformed(ActionEvent event)
	{
		int index = popupListener.getTabIndex();
		moveTab(index, index == (baseTabbedPane.getTabCount() - 1) ? 2 : (index + 1));
	}
	...
	void baseTabbedPane_changePanel(ChangeEvent c)
	{
		...
		else
		{
			...
			characterPane.featList_Changed();
	...
	void resetCharacterTabs()
	{
		if (characterPane == null)
			return;
		for (int i = 2; i < baseTabbedPane.getTabCount(); ++i)
	...
	void closeItem_actionPerformed(ActionEvent e)
	{
		...
		if (currTab > 1)
		{
			...
			if (baseTabbedPane.getTabCount() < 3)
			{
				newPCNameCount = 0;
			}
		}
	...
	private void disposePCTabAt(int index)
	{
		...
		Globals.getPCList().remove(index - 2);
		// Go to the source tab, not the dm tools, if no pc tabs
		baseTabbedPane.setSelectedIndex(newIndex == 1 ? 0 : newIndex);
	...
	private boolean closePCTabAt(int index, boolean isClosing)
	{
		...
		String fileName = aPC.getFileName();
		if (!savePC(aPC, fileName.equals(""))) return false;
	...
	public static void setTabName(PlayerCharacter pc, String aName)
	{
		...
		if (index >= 0)
			getBaseTabbedPane().setTitleAt(index + 2, aName);
	}
...
//			return ((CharacterInfo)getBaseTabbedPane().getComponentAt(index)).getSelectedFilterable();
		}
...
//			return (CharacterInfo)getBaseTabbedPane().getComponentAt(index);
	...
	class MenuItems extends JMenuBar
	{
		...
		class FileMenu extends JMenu implements MouseListener
		{
			FileMenu()
			{
				super("File");
		...
		public MenuItems()
		{
			...
			//Preferences Menu
//			preferencesItem = Utility.createMenuItem("Preferences...", frameActionListener.preferencesActionListener, "file.preferences", 'F', null, "Show the preferences dialog", "Preferences16.gif", true);
...
//			fileMenu.addSeparator();
			...
			exitItem = Utility.createMenuItem("Exit",
				...
				"file.exit", 'X', "control Q", "Quit the best PC generator on the planet", null, true);
			...
			JMenuItem converterItem = Utility.createMenuItem("Lst Converter",
				new ActionListener()
				{
					public void actionPerformed(ActionEvent e)
					{
						if (lstConverter == null)
							lstConverter = new LstConverter();
						lstConverter.show();
					}
				},
				"tools.converter",
			...
			JMenuItem openFiltersItem = Utility.createMenuItem("Open",
				new ActionListener()
				{
					public void actionPerformed(ActionEvent e)
					{
			...
			JMenuItem clearFiltersItem = Utility.createMenuItem("Clear",
				new ActionListener()
				{
					public void actionPerformed(ActionEvent e)
					{
			...
			JMenuItem customFiltersItem = Utility.createMenuItem("Custom",
				new ActionListener()
				{
					public void actionPerformed(ActionEvent e)
					{
			...
			JMenuItem editFiltersItem = Utility.createMenuItem("Edit",
				new ActionListener()
				{
					public void actionPerformed(ActionEvent e)
					{
...
//			debugMenu.add(experimentalCursor);
	...
	{
		if (characterPane != null)
			characterPane.eqList_Changed(newEq, purchase, true);
	}
	...
	public static void forceUpdate_CombatTools()
	{
		Globals.getRootFrame().toolsCombat.refreshTable();
	}
	...
	public static void forceUpdate_PlayerTabs()
	{
		...
		for (int i = 2, x = baseTabbedPane.getTabCount(); i < x; ++i)
		{
	...
	public static void forceUpdate_InfoEquipping()
	{
		if (characterPane != null)
		{
			characterPane.infoEquipping().setNeedsUpdate(true);
		}
	}
