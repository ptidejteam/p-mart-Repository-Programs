 ...
 */
public class Options extends JMenu // extends Preferences
{
	...
	private JRadioButtonMenuItem displayPlainName = new JRadioButtonMenuItem("Conan (Plain style)");
	private JRadioButtonMenuItem displayEpicName = new JRadioButtonMenuItem("Conan the Barbarian (Epic style)");
	private JRadioButtonMenuItem displayRaceName = new JRadioButtonMenuItem("Conan the Human (Race style)");
	private JRadioButtonMenuItem displayNetHackName = new JRadioButtonMenuItem("Conan the Human Barbarian (NetHack style)");
	...
	private void addLAFMenu(JMenu parent)
	{
		...
		Utility.setDescription(summaryTabShown, "<html>Show <b>Summary</t> tab in character's tabs</html>");
		...
		laf = new JRadioButtonMenuItem[UIFactory.getLnFCount()];
		ButtonGroup lafGroup = new ButtonGroup();
		for (int i = 0; i < laf.length; i++)
		{
			laf[i] = new JRadioButtonMenuItem();
			laf[i].setText(UIFactory.getLnFName(i));
			Utility.setDescription(laf[i], UIFactory.getLnFTooltip(i));
		...
		Utility.setDescription(skinnedLookFeel, "Sets the look to that of the select themepack (see next option), or to cross platform if there is no themepack selected. Probably needs a restart of pcgen to work correctly.");
		skinnedLookFeel.setMnemonic('K');
		skinnedLookFeel.addActionListener(lookFeelCheckBoxHandler);
		if (SettingsHandler.getLooknFeel() < laf.length)
		{
			laf[SettingsHandler.getLooknFeel()].setSelected(true);
		}
		else if (SettingsHandler.getLooknFeel() == laf.length)
		{
		...
		Utility.setDescription(themepack, "Change skin themepack (get some at http://javootoo.l2fprod.com/plaf/skinlf/index.php)");
	...
	private void jbInit() throws Exception
	{
		...
		addCharMenu(this);	// characters
		addConfigMenu(this);	// pcgen
	}
	...
	private final class LookFeelCheckBoxListener implements ActionListener
	{
		public void actionPerformed(ActionEvent actionEvent)
		{
			...
			{
				SettingsHandler.setLooknFeel(sourceIndex);
			...
			else if (source == skinnedLookFeel)
			{
				if ((SkinLFResourceChecker.getMissingResourceCount() == 0))
				{
					SettingsHandler.setLooknFeel(laf.length);
					try
					{
	...
	{
		method8_ = new JRadioButtonMenuItem[SettingsHandler.getInitialStatMax() - SettingsHandler.getInitialStatMin() + 1];

		method8.removeAll();
		ButtonGroup method8Group = new ButtonGroup();

		for (int i = 0; i < method8_.length; i++)
		{
			method8_[i] = new JRadioButtonMenuItem(Integer.toString(i + SettingsHandler.getInitialStatMin()));
			method8Group.add((AbstractButton)method8.add(method8_[i]));
			Utility.setDescription(method8_[i], "All " + (i + SettingsHandler.getInitialStatMin()) + "'s");
			method8_[i].addActionListener(allStatsHandler);
		}

		final int i = Math.min(SettingsHandler.getInitialStatMax(), SettingsHandler.getAllStatsValue());
		SettingsHandler.setAllStatsValue(i);
		if (SettingsHandler.getRollMethod() == 8)
			method8_[i - SettingsHandler.getInitialStatMin()].setSelected(true);
	}
	...
	private static final class AllStatsListener implements ActionListener
	{
		public void actionPerformed(ActionEvent actionEvent)
		{
			...
			for (int i = 0; i < method8_.length; i++)
			{
				if (method8_[i] != source) continue;

				SettingsHandler.setAllStatsValue(i + SettingsHandler.getInitialStatMin());
			}

			SettingsHandler.setRollMethod(8);
		}
	}
	...
	public final class ThemePackFilter extends FileFilter
	{
