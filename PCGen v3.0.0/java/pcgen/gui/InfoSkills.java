...
public class InfoSkills extends FilterAdapterPanel
{
	...
	private SkWrap skillA = new SkWrap((Skill)null, new Integer(0), new Float(0), new Integer(0));
	...
	public class SkWrap
	{
		...
		public SkWrap(Skill aSkill, Integer mod, Float ranks, Integer outputIndex)
		{
			_aSkill = aSkill;
			_mod = mod;
			_ranks = ranks;
...
//				splitPane.setDividerLocation(.5);
		...
		maxSkillRank.setText(new Integer(aPC.getTotalLevels() + 3).toString());
		...
		maxCrossSkillRank.setText(new Integer((aPC.getTotalLevels() + aPC.totalHitDice() + 3) / 2).toString());
...
//Globals.errorPrint("    uCI: before feats:");
		...
		{
			maxSkillRank.setText(Integer.toString(aPC.getTotalLevels() + aPC.totalHitDice() + 3));
			maxCrossSkillRank.setText(Integer.toString((aPC.getTotalLevels() + aPC.totalHitDice() + 3) / 2));
		}
		else
		{
			maxSkillRank.setText(Integer.toString(aPC.getTotalLevels() + 3));
			maxCrossSkillRank.setText(Integer.toString((aPC.getTotalLevels() + 3) / 2));
		}
		createModels();
		availableTable.updateUI();
		selectedTable.updateUI();
								...
								for (int k = 0; d != null && k < d.length; k++)
								{
									...
									if (!(d[k].getItem() instanceof PObject))
									{
										if (aSkill.isType(d[k].getItem().toString()))
										{
											...
											//aFN.setItem(aSkill);
											if (available)
												skillA = new SkWrap(aSkill, new Integer(0), new Float(0), new Integer(0));
											else
												skillA = new SkWrap(aSkill, aSkill.modifier(), aSkill.getTotalRank(), new Integer(aSkill.getOutputIndex()));
											aFN.setItem(skillA);
...
//Globals.errorPrint("      --done-- resetModel: available:"+available);
		...
		public Object getValueAt(Object node, int column)
		{
			...
			if (fn.getItem() instanceof SkWrap)
			{
				...
				outputIndex = (Integer)skillA.getSkWrapOutputIndex();
			}
		...
		public void setValueAt(Object aValue, Object node, int column)
		{
			...
			if (!(((PObjectNode)node).getItem() instanceof SkWrap))
				return; // can only use rows with Skills in them

			final PObjectNode fn = (PObjectNode)node;
			SkWrap skillA = (SkWrap)fn.getItem();
			Skill aSkill = (Skill)skillA.getSkWrapSkill();
