 ...
 *
 * $Id: RollInfo.java,v 1.1 2006/02/21 00:02:22 vauchers Exp $
 */
...
public class RollInfo
{
	...
	private String debugTokenize(StringTokenizer st, String delim, String rollString)
	{
		if (!st.hasMoreTokens())
		{
			Globals.errorPrint("Surprise!  Out of tokens in '" + rollString + "'");
			return null;
		}
		...
		{
			Globals.errorPrint("Darn--tokenizer barfed in '" + rollString + "': " + ex, ex);
			return null;
		}
	...
	public String toString()
	{
		...
		if (rerollBelow != Integer.MIN_VALUE) buf.append("m" + rerollBelow);
		if (rerollAbove != Integer.MAX_VALUE) buf.append("M" + rerollAbove);

		if (modifier > 0)
			buf.append("+").append(modifier);
		else if (modifier < 0) buf.append("-").append(-modifier);

		if (totalFloor != Integer.MIN_VALUE) buf.append("t").append(totalFloor);
		if (totalCeiling != Integer.MAX_VALUE) buf.append("T").append(totalCeiling);
