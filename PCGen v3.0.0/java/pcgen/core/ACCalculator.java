...
public class ACCalculator
{
	 ...
	 */
	private void setPlayerCharacter(PlayerCharacter aPC)
	{
		this.aPC = aPC;
	}
 ...
 */
	...
	{
		int acmod = aPC.calcStatMod(Constants.DEXTERITY);
	...
	private Set getSetFromString(String input)
	{
		...
		String t, boolean tb)
	{
		...
		Set sourceList = getSetFromString(s);
		Set typeList = getSetFromString(t);
		...
		for (Iterator it = acBonusMap.keySet().iterator(); it.hasNext();)
		{
			...
			if (xnor(sb, sourceList.contains(aSource)) &&
				xnor(tb, typeList.contains(aType)))
			{
				if (aBuffer.length() == 0)
				{
