...
public class PObject extends Object implements Cloneable, Serializable, Comparable
{
	...
	{
		return getSourceInForm(sourceDisplay, true);
	}

	public String getSourceInForm(int sourceDisplay, boolean includePage)
	{
...
//					Globals.errorPrint("In PObject.getSource the sourceDisplay " + sourceDisplay + " is unsupported.");
	...
	{
		return getSourceInForm(Constants.SOURCELONG, false);
	}
	...
	public Object clone()
	{
		...
		try
		{
			...
			retVal.cSkillList = cSkillList;
			retVal.ccSkillList = ccSkillList;
			retVal.spellList = spellList;
			retVal.weaponProfAutos = weaponProfAutos;
			if (specialAbilityList != null)
			{
		...
		while (aTok.hasMoreTokens())
		{
			...
			if (!aArrayList.isEmpty())
			{
				for (Iterator e1 = aArrayList.iterator(); e1.hasNext();)
				{
					...
					final pcgen.core.spell.Spell aSpell = (pcgen.core.spell.Spell)e1.next();
	...
	private boolean passesPrereqStat(String aList, PlayerCharacter aPC, boolean flag)
	{
		...
		if (i >= 0)
		{
			flag = aPC.adjStats(i) >= Integer.parseInt(aList.substring(aList.lastIndexOf('=') + 1));
		}
		else
		{
			Globals.errorPrint("The stat " + aList + " is unknown.");
		}
		return flag;
	}
	...
	private boolean removeMyType(String myType)
	{
		if (myTypeList == null)
		{
			return false;
