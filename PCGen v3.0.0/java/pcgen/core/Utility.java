 ...
 *
 * $Id: Utility.java,v 1.1 2006/02/21 00:02:22 vauchers Exp $
 */
package pcgen.core;
...
public class Utility
{
	...
	public static boolean modChoices(PObject obj, ArrayList availableList, ArrayList selectedList, boolean process)
	{
		...
		else if (choiceType.equals("SPELLLIST"))
		{
			...
			if (aClass != null && aClass.getLevel().intValue() > 0 && aPC.adjStats(Constants.INTELLIGENCE) > 11)
			{
				...
				maxNewSelections = aPC.calcStatMod(Constants.INTELLIGENCE);
			}
		}
					...
					if (aFeat.passesPreReqTests())
					{
						if (aFeat.isMultiples())
						{
							...
							if (!aFeat.isStacks())
							{
								...
								{
									int idx = aavailableList.indexOf((String)e.next());
									if (idx > -1)
									{
	...
	public static void buildSALIST(String aChoice, ArrayList aAvailable, ArrayList aBonus)
	{
		...
		for (Iterator e1 = aTemplateList.iterator(); e1.hasNext();)
		{
			...
			final ArrayList SAs = (ArrayList)aTempl.getSpecialAbilityList(aPC.getTotalLevels(), aPC.totalHitDice(), aPC.getSize());
