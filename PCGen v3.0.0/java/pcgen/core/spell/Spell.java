...
public class Spell extends PObject
{
	...
	public void setLevelString(final String aString)
	{
		...
		else
		{
			...
			levelString = levelString + aString;
		}
	}

	/**
	 * Returns boolean whether or not spell's levelString meets given criteria:
	 * levelString is | list of type|name|level
	 * @param mtype
	 * @param stype
	 * @param level
	 * @return a boolean
	 */
	public boolean containsKeyTypeLevel(final String mType, final String sType, final int level)
	{
		if (levelString.length() == 0)
			return false;
		...
		while (aTok.hasMoreTokens())
		{
			...
			if (!aTok.hasMoreTokens())
				break;
			final String sub = (String)aTok.nextToken(); // should be name of main object
			if (!aTok.hasMoreTokens())
				break;
			final int lev = Integer.parseInt(aTok.nextToken());
			if (main.equals(mType) && sub.equals(sType) && (level == -1 || lev == level))
				return true;
		}
		return false;
	}

	// Return -1 if spell doesn't have mType|sType key
	// mType should be CLASS or DOMAIN
	// sType should be name of Class or Domain
	...
	public int levelForKey(final String key)
	{
		if (levelString.length() == 0 || key.indexOf("|") == -1)
			return -1;
		return levelForKey(key.substring(0, key.indexOf("|")), key.substring(key.indexOf("|") + 1));
	}
