 ...
 *
 * $Id: Globals.java,v 1.1 2006/02/20 23:57:30 vauchers Exp $
 */
...
public class Globals implements Constants
{
	...
	private static ArrayList characterSpellList = new ArrayList();
   ...
   * bsmeister 11/20/2001
	...
	private static int equipTab_AvailableListMode = pcgen.gui.InfoEquipping.VIEW_TYPE;
	private static int equipTab_SelectedListMode = pcgen.gui.InfoEquipping.VIEW_NAME;
	...
	private static String selectedEqSetTemplate = templatePath.getAbsolutePath() + File.separator + "eqsheet.htm";
	...
	public static void setFreeClothesAtFirst(boolean freeClothesAtFirst)
	{
		Globals.freeClothesAtFirst = freeClothesAtFirst;
	}
	...
	public static ArrayList getCharacterSpellList()
	{
		return characterSpellList;
	}

	public static void addCharacterSpellList(CharacterSpell cs)
	{
		characterSpellList.add(cs);
	}
	...
	{
		return getClassNamed(aName, classList);
	}
	...
	{
		return getEquipmentNamed(name, equipmentList);
	}
	...
	public static Equipment getEquipmentNamed(String name, List aList)
	{
		for (Iterator e = aList.iterator(); e.hasNext();)
		{
			...
			if (eq.getName().equalsIgnoreCase(name) ||
				(eq.getKeyName().equalsIgnoreCase(name)))
			{
				return eq;
			}
		}
	...
	{
		return getDeityNamed(name, deityList);
	}
	...
	/**
	 * Returns an ArrayList of CharacterSpell with following criteria:
	 * @param aSpell  (optional, ignored if null),
	 * @param book          (optional, ignored if ""),
	 * @param level         (optional, ignored if < 0),
	 * @param owner         (Domain or PCClass, optional, ignored if null)
	 * @param owningClass   (optional, ignored if null)
	 * @return an ArrayList of CharacterSpell
	 */
	public static ArrayList getCharacterSpell(Spell aSpell, String book, int level, PObject owner, PCClass owningClass)
	{
		ArrayList aList = new ArrayList();
		String inOwnerS = "";
		String inOwningCS = "";
		...
		if (owner != null)
			inOwnerS = owner.getName();
		if (owningClass != null)
		{
			inOwningCS = owningClass.getName();
			inSubClassName = owningClass.getSubClassName();
		}
		...
		for (Iterator i = characterSpellList.iterator(); i.hasNext();)
		{
			...
			if (cs.getOwner() != null)
				csOwnerS = cs.getOwner().getName();
			if (cs.getOwningClass() != null)
				csOwningCS = cs.getOwningClass().getName();

			if ((aSpell == null || aSpell.equals(bSpell)) &&
				(book.length() == 0 || cs.getSpellBook().equals(book)) &&
				(level == -1 || cs.getLevel() == level || cs.getAdjustedLevel() == level) &&
				(owner == null || csOwnerS.equals(inOwnerS)) &&
				(owningClass == null || csOwningCS.equals(inOwningCS) || csOwningCS.equals(inSubClassName)))
			{
				aList.add(cs);
			}
		}
	...
	// Spell aSpell  (optional, ignored if null)
	// book          (optional, ignored if "")
	// level         (optional, ignored if < 0)
	// owner         (Domain or PCClass, optional, ignored if null)
	// owningClass   (optional, ignored if null)
	//
	public static CharacterSpell getFirstCharacterSpell(Spell aSpell, String book, int level, PObject owner, PCClass owningClass)
	{
		String inOwnerS = "";
		String inOwningCS = "";
		String inSubClassName = "";
	...
	private static int levelForClass(Spell aSpell, String ownerName)
	{
		ArrayList aList = getCharacterSpell(aSpell, Globals.getDefaultSpellBook(), -1, null, null);
		for (Iterator i = aList.iterator(); i.hasNext();)
		{
			CharacterSpell cs = (CharacterSpell)i.next();
			if (cs.getOwningClass().getName().equalsIgnoreCase(ownerName))
				return cs.getLevel();
		}
		return -1;
	}
	...
	{
		CharacterSpell cs = getFirstCharacterSpell(aSpell, Globals.getDefaultSpellBook(), -1, null, castingClass);
		if (cs != null)
			return cs.getLevel();
		return -1;
	}
	...
	{
		PCClass aClass = Globals.getClassNamed(castAs);
		CharacterSpell cs = getFirstCharacterSpell(aSpell, Globals.getDefaultSpellBook(), -1, null, aClass);
		if (cs != null)
			return cs.getLevel();
		return -1;
	}
	...
	*/
	...
	static Dimension getOptionsFromProperties()
	{
		...
		selectedEqSetTemplate = options.getProperty("pcgen.files.selectedEqSetTemplate", templatePath.getAbsolutePath() + File.separator + "eqsheet.htm");
		...
		freeClothesAtFirst = getPCGenOption("freeClothesAtFirst", true);
		...
		equipTab_AvailableListMode = getPCGenOption("EquipTab.availableListMode", pcgen.gui.InfoEquipping.VIEW_TYPE);
		equipTab_SelectedListMode = getPCGenOption("EquipTab.selectedListMode", pcgen.gui.InfoEquipping.VIEW_NAME);
	...
	static void setOptionsProperties()
	{
		...
		options.setProperty("pcgen.files.selectedEqSetTemplate", selectedEqSetTemplate);
		...
		setPCGenOption("freeClothesAtFirst", freeClothesAtFirst);
		...
		setPCGenOption("EquipTab.availableListMode", equipTab_AvailableListMode);
		setPCGenOption("EquipTab.selectedListMode", equipTab_SelectedListMode);
