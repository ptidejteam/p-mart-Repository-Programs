	...
	implements Serializable, EquipmentCollection, Comparable
{
	...
	private Float costMod = new Float(0);
	...
	public void setBaseItem(String baseName)
	{
		baseItem = baseName;
	}
	...
	public int canContain(Object obj)
	{
		if (obj instanceof Equipment)
		{
			...
			{
				if (true) //??? I assume there is a point to this, but I'll remove it the next time I happen to see it.
				{
...
/*
			...
			{
				eqModCost = new BigDecimal("0");
				for (Iterator e2 = 0; e2<aEqMod.getAssociatedCount(); e2++)
				{
					String modChoice = aEqMod.get(e2);
					final int offs = modChoice.lastIndexOf('|');
					int modValue = 0;
					try
					{
						modValue = Delta.parseInt(modChoice.substring(offs + 1));
					}
					catch (Exception exc)
					{
					}
...
*/
		...
		{
			java.util.Collections.sort(modifierCosts);
			//System.out.println(modifierCosts);
		}
		...
		return c.add(itemCost).add(new BigDecimal(costMod.doubleValue()));
	}
...
//			s_equipmentTypes.add(type);
	...
	{
		String it = infoType + "|";
		EquipmentModifier eqMod = getEqModifierKeyed("PCGENi_WEAPON", bPrimary);
		if (eqMod != null)
		{
...
//		return d_equipmentTypes;
	...
	public void setCostMod(Float aFloat)
	{
		costMod = aFloat;
	}
