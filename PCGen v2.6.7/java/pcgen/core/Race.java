...
//	private ArrayList languageAutos = new ArrayList();
	...
	public void addSpells(String line)
	{
		...
		{
			spellBuf = new StringBuffer(aTok.nextToken() + "|" + aTok.nextToken() + "|" + aTok.nextToken());
			if (preTag.length() > 0)
				spellBuf.append("|" + preTag);
...
//	}
	...
	public void rollHeightWeight()
	{
		...
		String bString = aPC.getTHeight();

		if (bString.equals(Constants.s_NONE))
			bString = heightString;
			...
			if (i >= 0)
			{
				switch (i)
				{
					...
					case 2:
						...
						break;
					case 3:
						final int bonus = Integer.parseInt(aString);
		...
		bString = aPC.getTWeight();

		if (bString.equals(Constants.s_NONE))
			bString = weightString;
	...
	public Set getLanguageAutos()
	{
		...
		aSet = Globals.extractLanguageList(languageAutos);
		return aSet;
	}
	...
	/** return the TreeSet of Language Auto Languages */
	public Set getAutoLanguages()
	{
		...
		aSet = Globals.extractLanguageList(languageAutos);
		// Let LANGAUTO take ALL (ack) and TYPE=x. For the CHOOSE:LANGAUTO function.
		// Non functional...
	...
	public void setLanguageAutos(String aString)
	{
		...
		while (aTok.hasMoreTokens())
		{
			tempTok = aTok.nextToken();
			if (tempTok.equals(".CLEAR"))
				languageAutos.clear();
			else
				languageAutos.add(tempTok);
		}
	}
	...
	public Object clone()
	{
		...
		aRace.languageAutos = (TreeSet)languageAutos.clone();
...
//		aRace.naturalWeapons = (ArrayList)naturalWeapons.clone();
		spellList = (ArrayList)spellList.clone();
		return aRace;
	}
