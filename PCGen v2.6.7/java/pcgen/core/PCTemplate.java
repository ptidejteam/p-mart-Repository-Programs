...
public class PCTemplate extends PObject
{
	...
	private String vision = Constants.s_NONE;
 ...
 */
	...
	public Object clone()
	{
		...
		aTemp.cSkillList = (ArrayList)cSkillList.clone();
		aTemp.ccSkillList = (ArrayList)ccSkillList.clone();
		...
		aTemp.weaponProfAutos = (ArrayList)weaponProfAutos.clone();
		aTemp.weaponProfBonus = (ArrayList)weaponProfBonus.clone();
		aTemp.SAs = (ArrayList)SAs.clone();
	...
	public void setMoveRates(String moveparse)
	{
		...
		if (moves.countTokens() == 1)
		{
			...
			/*if (tok.startsWith("*") || tok.startsWith("/"))
			{
				op = tok.charAt(0);
				if (op == '*') {
					tok = String.valueOf(getPCMovement(0) * Integer.valueOf(tok.substring(1)));
				} else {
					tok = String.valueOf(getPCMovement(0) / Integer.valueOf(tok.substring(1)));
				}
			}*/ // added by Syndaryl 31/03/02
			setMovement(new Integer(tok));
