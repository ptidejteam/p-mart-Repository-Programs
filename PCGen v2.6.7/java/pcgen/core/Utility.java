 ...
 *
 * $Id: Utility.java,v 1.1 2006/02/20 23:57:30 vauchers Exp $
 */
package pcgen.core;
...
public class Utility
{
	...
	public static void modChoices(PObject obj, boolean addIt, ArrayList availableList, ArrayList selectedList, boolean process)
	{
		availableList.clear();
		selectedList.clear();
		...
		if (obj.getChoiceString().startsWith("WEAPONPROF|"))
		{
		...
		int i = 0;
		int j = (int)((aPC.getFeats() + obj.getAssociatedCount()) / cost);
		int num = 0;
		...
		if (Globals.getWeaponTypes().contains(choiceType))
		{
			...
			j -= (int)(obj.getAssociatedCount() * cost);
		}
		//
		...
		else if (choiceType.equals("SPELLLIST"))
		{
			...
			{
				ArrayList aList = aClass.getCharacterSpell(null, Globals.getDefaultSpellBook(), -1, null, aClass);
...
*/		//removed because depended on additonal types in (nonexistant) eq
					...
					if (theFeat != null)
					{
						...
						if (theFeat.isMultiples())
						{
							...
							if (!theFeat.isStacks())
							{
								...
								{
									int idx = xavailableList.indexOf((String)e.next());
									if (idx > -1)
									{
		...
		if (aFeat != null)
		{
			...
			aFeat.getCcSkillList().clear(); // Why?
		}
		obj.clearAssociated();
		String objPrefix = "";
		...
		for (i = 0; i < chooser.getSelectedList().size(); i++)
		{
			...
			if (aFeat != null)
			{
				if (choiceType.equals("SKILLLIST") || choiceType.equals("SKILLSNAMEDTOCSKILL") || choiceType.equals("NONCLASSSKILLLIST"))
				{
					...
					{
						aFeat.getCSkillList().add(chosenItem);
					}
				}
				else if (choiceType.equals("CCSKILLLIST") || choiceType.equals("SKILLSNAMEDTOCCSKILL"))
				{
					if (rootArrayList.contains(chosenItem))
					{
						for (Iterator e2 = Globals.getSkillList().iterator(); e2.hasNext();)
						{
							...
							if (aSkill.getRootName().equals(chosenItem))
							{
								aFeat.getCcSkillList().add(aSkill.getName());
							}
						}
					}
					else
					{
						aFeat.getCcSkillList().add(chosenItem);
					}
				}
			}
		...
		if (!choiceType.equals("SPELLLIST"))
		{
			aPC.setFeats((int)((j - obj.getAssociatedCount() + selectedList.size()) * cost));
		}
	...
	/**
	 * Returns the index of the requested attribute abbrieviation,
	 * Currently the attributes are (in order): STR, DEX, CON, INT, WIS, CHA
	 *
	 * @deprecated Use Globals.getStatFromAbbrev instead (it loads the stats from a lst file, making it more configurable.)
	 *
	 * @param attribute to match, should be one of Constants.STRENGTH_ABBR,
	 *        or the like
	 * @return the index of the attribute, IE Constants.STRENGTH
	 *         returns -1 if the attribute is not matched (or null)
	 */
	public static int getAttributeIdxFromString(String attribute)
	{
		if (Constants.STRENGTH_ABBR.equals(attribute))
		{
			return Constants.STRENGTH;
		}
		else if (Constants.DEXTERITY_ABBR.equals(attribute))
		{
			return Constants.DEXTERITY;
		}
		else if (Constants.CONSTITUTION_ABBR.equals(attribute))
		{
			return Constants.CONSTITUTION;
		}
		else if (Constants.INTELLIGENCE_ABBR.equals(attribute))
		{
			return Constants.INTELLIGENCE;
		}
		else if (Constants.WISDOM_ABBR.equals(attribute))
		{
			return Constants.WISDOM;
		}
		else if (Constants.CHARISMA_ABBR.equals(attribute))
		{
