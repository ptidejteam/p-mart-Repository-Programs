/*
 * @(#) $Id: Feat.java,v 1.1 2006/02/20 23:57:30 vauchers Exp $
 *
...
public class Feat extends PObject
{
	...
	{
		return (Globals.isBoolBypassFeatPreReqs() || super.passesPreReqTestsForList(aPC, aObj, anArrayList));
	}
...
//						aPC.addWeaponProf(aProf.getName());
//					}
//				}
//				else
//				{
//					aPC.addWeaponProf(addSec);
//				}
//			}
//			else
//			{
...
//					if (aSkill.getType().indexOf(bString.substring(5)) >= 0)
	...
	public String qualifiedName()
	{
		...
		if (getAssociatedCount() > 0 && !name.endsWith("Weapon Proficiency"))
		{
			// has a sub-detail
			aStrBuf.append(" (");
			int i = 0;
	...
	public void modChoices(boolean addIt)
	{
		...
		Utility.modChoices(this, addIt, availableList, selectedList, true);
	}

	public void modChoices(boolean addIt, ArrayList availableList, ArrayList selectedList, boolean process)
	{

		Utility.modChoices(this, addIt, availableList, selectedList, process);
/*
		availableList.clear();
		selectedList.clear();
		...
		{
			super.getChoices(choiceString, null, availableList, selectedList, process);
			return;
		}
		...
		int i = 0;
		int j = (int)((aPC.getFeats() + getAssociatedCount()) / cost);
		int num = 0;
		...
		else if (choiceType.equals("STAT"))
		{
			...
			for (int x = 0; x < Globals.s_ATTRIBSHORT.length; x++)
			{
				...
				{
					availableList.add(Globals.s_ATTRIBLONG[x]);
				}
			}

			addAssociatedTo(selectedList);
			chooser.setPoolFlag(true);		// user is required to make changes
		}
		...
		else if (choiceType.equals("SPELLLIST"))
		{
			...
			if (aClass != null && aClass.getLevel().intValue() > 0 && aPC.adjStats(Constants.INTELLIGENCE) > 11)
			{
				...
				for (iter = aList.iterator(); iter.hasNext();)
				{
					...
					if (!containsAssociated(aSpell.getKeyName()))
...
//						if (aSkill.getType().indexOf(aString.substring(5)) >= 0)
		...
		else if (choiceType.equals("WEAPONFOCUS"))
		{
			...
			aFeat.addAssociatedTo(availableList);
			addAssociatedTo(selectedList);
		}
...
*//*		//removed because depended on additonal types in (nonexistant) eq
		...
		else if (choiceType.equals("HP"))
		{
			...
			for (int e1=0; e1 = getAssociatedCount(); e1++)
		...
		for (i = 0; i < chooser.getSelectedList().size(); i++)
		{
			...
			{
				if (!containsAssociated(chooser.getSelectedList().get(i)))
					addAssociated(chooser.getSelectedList().get(i));
			}
			else
			{
				String aString = (String)chooser.getSelectedList().get(i);
				//
				// Translate stat long name to abbreviation for calculations
				//
				if (choiceType.equals("STAT"))
				{
					aString = Globals.s_ATTRIBSHORT[Globals.getStatFromLongName(aString)];
				}
				final String prefix = aString + "|";
				addAssociated(aString);
		...
		if (!choiceType.equals("SPELLLIST"))
			aPC.setFeats((int)((j - getAssociatedCount() + selectedList.size()) * cost));
*/
	}
