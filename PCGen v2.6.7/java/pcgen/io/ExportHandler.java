...
public class ExportHandler
{
	...
	public void write(PlayerCharacter aPC, BufferedWriter out)
	{
		...
		aPC.populateSkills(Globals.getIncludeSkills());
		ArrayList skillList = (ArrayList)Globals.sortPObjectList(aPC.getSkillList());
		ArrayList featList = (ArrayList)Globals.sortPObjectList(aPC.getFeatList());
		...
		{
			System.err.println("Error in ExportHandler::write");
			exc.printStackTrace();
		}
		csheetTag2 = "\\";
	...
	public FORNode parseFORs(StringTokenizer tokens)
	{
	...
	public FORNode parseFORs(String forLine, StringTokenizer tokens)
	{
	...
	public void loopFOR(FORNode node, int min, int max, int step, BufferedWriter output, FileAccess fa)
	{
	...
	public boolean evaluateExpression(String expr)
	{
		...
		if (expr.endsWith("UNTRAINED"))
		{
			...
			if (fString.length() > 5)
			{
				...
				if (i <= aPC.getSkillList().size() - 1)
					aSkill = (Skill)aPC.getSkillList().get(i);
			}
			if (aSkill == null)
				return false;
	...
	private int replaceToken(String aString, BufferedWriter output)
	{
		try
		{
				...
				aString.lastIndexOf('<') == -1 && aString.lastIndexOf('>') == -1)
			{
				...
				{
					ArrayList aArrayList = aPC.getEquipmentOfType("SHIELD", 3);
					if (Integer.parseInt(aString.substring(aString.length() - 1)) >= aArrayList.size())
						canWrite = false;
					return 0;
				}
				...
				{
					ArrayList aArrayList = aPC.getEquipmentOfType("ARMOR", 3);
					if (Integer.parseInt(aString.substring(aString.length() - 1)) >= aArrayList.size())
						canWrite = false;
					return 0;
				}
			...
			{
				replaceTokenPaperInfo(aString, fa, output);
			}
			...
			{
				String tempRaceName = aPC.getRace().getRaceOutputName();
...
//				FileAccess.encodeWrite(output, aPC.getSubRegion());
				...
				FileAccess.write(output, Integer.toString(aPC.totalLevels()));
			else if (aString.equals("CR"))
				FileAccess.write(output, Integer.toString(aPC.calcCR()));
			...
			{
				replaceTokenBonusList(aString, fa, output);
			}
			...
			{
				replaceTokenTemplate(aString, fa, output);
			}
			else if (aString.startsWith("CLASS"))
			{
				replaceTokenClass(aString, fa, output);
			}
			else if (aString.equals("HITDICE"))
			{
				replaceTokenHitDice(aString, fa, output);
			}
			else if (aString.equals("EXP.CURRENT"))
			{
				FileAccess.write(output, aPC.getExperience().toString());
			}
			else if (aString.equals("EXP.NEXT"))
			{
				FileAccess.write(output, Integer.toString(Globals.minExpForLevel(aPC.totalLevels() + 1 + aPC.levelAdjustment())));
			}
			...
			{
				final int initiativeMisc = aPC.initiativeMod() - aPC.calcStatMod(Constants.DEXTERITY);
				writeToken(initiativeMisc, fa, output);
			}
			else if (aString.startsWith("MOVEMENT"))
			{
				replaceTokenMovement(aString, fa, output);
			}
			...
			{
				printFeatList(aString.substring(11), aPC.aggregateFeatList(), fa, output);
			}
			else if (aString.startsWith("FEATAUTOLIST"))
			{
				printFeatList(aString.substring(12), aPC.featAutoList(), fa, output);
			}
			else if (aString.startsWith("FEATLIST"))
			{
				printFeatList(aString.substring(8), aPC.getFeatList(), fa, output);
			}
			else if (aString.startsWith("VFEATLIST"))
			{
				printFeatList(aString.substring(9), aPC.vFeatList(), fa, output);
			}
			else if (aString.startsWith("FEATALL"))
			{
				printFeat(7, aString, aPC.aggregateFeatList(), fa, output);
			}
			else if (aString.startsWith("FEATAUTO"))
			{
				printFeat(8, aString, aPC.featAutoList(), fa, output);
			}
			else if (aString.startsWith("FEAT"))
			{
				printFeat(4, aString, aPC.getFeatList(), fa, output);
			}
			else if (aString.startsWith("VFEAT"))
			{
				printFeat(5, aString, aPC.vFeatList(), fa, output);
			}
			...
			{
				FileAccess.write(output, aPC.getAttackString(Constants.ATTACKSTRING_MELEE,
					aPC.calcStatMod(Constants.STRENGTH) + aPC.modForSize()));
			}
			...
			{
				FileAccess.write(output, aPC.getAttackString(Constants.ATTACKSTRING_RANGED,
					aPC.calcStatMod(Constants.DEXTERITY) + aPC.modForSize()));
			}
			...
			{
				replaceTokenSpellList(aString, fa, output);
			}
			...
			{
				FileAccess.write(output, Globals.maxLoadForStrength(aPC.adjStats(Constants.STRENGTH)).toString());
			}
			...
			else if (aString.startsWith("MISC."))
			{
				...
				FileAccess.write(output, aPC.defense().toString());
			...
			else if (aString.equals("DEFENSE.CTOTAL"))
			{
				...
				FileAccess.write(output, aPC.getFPoints());
...
//		catch (IOException exc)
	...
	private void replaceTokenRaceAbilityList(FileAccess fa, BufferedWriter output)
	{
		int i = 0;
		String bString = null;
		for (Iterator e = aPC.getRace().getSpecialAbilities().iterator(); e.hasNext();)
		{
	...
	private void replaceTokenBonusList(String aString, FileAccess fa, BufferedWriter output)
	{
	...
	private void replaceTokenClass(String aString, FileAccess fa, BufferedWriter output)
	{
		int i = 0;
		int y = 0;
		int cmp = 0;
	...
	private void replaceTokenDir(String aString, FileAccess fa, BufferedWriter output)
	{
		if (aString.endsWith("PCGEN"))
			FileAccess.write(output, Globals.getPcgenSystemDir().getAbsolutePath());
		else if (aString.endsWith("TEMPLATES"))
			FileAccess.write(output, Globals.getTemplatePath().getAbsolutePath());
		else if (aString.endsWith("PCG"))
			FileAccess.write(output, Globals.getPcgPath().getAbsolutePath());
		else if (aString.endsWith("HTML"))
			FileAccess.write(output, Globals.getHtmlOutputPath().getAbsolutePath());
		else if (aString.endsWith("TEMP"))
			FileAccess.write(output, Globals.getTempPath().getAbsolutePath());
		else
		{
	...
	private void replaceTokenHitDice(String aString, FileAccess fa, BufferedWriter output)
	{
		String del = "";
		...
		//
		int temp = aPC.calcStatMod(Constants.CONSTITUTION);
		temp *= (aPC.totalLevels() + aPC.getRace().hitDice());
		//
		// Add in feat bonus
		//
	...
	private void replaceTokenMovement(String aString, FileAccess fa, BufferedWriter output)
	{
		if ((aPC.getRace() != null) && !aPC.getRace().equals(Globals.s_EMPTYRACE))
		{
			if (aString.length() > 9)
	...
	private void replaceTokenPaperInfo(String aString, FileAccess fa, BufferedWriter output)
	{
	...
	private void replaceTokenTemplate(String aString, FileAccess fa, BufferedWriter output)
	{
		...
		else
		{
			...
			for (Iterator e = aPC.getTemplateList().iterator(); e.hasNext();)
			{
				...
				// --- arcady.
				{
					...
					{
						FileAccess.write(output, aTemplate.getSAs(aPC.totalLevels(),
							aPC.totalHitDice(),
							aPC.getSize()).toString());
					}
					else if (aLabel.equals("FEAT"))
					{
						FileAccess.write(output, aTemplate.feats(aPC.totalLevels(),
							aPC.totalHitDice(),
							aPC.getSize()).toString());
					}
					...
					{
						FileAccess.write(output,
							Integer.toString(aTemplate.getSR(aPC.totalLevels(),
								aPC.totalHitDice(),
								aPC.getSize())));
					}
					...
					{
						FileAccess.write(output,
							Integer.toString(aTemplate.getCR(aPC.totalLevels(),
								aPC.totalHitDice(),
								aPC.getSize())));
					}
					else
	...
	private void replaceTokenWeight(String aString, FileAccess fa, BufferedWriter output)
	{
		...
		if (aString.endsWith("MEDIUM"))
			...
			i = 3;
		FileAccess.write(output, new Float(i * Globals.maxLoadForStrength(aPC.adjStats(Constants.STRENGTH)).intValue() / 3).toString());
	}
	...
	public String replaceTokenEq(String aString)
	{
		...
		if (aString.startsWith("EQTYPE"))
		{
			...
			{
				aList = aPC.getEquipmentOfType(aType, 3);
			}
		}
		else
		{
		...
		while (aTok.countTokens() > 2)	//should be ok, assumes last two fields are # and a Param
		{
			...
			if (bString.equalsIgnoreCase("NOT"))
				aList = new ArrayList(aPC.removeEqType(aList, aTok.nextToken()));
			else if (bString.equalsIgnoreCase("ADD"))
				aList = new ArrayList(aPC.addEqType(aList, aTok.nextToken()));
			else if (bString.equalsIgnoreCase("IS"))
				aList = new ArrayList(aPC.removeNotEqType(aList, aTok.nextToken()));
		}
		//End Not code...
	...
	private int replaceTokenSpecialAbility(String aString, FileAccess fa, BufferedWriter output)
	{
		...
		if (specialability >= 0 && specialability < len)
			if (aString.indexOf(".DESCRIPTION") > -1)
				replaceWithDelimiter(fa, output, getItemDescription("SA", aPC.getSpecialAbilityTimesList().get(specialability).toString(), ""), sDelim);
			else
	...
	{
		int i = 0;
		for (Iterator e = aPC.getSkillList().iterator(); e.hasNext();)
		{
			final Skill aSkill = (Skill)e.next();
			int modSkill = -1;
			if (aSkill.getKeyStat().compareToIgnoreCase(Constants.s_NONE) != 0)
			{
				modSkill = aSkill.modifier().intValue() - aPC.calcStatMod(Globals.getStatFromAbbrev(aSkill.getKeyStat()));
			}
	...
	{
		FileAccess.write(output,
			Delta.toString(aPC.calculateSaveBonus(1, "Fortitude",
	...
	{
		FileAccess.write(output,
			Delta.toString(aPC.calculateSaveBonus(2, "Reflex",
				Constants.DEXTERITY,
				aString)));
	}
	...
	{
		FileAccess.write(output,
			Delta.toString(aPC.calculateSaveBonus(3, "Willpower",
				Constants.WISDOM,
				aString)));
	}
		...
		if (eq != null)
		{
			...
			else if (aString.startsWith("HIT") || aString.startsWith("TOTALHIT"))
			{
				String mString = aPC.getAttackString(Constants.ATTACKSTRING_MELEE,
					aPC.calcStatMod(Constants.STRENGTH));
				String rString = aPC.getAttackString(Constants.ATTACKSTRING_MELEE,
					aPC.calcStatMod(Constants.DEXTERITY));
				if (eq.isMonk())
				{
				...
				else if (isDouble || eq.getHand() == Equipment.TWOWEAPON_HANDS || aPC.isPrimaryWeapon(eq) || aPC.isSecondaryWeapon(eq))
				{
					    ...
					    (!aPC.getPrimaryWeapons().isEmpty() && aPC.isSecondaryWeapon(eq)))
					{
						...
						if (eq.isRanged())
						{
							final String aKey = "COMBAT.TOHIT-SECONDARY.NOTRANGED";
							final String aBonus = (String)aPC.getBonusMap().get(aKey);
							if (aBonus != null)
							{
								secondaryBonus -= Integer.parseInt(aBonus);
							}
						}
					}
					if (isDouble ||
					    ...
					    (!aPC.getSecondaryWeapons().isEmpty() && aPC.isPrimaryWeapon(eq)))
					{
						...
						if (eq.isRanged())
						{
							final String aKey = "COMBAT.TOHIT-PRIMARY.NOTRANGED";
							final String aBonus = (String)aPC.getBonusMap().get(aKey);
							if (aBonus != null)
							{
								primaryBonus -= Integer.parseInt(aBonus);
							}
						}
					}
				}
				...
				{
					final String aKey = "WEAPONPROF=" + profName.toUpperCase() + ".TOHIT.NOTRANGED";
					String aBonus = (String)aPC.getBonusMap().get(aKey);
					if (aBonus != null)
					{
...
//				final int magichit = eq.bonusTo("WEAPON", "TOHIT") + eq.bonusTo("WEAPONPROF=" + profName, "TOHIT");
			...
			else if (aString.endsWith("DAMAGE"))
			{
				...
				int bonus = 0;
				int strMod = aPC.calcStatMod(Constants.STRENGTH);
				...
				if (!aString.startsWith("BASE"))
				{
					...
					if ((strMod > 0) && eq.isNatural() && eq.modifiedName().endsWith("Secondary"))
					{
						strMod /= 2;
					}
					...
					{
						if (aPC.isSecondaryWeapon(eq) && (aPC.getPrimaryWeapons().indexOf(eq) == -1) && (strMod > 0))
						{
							bInt = new Integer(bInt.intValue() + aPC.doOffhandMod(strMod));
						}
						else
						{
				...
				if (isDouble || eq.getHand() == Equipment.TWOWEAPON_HANDS)
				{
					...
					if (strMod > 0)
					{
						bonus += aPC.doOffhandMod(strMod);	// only get half strength bonus
					}
					else
					{
						bonus += strMod;
	...
	private int replaceTokenStat(String aString, FileAccess fa, BufferedWriter output)
	{
		...
		if (target.startsWith("STAT"))
		{
			if (aPC.isNonability(i))
				...
				FileAccess.write(output, Integer.toString(aPC.adjStats(i)));
		}
		else if (target.equals("BASE"))
		{
			if (aPC.isNonability(i))
				...
				FileAccess.write(output, Integer.toString(aPC.unAdjStats(i)));
		}
		...
		else if (target.equals("BASEMOD"))
		{
			...
			{
				int temp = aPC.calcUnAdjStatMod(i);
				FileAccess.write(output, Delta.toString(temp));
			}
		}
...
//  			FileAccess.write(output, Delta.toString(eq.getAcMod()));
		...
		int len, FileAccess fa, BufferedWriter output)
	{
		Equipment eq;
		ArrayList aArrayList = new ArrayList();
		for (Iterator mapIter = aPC.getEquipmentOfType(type, subtype, equipped).iterator(); mapIter.hasNext();)
		{
	 ...
	 */
	private int _replaceTokenArmorItem(int item, String subtype, String property, int equipped, int len, FileAccess fa, BufferedWriter output)
	{
	...
	private int _replaceTokenArmorShield(int shield, String subtype, String property, int equipped,
		int len, FileAccess fa, BufferedWriter output)
	{
		ArrayList aArrayList = aPC.getEquipmentOfType("Shield", subtype, equipped);
	...
	private int _replaceTokenArmorShirt(int shirt, String subtype, String property, int equipped,
		int len, FileAccess fa, BufferedWriter output)
	{
		ArrayList aArrayList = aPC.getEquipmentOfType("Shirt", subtype, equipped);
	...
	private int _replaceTokenArmorSuit(int suit, String subtype, String property, int equipped,
		int len, FileAccess fa, BufferedWriter output)
	{
		ArrayList aArrayList = aPC.getEquipmentOfType("Suit", subtype, equipped);
		//
		// Temporary hack until someone gets around to fixing it properly
		//
//  		aArrayList.addAll(aPC.getEquipmentOfType("Shirt", subtype, equipped));
	...
	private int _replaceTokenArmor(int armor, String property, int equipped,
		int len, FileAccess fa, BufferedWriter output)
	{
		ArrayList aArrayList = aPC.getEquipmentOfType("Armor", equipped);
		ArrayList bArrayList = aPC.getEquipmentOfType("Shield", equipped);
	...
	private int replaceTokenArmor(String aString, int len, FileAccess fa, BufferedWriter output)
	{
		...
		else if ((tokens.length == 4) || (tokens.length == 5))
		{
			...
			{
				return _replaceTokenArmorSuit(index, subtype, property, equipped, len, fa, output);
			}
			/**
			 * ARMOR.SHIRT.ALLx.property
			 * ARMOR.SHIRT.EQUIPPEDx.property
			...
			{
				return _replaceTokenArmorShirt(index, subtype, property, equipped, len, fa, output);
			}
			/**
			 * ARMOR.SHIELD.ALLx.property
			 * ARMOR.SHIELD.EQUIPPEDx.property
			...
			{
				return _replaceTokenArmorShield(index, subtype, property, equipped, len, fa, output);
			}
			/**
			 * ARMOR.ITEM.ALLx.property
			 * ARMOR.ITEM.EQUIPPEDx.property
			...
			{
				return _replaceTokenArmorItem(index, subtype, property, equipped, len, fa, output);
			}
			/**
			 * ARMOR.type.ALLx.property
			 * ARMOR.type.EQUIPPEDx.property
			...
			{
				return _replaceTokenArmorVarious(index, tokens[1], subtype, property, equipped, len, fa, output);
			}
		}
	...
	private void replaceTokenSpellList(String aString, FileAccess fa, BufferedWriter output)
	{
		//SPELLLISTCAST0.0 KNOWN0.0 BOOK0.0 TYPE0
		int cmp = 0;
		...
		if (aObject != null)
		{
			...
			switch (cmp)
			{
				...
				case 2:
					...
					ArrayList spells = aObject.getCharacterSpell(null, Globals.getDefaultSpellBook(), level, null, null);
		...
		if (aObject != null || classNum == -1)
		{
			...
			if (!bookName.equals(""))
			{
				...
				else if (aObject != null)
				{
					...
					canWrite = false;
				if (found && aSpell != null)
				{
					if (aLabel.equals("NAME"))
						FileAccess.write(output, cs.toString());
					else if (aLabel.equals("TIMES"))
						FileAccess.write(output, String.valueOf(cs.getTimes()));
					else if (aSpell != null)
					{
						...
						else if (aLabel.equals("FULLSCHOOL"))
						{
							...
							FileAccess.encodeWrite(output, aSpell.getSR());
						...
						else if (aLabel.startsWith("BONUSSPELL"))
						{
							...
							if (aObject != null && cs != null && cs.isInSpecialty() && (aObject instanceof PCClass))
							{
	...
	private int replaceTokenSkill(String aString, int len, FileAccess fa, BufferedWriter output)
	{
		...
		if (fString.startsWith("SKILLSUBSET"))	// Matches skills known which start with the second dotted field ie. SKILLSUBSET%.Read Language.NAME
		{
			...
			for (Iterator iter = aPC.getSkillList().iterator(); iter.hasNext();)
			{
		...
		else if (fString.length() > 5)
		{
			...
			if (i >= aPC.getSkillList().size() - 1 && existsOnly)
				noMoreItems = true;
			if (i > aPC.getSkillList().size() - 1)
				len = 0;
			else
		...
		return _writeSkillProperty(aSkill, property, fa, output);
	}

	/*
	 * This writes the Skill info out to the chseet
	 */
	private int _writeSkillProperty(Skill aSkill, String property, FileAccess fa, BufferedWriter output)
	{
		int len = 0;
		int cmp = 0;
			...
			if ((cmp == 5 || cmp == 6) && aSkill.getKeyStat().equals(Constants.s_NONE))
				...
				switch (cmp)
				{
					...
					case 4:
						...
						return len;
					case 5:
						FileAccess.write(output, Integer.toString(aPC.calcStatMod(Globals.getStatFromAbbrev(aSkill.getKeyStat()))));
						return len;
					case 6:
						FileAccess.write(output, Integer.toString(aSkill.modifier().intValue() - aPC.calcStatMod(Globals.getStatFromAbbrev(aSkill.getKeyStat()))));
						return len;
					case 7:
	...
	private int replaceTokenSpellListBook(String aString)
	{
		...
		if (aObject != null)
		{
			...
			CharacterSpell cs = aObject.getFirstCharacterSpell(null, bookName, levelNum, null, null);
			if (cs != null)
			{
				canWrite = true;
			}
	...
	private void replaceTokenAlignmentShort(FileAccess fa, BufferedWriter output)
	{
		...
		FileAccess.write(output, alString);
	}
		...
		for (Iterator e = anArrayList.iterator(); e.hasNext();)
		{
			...
			if (i == 0 && (aFeat.isVisible() == 1 || aFeat.isVisible() == 2))
			{
				if (aString.endsWith(".DESC"))
					...
					replaceWithDelimiter(fa, output, getItemDescription("FEAT", aFeat.getName(),
	...
	private void writeToken(final int value, FileAccess fa, BufferedWriter output)
	{
