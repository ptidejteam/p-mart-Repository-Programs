	...
	public void parsePCG(String[] lines) throws PCGParseException
	{
		...
		if (pcgVersion <= 253)
		{
			for (Iterator it = aPC.getClassList().iterator(); it.hasNext();)
			{
				...
				{
					parseOldClassSpellLine((PCClass)it.next(), lines[i++]);
				}
			}
...
//		parseWeaponProfLine(lines[i++]);
	...
	private void parseSkillsLine(String line) throws PCGParseException
	{
		...
		while (skillTokenizer.hasMoreElements())
		{
			...
			if (aSkill != null)
			{
				...
				if (aSkill.getChoiceList().size() > 0 && aFloat.intValue() > 0)
				{
					for (int i = 0; i < aFloat.intValue(); i++)
					{
						aSkill.addAssociated(skillTokenizer.nextToken());
					}
				}
			}
			else
	...
	/**
	 * This parses the old style spell line (all one one line)
	 **/
	private void parseOldClassSpellLine(PCClass aClass, String line)
	{
		String className = new String(aClass.getKeyName());
		String castName = className;
		if (aClass.getCastAs().length() > 0)
			castName = aClass.getCastAs();
		PCClass castingClass = Globals.getClassNamed(castName);

		//System.out.println("PCGVer0Parser:pOldCSL: "+castName+":"+className);
		...
		while (classSpellTokenizer.hasMoreTokens())
		{
			...
			if (aSpell != null)
			{
				CharacterSpell gcs = Globals.getFirstCharacterSpell(aSpell, Globals.getDefaultSpellBook(), -1, null, castingClass);
				if (gcs == null)
					continue;

				// do not load auto known Spells into default
				if (aClass.isAutoKnownSpell(aSpell.getKeyName(), gcs.getLevel()))
					continue;

				// if this spell is already present on this
				// players spell list, don't add it again
				CharacterSpell charSpell = aClass.getFirstCharacterSpell(aSpell, Globals.getDefaultSpellBook(), gcs.getLevel(), null, null);
				// it's already on the list, so check it to make
				// sure it's the same level. If so, skip it
				if (charSpell != null && charSpell.getAdjustedLevel() == gcs.getAdjustedLevel())
					continue;

				// ok, everything looks good. Now let's add it!

				CharacterSpell cs = new CharacterSpell(castingClass, aClass, aSpell);
				cs.setLevel(gcs.getLevel());

				//System.out.println("PCGVer0Parser:pOldCSL: " + cs.getSpell().getName()+":"+cs.getOwner().getName()+":"+cs.getOwningClass().getName());
				...
				while (bTok.hasMoreTokens())
				{
					...
					CharacterSpell ccs = aClass.getFirstCharacterSpell(aSpell, bookName, -1, null, aClass);
					if (ccs != null)
					{
					...
					{
						cs = new CharacterSpell(castingClass, aClass, aSpell);
						cs.setLevel(gcs.getLevel());
						cs.setSpellBook(bookName);
						if (bTok.hasMoreTokens())
							cs.setTimes(Integer.parseInt(bTok.nextToken()));
						//System.out.println("PCGVer0Parser:pOldCSL: " + cs.getSpell().getName());
						aClass.addCharacterSpell(cs);
					} // end else
				} // end while spellbooks
	...
	private int parseSpellLine(String[] lines, int i)
	{
		...
		while (aString.startsWith("SPELL:"))
		{
			...
			if (aObject == null && className.equals(pdName))
			{
				aObject = (PObject)aClass;
			}

			// see if it's a subclass
			if (aObject == null && aClass.getSubClassName().equals(pdName))
			{
				aObject = new PObject();
				aObject.setName(pdName);
			}

			// it's not a class or domain spell, so it
			// must be that wacky CastAs stuff
			if (aObject == null && aClass.getCastAs().length() > 0)
			{
				aObject = (PObject)Globals.getClassNamed(aClass.getCastAs());
				usesCastAs = true;
			}
			...
			if (aObject == null)
			{
				final String message = "Bad spell info - no domain named " + pdName;
				warnings.add(message);
				continue;
			}

			CharacterSpell gcs = Globals.getFirstCharacterSpell(aSpell, Globals.getDefaultSpellBook(), -1, aObject, null);

			if (gcs == null)
			{
				final String message = "Bad spell info - no spell for " + aSpell.getName() + " in " + pdName + " class";
				warnings.add(message);
				continue;
			}
			...
			if (book.equals(Globals.getDefaultSpellBook()) &&
				aClass.isAutoKnownSpell(aSpell.getKeyName(), gcs.getLevel()) &&
				aPC.getAutoSpells())
			{
				continue;
			}

			CharacterSpell charSpell = aClass.getFirstCharacterSpell(aSpell, book, gcs.getLevel(), null, null);

			CharacterSpell cs = new CharacterSpell(aObject, aClass, aSpell);
			cs.setLevel(gcs.getLevel());
			cs.setTimes(times);
			...
			if (featList != null)
				cs.setFeatList(featList);

			// just to make sure the spellbook is present
			aPC.addSpellBook(book);

			// if the spell is already on the list, check adjusted
			// spell level, and if they are the same, don't add
			if (charSpell != null)
			{
				if (charSpell.getSpellBook().equals(cs.getSpellBook()) &&
					charSpell.getAdjustedLevel() == cs.getAdjustedLevel())
				{
					System.out.println("Already know "+aSpell.getName());
					continue;
				}
			}

			//System.out.println("PCGVer0Parser:pCSL:("+gcs.getLevel()+")"+aSpell.getName()+":"+times+":"+book+":"+cs.getOwner().getName()+":"+cs.getOwningClass().getName());
...
//		}
	...
	private void parseEquipmentLine(String line) throws PCGParseException
	{
		...
		for (Iterator it = containers.keySet().iterator(); it.hasNext();)
		{
			...
			if (eq != null)
			{
				aParent = (Equipment)aPC.getEquipmentNamed((String)containers.get(aName));
...
//System.out.println("PGCVer0Parse:pESL: "+aString);
				...
				else
				{
					...
					if (iTok.countTokens() > 3)
					{
						...
						if (eqT != null)
						{
							eqT.insertChild(eq);
							eq.setParent(eqT);
//System.out.println("PGCVer0Parse:pESL: Parent:"+eqT.getName()+" child:"+eq.getName());
						}
					}
					aSet.setItem(eq);
				}
