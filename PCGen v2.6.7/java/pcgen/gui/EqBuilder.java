/*
 * EqBuilder.java
 * @(#) $Id: EqBuilder.java,v 1.1 2006/02/20 23:57:36 vauchers Exp $
 *
...
public class EqBuilder extends JPanel
{
	...
	private void setGuiTextInfo(Object obj, String in_String)
	{
...
*/
		...
		setGuiTextInfo(jButtonOkay, "Okay");
		...
		jButtonOkay.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent evt)
			{
				jButtonOkayActionPerformed(false);
			}
		});
		...
		jButtonPurchase.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent evt)
			{
				jButtonOkayActionPerformed(true);
			}
		});
...
//		jScroll_ListAvailable.setViewportView(jPanel7);
		...
		//
		jComboBoxSize.setModel(new DefaultComboBoxModel(Constants.s_SIZELONG));
		...
		for (Iterator e = Globals.getClassList().iterator(); e.hasNext();)
		{
			...
			if ((baseStat != null) && (baseStat.length() > 0) && !baseStat.equalsIgnoreCase(Constants.s_NONE))
			{
				...
				for (Iterator e2 = Globals.getSpellMap().values().iterator(); e2.hasNext();)
				{
					...
					if (Globals.getSpellLevel(aSpell, aClass) < 9999)
					{
		...
		jSplitPane2.setDividerLocation(Globals.getCustomizerSplit1());
		jSplitPane3.setDividerLocation(Globals.getCustomizerSplit2());
	...
	private void jButtonNameActionPerformed()
	{
		...
		if (selectedValue != null)
		{
			...
			{
				customName = aString;
				StringBuffer oldName = new StringBuffer("(" + aNewEq.getItemNameFromModifiers() + ")");
				//
				...
				final String aSize = Constants.s_SIZESHORT[getItemSize()];
	...
	private void jButtonWeightActionPerformed()
	{
		...
		if (selectedValue != null)
		{
			...
			try
			{
				Float newWeight = new Float(aString);
				...
				aNewEq.setWeightMod(new Float(0));
				aNewEq.setWeightMod(new Float(newWeight.doubleValue() - aNewEq.getWeight().doubleValue()));
				showItemInfo();
			}
			catch (Exception e)
			{
	...
	private void jButtonCostActionPerformed()
	{
		...
		if (selectedValue != null)
		{
			...
			try
			{
				Float newCost = new Float(aString);
				...
				aNewEq.setCostMod(new Float(0));
				aNewEq.setCostMod(new Float(newCost.doubleValue() - aNewEq.getCost().doubleValue()));
				showItemInfo();
			}
			catch (Exception e)
			{
...
//			System.out.println("castingClass: " + ((PObject)castingClass).getName());
//			System.out.println("theSpell: " + theSpell);
//			System.out.println("variant: " + variant);
//			System.out.println("spellType: " + spellType);
//			System.out.println("baseSpellLevel: " + baseSpellLevel);
//			System.out.println("casterLevel: " + casterLevel);
//			System.out.println("metamagic: " + metamagicFeats);
			...
			if (eqMod.getMinCharges() > 0)
			{
				...
				{
					Object selectedValue = JOptionPane.showInputDialog(null,
						"Enter Number of Charges (" + Integer.toString(eqMod.getMinCharges()) + "-" + Integer.toString(eqMod.getMaxCharges()) + ")",
						Constants.s_APPNAME,
						JOptionPane.INFORMATION_MESSAGE,
						null,
						null,
	...
	private Float getCostForItem(int itemType, int spellLevel, int casterLevel, double levelMult, Spell aSpell)
	{
		double cost = 0;
		int count = 1;
		switch (itemType)
		...
		if (iSelected >= 0)
		{
			...
			if (iSelected >= 0)
			{
				if (dataModel.getTableType() != EquipmentModModel.TABLETYPE_SPELLS)
				{
					...
					if (eqMod.getChoiceString().equals("EQBUILDER.SPELL"))
					{
	...
	{
		if ((aSpell == null) ||
			(spellVariant.length() == 0) ||
			(jComboBoxClass.getSelectedIndex() < 0) ||
			(jComboBoxLevel.getSelectedIndex() < 0))
		{
			return null;
		}
		...
		if (castingClass != null)
		{
			...
			final int spellLevel = Globals.getSpellLevel(aSpell, castingClass, castAs);
	...
	private void showSpellInfo(boolean bSelectSpell)
	{
		...
		if (newName != null)
		{
			itemName.append("\nNew Item: ").append(newName);
		}

		if (aNewEq != null)
		{
			itemName.append("\nCost: ").append(aNewEq.getCost().toString());
	...
	private void saveScreenInfo()
	{
		...
		Globals.setCustomizerSplit1(jSplitPane2.getDividerLocation());
		Globals.setCustomizerSplit2(jSplitPane3.getDividerLocation());
	...
	private void jButtonOkayActionPerformed(boolean bPurchase)
	{
		...
		((PCGen_Frame1)Globals.getRootFrame()).eqList_Changed(aNewEq, bPurchase);
	...
	public boolean setEquipment(Equipment aEq, boolean bReloading)
	{
		listModel1.clear();
		listModel2.clear();
...
//		}
		...
		{
System.out.println("No base equipment found: " + aEq.getName() + ", aborting EqBuilder:" + sBaseKey);
			return false;
		}
...
*/		{
			...
			for (int i = 0; i < Constants.s_SIZESHORT.length; i++)
			{
				if ((Globals.sizeAdjustmentCostMultiplier(Constants.s_SIZESHORT[i], typeList) != 1.0) ||
					(aEq.isArmor() && (Globals.sizeAdjustmentAcModMultiplier(Constants.s_SIZESHORT[i], typeList) != 1.0)) ||
					((aEq.getWeight().floatValue() != 0.0) && Globals.sizeAdjustmentWeightMultiplier(Constants.s_SIZESHORT[i], typeList) != 1.0) ||
					(aEq.isContainer() && (Globals.sizeAdjustmentCapacityMultiplier(Constants.s_SIZESHORT[i], typeList) != 1.0)))
				{
					showSize = true;
					break;
				}
			...
			// Set item size
			//
			setItemSize(Constants.s_SIZELONG[iSize]);
		}
...
//				jButtonSpellActionPerformed();
	...
	private int getItemSize()
	{
		...
		if (idx >= 0)
		{
			...
			for (int i = Constants.SIZE_F; i <= Constants.SIZE_C; i++)
			{
				if (Constants.s_SIZELONG[i].equalsIgnoreCase(aSize))
				{
					return i;
				}
			}
...
//		}
	...
	private void showItemInfo()
	{
		...
		if (customName.length() != 0)
		{
			aInfo.append("Name: " + customName).append('\n');
		}
		String sprop = "";
...
*/
	...
	public final class EquipmentModModel extends AbstractTableModel
	{
		...
		private final Object[][] s_columnTypeNames_eqMod2 = new Object[][]
		{
			{String.class, "Q1"},
			{String.class, "Q2"},
			{String.class, "Name"},
		...
		private final Object[][] s_columnTypeNames_eqMod1 = new Object[][]
		{
			{String.class, "Q"},
			{String.class, "Name"},
			{String.class, "PreReqs"},
		...
		private final Object[][] s_columnTypeNames_spells = new Object[][]
		{
			{String.class, "Spell"},
//			{ Integer.class, "Level" },
...
//			{ Integer.class, "Normal Level" },
			{String.class, "Level"},
			{String.class, "Minimum Level"},
			{String.class, "Normal Level"},
			{String.class, "Cost"},
			{String.class, "Source"}
		};
...
//		}
		...
		public void setFilter(ArrayList aFilter, int listCount)
		{
		...
		public void setClassFilter(String className, int casterLevel, int levelAdjustment)
		{
		...
		{
			return Globals.getSpellLevel(aSpell, castingClass, castAs) + levelAdjustment;
		}
		...
		{
			return Globals.minCasterLevel(aSpell, castingClass, castAs, allowBonus, levelAdjustment);
		}
		...
		private Object getEqModTableValueAt(int row, int column, boolean twoLists)
		{
			...
			switch (column)
			{
				...
				case 4:
					...
					if (iPlus != 0)
					{
						eCost.append("Plus:").append(Integer.toString(iPlus));
					}
...
//					sRet = new Integer(iVal);
					sRet = new Integer(iVal).toString();
					break;
				case 2:
