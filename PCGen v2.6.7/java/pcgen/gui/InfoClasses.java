...
// create available table of class
	...
	private void experienceFocusLost(FocusEvent evt)
	{
		if (aPC.totalLevels() == 0)
			return;
		aPC.setDirty(true);
		Integer min = new Integer(Globals.minExpForLevel(aPC.totalLevels() + aPC.levelAdjustment()));
		...
		min = new Integer(Globals.minExpForLevel(aPC.totalLevels() + 1 + aPC.levelAdjustment()));
	...
	private void addClass(ActionEvent evt, int levels)
	{
		...
		if (levels > 0)
		{
			if (Globals.isFreeClothesAtFirst() && ((aPC.totalPCLevels() + aPC.totalNPCLevels()) == 1))
			{
				//
				...
				if (!hasClothes)
				{
					...
					if (selectedClothes.size() != 0)
					{
						...
						if (eq != null)
						{
							...
							{
								System.out.println("Cannot add duplicate equipment to PC");
							}
						}
					}
				}
