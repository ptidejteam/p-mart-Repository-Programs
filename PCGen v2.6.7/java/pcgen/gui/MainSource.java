 ...
 * decided to change the "Campaign" tab to "Source Materials" to avoid this confusion.  cu
...
public class MainSource extends FilterAdapterPanel
{
	...
	private void initComponents()
	{
		...
		viewMode = Globals.getPCGenOption("pcgen.options.sourceTab.availableListMode", VIEW_PUBFMTSET);
		...
		viewSelectMode = Globals.getPCGenOption("pcgen.options.sourceTab.selectedListMode", VIEW_PRODUCT);
		...
		Utility.setDescription(rightButton, "Click to select the Source");
		...
		Utility.setDescription(leftButton, "Click to deselect the Source");
		...
		removeAll.setToolTipText("Remove all sources from the above table");
		jPanel1s.add(removeAll);
		...
		website.setToolTipText("Go to the selected product's website");
		jPanel1s.add(website);
	...
	private void viewComboBoxActionPerformed(ActionEvent evt)
	{
		...
		if (index != viewMode)
		{
			...
			Globals.setPCGenOption("pcgen.options.sourceTab.availableListMode", viewMode);
		}
	}
	...
	private void viewSelectComboBoxActionPerformed(ActionEvent evt)
	{
		...
		if (index != viewSelectMode)
		{
			...
			Globals.setPCGenOption("pcgen.options.sourceTab.selectedListMode", viewSelectMode);
		}
	}
	...
	private void launchProductWebsite(ActionEvent e, boolean avail)
	{
		...
		{
			JOptionPane.showMessageDialog(null,
				"Select a product and try again.",
				Constants.s_APPNAME,
				JOptionPane.ERROR_MESSAGE);
		}
	}
