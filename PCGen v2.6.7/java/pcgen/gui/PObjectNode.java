...
//				return "|" + Globals.getFeatVirtualColor() + "|" + item.toString();
		...
		if (item instanceof CharacterSpell)
		{
			CharacterSpell spellA = (CharacterSpell)item;
			boolean isSpecial = spellA.isInSpecialty();
			int times = spellA.getTimes();

			String val = "";	//"<html>";

			// first, set the name
			val += spellA.toString(); // gets name + featList
			// now tack on any extra crap such as domains, etc
			if (isSpecial &&
				(spellA.getOwner() instanceof Domain))
			{
				val += " [" + spellA.getDescriptor(", ") + "]";
			}
