...
public class InfoSpells extends FilterAdapterPanel
{
	...
	private int selRow;
	private CharacterSpell lastSpell = null;
	...
	private void initComponents()
	{
		...
		//
		int iView = Globals.getSpellsTab_AvailableListMode();
		if ((iView >= VIEW_CLASS) && (iView <= VIEW_TYPE))
			viewMode = iView;
		Globals.setSpellsTab_AvailableListMode(viewMode);
		iView = Globals.getSpellsTab_SelectedListMode();
		if ((iView >= VIEW_CLASS) && (iView <= VIEW_TYPE))
			viewSelectMode = iView;
		Globals.setSpellsTab_SelectedListMode(viewSelectMode);
	...
	private void initActionListeners()
	{
		...
		addComponentListener(new ComponentAdapter()
		{
			public void componentResized(ComponentEvent e)
			{
				...
				if (s > 0)
					Globals.setPCGenOption("InfoSpells.splitPane", s);
				s = asplit.getDividerLocation();
				if (s > 0)
					Globals.setPCGenOption("InfoSpells.asplit", s);
				s = bsplit.getDividerLocation();
				if (s > 0)
					Globals.setPCGenOption("InfoSpells.bsplit", s);
			}
		});
	 ...
	 */
	private void setInfoLabelText(CharacterSpell spellA)
	{
		lastSpell = spellA; //even if that's null
		if (spellA != null)
		{
			Spell aSpell = spellA.getSpell();
			StringBuffer b = new StringBuffer();
			b.append("<html><font size=+1><b>").append(aSpell.getName()).append("</b></font>");
			if (spellA.getLevel() != spellA.getAdjustedLevel())
			{
				b.append("&nbsp;<font size=-2>[");
				for (Iterator i = spellA.getFeatList().iterator(); i.hasNext();)
					b.append(i.next().toString()).append(" &nbsp;");
				b.append("]</font>&nbsp;&nbsp; ");


			}
			b.append(" &nbsp;<b>Level:</b>&nbsp; ").append(spellA.getLevel());
			...
			b.append(" &nbsp;<b>Spell Resistance:</b>&nbsp; ").append(aSpell.getSR());
	...
	private void setClassLabelText(PCClass aClass)
	{
		if (aClass != null)
		{
			...
			if (Globals.isDeadlandsMode())
			{
				...
				a.append("<html><table border=1><tr><td><font size=-1><b>").append(aClass.getName()).append(" SpellPoints</b></font></td>");
				for (int i = 0; i <= 9; i++)
			...
			public void valueChanged(ListSelectionEvent e)
			{
				if (!e.getValueIsAdjusting())
				{
					...
					if (!atree.isSelectionEmpty())
					{
						...
						String className = "";
						if (viewMode == VIEW_CLASS)
							className = avaCPath.getPathComponent(1).toString();
						else if ((viewMode == VIEW_LEVEL) && (avaCPath.getPathCount() > 2))
							className = avaCPath.getPathComponent(2).toString();
						else if ((viewMode == VIEW_TYPE) && (lastClass != null))
							className = lastClass;
					...
					if (fNode.getItem() instanceof CharacterSpell)
					{
						CharacterSpell spellA = (CharacterSpell)fNode.getItem();
						if (spellA.getSpell() instanceof Spell)
						{
							addSpellButton.setEnabled(true);
							setInfoLabelText(spellA);
						}
					}
				}
			}
	...
	private void formComponentShown(ComponentEvent evt)
	{
		...
		{
			hasBeenSized = true;
			s = Globals.getPCGenOption("InfoSpells.splitPane", (int)(this.getSize().getWidth() * 2 / 10));
			t = Globals.getPCGenOption("InfoSpells.bsplit", (int)(this.getSize().getHeight() - 101));
			u = Globals.getPCGenOption("InfoSpells.asplit", (int)(this.getSize().getWidth() - 408));
		...
		if (t > 0)
		{
			...
			Globals.setPCGenOption("InfoSpells.bsplit", t);
		}
		if (u > 0)
		{
			asplit.setDividerLocation(u);
			Globals.setPCGenOption("InfoSpells.asplit", u);
		}
	...
	private void addSpellButton(ActionEvent evt)
	{
		...
		if (spellA.getSpell() instanceof Spell)
		{
			...
			if (className.length() == 0)
				className = (String)spellA.getOwningClass().getName();
			spLevel = (int)spellA.getAdjustedLevel();
		}
		else
			return;
		...
		final String aString = aPC.addSpell(spellA, featList, className, bookName);
...
//System.err.println("MMaddSB:MMaddSpell: " + className + ":" + spLevel + ":" + aSpell.getName() + ":" + bookName + ":feats:"+selFeatList.size());

		final String aString = aPC.addSpell(spellA, selFeatList, className, bookName);
...
/*
		// if no mmFrame, init one
		if (mmFrame == null)
			mmFrame = new MMFrame();
		mmFrame.setVisible(true);
		mmFrame.toFront();

		// now set the spell, make visible and bring to front
		if (mmFrame.setSpell(aSpell))
		{
			// blah!
		}
*/
	...
	private void delSpellButton(ActionEvent evt)
	{
		...
		final String aString = aPC.delSpell(spellA, aClass, bookName);
	...
	private void addBookButton(ActionEvent evt)
	{
		...
		{
			System.err.println("addBookButton:failed");
			return;
		}
		return;
	}
	...
	private void delBookButton(ActionEvent evt)
	{
		...
		{
			System.err.println("You may not delete the default spellbook");
			return;
		}
		...
		{
			System.err.println("delBookButton:failed ");
			return;
		}
	 ...
	 **/
	public class SpellModel extends AbstractTreeTableModel implements TreeTableModel
	{
		...
		public void resetModel(int mode, boolean available, boolean newCall)
		{
			...
			if (available)
			{
				...
				switch (mode)
				{
					case VIEW_CLASS:     // class/level/spell
						setRoot(avaRoot);
						...
						for (int iClass = 0; iClass < classList.size(); iClass++)
						{
							...
							for (int iLev = 0; iLev <= 9; iLev++)
							{
								...
								{
									charSpells = Globals.getCharacterSpell(null, Globals.getDefaultSpellBook(), iLev, null, aClass);
									addDomainSpellsForClass(className, spLev[iLev], iLev);
								}
								else
								{
									charSpells = aClass.getCharacterSpell(null, Globals.getDefaultSpellBook(), iLev, null, null);
								}
								setNodeSpells(charSpells, spLev[iLev], iLev);
					...
					case VIEW_LEVEL:
						...
						for (int iLev = 0; iLev <= 9; iLev++)
						{
							...
							for (int iClass = 0; iClass < spClass.length; iClass++)
							{
								...
								setNodeSpells(charSpells, spClass[iClass], iLev);
								// add if !empty
						...
						for (int iBook = 0; iBook < bookList.size(); iBook++)
						{
							...
							for (int iClass = 0; iClass < classList.size(); iClass++)
							{
								...
								for (int iLev = 0; iLev <= 20; iLev++)
								{
									...
									ArrayList charSpells = aClass.getCharacterSpell(null, bookName, iLev, null, null);
									setNodeSpells(charSpells, sbLev[iLev], iLev);
									// add if !empty
							...
							final ArrayList rList = aPC.getRace().getCharacterSpell(null, bookName, -1, null, null);
							if (rList.size() > 0)
							{
								PObjectNode rnode = new PObjectNode();
								rnode.setItem(aPC.getRace());
								cb[iBook].addChild(rnode);
								setNodeSpells(rList, rnode, 0);
							}
							// now add to root if !empty
							if (!cb[iBook].isLeaf())
							{
						...
						for (int iBook = 0; iBook < bookList.size(); iBook++)
						{
							...
							for (int iLev = 0; iLev <= 20; iLev++)
							{
								...
								for (int iClass = 0; iClass < classList.size(); iClass++)
								{
									...
									ArrayList charSpells = aClass.getCharacterSpell(null, bookName, iLev, null, null);
									setNodeSpells(charSpells, sbClass[iClass], iLev);
									// add if !empty
		...
		public void addDomainSpellsForClass(String className, PObjectNode theParent, int iLev)
		{
			...
			for (int iDom = 0; iDom < aPC.getCharacterDomainList().size(); iDom++)
			{
				...
				{
					ArrayList domainSpells = Globals.getCharacterSpell(null, "", iLev, aDom, null);
					p.setParent(theParent);
					if (!dom)
						theParent.addChild(p);
					dom = true;
					setNodeSpells(domainSpells, p, iLev);
				}
			}
		}
		...
		public Object getValueAt(Object node, int column)
		{
			...
			if (fn == null)
			{
				Globals.debugErrorPrint("Somehow we have no active node when doing getValueAt in InfoSpells.");
				return null;
			}

			if (fn.getItem() instanceof CharacterSpell)
			{
				spellA = (CharacterSpell)fn.getItem();
				aSpell = (Spell)spellA.getSpell();
				className = (String)spellA.getOwningClass().getName();
				spLevel = (int)spellA.getAdjustedLevel();
			}
	...
	private void setNodeSpells(ArrayList charSpells, PObjectNode tNode, int iLev)
	{
		Collections.sort(charSpells);
		for (Iterator fI = charSpells.iterator(); fI.hasNext();)
		{
			final CharacterSpell cSpell = (CharacterSpell)fI.next();
			if (iLev != cSpell.getAdjustedLevel())
				continue;
			PObjectNode fCN = new PObjectNode();
			fCN.setItem(cSpell);
			fCN.setParent(tNode);
			tNode.addChild(fCN);
		} // end spells loop
...
//  			contentPane.add(mainMM);
	...
	public class MainMM extends JPanel
	{
		...
		private void jbInit() throws Exception
		{
			...
			doneButton.addActionListener(new ActionListener()
			{
				...
				{
					// mmAddSpell();
					System.err.println("DONE: " + getParent());
					mmFrame.dispose();
				}
			});
		...
		private void mmTableMouseClicked(MouseEvent evt)
		{
			...
			if (aFeat != null)
				System.err.println("mmTableMouseClick: " + aFeat.getName());
			System.err.println("mmTableMouseClick:Total: " + totalLevels);

		}

		public boolean setSpell(Spell aSpell)
		{
			System.err.println("MainMM:setSpell: " + aSpell.getName());
			return true;
		}
