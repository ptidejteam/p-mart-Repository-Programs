...
public class InfoFeats extends FilterAdapterPanel
{
	...
	public static final int VIEW_TYPENAME = 0;		// view mode for Type->Name
	public static final int VIEW_NAMEONLY = 1;		// view mode for Name (essentially a JTable)
	public static final int VIEW_PREREQTREE = 2;		// view in requirement tree mode
	...
	//
	private int xlatInternalType(int featType)
	{
		int iRetVal;
		switch (featType)
		{
	...
	void initComponents()
	{
		...
		pcgen.gui.Utility.setDescription(viewComboBox, "You can change how the feats in the Available and Selected Tables are listed - either by name or in a directory-like structure.");
		...
		pcgen.gui.Utility.setDescription(viewSelectComboBox, "You can change how the feats in the Selected Tables are listed - either by name or in a directory-like structure.");
...
// create available table of feats
		...
		pcgen.gui.Utility.setDescription(rightButton, "Click to add the selected feat from the Available list of feats");
		...
		pcgen.gui.Utility.setDescription(sButton, "Click to change orientation of tables");
		...
		pcgen.gui.Utility.setDescription(numFeatsField, "How many feats you have left to choose (editable).");
		...
		pcgen.gui.Utility.setDescription(leftButton, "Click to remove the selected feat from the Selected list of feats");
		...
		pcgen.gui.Utility.setDescription(cScroll, "Any requirements you don't meet are in italics.");
		...
		pcgen.gui.Utility.setDescription(aButton, "Click to select any optional weapon proficiencies.");
	...
	private void formComponentShown(ComponentEvent evt)
	{
		...
		{
			hasBeenSized = true;
			s = Globals.getPCGenOption("InfoFeats.splitPane", (int)(this.getSize().getWidth() * 6 / 10));
			t = Globals.getPCGenOption("InfoFeats.bsplit", (int)(this.getSize().getHeight() * 75 / 100));
			u = Globals.getPCGenOption("InfoFeats.asplit", (int)(this.getSize().getWidth() * 5 / 10));
		...
		if (s > 0)
		{
			...
			Globals.setPCGenOption("InfoFeats.splitPane", s);
		}
		if (t > 0)
		{
			bsplit.setDividerLocation(t);
			Globals.setPCGenOption("InfoFeats.bsplit", t);
		}
		if (u > 0)
		{
			asplit.setDividerLocation(u);
			Globals.setPCGenOption("InfoFeats.asplit", u);
		}
	}
...
//		final Feat aFeat = aPC.getFeatNamed(aString);
		final Feat aFeat = aPC.getFeatNonAggregateNamed(aString);
					...
					while (fList.size() > 0 && loopmax-- > 0)
					{
						for (int i = 0; i < fList.size(); i++)
						{
							...
							for (int j = 0; j < rootAsPObjectNode.getChildCount(); j++)
							{
								...
								{
									String pString = (String)aFeat.getPreReq(pi);
									if (!pString.startsWith("PREFEAT:"))
										aFeat.removePreReq(pString);
								}
								placed = placedThisFeatInThisTree(aFeat, po, aFeat.preReqStrings(), 0);
								if (placed > 0)
									break;
							}
