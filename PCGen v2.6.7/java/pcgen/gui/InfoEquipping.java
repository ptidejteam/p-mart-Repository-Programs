 ...
 * Last Edited: $Date: 2006/02/20 23:57:36 $
 *
 */
...
public class InfoEquipping extends FilterAdapterPanel
{
	...
	protected EquipModel availableModel = null;  // Model for JTreeTable
	protected EquipModel selectedModel = null;   // Model for JTreeTable
	protected JTreeTable availableTable;         // available Equipment
	protected JTreeTable selectedTable;	     // Equipment Sets
	...
	private static String defaultEquipSet = "Default Set";
	...
	private void initComponents()
	{
		...
		//
		int iView = Globals.getEquipTab_AvailableListMode();
		if ((iView >= VIEW_TYPE) && (iView <= VIEW_NAME))
			viewMode = iView;
		Globals.setEquipTab_AvailableListMode(viewMode);
		iView = Globals.getEquipTab_SelectedListMode();
		if ((iView >= VIEW_TYPE) && (iView <= VIEW_LOCATION))
			viewSelectMode = iView;
		Globals.setEquipTab_SelectedListMode(viewSelectMode);
		...
		// create the horizontal/vertical switcher button
		...
		asplit = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, bLeftPane, bRightPane);
		...
		templateTextField.setText(Globals.getSelectedEqSetTemplate());
		...
		bsplit = new JSplitPane(JSplitPane.VERTICAL_SPLIT, topPane, botPane);
	...
	private void initActionListeners()
	{
		...
		addComponentListener(new ComponentAdapter()
		{
			public void componentResized(ComponentEvent e)
			{
				...
				if (s > 0)
					Globals.setPCGenOption("InfoEquipping.splitPane", s);
				s = asplit.getDividerLocation();
				if (s > 0)
					Globals.setPCGenOption("InfoEquipping.asplit", s);
				s = bsplit.getDividerLocation();
				if (s > 0)
					Globals.setPCGenOption("InfoEquipping.bsplit", s);
			}
		});
	...
	{
//System.out.println("setInfoLabelText: "+eqI.getName()+":");
		lastEquip = eqI; //even if that's null
		if (eqI != null)
		{
			StringBuffer b = new StringBuffer();
			b.append("<html><font size=+1><b>").append(eqI.getName()).append("</b></font>");
			if (!eqI.longName().equals(eqI.getName()))
				b.append("(").append(eqI.longName()).append(")");
			b.append(" &nbsp;<b>Type:</b>&nbsp; ").append(eqI.getType());
			b.append(" <b>PROFICIENT</b>:").append(((aPC.isProficientWith(eqI) && eqI.meetsPreReqs()) ? "Y" : "<font color='red'>N</font>"));
	...
	/*
	 * set the class info text in the Combat Info panel
	 * to the currently selected EquipSet Profile
	 */
	private void setCombatLabelText(PCClass aClass)
	{
		if (aClass != null)
		{
//System.out.println("setCombatLabelText: "+aClass.getName());

			StringBuffer b = new StringBuffer();
			b.append("<html><table border=1><tr><td><font size=-2><b>").append(aClass.getName()).append(" [").append(String.valueOf(aClass.getLevel().intValue() + aPC.getTotalBonusTo("PCLEVEL", aClass.getName(), true))).append("]</b></font></td>");
			for (int i = 0; i <= 9; i++)
			...
			b.append("</html>");
			combatLabel.setText(b.toString());
		}
	}
	...
	{
//System.out.println("--start-- formComponentShown");
		requestFocus();
		PCGen_Frame1.getStatusBar().setText("");

//System.out.println("fCS:before:updateCharacterInfo");
		...
		{
			hasBeenSized = true;
			s = Globals.getPCGenOption("InfoEquipping.splitPane", (int)(this.getSize().getWidth() * 7 / 10));
			t = Globals.getPCGenOption("InfoEquipping.bsplit", (int)(this.getSize().getHeight() - 101));
			u = Globals.getPCGenOption("InfoEquipping.asplit", (int)(this.getSize().getWidth() - 408));
		...
		if (t > 0)
		{
			...
			Globals.setPCGenOption("InfoEquipping.bsplit", t);
		}
		if (u > 0)
		{
			asplit.setDividerLocation(u);
			Globals.setPCGenOption("InfoEquipping.asplit", u);
		}

		availableTable.updateUI();
		selectedTable.updateUI();

//System.out.println("--done-- formComponentShown");
	}
	...
	{
//System.out.println("  --start-- uCI: needsUpdate: "+needsUpdate);
...
//System.out.println("  --done-- uCI: needsUpdate: "+needsUpdate);
	}


	private void addEquipButton(ActionEvent evt)
	{
//System.out.println("------------------addEquipButton---------------- ");
		...
		System.out.println("addEB: eqI: " + eqI.getName() + "  equipSetName:" + equipSetName);
		...
		else
		{
			...
			if (locName.length() == 0)
			{
				...
				c.setTitle("Choose Location for Equipment");
		...
		if (locName.equals("") || locName.length() == 0)
			return;
		...
		System.out.println("--addEB-- IdPath:" + id + "  Parent:" + eSet.getIdPath() + " Location:" + locName + " eqName:" + eqI.getName() + "  eSet:" + eSet.getName());
	...
	private void delEquipButton(ActionEvent evt)
	{
		...
		EquipSet eSet = (EquipSet)fNode.getItem();
		Equipment eqI = Globals.getEquipmentNamed(eSet.getValue());
		...
		System.out.println("--delEB--  getId:" + eSet.getId() + " Parent:" + eSet.getParentIdPath() + " eqName:" + eqI.getName() + " aSet:" + eSet.getName());

		// remove it from EquipSet
		aPC.getEquipSet().remove(eSet);
	...
	private void setQtyButton()
	{
		...
		//System.out.println("--setQty--  eSet:"+eSet.getName()+" eqName:"+eqI.getName()+" num:"+eSet.getQty()+" newNum:"+newNum);
	...
	{
		String aString = equipSetTextField.getText();

		System.out.println("addEquipSetButton:aString: " + aString);

		if (aString.equals(selectedEquipSet))
		{
			System.out.println("addESB: Set already exists: aString: " + aString + " == " + selectedEquipSet);
			return;
		}
		EquipSet pcSet = aPC.getEquipSetByName(aString);
		if (pcSet != null)
		{
			System.out.println("addESB: Set already exists: " + aString);
			return;
		}
		...
		EquipSet eSet = new EquipSet(id, aString);

		aPC.setDirty(true);
		selectedEquipSet = aString;
	...
	{
		String aString = equipSetTextField.getText();
		System.out.println("delEquipSetButton:aString: " + aString);

		EquipSet eSet = aPC.getEquipSetByName(aString);

		if (eSet == null)
		{
			System.out.println("delEquipSetButton: No EquipSet named: " + aString);
			return;
		}
		...
		{
			System.out.println("delEquipSetButton:failed ");
			return;
		}
	 ...
	 *
	 */
	private String getEqTypeName(Equipment eqI)
	{
		String locTypeName = "";
	 ...
	 */
	protected ArrayList locationChoices(Equipment eqI)
	{
		...
		if (eqI.isWeapon())
		{
			String aWProf = eqI.profName(Equipment.BOTH_HANDS);
	 ...
	 **  **  **    **  ****  **     **
	 **  **  **    **  ** ** **     **
	 **Creates the EquipModel that will be used.
	 **       **  **   **   ***     **
	 **        ****    **    **     **
	 */
	 ...
	 */
	public class EquipModel extends AbstractTreeTableModel implements TreeTableModel
	{
		 ...
		 */
		public EquipModel(int mode, boolean available)
		{
			super(null);
		...
		public void resetModel(int mode, boolean available, boolean newCall)
		{
			...
			if (aPC.getEquipSet().size() == 0)
			{
				...
				aPC.addEquipSet(eSet);
				System.out.println("Adding EquipSet: " + defaultEquipSet);
		...
		public Object getValueAt(Object node, int column)
		{
			...
			PObjectNode fNode = fn;
			EquipSet eSet = null;
			Equipment eqI = null;

			if (fn == null)
			{
				Globals.debugErrorPrint("Somehow we have no active node when doing getValueAt in InfoEquipping.");
				return null;
			}

			column = adjustAvailColumnConst(column);

//System.out.println("getValueAt:before eqI  model("+modelType+")("+column+") "+fn.toString());
			...
			switch (column)
			{
				...
				default:
					...
					{
						Globals.debugErrorPrint("Somehow we have no active node when doing getValueAt in InfoEquip.");
						return null;
					}
...
//System.out.println("aETN: aSet:"+aSet.getIdPath()+"  name: "+aSet.getName()+"  value:"+aSet.getValue()+"  getPId:"+aSet.getParentIdPath());
		}
...
//System.out.println("aETN: es:"+es.getIdPath()+"  name: "+es.getName()+"  value:"+es.getValue()+"  getPId:"+es.getParentIdPath());
			}
	...
	private void selectTemplateButton(ActionEvent e)
	{
		...
		fc.setSelectedFile(new File(Globals.getSelectedEqSetTemplate()));
		if (fc.showOpenDialog(InfoEquipping.this) == JFileChooser.APPROVE_OPTION)
		{
			Globals.setSelectedEqSetTemplate(fc.getSelectedFile().getAbsolutePath());
	...
	private void viewEqSetButton(ActionEvent e)
	{
		...
		try
		{
			...
			//
			if (osName.startsWith("Windows ") && (Globals.getBrowserPath() == null))
			{
	...
	{
		final String template = Globals.getSelectedEqSetTemplate();
		...
		fcExport.setSelectedFile(new File(Globals.getHtmlOutputPath() + File.separator + "eqset_" + aPC.getName() + ext));
		fcExport.setDialogTitle("Export EquipSet " + aPC.getName());
		...
		{
			JOptionPane.showMessageDialog(null, "Could not export " + aPC.getName() + ". Try another filename", "PCGen", JOptionPane.ERROR_MESSAGE);
			ex.printStackTrace();
		}
	}
	...
	private File getTempPreviewFile()
	{
		...
		try
		{
			...
			tempFile = File.createTempFile("equipSets", ext, Globals.getTempPath());
		}
	...
	{
		File esTemplate = new File(Globals.getSelectedEqSetTemplate());
		...
		*/
		...
		(new pcgen.io.ExportHandler(reParse)).write(aPC, bw);
	 ...
	 *
	 **/
	private int[] eqsheetSplit(File template, File tmpFile, int tests[])
	{
		boolean done = false;
		boolean eqset = false;
		int ret[] = new int[2];
		...
		try
		{
			...
			{
				line = (String)lines.elementAt(lineNum);
	...
	private void equipItch(File template, BufferedWriter out)
	{
		...
		for (Iterator i = eqRootList.iterator(); i.hasNext();)
		{
			...
			System.out.println("Preview:(" + pid + ") " + esRL.getName());
			...
			(new pcgen.io.ExportHandler(template)).write(aPC, out);
