 ...
 *
 * $Id: ThermometerDemo.java,v 1.1 2007/10/10 19:25:26 vauchers Exp $
 *
...
public class ThermometerDemo extends JPanel {
  ...
  void jbInit() throws Exception {
    ...
    thermo[0].addSubtitle("Sea Water Temp");
    thermo[1].addSubtitle("Air Temp", new Font("SansSerif", Font.PLAIN, 16));
    thermo[2].addSubtitle("Ship Temp",
    ...
    pickShow[0] = pickShow0;
    pickShow[1] = pickShow1;
    pickShow[2] = pickShow2;

    pickAxis[0] = pickAxis0;
    pickAxis[1] = pickAxis1;
    pickAxis[2] = pickAxis2;

    pickAxis[0].setSelectedIndex(2);
    pickAxis[1].setSelectedIndex(2);
    pickAxis[2].setSelectedIndex(2);
    ...
    pickShow0.addActionListener(new java.awt.event.ActionListener() {
      public void actionPerformed(ActionEvent e) {
        setShowValue(0);
      }
    });
    ...
    pickAxis0.addActionListener(new java.awt.event.ActionListener() {
      public void actionPerformed(ActionEvent e) {
        setShowAxis(0);
      }
    });
    pickAxis1.addActionListener(new java.awt.event.ActionListener() {
      public void actionPerformed(ActionEvent e) {
        setShowAxis(1);
      }
    });
    pickAxis2.addActionListener(new java.awt.event.ActionListener() {
      public void actionPerformed(ActionEvent e) {
        setShowAxis(2);
      }
    });
    ...
    jPanel9.add(pickShow2, null);
    jPanel9.add(pickAxis2, null);
    add(jPanel10, null);
    ...
    jPanel8.add(pickShow1, null);
    jPanel8.add(pickAxis1, null);
    ...
    jPanel7.add(pickShow0, null);
    jPanel7.add(pickAxis0, null);
    add(jPanel3, null);
    add(jPanel1, null);
  ...
  private void setValue(int thermometer, double value) {
    if ( (thermometer >= 0) && (thermometer < 3)) {
      try {
        thermo[thermometer].setValue(thermo[thermometer].getValue().doubleValue() +
                                     value);
      }
      catch (Exception ex) {
        ex.printStackTrace();
  ...
  private void setShowValue(int thermometer) {
    if ( (thermometer >= 0) && (thermometer < 3)) {
      thermo[thermometer].setValueLocation(pickShow[thermometer].
                                           getSelectedIndex());
    }
  }
  ...
  private void setShowAxis(int thermometer) {
    if ( (thermometer >= 0) && (thermometer < 3)) {
      thermo[thermometer].setShowAxisLocation(pickAxis[thermometer].
                                           getSelectedIndex());
    }
  }
