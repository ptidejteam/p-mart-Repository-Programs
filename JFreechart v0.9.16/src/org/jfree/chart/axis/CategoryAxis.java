 ...
 * Contributor(s):   -;
 *
 * $Id: CategoryAxis.java,v 1.1 2007/10/10 19:25:36 vauchers Exp $
 *
...
public class CategoryAxis extends Axis implements Cloneable, Serializable {
    ...
    public CategoryAxis(String label) {
        ...
        this.maxCategoryLabelWidthRatio = 0.9f;
     ...
     * listeners.
     *
     * @param margin  the margin (a percentage of the axis length).
     */
     ...
     * listeners.
     * <P>
     * The overall category margin is distributed over N-1 gaps, where N is the number of categories on the
     * axis.
     *
     * @param margin  the margin (a percentage of the axis length).
     */
     ...
     * 
     * @return The category label width ratio.
     */
     ...
     * 
     * @param positions  the category label position specification.
     */
    ...
    public List refreshTicks(Graphics2D g2, 
        ...
        if (categories != null) {
            float maxCategoryLabelWidth 
                = (float) (this.maxCategoryLabelWidthRatio 
                  * calculateCategorySize(categories.size(), dataArea, edge));
            int categoryIndex = 0;
            Iterator iterator = categories.iterator();
            while (iterator.hasNext()) {
                Comparable category = (Comparable) iterator.next();
                TextBlock label = createLabel(category, maxCategoryLabelWidth, edge, g2);
                CategoryLabelPosition position = this.categoryLabelPositions.getLabelPosition(edge);
                if (edge == RectangleEdge.TOP || edge == RectangleEdge.BOTTOM) {
        ...
        state.setMax(max + this.categoryLabelPositionOffset);
        return ticks;
    ...
    protected TextBlock createLabel(Comparable category, float width, 
        ...
        TextBlock label = TextUtilities.createTextBlock(
            category.toString(), this.getTickLabelFont(), width, new G2TextMeasurer(g2)
        );  
        return label; 
    }
